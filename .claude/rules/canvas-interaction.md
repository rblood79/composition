...
- **페이지 헤더는 DOM 층이다** (ADR-221, 2026-09-17 — 그전엔 Skia 렌더 + `pageTitleBoundsMap` point 판정이었다). 헤더 띠·타이틀·이름 편집은 `overlay/pageHeader/PageHeaderLayer` 가 `.canvas-container` 안에서 그리고, `usePageHeaderPlacement` 가 Skia 프레임 presentation 을 구독해 화면 px 로 배치한다. **페이지 간 occlusion 은 clip-path** — 위 (페인트 순서 뒤) 페이지의 body ∪ 헤더 rect 에 가려지는 구간을 `inset()` 하나로 잘라 보이지도 잡히지도 않게 한다 (clip 밖은 pointer 대상 아님). 히트는 헤더 노드 자체의 native pointerdown/dblclick (컨테이너 bubble pan·중앙 핸들러보다 먼저) 이고, BuilderCanvas pointerdown capture 는 `[data-page-header]` 자손이면 Skia 선판정만 건너뛴다. 카메라 제스처 (휠 pan/zoom · 스페이스 pan) 중에는 `cameraGestureActive` 게이트로 층을 숨기고 배치를 멈추며 (제스처 중 DOM 쓰기 0), 층 컴포넌트는 `frames` 를 마지막 settle 스냅샷으로 **동결**해 mount/unmount 도 0 이다 — gate-off 커밋의 layoutEffect 가 최신 집합 배치 1회 뒤 `data-hidden` 을 뗀다 (commit-before-reveal, ADR-226). 헤더 화면 폭 (`frame.width × settle zoom`) 이 96 px 미만이면 `data-lod="compact"` — 액션 버튼 미렌더 · gap 0 (판정 입력은 settle 스냅샷뿐, 편집 중은 항상 full). 프레임 타이틀 (`FrameAreaGroup`) 은 여전히 Skia `renderPageTitle` — 히트가 없어 이관 대상이 아니다.
