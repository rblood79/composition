# 전제·관점 의문 처리 — 결정 지점 한정

> 정본. CLAUDE.md 는 4개 결정 지점 요약만 두고 호출 규약 · 종결 계약 · 재개 조건 · 결정 지점의 판단 재료 전문은 이 파일이 보유한다 (2026-09-02 CLAUDE.md 에서 이관, 내용 무변경). frontmatter `paths` 없음 = 상시 로드.
...
**전제 확정 종결 계약 (terminal state)**: 다음 중 하나가 성립하면 해당 ADR 의 전제·관점은 **확정**이며, scope 무변경인 한 구현 전 과정 (execute-adr 포함) 에서 재질문·재검토 금지:
...
**결정 지점에서 따질 것**: 어느 ADR 의 어느 전제가 흔들리는지 · 코드 실측 근거가 무엇인지 · 반대 방향의 설명도 성립하는지 · 사용자에게 무엇을 물어야 답이 갈리는지. codex review / cross-check 는 본문 정합 layer 일 뿐 전제·관점 layer 가 아니다. 절차 컴플라이언스 (Risk 표 / Gate 매핑 / type-check PASS / codex review PASS) 통과가 사용자 confirm 을 대체하지 못한다.
