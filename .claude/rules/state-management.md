...
| 경로                                                                      | 위치                                                                                             | 잔존 패턴                                                                                                                                                                                                                                                                                                       |
| ------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `createInstance` / `resetInstanceOverrideField` / instance snapshot batch | `apps/builder/src/builder/stores/utils/instanceActions.ts:579-637` (`applyElementSnapshotBatch`) | `set` 1차 (line 604-626) → `syncInstanceElementsToCanonical` 2차 (line 629; createInstance line 688 / resetInstanceOverrideField line 952). JSDoc + 3 inline 코멘트가 `ADR-122 §Residual set 1차 (canonical-first 아님)` 로 정정됨 (2026-06-22) — 코멘트와 동작 일치, 호출 순서 reverse 정정은 여전히 후속 분리 |
