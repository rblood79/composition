...
- **field 가족의 FieldError · Label · Input · DateInput 자식은 read-only sub-part** (판정 A × 2, 2026-09-03): `isDelegatedSubpartChild(child.type, parent.type)` 가 참이면 layout 자식 visit 이 인라인 style 을 통째로 무시하고 투영 `display` + (FieldError) delegation fontSize + (Input/DateInput) `width:100%` 만 남긴다 (줄 높이 명시 주입 금지 — 빈 FieldError 높이 0). **3.6 implicit 패치는 sub-part 자식에 대해 implicit 이 추가·변경한 키만 덮는다** — implicitStyles 가 `{...cs, 주입}` 으로 복사한 전체 style 을 덮으면 걷어낸 인라인이 되살아난다 (Label/Input 에서 실측). overlay margin 보고도 batch 를 읽는다. 게이트: bridge `read-only sub-part` 2 · browser FieldError 옛 문서 케이스 · `adr923FieldSubpartProjection.browser.test.ts` (junk == clean).
