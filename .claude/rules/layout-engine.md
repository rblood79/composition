...
| 13  | 백분율 — 인라인 축은 상속 available 로 확정, 블록 축은 `explicit_h>0` 만; `%` ctx 와 자식 재귀 available **두 게이트** 모두. **`explicit_h` 자리엔 부모 배치가 확정한 used 값도 온다** (ADR-206 — stretch (a) 커널 전 · (b) 커널 뒤 wrap/min-max · (c) column post-flexing main · grid area · aspect `%` base 는 `pct_base_h` 별도 채널, 소비자 게이트로 2^d 차단) | `solve_flex`/`solve_block` ctx + `child_containing_h` · `TreeNode.definite_h` · `solve_node_definite` · `definite_consumer_flags` | percentSize §ADR-206 · containerIntrinsic K · golden N11/N12 |
...
| 26  | auto 축 solve 반환은 **content-box** — leaf 스칼라 경로도 (layout 은 border-box 별도) · grid 기여/배치 (`col_contribution` · 행 기여 · `place_grid_axis` real_size) 가 auto 축 자식에 pad/border 가산 (`child_auto_*_pad_border`, 기여의 `%` padding 은 0) · flex `off+19` row 는 content 공간 (Taffy #1018) | `resolve_leaf_intrinsic_width` · `solve_grid` · `write_flex_item` | paddedLeafIntrinsic · shrinkToFitInline (padding 132) |
