...
- **field 5 의 FieldError 자식은 read-only sub-part** (잔여 1 판정 A, 2026-09-03): `hasDelegatedChild(parent.type, FIELD_ERROR_CHILD_SELECTOR)` 가 참이면 layout·Skia 는 자식 인라인 style 을 통째로 무시하고 투영 `display` + delegation fontSize 만 소비한다 (줄 높이 명시 주입 금지 — 빈 FieldError 높이 0). overlay margin 보고도 raw store style 이 아니라 batch 를 읽는다. 게이트: bridge `read-only sub-part` · browser 옛 문서 케이스 (marginTop·padding·color).
