...
| 8   | Demand characteristics | 측정 조건이 결과를 만든다 — hidden 탭 rAF 정지로 배선 결함처럼 보임, `HEAPU8.byteLength` 고수위선 delta 0, allocator ~100 MB grow 앨리어싱. browser gate DOM leg 이 Preview 전역 reset (`* { box-sizing: border-box }`) 을 안 실어 Input 426 > 400 이 "결함" 으로 3 문서를 지나감 — production 342 = 342 (ADR-923, 2026-09-03)                                                     | 측정 조건 기록 (visibilityState · 처녀 힙 · 60+ 대량 기울기) — 조건 없는 수치는 인용 금지       | feedback-paragraph-fontvariations-per-collection-instancing · reference-chrome-mcp-hidden-tab-raf-pause-stale-overlay     |
