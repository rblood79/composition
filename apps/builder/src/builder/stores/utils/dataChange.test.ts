...
const dbMock = {
  collections: {
    ...
    update: vi.fn(
      async (_id: string, _u: Partial<DataTable>) => ({}) as DataTable,
    ),
...
function roundTrip(ops: DataOp[]) {
  ...
  const strip = (m: Map<string, DataTable>) =>
    [...m.values()].map(
      ({ runtimeData: _r, updated_at: _u, created_at: _c, ...rest }) => rest,
    );
  expect(strip(back.collections)).toEqual(strip(before));
  // inverse 의 inverse 는 forward 와 같은 효과
  const again = reduceDataOps(back.collections, back.inverse, {
    projectId: "p",
  });
...
describe("reduceDataOps — op 별 결과 + 역연산 왕복", () => {
  it("set_cell", () => {
    ...
    expect(r.collections.get("c1")!.mockData[1]).toEqual({
      name: "B",
      email: "b@x",
    });
  ...
  it("remove_rows (비연속) → inverse 는 오름차순 insert_rows", () => {
    const r = roundTrip([
      { op: "remove_rows", collectionId: "c1", rowIndexes: [2, 0] },
    ]);
    expect(r.collections.get("c1")!.mockData).toEqual([
      { name: "b", email: "b@x" },
    ]);
  ...
  it("insert_rows (at 생략 = 끝) / replace_rows (CSV)", () => {
    const ins = roundTrip([
      { op: "insert_rows", collectionId: "c1", rows: [{ name: "d" }] },
    ]);
    expect(ins.collections.get("c1")!.mockData).toHaveLength(4);
    expect(ins.inverse).toEqual([
      { op: "remove_rows", collectionId: "c1", rowIndexes: [3] },
    ]);
    const rep = roundTrip([
      { op: "replace_rows", collectionId: "c1", rows: [{ name: "z" }] },
    ]);
  ...
  it("add_field (id 없음 → 부여, applied 에 id 실림) / remove_field (schema 만, 행 값 유지)", () => {
    ...
    expect((add.applied[0] as { field: { id?: string } }).field.id).toBe(
      added.id,
    );
    expect(add.inverse).toEqual([
      { op: "remove_field", collectionId: "c1", fieldId: added.id },
    ]);

    const rm = roundTrip([
      { op: "remove_field", collectionId: "c1", fieldId: "f_email" },
    ]);
    expect(rm.collections.get("c1")!.schema.map((f) => f.key)).toEqual([
      "name",
    ]);
    expect(rm.collections.get("c1")!.mockData[0]).toEqual({
      name: "a",
      email: "a@x",
    });
    expect(rm.inverse).toEqual([
      {
        op: "add_field",
        collectionId: "c1",
        field: { id: "f_email", key: "email", type: "email" },
        index: 1,
      },
    ]);
  });
  ...
  it("create_collection / delete_collection — 같은 id 로 되살아난다 (바인딩 참조 보존)", () => {
    ...
    expect(del.inverse[0]).toMatchObject({
    ...
    const cr = reduceDataOps(
      new Map(),
      [
        {
          op: "create_collection",
          name: "Roles",
          schema: [{ key: "role", type: "string" }],
        },
      ],
      { projectId: "p" },
    );
    ...
    expect(cr.inverse).toEqual([
      { op: "delete_collection", collectionId: created.id },
    ]);
  });
  ...
  it("update_collection / set_source", () => {
    ...
    // inverse 는 역순
    expect(r.inverse.map((o) => o.op)).toEqual([
      "set_source",
      "update_collection",
    ]);
  });

  it("set_execution_policy (ADR-218) — 정책 설정 + inverse 는 이전 정책(없었으면 null)", () => {
    const r = roundTrip([
      {
    ...
    const c = r.collections.get("c1")!;
    expect(c.executionPolicy).toEqual({ mode: "interval", intervalSec: 30 });
    // 이전에 정책이 없었으므로 inverse = null (필드 제거)
    expect(r.inverse).toEqual([
      { op: "set_execution_policy", collectionId: "c1", policy: null },
    ]);
  });

  it("set_execution_policy — policy:null 은 필드 제거, inverse 는 이전 정책 복원", () => {
    const before = new Map<string, DataTable>([
      ["c1", { ...users(), executionPolicy: { mode: "auto" } }],
    ]);
    ...
    expect(forward.collections.get("c1")!.executionPolicy).toBeUndefined();
    ...
    expect(back.collections.get("c1")!.executionPolicy).toEqual({
      mode: "auto",
    });
  });
  ...
  it("검증 실패는 throw — 없는 collection · 범위 밖 row · 미배선 op (bind_element · endpointId)", () => {
    ...
    expect(() =>
      reduceDataOps(
        m,
        [{ op: "bind_element", elementId: "e", collectionId: "c1" }],
        {},
      ),
    ).toThrow(DataChangeError);
    expect(() =>
...
describe("applyDataChange — DB · 메모리 · History · record:false", () => {
  function makeStore() {
    ...
    return {
      ...
      collections: () => state.collections as Map<string, DataTable>,
    };
  }
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("셀 편집 → DB update 1 · 메모리 갱신 · History data entry (inverse 동봉)", async () => {
    ...
    const entry = addEntry.mock.calls[0][0] as {
      ...
      data: {
        dataChangeEvent: {
          change: { ops: DataOp[]; origin: string };
          inverse: DataOp[];
        };
      };
    };
    ...
    await expect(
      apply({
        ops: [
          ...
          { op: "delete_collection", collectionId: "nope" },
        ],
        origin: "user",
      }),
...
describe("applyDataChange — bind_element (ADR-213 Phase 2, canonical 축 consumer + cross-store 원자성)", () => {
  ...
  const consumer: DataBindingConsumer = {
    ...
    apply: (id, write) => {
      ...
      if (write.restore) {
        nodes.set(id, {
          props: write.restore.props,
          extension: write.restore.extension,
        });
  ...
  it("bind → props.dataBinding 이 사람 UI 형상 {source:dataTable, collectionId, name} 으로 기록 · History 1 · inverse 는 restore 스냅샷", async () => {
    ...
    const entry = addEntry.mock.calls[0][0] as {
      elementIds: string[];
      data: { dataChangeEvent: { change: { origin: string } } };
    };
    ...
    expect(result.inverse).toEqual([
      {
        ...
        elementId: "e_free",
        collectionId: null,
        restore: { props: undefined, extension: undefined },
      },
    ]);
    expect(dbMock.collections.update).not.toHaveBeenCalled();
  });
  ...
  it("undo(inverse, record:false) 는 legacy extension 형태까지 원상 복구한다", async () => {
    ...
    const forward = await apply({
      ops: [{ op: "bind_element", elementId: "e_legacy", collectionId: "c1" }],
      origin: "agent",
    });
    expect(nodes.get("e_legacy")).toEqual({
  ...
  it("collectionId:null 은 해제 — inverse 로 다시 붙는다", async () => {
    const { apply } = makeStore();
    await apply({
      ops: [{ op: "bind_element", elementId: "e_free", collectionId: "c1" }],
      origin: "user",
    });
    const unbound = await apply({
      ops: [{ op: "bind_element", elementId: "e_free", collectionId: null }],
      origin: "user",
    });
    expect(nodes.get("e_free")).toEqual({});
  ...
  it("preflight — 없는 요소 · 없는 collection 은 아무것도 바꾸지 않고 throw (같은 change 안에서 만든 collection 은 허용)", async () => {
    ...
    await expect(
      apply({
        ops: [{ op: "bind_element", elementId: "ghost", collectionId: "c1" }],
        origin: "ai",
      }),
    ).rejects.toThrow(DataChangeError);
    await expect(
      apply({
        ops: [
          ...
          { op: "bind_element", elementId: "e_free", collectionId: "nope" },
        ],
        origin: "ai",
      }),
    ).rejects.toThrow(DataChangeError);
    ...
    const created = await apply({
      ops: [
        ...
        { op: "bind_element", elementId: "e_free", collectionId: "c_new" },
      ],
      origin: "ai",
    });
    expect(nodes.get("e_free")).toEqual({
      props: { source: "dataTable", collectionId: "c_new", name: "Roles" },
    });
    expect(created.inverse.map((o) => o.op)).toEqual([
      "bind_element",
      "delete_collection",
    ]);
    expect(addEntry).toHaveBeenCalledTimes(1);
