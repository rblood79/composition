...
/**
 * ADR-231 — 시스템 페이지 (Components …) 집합. 배치 함수는 이 집합을 격자에서 빼고 왼쪽 세로
 * 열에 두며, 위치는 breakpoint 공통값 하나다 (전환은 읽지도 쓰지도 않는다 · 쓰기는 세 breakpoint 동시).
 */
export function resolveSystemPageIds(
  pages: readonly Pick<Page, "id" | "slug" | "title">[],
): ReadonlySet<string> {
