...
/**
 * `PropagationUpdate[]` → store batch update entry (`BatchPropsUpdate`) 변환.
 *
 ...
 * 화면 코드가 같은 매핑을 인라인으로 재작성하면 그 누락을 어떤 게이트도 못 본다 — 그래서
 * 이 transport 는 함수 하나로 고정하고, 유일한 production 호출자는 `semanticUpdateDispatch`
 * 다 (round 4 fe3m1 · round 5 fe4m1). 반환 타입은 store 계약 (`BatchPropsUpdate`) 을 직접
 * 가리킨다 — `elementUpdate` 는 이 모듈을 import 하지 않아 순환이 없다 (round 5 fe4l1).
 */
export function toBatchPropsUpdates(
  updates: readonly PropagationUpdate[],
): BatchPropsUpdate[] {
  return updates.map((update) => ({
    elementId: update.elementId,
    props: update.props as BatchPropsUpdate["props"],
