 ...
 * 뜬다 (2026-09-08 Monitor 패널 live 재현 — 컨테이너 2 · 토스트 4 · 랜드마크 2).
...
describe("builder toast — 단일 컨테이너 · 단일 store", () => {
  it("ToastContainer 를 마운트하는 컴포넌트는 BuilderCore 하나다", () => {
    ...
    expect(read("builder/panels/monitor/MonitorPanel.tsx")).not.toContain(
      "ToastContainer",
    );
  });
  ...
  it("로컬 훅 경로는 없다 — 발신은 useToastStore 뿐", () => {
    ...
    for (const rel of [
      ...
      "builder/panels/monitor/MonitorPanel.tsx",
    ]) {
      expect(read(rel)).toContain("useToastStore((state) => state.showToast)");
    }
  });
