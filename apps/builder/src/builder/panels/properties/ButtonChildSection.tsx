...
/**
 * Button 자식으로 추가할 Icon element 를 생성한다. handleAddElement 대신 직접 생성하는
 *   이유: handleAddElement 는 생성 직후 setSelectedElement 로 Button 선택을 풀어 셀렉트가
 *   사라진다. id 를 미리 만들어 selection 변경 없이 생성.
 */
function buildIconChild(
  parentId: string,
  pageId: string,
 ...
 *
 * RSP 공식 모델 (Button "With Icon and Label"): `<Button><Icon/>label</Button>` — 아이콘은
 *   자식 element, label 은 Button 자체의 children. 본 셀렉트는 **Icon 자식 element 만**
 *   생성/수정/삭제하고 Button 의 string children(label, "Text" 필드)은 건드리지 않는다.
 *   string children + Icon 자식 element 공존은 CanonicalNodeRenderer 의
 *   `ICON_HOST_BUTTON_TYPES` 분기가 렌더(둘 다 그림) → label 도 화면에 유지된다.
 *   - None → 아이콘: Icon 자식 element 생성(selection 유지).
 *   - 아이콘 → 다른 아이콘: 기존 자식 Icon 의 iconName 만 수정.
 *   - 아이콘 → None(clear): Icon 자식 element 삭제.
...
export const ButtonChildSection = memo(function ButtonChildSection({
  ...
  const handleSelectIcon = useCallback(
    (iconName: string) => {
      ...
      // None → 아이콘: Icon 자식 element 생성. Button.children(label)은 건드리지 않는다
      //   (렌더러가 string children + Icon 자식 공존을 그린다).
      ...
      addElement(
        buildIconChild(elementId, currentPageId, pageElements, iconName),
      );
    },
