...
import {
  ...
  ImageSpec,
  // ADR-912 단계5 step4 (2026-06-17): InlineAlertSpec import 제거 — InlineAlert.spec 물리 삭제(catalog cutover).
  ...
  //   binding.accepts D2 (TagGroup.binding.ts). getPropertyEditorSpec("TagGroup") dead 경로.
...
> = {
  ...
  // InlineAlert — ADR-912 단계5 step4 (2026-06-17): catalog cutover → GenericPropertyEditor 가
  //   binding.accepts(InlineAlert.binding.ts) 로 properties 생성(spec.properties dead). spec 삭제로 등록 제거.
  ...
  // ADR-912 단계5 step4 (2026-06-17): TagGroup catalog cutover → entry 제거 (binding.accepts D2). spec 삭제.
