...
describe("LayoutBodyEditor visible naming", () => {
  it("exposes only layout preset controls for frame body properties", async () => {
    const source = await readFile(
      resolve(__dirname, "LayoutBodyEditor.tsx"),
      "utf-8",
    );

    expect(source).toContain('<PropertySection title="Layout Preset">');
    expect(source).toContain("useCanonicalPropertyElement");
    expect(source).not.toContain("canonicalElementSnapshot");
    expect(source).not.toContain('<PropertySection title="Frame Preset">');
