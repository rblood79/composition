...
const PALETTE_ORDER: ReadonlyArray<{
  ...
  // 2026-08-21 결손 보강 — 아래 3종도 TextArea 와 같은 형태였다: catalog panel meta +
  //   factory creator + rendererMap 전용 렌더러를 모두 갖췄는데 본 배열에만 없어 꺼낼 수
  //   없었다(코드 어디에도 제외 사유 없음). creator↔팔레트 전수 대조로 적발.
  // Pagination: 페이지 이동 UI — Breadcrumbs/Link/Nav 내비게이션 묶음 뒤.
  { type: "Pagination", source: "catalog" },
  ...
  // ADR-148 Phase 2 — 첫 신규 reusable (kind:"reusable" entry, REUSABLE_BY_TYPE 파생).
  { type: "IconButton", source: "catalog" },
  ...
  // ADR-201 (2026-09-17): 대용량 파일 업로드 compound — DropZone/FileTrigger 뒤.
  { type: "FileUpload", source: "catalog" },
