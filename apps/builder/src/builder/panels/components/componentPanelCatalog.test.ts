...
describe("ADR-142 Component Panel catalog bridge", () => {
  it("maps active componentCatalog entries into panel definitions", () => {
    ...
    const menu = catalogItems.find((item) => item.type === "Menu");
    ...
    expect(listBox?.source).toBe("catalog");
    expect(listBox?.categoryKey).toBe("collections");
    expect(listBox?.label).toBe("list box");
    ...
    expect(menu?.source).toBe("catalog");
    expect(menu?.categoryKey).toBe("buttons");
    expect(menu?.label).toBe("menu");
    expect(menu?.icon).toBe(Menu);
    expect(comboBox?.source).toBe("catalog");
    expect(comboBox?.categoryKey).toBe("forms");
    expect(comboBox?.label).toBe("combo box");
    expect(select?.source).toBe("catalog");
    expect(select?.categoryKey).toBe("forms");
