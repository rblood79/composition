/**
 * engine WASM 모듈 초기화 래퍼 (ADR-916 Phase 2-B seam C-2a)
 *
 * 엔진 없는 자체 레이아웃 엔진(`packages/engine`)의 wasm-pack
 * `--target bundler` 산출물(`engine-pkg/`)을 전역 로드한다.
 * 비동기 로드로 전역 캐시를 채우고, 동기 wrapper(`EngineLayout`)
 ...
 * `initEngineWasm()` 을 startup(init.ts)에서 호출해 전역 캐시를 채우고,
...
export interface RawEngineLayout {
  isAvailable(): boolean;
...
// engine(taffy-free) crate 로 이동 — endgame Taffy 완전 제거의 crate
...
interface EngineModule {
  LayoutEngine: { new (): RawEngineLayout };
...
export async function initEngineWasm(): Promise<void> {
  ...
  initPromise = (async () => {
    try {
      ...
      // `engine-pkg/`(dev 서버 root
      // 안)로 지정한다 — `/packages/engine/pkg/...` 절대 URL 은 dev
      ...
      if (import.meta.env.DEV) {
        console.log("engine WASM initialized");
      }
    } catch (err) {
...
export function getEngineWasm(): EngineModule | null {
  return engineModule;
}
