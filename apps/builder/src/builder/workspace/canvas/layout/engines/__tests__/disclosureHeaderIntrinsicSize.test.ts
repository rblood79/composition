 ...
 *   INLINE_BLOCK_TAGS/TEXT_LEAF_TAGS 미등록 → enrichWithIntrinsicSize 가 width 미산출 →
...
describe("DisclosureHeader intrinsic size (ADR-912 B+icon width 회귀)", () => {
  test("INLINE_BLOCK_TAGS 에 disclosureheader 등록 — width 주입 대상", () => {
    expect(INLINE_BLOCK_TAGS.has("disclosureheader")).toBe(true);
  });
