 ...
 */
function ensureListBoxScrollStyle(node: CanonicalNode): CanonicalNode {
  const props = isRecord(node.props) ? node.props : undefined;
  const style = isRecord(props?.style)
    ? (props?.style as Record<string, unknown>)
    : undefined;
  const hasHeight = style?.maxHeight != null || style?.height != null;
  const hasOverflow = style?.overflow != null || style?.overflowY != null;
  if (hasHeight || hasOverflow) return node;
  return {
    ...node,
    props: {
      ...(props ?? {}),
 ...
 * - 스크롤 style 보강은 `ensureListBoxScrollStyle` — height/overflow 미명시 default 만 대상.
 * - 멱등: anchor 가 이미 없고 style 도 보강 불필요하면 동일 참조를 반환한다.
 */
export function migrateLegacyListBoxTemplatesToOrigins(
  ...
  function migrateNode(node: CanonicalNode): CanonicalNode {
    ...
    let next: CanonicalNode = node.children
      ? { ...node, children: migratedChildren }
      : node;
    ...
    if (isPageListBoxInstance(node)) {
      ...
      if (strippedChildren.length !== migratedChildren.length) {
        next = { ...next, children: strippedChildren };
      }
      next = ensureListBoxScrollStyle(next);
    }
