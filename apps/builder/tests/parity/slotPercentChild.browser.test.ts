 ...
 * - **컨테이너 flex item 의 intrinsic 부재** → **ADR-169 로 이관** (Phase 0 fixture 완료).
 *   `solve_flex` 1단계가 각 item 을 컨테이너 available 로 solve 해 그 결과를 고유 폭으로
 *   쓰는 구조. 발산 7형태 + R8 masking 판정은 `containerIntrinsic.browser.test.ts` 가 소유한다.
 */
const ROOT_W = 1920;
