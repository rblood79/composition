...
const elementById = (page, id) =>
  state(
    page,
    (id) => {
      const e = window.__composition_STORE__.getState().elements.find(
        (x) => x.id === id,
      );
      return e
        ? {
...
async function addFromPalette(page, label, type) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (text.replace(/\s+/g, "").toLowerCase() === label.replace(/\s+/g, "").toLowerCase()) {
      item = items.nth(i);
      break;
    }
  }
  ...
  return state(
    ...
    (id) => {
      for (const frame of document.querySelectorAll("iframe")) {
        ...
        const matches = [...(doc?.querySelectorAll(`[data-element-id="${id}"]`) ?? [])];
        const el =
          matches.find((m) => m.getBoundingClientRect().width > 0) ?? matches[0];
        if (!el) continue;
...
async function updateOrigin(page, originId, patch) {
  await state(
    page,
    ({ originId, patch }) => {
      window.__composition_STORE__.getState().updateElementProps(originId, patch);
    },
    { originId, patch },
  );
...
async function waitPersisted(page, nodeId, predicate, timeoutMs = 15_000) {
  ...
  while (Date.now() - start < timeoutMs) {
    const value = await state(page, async (nodeId) => {
      ...
      const rows = await new Promise((res, rej) => {
        const r = db.transaction("document_parts", "readonly").objectStore("document_parts").getAll();
        r.onsuccess = () => res(r.result);
        r.onerror = () => rej(r.error);
      });
      db.close();
      const pid = location.pathname.split("/builder/")[1];
      const row = rows.find((r) => r.project_id === pid && r.key === `node:${nodeId}`);
...
try {
  ...
  const origins = await state(page, () => {
    ...
    return {
      ...
      roots: roots.map((e) => ({ id: e.id, type: e.type, reusable: e.reusable })),
    };
  });
  ...
  const expected = [
    ...
    "component-gridlist",
    ...[
      "Badge","ProgressBar","Avatar","AvatarGroup","StatusLight","ProgressCircle","Tabs","Breadcrumbs","Link","Nav","Pagination","DisclosureGroup","Disclosure","CardView","Button","ToggleButton","ToggleButtonGroup","ButtonGroup","Menu","TextField","TextArea","NumberField","SearchField","ColorField","Checkbox","CheckboxGroup","RadioGroup","Select","ComboBox","Switch","Slider","Meter","DropZone","FileTrigger","FileUpload","Table","Tree","TagGroup","TableView","Chart","Calendar","DatePicker","DateRangePicker","DateField","TimeField","RangeCalendar","Dialog","Modal","Popover","Tooltip",
  ...
  for (const type of TYPES) {
    ...
    record(
      ...
      JSON.stringify({ type: el?.type, ref: el?.ref, props: el?.props, page: el?.page_id }),
    );
  }
  ...
  const chartPatchOnlyDiffers = Object.entries(chart?.props ?? {}).every(
    ([k, v]) => k === "chartType" || JSON.stringify(chartOrigin?.props?.[k]) !== JSON.stringify(v),
  );
  record(
  ...
  await state(page, (id) => window.__composition_STORE__.getState().setSelectedElement(id), created.Button);
  await page.waitForTimeout(600);
  ...
  if (values.includes("Button")) {
    ...
    overrideOk = after?.props.children === "Buy now" && Object.keys(after.props).length === 1;
    overrideDetail = JSON.stringify(after?.props);
    const preview = await waitPreview(page, created.Button, (p) => p.found && p.text.includes("Buy now"));
    record(
  ...
  const dialog1 = await updateOrigin(page, "component-button", { variant: "accent" });
  const propagated = await waitPreview(page, created.Button, (p) => p.variant === "accent");
  record(
  ...
  await page.waitForTimeout(800);
  const dup = await state(page, (id) => {
    const st = window.__composition_STORE__.getState();
    const refs = st.elements.filter((e) => e.ref === "component-button" && e.id !== id);
  ...
  const afterUndo = await state(page, () =>
    window.__composition_STORE__.getState().elements.filter((e) => e.ref === "component-button").length,
  );
  ...
  record("G2-c: Undo → 1 · Redo → 2", afterUndo === 1 && afterRedo === 2, `${afterUndo} / ${afterRedo}`);

  // ── G2-d: 같은 props 의 plain 노드와 layout rect 동일 (Skia leg) ──
  const plainId = await state(page, (refId) => {
    const st = window.__composition_STORE__.getState();
  ...
  record(
    ...
    !!refRect && !!plainRect && refRect.w === plainRect.w && refRect.h === plainRect.h,
  ...
  record(
    ...
    refPrev.found && plainPrev.found && refPrev.w === plainPrev.w && refPrev.h === plainPrev.h && refPrev.variant === plainPrev.variant,
  ...
  const chartDialog = await updateOrigin(page, "component-chart", { style: { width: 480 } });
  await page.waitForTimeout(1200);
  ...
  record(
    ...
    chartRect0?.w === 320 && chartRect1?.w === 480 && chartInst1?.props.chartType === "bar" && !("style" in chartInst1.props),
  ...
  const beforeDel = await state(page, () => window.__composition_STORE__.getState().elements.length);
  await state(page, () => window.__composition_STORE__.getState().removeElement("component-badge"));
  await page.waitForTimeout(500);
  const afterDel = await state(page, () => ({
    n: window.__composition_STORE__.getState().elements.length,
    still: !!window.__composition_STORE__.getState().elements.find((e) => e.id === "component-badge"),
  }));
  record(
  ...
  const persisted = await waitPersisted(page, "component-badge", (v) => v.includes("Edited origin"));
  record("G3-b: origin 편집이 IndexedDB document_parts 에 persist", persisted, `node:component-badge ${persisted}`);
  ...
  record(
    ...
    afterReload.badge?.children === "Edited origin" &&
      ...
      JSON.stringify(afterReload.order) === JSON.stringify(beforeReload.order) &&
  ...
  record("page error 0 · dialog 0", errors.length === 0 && dialogs === 0, `${errors.length} / ${dialogs}`);
} catch (e) {
