// tabs-panel-wrapper-live.mjs — TabPanels 래퍼 padding (Skia 전용 채널) 제거 후 Tabs 높이·TabPanel y Skia ↔ Preview DOM live (2026-09-18, ADR-223 후속 3).
//   사용: node apps/builder/scripts/tabs-panel-wrapper-live.mjs  (dev 5173 · .auth-session.json · headed)
// Tabs root h + TabPanel 의 Tabs 기준 y/h — 수리 전 Skia 77 / DOM 53 · TabPanel y 41 / 29
import { chromium } from "playwright";
import { resolve } from "node:path";
import { waitReady } from "./perf-baseline.mjs";
const BASE_URL = process.env.BUILDER_URL ?? "http://localhost:5173";
const STORAGE_STATE = resolve("apps/builder/scripts/.auth-session.json");
const log = (...a) => console.log("[tabs live]", ...a);
...
async function addFromPalette(page, type) {
  ...
  const idx = n === 1 ? 0 : names.findIndex(l => l.replace(/\s+/g,"").toLowerCase()===type.toLowerCase() || l === "탭");
...
try {
  ...
  await input.fill(`tabslive-${Date.now()}`); await input.press("Enter");
  await page.waitForURL(/\/builder\/[^/?]+$/, { timeout: 90000 }); await waitReady(page);
  const navId = await addFromPalette(page, "Tabs");
  const info = await page.evaluate((id) => {
    const st = window.__composition_STORE__.getState();
    const nav = st.elements.find(e => e.id === id);
    const panels = st.elements.find(e => e.parent_id === id && e.type === "TabPanels");
    const panel = st.elements.find(e => e.parent_id === panels?.id && e.type === "TabPanel");
    ...
    // TabPanel 의 Tabs 기준 rect = TabPanels(부모 기준) + TabPanel(부모 기준)
    const pp = r(panels?.id), p = r(panel?.id);
    const panelInTabs = pp && p ? [pp[0] + p[0], pp[1] + p[1], p[2], p[3]] : null;
    return { style: nav?.props?.style, nav: r(id), kids: [panelInTabs], kidIds: [panel?.id], panelsRect: pp };
  }, navId);
  ...
  log("skia tabs", info.nav, "TabPanels(래퍼)", JSON.stringify(info.panelsRect), "TabPanel(Tabs 기준)", JSON.stringify(info.kids));
  ...
  // Skia: nav 는 자기 부모 기준이라 (0,0) 으로, TabPanel 은 이미 Tabs 기준. DOM 도 Tabs 기준.
  const skiaRel = [[0, 0, info.nav[2], info.nav[3]], ...info.kids];
  ...
  log(pass ? "PASS — Tabs 높이 · TabPanel (Tabs 기준) y/h Skia = DOM (Δ ≤ 1 — 수리 전 77/53 · y 41/29)" : "FAIL — rect 불일치");
