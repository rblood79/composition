...
await context.addInitScript(({ MODEL }) => {
  const profile = { provider: "openai-compatible", baseUrl: "http://localhost:11434/v1", model: MODEL };
  localStorage.setItem("composition.ai.profiles", JSON.stringify({ main: profile, planner: profile, executor: profile }));
  window.__llmBodies = [];
  const orig = window.fetch.bind(window);
  window.fetch = async (input, init) => {
    const url = typeof input === "string" ? input : input?.url ?? "";
    if (url.includes("/chat/completions") && init?.body) {
      try { window.__llmBodies.push(JSON.parse(init.body)); } catch { window.__llmBodies.push({ raw: String(init.body).slice(0, 200) }); }
    }
...
const createVariable = async (name, defaultValue) => {
  ...
  const defaultInput = editor.locator("fieldset", { has: page.locator('legend:has-text("Default Value")') }).locator("input").first();
...
const readProjectVarRows = () => page.evaluate(() => JSON.stringify([...document.querySelectorAll('[data-variable-group="project"] .list-item')].map((e) => e.textContent)));

try {
  const ps = await fetch(`${OLLAMA}/api/ps`).then((r) => r.json()).catch(() => null);
  log("ollama ps", JSON.stringify(ps?.models?.map((m) => ({ name: m.name, ctx: m.context_length })) ?? ps));
  ...
  await pollUntil(() => stateSection.locator('.state-def[data-implicit="isSelected"] .state-def-name').textContent().catch(() => null), (t) => t === "agree", 8000, 200);

  // AI 패널 → 전송
  await setPanel(page, "ai", true);
  const textarea = page.locator(".ai-composer textarea");
  await textarea.waitFor({ timeout: 10_000 });
  await textarea.fill("list_variables 도구를 호출해서 이 프로젝트의 변수 목록을 읽고, 변수마다 이름과 소유자 종류(project/page/element)를 한 줄씩 알려줘. 다른 도구는 쓰지 마.");
  await page.locator('.ai-composer button[aria-label="질문 제출"], .ai-composer button[aria-label="Submit question"], .ai-composer button[type="submit"]').first().click();
  ...
  const toolResults = await pollUntil(
    () => page.evaluate(() => [...document.querySelectorAll(".tool-result-message")].map((e) => ({ label: e.querySelector(".tool-result-label")?.textContent ?? "", success: e.getAttribute("data-success") }))),
  ...
  record("② 요청 본문 tools 에 list_variables 정의 · 모델이 list_variables 를 호출 (패널 tool-result 행 · toolLabels 어휘)", hasDef && toolResults.some((r) => /변수 목록|variable list/i.test(r.label) && r.success === "true"), JSON.stringify({ toolsCount: toolNames.length, hasDef, toolResults, requests: bodies.length, elapsedMs }));

  // tool 결과 메시지 (role:"tool") — 후속 요청 본문에 실린다
  const toolMsgs = bodies.flatMap((b) => (b.messages ?? []).filter((m) => m.role === "tool")).map((m) => String(m.content ?? ""));
  const lvResult = toolMsgs.map((c) => { try { return JSON.parse(c); } catch { return null; } }).find((v) => v && JSON.stringify(v).includes('"count"') && JSON.stringify(v).includes('"agree"'));
  const rows = Array.isArray(lvResult) ? lvResult : Array.isArray(lvResult?.data) ? lvResult.data : Array.isArray(lvResult?.result) ? lvResult.result : null;
  ...
  record("③ tool 결과: count → owner project · agree → owner element (Checkbox) · usedBy 수 · 런타임 값 키 없음", !!countRow && countRow.owner?.kind === "project" && !!agreeRow && agreeRow.owner?.kind === "element" && agreeRow.owner?.elementType === "Checkbox" && typeof countRow.usedBy === "number" && !keys.some((k) => /^(value|runtimeValue|current)$/i.test(k)), JSON.stringify({ toolMsgs: toolMsgs.length, countRow, agreeRow, keys }).slice(0, 600));

  const assistant = await pollUntil(
    () => page.evaluate(() => [...document.querySelectorAll('.ai-message[data-role="assistant"] .ai-message-bubble')].map((e) => e.textContent ?? "").at(-1) ?? ""),
    (t) => /count/.test(t) && /agree/.test(t) && /project|프로젝트/i.test(t) && /element|요소/i.test(t), 120_000, 1500, // 스트리밍 중 조기 판정 방지 — 답변 완성분까지
  );
  record("① 답변에 count · agree 와 소유자 종류가 언급된다", /count/.test(assistant) && /agree/.test(assistant) && /project|프로젝트/i.test(assistant) && /element|요소/i.test(assistant), JSON.stringify(assistant.slice(0, 300)));

  await setPanel(page, "datatable", true);
  await page.locator(".datatable-panel .panel-tab").nth(2).click();
  await page.waitForTimeout(300);
  const varsAfter = await readProjectVarRows();
  const indexKinds = await page.locator('[data-variable-group="index"] .variable-index-item').evaluateAll((els) => els.map((e) => e.getAttribute("data-owner-kind")));
  record("④ 쓰기 0 — Data 탭 프로젝트 변수 목록 · 인덱스 (element 1) 가 호출 전과 같다", varsAfter === varsBefore && indexKinds.length === 1 && indexKinds[0] === "element", JSON.stringify({ varsBefore, varsAfter, indexKinds }));

  // 절단 여부 — tool 이 실린 요청을 stream:false 로 다시 보내 prompt_tokens 확인 (ollama 기본 num_ctx 4096 함정)
  if (toolBearing) {
    const usage = await fetch(`${OLLAMA}/v1/chat/completions`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ ...toolBearing, stream: false }) }).then((r) => r.json()).then((j) => j.usage).catch((e) => ({ error: String(e) }));
    const bytes = JSON.stringify(toolBearing).length;
    record("환경 — tool 실린 요청의 prompt_tokens 가 본문 크기에 걸맞다 (절단 없음)", (usage?.prompt_tokens ?? 0) > bytes / 8, JSON.stringify({ bytes, usage }));
  }
  record("native dialog 0", dialogs === 0, String(dialogs));
  record("page error 0", pageErrors.length === 0, pageErrors.join(" | ").slice(0, 200));
  writeFileSync(resolve(OUT_DIR, "llm-bodies.json"), JSON.stringify(bodies, null, 2));
  ...
  log(`done — ${findings.length - failed}/${findings.length} PASS → ${OUT_DIR}`);
