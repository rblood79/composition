// pagination-preview-chrome-live.mjs — renderPagination 의 catalogChrome 배선 후 Skia ↔ Preview DOM rect live (2026-09-18, ADR-223 후속 1).
//   사용: node apps/builder/scripts/pagination-preview-chrome-live.mjs  (dev 5173 · .auth-session.json · headed)
// Pagination root + Button 5 — Skia layout rect (부모 기준) vs Preview DOM rect · computed flex/space-between/gap
import { chromium } from "playwright";
import { resolve } from "node:path";
import { waitReady } from "./perf-baseline.mjs";
const BASE_URL = "http://localhost:5173";
const STORAGE_STATE = resolve("apps/builder/scripts/.auth-session.json");
const log = (...a) => console.log("[pagination live]", ...a);
...
async function addFromPalette(page, type) {
  ...
  const idx = n === 1 ? 0 : names.findIndex(l => l.replace(/\s+/g,"").toLowerCase()===type.toLowerCase() || l === "페이지네이션");
...
try {
  ...
  await input.fill(`pglive-${Date.now()}`); await input.press("Enter");
  await page.waitForURL(/\/builder\/[^/?]+$/, { timeout: 90000 }); await waitReady(page);
  const navId = await addFromPalette(page, "Pagination");
  ...
  const dom = await page.evaluate((ids) => {
    for (const f of document.querySelectorAll("iframe")) {
      ...
      const els = ids.map(id => d.querySelector(`nav[data-element-id^="${id}"], button[data-element-id^="${id}"]`) ?? d.querySelector(`[data-element-id^="${id}"]`)); if (els.some(e=>!e)) continue;
  ...
  // 폭은 iframe 을 페이지 폭으로 강제했으므로 root 폭도 비교한다 — space-between 의 마지막 버튼 x 가 폭에 종속.
  let pass = !!dom && skiaRel.every((s, i) => s && dom.rects[i] && s.every((v, k) => Math.abs(v - dom.rects[i][k]) <= 2.5));
  pass = pass && dom.cls.includes("react-aria-Pagination") && dom.computed.display === "flex" && dom.computed.justifyContent === "space-between" && dom.computed.gap === "8px" && dom.computed.alignItems === "center";
