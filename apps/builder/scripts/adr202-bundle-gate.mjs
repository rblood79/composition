...
const files = [...new Set([...aiClosure].map(key => manifest[key].file))].filter(file => !initialFiles.has(file));
const sum = (fn) => files.reduce((n, file) => n + fn(readFileSync(resolve(dist, file))), 0);
const before = { builder: beforeBuilder.initial.js.gzipBytes, preview: beforePreview.initial.js.gzipBytes };
const after = { builder: afterBuilder.initial.js.gzipBytes, preview: afterPreview.initial.js.gzipBytes };
const delta = { builder: after.builder - before.builder, preview: after.preview - before.preview };
const checks = {
  sameLockfile: [beforePreview, afterBuilder, afterPreview].every(value => value.revision.lockfileSha256 === beforeBuilder.revision.lockfileSha256),
  ...
  previewAbsolute: after.preview <= 675021,
  builderDelta: delta.builder <= 3.5 * 1024,
  previewDelta: delta.preview <= 0,
  budgetCurrent: new Date().toISOString().slice(0, 10) <= '2026-10-14',
  aiImplementationLazy: !initialFiles.has(manifest[aiEntry].file),
  commandLazy: !initialFiles.has(manifest[commandEntry].file),
};
const output = { before, after, delta, checks, firstOpen: { files, rawBytes: sum(buffer => buffer.length), gzipBytes: sum(buffer => gzipSync(buffer, { level: 9 }).length) }, pass: Object.values(checks).every(Boolean) };
