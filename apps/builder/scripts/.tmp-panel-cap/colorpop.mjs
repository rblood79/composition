...
await rec("base_rgba", ".property-color-popover .color-input-fields, .property-color-popover .color-input-number-field, .property-color-popover .color-input-number-field__scrub, .property-color-popover .color-input-number-field__label, .property-color-popover .color-input-number-field input", 8);
...
await rec("fill_color", `${P}, ${P} .fill-detail-popover, ${P} .fill-type-selector, ${P} .fill-type-selector legend, ${P} .fill-type-selector .react-aria-ToggleButtonGroup, ${P} .fill-type-selector .react-aria-ToggleButton, ${P} .color-picker-panel, ${P} .fill-detail-popover__opacity, ${P} .fill-detail-popover__opacity input, ${P} .fill-detail-popover__divider, ${P} .blend-mode, ${P} .blend-mode legend, ${P} .blend-mode .react-aria-Button, ${P} .blend-mode button`, 16);
...
await rec("fill_gradient", `${P}, ${P} .gradient-editor, ${P} .gradient-type-select, ${P} .gradient-type-select .react-aria-Button, ${P} .gradient-bar, ${P} .gradient-bar__handle, ${P} .gradient-editor .color-picker-panel, ${P} .gradient-editor__divider, ${P} .gradient-controls, ${P} .gradient-controls fieldset, ${P} .gradient-controls input, ${P} .gradient-stop-list, ${P} .gradient-stop-list__row, ${P} .gradient-stop-list__swatch, ${P} .gradient-stop-list__position, ${P} .gradient-stop-list__delete, ${P} .gradient-stop-list__add-btn`, 20);
await shot("pop-fill-gradient-linear", page.locator(P).first());
// Radial via Select
const gsel = page.locator(`${P} .gradient-type-select .react-aria-Button`).first();
await gsel.click(); await page.waitForTimeout(400);
await rec("gradient_select_menu", ".react-aria-Popover .react-aria-ListBox, .react-aria-Popover .react-aria-ListBoxItem", 6);
await page.getByRole("option", { name: "Radial" }).click(); await page.waitForTimeout(600);
...
await gsel.click(); await page.waitForTimeout(400); await page.getByRole("option", { name: "Mesh" }).click(); await page.waitForTimeout(700);
