...
// ── catalog 정적 참조 seam (ADR-916 P2-CAT ③) ──────────────────────────────────
//
// catalog 스냅샷은 per-instance(`LayoutEngine`)가 아니라 `catalog::CATALOG`
// thread_local static(조항 5)이라, 주입/조회를 **free function** 으로 노출한다
// (`LayoutEngine` 메서드 아님). wasm.rs seam 관습(wrapper 존재 ≠ flag 전환) 준수 —
// JS init 실배선(`initCompositionEngineWasm` 에서 이 export 호출)은 소비 phase
// P2-PROP 소유. 본 export 존재만으로는 live 영향 0(no-dormant-foundation-ahead-of-flip).

/// JS `serializeCatalogSnapshot` 출력 JSON → thread_local static 원자 주입.
///
/// # Errors
/// JSON 파싱 실패 시 `Err(JsValue)`(no-silent-drop — buildTreeBatch 관습과 동일).
#[wasm_bindgen(js_name = injectCatalogSnapshot)]
pub fn inject_catalog_snapshot(json: &str) -> Result<(), JsValue> {
    crate::catalog::inject_catalog_snapshot(json).map_err(|e| JsValue::from_str(&e))
}

/// 조항 5 — catalog 주입 여부. JS `isAvailable()` 2조건 중 `catalogInjected` 담당.
#[wasm_bindgen(js_name = isCatalogInjected)]
pub fn is_catalog_injected() -> bool {
    crate::catalog::is_catalog_injected()
}

/// (type×size) 조회 → `[fontSize, lineHeight, iconSize]` flat f32 with NaN sentinel.
///
/// 조항 2 fallback(`sizes[size] ?? sizes[defaultSize]`) 적용 결과. 미주입/미존재
/// type/미존재 size(default 없음) → 빈 배열(호출측 fail-loud 판정). partial metric 의
/// 미존재 key 는 `NaN`(JS 는 `Number.isNaN` 으로 None 판별 → 침묵 0 default 아님).
#[wasm_bindgen(js_name = lookupCatalogMetric)]
pub fn lookup_catalog_metric(component_type: &str, size: &str) -> Box<[f32]> {
    match crate::catalog::lookup_catalog_metric(component_type, size) {
        Some(m) => Box::new([
            m.font_size.unwrap_or(f32::NAN),
            m.line_height.unwrap_or(f32::NAN),
            m.icon_size.unwrap_or(f32::NAN),
        ]),
        None => Box::new([]),
    }
}
