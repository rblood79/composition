...
pub struct LayoutTree {
    ...
    mutation_gen: u64,
    /// min/max 크기에 intrinsic 키워드를 쓰는 노드 수. 0 인 일반 문서는 측정 순회를 건너뛴다.
    intrinsic_size_constraint_count: usize,
...
impl LayoutTree {
    ...
    fn alloc_handle(&mut self, node: TreeNode) -> usize {
        ...
        if has_intrinsic_size_constraint(&node.style) {
            self.intrinsic_size_constraint_count += 1;
        }
    ...
    pub fn update_style(&mut self, handle: usize, style: NodeStyle) {
        let Some(was_constrained) = self
            .get(handle)
            .map(|node| has_intrinsic_size_constraint(&node.style))
        else {
            return;
        };
        let is_constrained = has_intrinsic_size_constraint(&style);
        if was_constrained != is_constrained {
            if is_constrained {
                self.intrinsic_size_constraint_count += 1;
            } else {
                self.intrinsic_size_constraint_count -= 1;
    ...
    pub fn remove_node(&mut self, handle: usize) {
        if handle < self.nodes.len() && self.nodes[handle].is_some() {
            if self
                .get(handle)
                .map(|n| has_intrinsic_size_constraint(&n.style))
                .unwrap_or(false)
            {
                self.intrinsic_size_constraint_count -= 1;
            }
    ...
    pub fn clear(&mut self) {
        ...
        self.intrinsic_size_constraint_count = 0;
    ...
    pub fn compute_layout(&mut self, root: usize, available_width: f32, available_height: f32) {
        ...
        // min/max 크기의 intrinsic 키워드는 고정 길이가 아니라 자손의 intrinsic 크기다.
        // 길이 파서(resolve_dimension_opt)는 이 키워드를 None 으로 남기므로, 실제 solve 동안만
        // content 측정값을 px 제약으로 투영한다. 측정할 때는 자기 width/min/max 를 걷어
        // 명시 width(예: 20px)가 intrinsic 폭으로 잘못 보고되지 않게 한다.
        let intrinsic_size_constraints = if self.intrinsic_size_constraint_count > 0 {
            self.materialize_intrinsic_size_constraints(root, available_width)
        } else {
            Vec::new()
        };
        ...
        // flex/grid 분배 뒤 폭이 달라진 컨테이너만 확정 폭으로 높이 제약을 다시 잰다.
        // 폭과 무관한 leaf 스칼라는 TS 재측정 경로가 소유한다. 무조건 반복 solve 하지 않는다.
        if self.refresh_intrinsic_height_constraints(&intrinsic_size_constraints) {
            self.solve_node(root, solve_w, available_height);
        }
        self.fixup_root_self_size(root, available_width, available_height);
        for original in intrinsic_size_constraints {
            if let Some(node) = self.get_mut(original.handle) {
                node.style.min_width = original.min_width;
                node.style.max_width = original.max_width;
                node.style.min_height = original.min_height;
                node.style.max_height = original.max_height;
                node.intrinsic_w = None;
            }
        }
    }

    /// intrinsic 제약의 측정/복원은 공통 순회로 처리한다. 폭은 min/max-content,
    /// 높이는 해당 폭에서 auto block-size 를 측정하며 두 축의 측정 공식은 분리한다.
    fn materialize_intrinsic_size_constraints(
        &mut self,
        root: usize,
        available_width: f32,
    ) -> Vec<IntrinsicConstraintRestore> {
        let mut stack = vec![(root, false, available_width)];
        let mut restore = Vec::new();
        while let Some((handle, visited, containing_width)) = stack.pop() {
            if !visited {
                ...
                if let Some(node) = self.get(handle) {
                    ...
                    for &child in node.children.iter().rev() {
                        stack.push((child, false, child_width));
                    }
                }
                continue;
            }
            let Some(node) = self.get(handle) else { continue };
            if !has_intrinsic_size_constraint(&node.style) {
                continue;
            }
            let mut original = IntrinsicConstraintRestore {
                handle,
                min_width: node.style.min_width.clone(),
                max_width: node.style.max_width.clone(),
                min_height: node.style.min_height.clone(),
                max_height: node.style.max_height.clone(),
                containing_width,
                measured_width: None,
            };
            ...
            if width_constrained {
                ...
                let (min_content, max_content) = self.measure_intrinsic_width(handle).unwrap_or((0.0, 0.0));
                let min = min_content + pad_border_w;
                let max = max_content + pad_border_w;
                let fit = containing_width.clamp(min, max);
                if let Some(node) = self.get_mut(handle) {
                    node.style.width = original_width;
                    node.style.min_width = materialized_constraint(&original.min_width, min, max, fit);
                    node.style.max_width = materialized_constraint(&original.max_width, min, max, fit);
                    node.intrinsic_w = None;
                }
            }
    ...
    /// 블록 축 intrinsic 크기는 내용의 auto 높이다. 측정 동안 자기 height/min/max만 제거한다.
    /// TS leaf는 contentHeight, 컨테이너는 기존 block/flex/grid solve의 내용 높이를 쓴다.
    fn measure_intrinsic_height(
        &mut self, handle: usize, containing_width: f32, used_width: Option<f32>,
    ) -> (f32, f32) {
        let Some(node) = self.get(handle) else { return (0.0, 0.0) };
        let width = node.style.width.clone();
    ...
    fn refresh_intrinsic_height_constraints(&mut self, originals: &[IntrinsicConstraintRestore]) -> bool {
        ...
        // postorder: 자손 제약을 먼저 갱신해야 부모의 내용 높이가 정확하다.
        for original in originals {
            let Some(measured_width) = original.measured_width else { continue };
            let Some(node) = self.get(original.handle) else { continue };
            if node.children.is_empty()
                || (!node.subtree_dirty && (node.layout.width - measured_width).abs() <= 0.5)
            {
                continue;
            }
            ...
            if let Some(node) = self.get_mut(original.handle) {
                ...
                node.style.min_height = min;
                node.style.max_height = max;
            }
            self.propagate_dirty(original.handle);
            changed = true;
        }
        changed
    }
    ...
    fn col_contribution(
        ...
        if let Some((mn, mx)) = self.measure_intrinsic_width(c) {
            ...
            let (mx, _) = self.track_contribution(c, mx + pb_h, 0.0, container_w, container_h);
            return ((mn + m_h).min(mx + m_h), mx + m_h, false);
        }
        let (cw, ch) = self.solve_node(c, container_w, container_h);
        let (cw, _) = self.track_contribution(c, cw + pb_h, ch, container_w, container_h);
        (cw + m_h, cw + m_h, true)
    }
    ...
    fn child_auto_block_pad_border(&self, c: usize, container_w: f32, container_h: f32) -> f32 {
        let Some(n) = self.get(c) else { return 0.0 };
        let h = n.style.height.as_deref();
        let is_auto = resolve_dimension_opt(h, &self.ctx_for(container_h)).is_none()
            && !size_is_intrinsic_keyword(h);
        if is_auto {
            axis_pad_border(&n.style, &self.ctx_for(container_w.max(0.0)), false)
        } else {
            0.0
        }
    }
    ...
    /// pass 에서 확정 폭으로 풀면 auto 트랙이 부풀어 Chrome 과 갈린다 (실측 shrink-to-fit grid
    /// + `paddingLeft:10%` 자식: Chrome 120 / 확정 기준 132).
    fn child_auto_inline_pad_border(&self, c: usize, container_w: f32) -> f32 {
        let Some(n) = self.get(c) else { return 0.0 };
        let ctx = self.ctx_for(container_w.max(0.0));
    ...
    fn resolve_leaf_intrinsic_width(
        ...
        match sentinel {
            ...
            Some(n) if n == MIN_CONTENT => bb(min_c),
            Some(n) if n == MAX_CONTENT => bb(max_c),
            Some(n) if n == FIT_CONTENT => {
                ...
                bb(stretch_fit.clamp(min_c, max_c))
            }
            // auto (None) / 해석 불가 — content 제안값 = max-content, **content-box 로 보고**.
            ...
            _ if intrinsic_mode(avail_w) == Some(IntrinsicMode::Min) => {
                (min_c, min_c + pad_border_h)
            }
            _ => (max_c, max_c + pad_border_h),
        }
    }
...
/// 한 번의 solve 동안만 치환한 제약과 높이 측정 문맥. 저작 선언은 완료 뒤 복원한다.
struct IntrinsicConstraintRestore {
    handle: usize,
...
fn materialized_constraint(value: &Option<String>, min: f32, max: f32, fit: f32) -> Option<String> {
    match value.as_deref().map(str::trim) {
        Some(v) if v.eq_ignore_ascii_case("min-content") => Some(format!("{min}px")),
        Some(v) if v.eq_ignore_ascii_case("max-content") => Some(format!("{max}px")),
        Some(v) if v.eq_ignore_ascii_case("fit-content") => Some(format!("{fit}px")),
        _ => value.clone(),
    }
}

/// intrinsic 키워드는 auto와 달리 align/justify-self stretch 대상에서 빠진다.
...
mod tests {
    ...
    #[test]
    fn padded_scalar_leaf_in_flex_row_is_content_plus_padding_once() {
        let (t, h) = solve(
            r#"[
            {"style":{"width":"auto","height":"20px","paddingLeft":"12px","paddingRight":"8px","contentMinWidth":40,"contentMaxWidth":80},"children":[]},
            {"style":{"display":"flex","flexDirection":"row","width":"400px"},"children":[0]}
        ]"#,
            1, 400.0, -1.0,
        );
        assert_eq!(t.get_layout(h[0]).width, 100.0, "80 + 12 + 8 (구 엔진 120)");
    }

    /// 대조 — padding 있는 **컨테이너** (auto 폭) 도 flex row 에서 content + padding 한 번.
    #[test]
    fn padded_auto_container_in_flex_row_is_content_plus_padding_once() {
        let (t, h) = solve(
            r#"[
            {"style":{"width":"50px","height":"20px"},"children":[]},
            {"style":{"display":"flex","flexDirection":"row","paddingLeft":"10px","paddingRight":"10px"},"children":[0]},
            {"style":{"display":"flex","flexDirection":"row","width":"400px"},"children":[1]}
        ]"#,
            2, 400.0, -1.0,
        );
        assert_eq!(t.get_layout(h[1]).width, 70.0, "50 + 10 + 10");
    }

    /// grid `auto` 트랙 기여 — padded leaf 와 padded 컨테이너 모두 border-box (+ margin) 기여.
    #[test]
    fn grid_auto_track_contribution_is_border_box_for_leaf_and_container() {
        let (t, h) = solve(
            r#"[
            {"style":{"width":"auto","height":"20px","paddingLeft":"12px","paddingRight":"8px","contentMinWidth":40,"contentMaxWidth":80},"children":[]},
            {"style":{"display":"grid","gridTemplateColumns":["auto","1fr"],"width":"400px"},"children":[0]}
        ]"#,
            1, 400.0, -1.0,
        );
        assert_eq!(t.get_layout(h[0]).width, 100.0, "leaf: auto 트랙 = 80 + 20");
        let (t, h) = solve(
            r#"[
            {"style":{"width":"50px","height":"20px"},"children":[]},
            {"style":{"display":"flex","flexDirection":"row","paddingLeft":"10px","paddingRight":"10px"},"children":[0]},
            {"style":{"display":"grid","gridTemplateColumns":["auto","1fr"],"width":"400px"},"children":[1]}
        ]"#,
            2, 400.0, -1.0,
        );
        assert_eq!(t.get_layout(h[1]).width, 70.0, "컨테이너: auto 트랙 = 50 + 20");
    }

    /// grid `justify-items: start` — auto 폭 자식의 배치 폭도 border-box (leaf · 컨테이너).
    #[test]
    fn grid_justify_start_uses_border_box_for_auto_width_leaf_and_container() {
        let (t, h) = solve(
            r#"[
            {"style":{"width":"auto","height":"20px","paddingLeft":"12px","paddingRight":"8px","contentMinWidth":40,"contentMaxWidth":80},"children":[]},
            {"style":{"display":"grid","gridTemplateColumns":["300px"],"justifyItems":"start","width":"400px"},"children":[0]}
        ]"#,
            1, 400.0, -1.0,
        );
        ...
        let (t, h) = solve(
            ...
            {"style":{"width":"50px","height":"20px"},"children":[]},
            {"style":{"display":"flex","flexDirection":"row","paddingLeft":"10px","paddingRight":"10px"},"children":[0]},
            {"style":{"display":"grid","gridTemplateColumns":["300px"],"justifyItems":"start","width":"400px"},"children":[1]}
        ]"#,
            2, 400.0, -1.0,
        );
        assert_eq!(t.get_layout(h[1]).width, 70.0, "컨테이너 50 + 20");
    }

    /// block `width: fit-content` 부모 안 padded leaf — shrink-to-fit 폭도 한 번만.
    #[test]
    /// 세로 축 대조 — grid auto 행 안 padded 컨테이너 (auto 높이) 의 행 높이 · align start 배치.
    #[test]
    fn grid_auto_row_padded_container_block_axis() {
        let (t, h) = solve(
            r#"[
            {"style":{"width":"50px","height":"20px"},"children":[]},
            {"style":{"display":"flex","flexDirection":"row","paddingTop":"10px","paddingBottom":"10px"},"children":[0]},
            {"style":{"display":"grid","gridTemplateColumns":["100px"],"width":"400px"},"children":[1]}
        ]"#,
            2, 400.0, -1.0,
        );
        assert_eq!(t.get_layout(h[1]).height, 40.0, "auto 행 = 20 + 20");
        assert_eq!(t.get_layout(h[2]).height, 40.0, "grid 높이 = 행");
        let (t, h) = solve(
            r#"[
            {"style":{"width":"50px","height":"20px"},"children":[]},
            {"style":{"display":"flex","flexDirection":"row","paddingTop":"10px","paddingBottom":"10px"},"children":[0]},
            {"style":{"display":"grid","gridTemplateColumns":["100px"],"gridTemplateRows":["100px"],"alignItems":"start","width":"400px"},"children":[1]}
        ]"#,
            2, 400.0, -1.0,
        );
    ...
    fn padded_scalar_leaf_in_fit_content_block_parent() {
        let (t, h) = solve(
            r#"[
            {"style":{"width":"auto","height":"20px","paddingLeft":"12px","paddingRight":"8px","contentMinWidth":40,"contentMaxWidth":80},"children":[]},
            {"style":{"display":"flex","flexDirection":"row","width":"fit-content"},"children":[0]},
    ...
    #[test]
    fn intrinsic_width_constraints_follow_content_mutation_and_keep_authored_style() {
        let mut tree = LayoutTree::new();
        let child = tree.create_node(
            serde_json::from_str(r#"{"width":"100px","height":"10px"}"#).unwrap(),
        );
        let owner = tree.create_node(
            serde_json::from_str(r#"{"width":"20px","height":"20px","paddingLeft":"10px","paddingRight":"10px","minWidth":"min-content"}"#).unwrap(),
        );
        let root = tree.create_node(serde_json::from_str(r#"{"width":"320px"}"#).unwrap());
        tree.set_children(owner, vec![child]);
        tree.set_children(root, vec![owner]);

        tree.compute_layout(root, 320.0, 400.0);
        assert_eq!(tree.get_layout(owner).width, 120.0);
        assert_eq!(
            tree.get(owner).unwrap().style.min_width.as_deref(),
            Some("min-content")
        );
        tree.compute_layout(root, 320.0, 400.0);
        assert_eq!(tree.get_layout(owner).width, 120.0);
        ...
        tree.update_style(
            child,
            serde_json::from_str(r#"{"width":"150px","height":"10px"}"#).unwrap(),
        );
        tree.compute_layout(root, 320.0, 400.0);
        assert_eq!(tree.get_layout(owner).width, 170.0);

        tree.update_style(
            owner,
            serde_json::from_str(r#"{"width":"250px","height":"20px","paddingLeft":"10px","paddingRight":"10px","maxWidth":"fit-content"}"#).unwrap(),
        );
        tree.compute_layout(root, 320.0, 400.0);
        assert_eq!(tree.get_layout(owner).width, 170.0);
        assert_eq!(
            tree.get(owner).unwrap().style.max_width.as_deref(),
            Some("fit-content")
        );

        tree.update_style(
            owner,
            serde_json::from_str(r#"{"width":"250px","height":"20px"}"#).unwrap(),
        );
        assert_eq!(tree.intrinsic_size_constraint_count, 0);
        tree.compute_layout(root, 320.0, 400.0);
        assert_eq!(tree.get_layout(owner).width, 250.0);
    }

    #[test]
    fn intrinsic_height_constraint_follows_flex_used_width_and_content_mutation() {
        let mut tree = LayoutTree::new();
        let handles = tree.build_tree_batch(r#"[
            {"style":{"width":"60px","height":"20px","flexShrink":0},"children":[]},
            {"style":{"width":"60px","height":"20px","flexShrink":0},"children":[]},
            {"style":{"display":"flex","flexWrap":"wrap","width":"200px","height":"10px","minWidth":"0px","minHeight":"min-content"},"children":[0,1]},
            {"style":{"display":"flex","width":"100px"},"children":[2]}
        ]"#).unwrap();
        for _ in 0..2 {
            tree.compute_layout(handles[3], 100.0, 400.0);
            assert_eq!(tree.get_layout(handles[2]).width, 100.0);
            assert_eq!(tree.get_layout(handles[2]).height, 40.0);
            let style = &tree.get(handles[2]).unwrap().style;
            assert_eq!(style.width.as_deref(), Some("200px"));
            assert_eq!(style.height.as_deref(), Some("10px"));
            assert_eq!(style.min_height.as_deref(), Some("min-content"));
        }
        tree.update_style(handles[0], serde_json::from_str(
            r#"{"width":"60px","height":"50px","flexShrink":0}"#,
        ).unwrap());
        tree.compute_layout(handles[3], 100.0, 400.0);
        assert_eq!(tree.get_layout(handles[2]).height, 70.0);
    }
