...
- ADR-142 의 spec 이행(theme/tokens 저작 / `PrimitiveBinding` 작성 / reusable 문서)에서
  **필수 선행 입력**으로 쓰인다 — token 추출 규칙(§2), 상태 규칙(§3), 유틸 패턴(§4),
  구조 패턴(§5)을 spec 이행 전 확정 입력으로 제공한다.
...
레이아웃 변수**다. 본 문서의 token 표(§2)는 후자를 포함하지 않는다.

- design token: `theme.css` / `utilities.css` 가 정의 — §2~§4 의 대상.
...
축: **color / radius / spacing / typography / motion / elevation**. 정의 원천은
`theme.css` (전역) + `utilities.css` (유틸 진입점).
...
> 미사용 토큰. theme/tokens 저작 시 채택 보류 후보(§6).
...
완전 둥근 형태(`9999px`)는 토큰이 아니라 반복 리터럴이다 → **pill 패턴**(§5.1).
...
- `.button-base` / `.indicator` / `.inset`: 다중 inset shadow 레시피(§4). 라이트/다크에서
  하이라이트·그림자 방향이 반전.
- 흰색/검정 alpha 오버레이(`rgb(255 255 255/.N)`, `rgb(0 0 0/.N)`)는 specular highlight·
  inner shadow 용 — 토큰화 안 됨, 레시피는 `utilities.css`(§4).
...
### 3.3 hover / pressed / disabled

- hover/pressed 는 유틸 패턴(§4)의 진입점 변수(`--button-color` 등)나 시맨틱 `-hover`/
  `-pressed` 토큰으로 파생.
...
token 으로 정규화되지 않았지만 starter/src 전반에 **반복되는 구조 패턴**. theme/tokens
저작 시 명명된 규칙으로 흡수할 후보다.
...
### 6.1 채택 — starter upstream 귀속 항목

starter 업데이트로 다시 들어올 항목. theme/tokens 가 starter 에서 가져온다.

- §2 token taxonomy (color OKLCH 모델 / radius / spacing / typography / motion / elevation)
- §3 상태 토큰 규칙 (`{prop}-hover/-pressed/-disabled` 네이밍, focus-visible 전역 규칙)
- §4 유틸 패턴 (button-base / indicator / inset 의 진입점 + 파생 메커니즘)
- §5 구조 패턴 (pill / group-seam / overlay-size / field-height)

### 6.2 제외 — composition 고유 디자인 언어 (의도적 발산)
...
- composition 시맨틱 변수 체계 — `--bg-*` / `--fg-*` / `--border-*` / `--accent-*`
  ...
  `--field-background`)을 그대로 채택하지 않고 §7 매핑을 거친다.
...
- 회색지대 → §7 매핑 표에 starter rule 과 composition 시맨틱을 병기해 판정 보류.

---

## 7. 매핑 표 — starter rule → composition token semantic → ADR-142 반영 대상
...
| starter rule (§2~§5)                      | composition 시맨틱 (css-tokens.md)         | ADR-142 반영 대상                                        |
| ----------------------------------------- | ------------------------------------------ | -------------------------------------------------------- |
...
| `--font-size*`                            | typography 토큰                            | theme/tokens root collection                             |
| §3 focus-visible 전역 규칙                | focus 상태 규칙                            | theme/tokens (generic 렌더러 상태 CSS)                   |
| §4 `.button-base` 유틸                    | 버튼 시각 패턴                             | theme/tokens + `PrimitiveBinding`(Button)                |
| §4 `.indicator` 유틸                      | 인디케이터 패턴                            | theme/tokens + `PrimitiveBinding`(Checkbox/Radio/Switch) |
| §4 `.inset` 유틸                          | 입력 inset 패턴                            | theme/tokens + `PrimitiveBinding`(field 계열)            |
| §5.1 pill (`9999px`)                      | radius full/pill 단계                      | theme/tokens root collection                             |
| §5.2 group-seam                           | 그룹 컨테이너 구조 규칙                    | reusable canonical 문서 (그룹 컴포넌트)                  |
| §5.4 field-height (`--spacing-8`)         | control-height 규칙                        | theme/tokens + `PrimitiveBinding`                        |
| §5.3 overlay-size                         | (판정 보류 — §6.3)                         | 별도 판정                                                |
...
- **토큰/유틸 (2)**: `theme.css`, `utilities.css` — 본 문서 §2~§4 의 정의 원천.
...
> 스냅샷이 바뀌면 §2~§5 의 token/패턴을 재추출하고 §7 매핑을 갱신한다. 본 문서는 그
