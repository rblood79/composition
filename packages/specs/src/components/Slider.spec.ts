...
export const SliderSpec: ComponentSpec<SliderProps> = {
  ...
  propagation: {
    rules: [
      ...
      // SliderThumb 은 손자(Slider → SliderTrack → SliderThumb)이므로 배열 childPath
      //   로 명시 (string 은 직계 자식만 매칭 → DEAD). Select 의
      //   ["SelectTrigger","SelectValue"] 와 동형 — propagationEngine 단계별 순회 지원.
      {
        parentProp: "size",
