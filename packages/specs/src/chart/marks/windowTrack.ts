...
/**
 * 트랙 막대 (`grid` 토큰 채움) + 창 시작 thumb (`axis` 토큰 채움). 두 마크 모두 `fillRole` 이라
 * 시리즈 팔레트를 쓰지 않는다 — 데이터가 아니라 컨트롤의 자리다.
 */
export function buildWindowTrackMarks(track: Rect): PathMark[] {
  ...
  const thumb: PathMark = {
    kind: "path",
    d: circlePath(track.x, cy, r),
    bbox: { x: track.x - r, y: cy - r, w: r * 2, h: r * 2 },
    fillRole: "axis",
  };
  return [bar, thumb];
}
