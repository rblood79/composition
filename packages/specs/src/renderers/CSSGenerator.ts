...
export function emitContainerStyles(c: ContainerStylesSchema): string[] {
  ...
  // 컨테이너 root elevation — states / indicatorMode 와 동일하게 토큰 해석을 거친다
  //   (ADR-166 Phase 2). `{shadow.md}` → `var(--shadow-md)`, raw CSS 는 그대로 통과.
  if (c.boxShadow) {
...
function generateChartVariables<Props>(spec: ComponentSpec<Props>): string[] {
  ...
  // ADR-217 — 기준선 토큰 (Skia 는 `chart.reference` 를 같은 자리에서 읽는다).
  lines.push(
    `  --chart-reference: ${tokenToCSSVar((chart.reference ?? CHART_REFERENCE_FALLBACK_TOKEN) as TokenRef)};`,
  );
