/**
 * Page URL — Nested Routes & Slug System (preview 라우터 · publish navigate 공용 SSOT)
 *
 ...
 * 4. 그 외 → "/" + Page.slug
 *
 * 2026-09-20: `apps/builder/src/utils/urlGenerator.ts` 에서 shared 로 이동. preview 는 이
 * URL 을 react-router 라우트로 쓰고, publish 는 `resolvePageIdByPath` 로 navigate 규칙의
 * path 를 같은 표에 대조한다 — 해석이 두 벌이면 같은 path 가 preview 에서만 (또는
 * publish 에서만) 통한다.
 */
...
// ============================================
// Path → Page 해석 (publish navigate)
// ============================================

export interface ResolvePageIdByPathOptions<P extends UrlPage> {
  /** 페이지에 적용된 layout 의 slug (preview 의 frame binding 경로). 없으면 rule 2 생략 */
  layoutSlugOf?: (page: P) => string | null | undefined;
}

/**
 * 매칭용 정규화 — react-router 기본값 (trailing slash 허용 · caseSensitive=false) 과
 * 같은 폭으로 맞춘다. query/hash 는 페이지 판정에 안 쓴다.
 */
...
/**
 * navigate path 가 가리키는 페이지 id. preview `CanvasRouter` 가 `generatePageUrl` 로 만든
 * 라우트 표와 같은 순서 (정적 우선 · 세그먼트 많은 것 우선) 로 대조한다.
 *
 * @returns 매칭된 페이지 id, 없으면 null
 */
export function resolvePageIdByPath<P extends UrlPage>(
  path: string,
  pages: readonly P[],
  options: ResolvePageIdByPathOptions<P> = {},
): string | null {
  const target = normalizePathForMatch(path);
  const allPages: UrlPage[] = [...pages];
  const routes = pages
    .map((page) => {
      const layoutSlug = options.layoutSlugOf?.(page);
      const url = generatePageUrl({
        page,
        layout: layoutSlug ? { slug: layoutSlug } : null,
        allPages,
      });
      return { pageId: page.id, url, isDynamic: hasDynamicParams(url) };
    })
    .sort((a, b) => {
  ...
  for (const route of routes) {
    const pattern = normalizePathForMatch(route.url);
    if (!route.isDynamic) {
      if (pattern === target) return route.pageId;
      continue;
    }
    // 동적 세그먼트 이름은 lowercase 돼도 `:name` 패턴 자체는 유지된다
    if (matchDynamicUrl(pattern, target)) return route.pageId;
  }
  return null;
}
