 ...
 */
export function applyCollectionItemsMigration(elements) {
    ...
    const selectItemChildrenByParent = new Map();
    const comboBoxItemChildrenByParent = new Map();
    const listBoxItemChildrenByParent = new Map();
    // ADR-097: TagGroup 2단 이전 — Tag 는 TagList (중간 컨테이너) 의 자식이므로
    //   parent_id 가 TagList ID. TagList 자체는 TagGroup 의 자식으로 유지.
    const tagChildrenByTagListId = new Map();
    const tagListIdToTagGroupId = new Map();
    for (const el of elements) {
        if (!el.parent_id)
            continue;
    ...
    const hasAnyWork = selectItemChildrenByParent.size > 0 ||
    ...
    const tagListNeedsItemsPropagation = [];
    if (!hasAnyWork) {
        const tagGroupById = new Map();
        const tagListsByTagGroupId = new Map();
        for (const el of elements) {
            if (el.type === "TagGroup")
                tagGroupById.set(el.id, el);
        }
        for (const el of elements) {
        ...
        for (const [tagGroupId, tg] of tagGroupById) {
            const tgItems = (tg.props ?? {}).items;
            if (!Array.isArray(tgItems) || tgItems.length === 0)
                continue;
            const tls = tagListsByTagGroupId.get(tagGroupId) ?? [];
            for (const tl of tls) {
                const tlItems = (tl.props ?? {}).items;
                if (Array.isArray(tlItems) && tlItems.length > 0)
                    continue;
    ...
    const listBoxAbsorbParents = new Set();
    for (const [parentId, lbiChildren] of listBoxItemChildrenByParent) {
        ...
        if (!anyTemplate)
            listBoxAbsorbParents.add(parentId);
    }
    ...
    for (const [tagListId, tagChildren] of tagChildrenByTagListId) {
        if (!tagListIdToTagGroupId.has(tagListId))
            continue;
    ...
    for (const el of elements) {
        ...
        if (el.type === "ListBox" && listBoxAbsorbParents.has(el.id)) {
            ...
            const items = listBoxItemChildrenToItemsArray(lbiChildren, childrenByParent);
            const nextProps = {
                ...(el.props ?? {}),
                items,
            };
            ...
            if (Array.isArray(rawSelectedIndices) &&
                ...
                const keys = rawSelectedIndices
                    .map((idx) => (typeof idx === "number" ? items[idx]?.id : undefined))
                    .filter((k) => typeof k === "string");
                if (keys.length > 0)
                    nextProps.selectedKeys = keys;
            }
 ...
 */
export function applySelectComboBoxMigration(elements) {
...
function collectSubtreeIds(rootId, childrenByParent, target) {
    const stack = [rootId];
...
export function selectItemChildrenToItemsArray(selectItemChildren) {
    return selectItemChildren.map((child) => {
        const p = child.props ?? {};
        return {
            id: child.id,
            label: typeof p.label === "string" && p.label.length > 0 ? p.label : child.id,
...
export function comboBoxItemChildrenToItemsArray(comboBoxItemChildren) {
 ...
 */
export function listBoxItemChildrenToItemsArray(lbiChildren, childrenByParent) {
    return lbiChildren.map((lbi) => {
        ...
        return {
            id: lbi.id,
            label: typeof p.label === "string" && p.label.length > 0
                ? p.label
            ...
            textValue: typeof p.textValue === "string" ? p.textValue : undefined,
            isDisabled: p.isDisabled === true || undefined,
            description: typeof p.description === "string" ? p.description : descFromChild,
            href: typeof p.href === "string" && p.href ? p.href : undefined,
        };
    });
}
function extractStringChildren(el) {
    if (!el)
        return undefined;
 ...
 */
export function tagChildrenToItemsArray(tagChildren) {
    return tagChildren.map((type) => {
        const p = type.props ?? {};
        const labelFromChildren = typeof p.children === "string" && p.children.length > 0
            ? p.children
            : undefined;
        return {
            id: type.id,
            ...
            isDisabled: p.isDisabled === true || undefined,
            allowsRemoving: typeof p.allowsRemoving === "boolean" ? p.allowsRemoving : undefined,
        };
    });
}
//# sourceMappingURL=migrateCollectionItems.js.map