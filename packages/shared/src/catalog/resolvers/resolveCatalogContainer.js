...
export function resolveCatalogStructure(type, doc) {
 ...
 */
export function resolveCatalogContainerBase(type, doc) {
    const rule = resolveComponentRule(type, doc);
    if (!rule)
        return {};
    const merged = {};
    const composition = rule.structure?.composition;
 ...
 */
export function resolveCatalogContainerVariants(type, props, doc) {
    const rule = resolveComponentRule(type, doc);
    const variants = rule?.containerVariants ??
        rule?.structure?.composition?.containerVariants;
    if (!variants)
    ...
    for (const [dataAttr, valueMap] of Object.entries(variants)) {
        ...
        const propEntry = Object.entries(props).find(([propKey]) => camelToKebab(propKey) === dataAttr);
        if (!propEntry)
            continue;
 ...
 */
export function resolveCatalogSizeField(type, size, field, doc) {
    const rule = resolveComponentRule(type, doc);
    return rule?.sizes?.[size]?.[field];
}
//# sourceMappingURL=resolveCatalogContainer.js.map