 ...
 */
export const UNIVERSAL_STYLE_CONTRACTS = {
    // typography
 ...
 */
function deriveOptions(contract, rule) {
    if (contract.options)
 ...
 */
export function resolveEditContract(node, doc) {
    ...
    const accepts = entry?.kind === "primitive" ? entry.binding.props.accepts : undefined;
    if (accepts) {
    ...
    const { base } = resolveMergedStyle(node, doc);
    for (const [key, contract] of Object.entries(UNIVERSAL_STYLE_CONTRACTS)) {
        ...
        const baseValue = base && Object.hasOwn(base, key)
            ? base[key]
            : undefined;
        fields.push({
            key,
...
//# sourceMappingURL=resolveEditContract.js.map