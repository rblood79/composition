...
import { toRuntimeCollection, resolveCollectionSnapshot } from "../collectionSnapshot";
...
const table = {
  ...
  name: "Users",
  project_id: "p",
  schema: [{ id: "f-name", key: "name", type: "string", label: "Name", required: true }],
  mockData: [{ name: "a" }],
  runtimeData: [{ name: "rt" }],
  useMockData: false,
...
describe("ExportedProjectSchema — schema[].id 통과", () => {
  it("검증 결과 (strip) 에 필드 id 가 남는다", () => {
    ...
    expect(result.data?.collections?.[0].schema?.[0]).toMatchObject({ id: "f-name", key: "name" });
  });
});
