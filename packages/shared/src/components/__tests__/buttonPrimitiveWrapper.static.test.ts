...
describe("ADR-142 Button primitive wrapper boundary", () => {
  ...
  it("uses catalog toRacProps as the ColorSwatch prop projection source", () => {
    const source = fs.readFileSync(
      new URL("../ColorSwatch.tsx", import.meta.url),
      "utf8",
    );

    expect(source).toContain("toColorSwatchRacProps");
    expect(source).toContain("../catalog/outputs/toRacProps");
  });

  it("uses catalog toRacProps as the ColorArea prop projection source", () => {
    const source = fs.readFileSync(
      new URL("../ColorArea.tsx", import.meta.url),
      "utf8",
    );

    expect(source).toContain("toColorAreaRacProps");
    expect(source).toContain("../catalog/outputs/toRacProps");
  });

  it("uses catalog toRacProps as the ColorSlider prop projection source", () => {
    const source = fs.readFileSync(
      new URL("../ColorSlider.tsx", import.meta.url),
      "utf8",
    );

    expect(source).toContain("toColorSliderRacProps");
    expect(source).toContain("../catalog/outputs/toRacProps");
  });

  it("uses catalog toRacProps as the ColorWheel prop projection source", () => {
    const source = fs.readFileSync(
      new URL("../ColorWheel.tsx", import.meta.url),
      "utf8",
    );

    expect(source).toContain("toColorWheelRacProps");
    expect(source).toContain("../catalog/outputs/toRacProps");
  });
  ...
  it("uses catalog toRacProps as the RangeCalendar prop projection source", () => {
    const source = fs.readFileSync(
      new URL("../RangeCalendar.tsx", import.meta.url),
      "utf8",
    );

    expect(source).toContain("toRangeCalendarRacProps");
    expect(source).toContain("../catalog/outputs/toRacProps");
  });
  ...
  it("uses catalog toRacProps as the DropZone prop projection source", () => {
    const source = fs.readFileSync(
      new URL("../DropZone.tsx", import.meta.url),
      "utf8",
    );

    expect(source).toContain("toDropZoneRacProps");
    expect(source).toContain("../catalog/outputs/toRacProps");
  });

  it("uses catalog toRacProps as the Tooltip prop projection source", () => {
    const source = fs.readFileSync(
      new URL("../Tooltip.tsx", import.meta.url),
      "utf8",
    );

    expect(source).toContain("toTooltipRacProps");
    expect(source).toContain("../catalog/outputs/toRacProps");
  });

  it("uses catalog toRacProps as the Dialog prop projection source", () => {
    const source = fs.readFileSync(
      new URL("../Dialog.tsx", import.meta.url),
      "utf8",
    );

    expect(source).toContain("toDialogRacProps");
    expect(source).toContain("../catalog/outputs/toRacProps");
  });

  it("uses catalog toRacProps as the Popover prop projection source", () => {
    const source = fs.readFileSync(
      new URL("../Popover.tsx", import.meta.url),
      "utf8",
    );

    expect(source).toContain("toPopoverRacProps");
    expect(source).toContain("../catalog/outputs/toRacProps");
  });
