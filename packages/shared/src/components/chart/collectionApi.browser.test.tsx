...
import { useCollectionData, collectionDataCache } from "../../hooks/useCollectionData";
...
afterEach(() => {root?.unmount(); host?.remove(); host = undefined!; vi.unstubAllGlobals(); collectionDataCache.clear();});
...
function Consumer() {
  ...
  const list = useResolvedCollectionItems({dataBinding: binding, componentName: "ListBox", items:[{id:"fake"}]});
  return <><Chart chartType="bar" dataBinding={binding} data={[{category:"fake",value:99}]} isAnimationActive={false} style={{width:320,height:240}} />
    <output>{JSON.stringify({chart:chart.data,list:list.rows.map((row)=>row.item),loading:chart.loading,error:chart.error})}</output></>;
}
function render(endpoint: ApiEndpointDefinition) {
  ...
  root.render(<CollectionDataProvider services={createCollectionSnapshotServices([], [endpoint])}><Consumer /></CollectionDataProvider>);
}
const state = () => JSON.parse(host.querySelector("output")?.textContent || "{}");
const endpoint: ApiEndpointDefinition = {id:"api-a",name:"sales",baseUrl:"https://fixture.invalid",path:"/sales",method:"GET",responseMapping:{dataPath:"payload.rows"},queryParams:[{key:"page",value:"1"}]};
it("API 지연·빈 성공·오류·source 변경 취소: Chart/List는 한 공통 요청을 공유한다", async () => {
  ...
  vi.stubGlobal("fetch", vi.fn((url: URL, init: RequestInit) => new Promise<Response>((resolve,reject)=> {
  ...
  expect(host.querySelector(".react-aria-Chart")?.getAttribute("data-chart-status")).toBe("loading");
  ...
  await vi.waitFor(()=>expect(host.querySelectorAll(".recharts-bar-rectangle path").length).toBe(1));
  ...
  expect(state().chart).toEqual([]); expect(state().list).toEqual([]);
  expect(host.querySelector(".react-aria-Chart")?.getAttribute("data-chart-status")).toBe("empty");
  render({...endpoint,id:"api-d",path:"/error"});
  await vi.waitFor(()=>expect(calls.length).toBe(4));
  calls[3].resolve(new Response("offline",{status:503,statusText:"Unavailable"}));
  await vi.waitFor(()=>expect(state().error).toContain("503"));
  expect(host.querySelector(".react-aria-Chart")?.getAttribute("data-chart-status")).toBe("error");
});
