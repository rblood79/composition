...
describe("DataOpSchema", () => {
  it("11 op 전부 파싱된다", () => {
    const ops = [
      ...
      { op: "update_field", collectionId: "c1", fieldId: "f1", patch: { key: "fullName", label: null } },
      { op: "remove_field", collectionId: "c1", fieldId: "f1" },
      { op: "set_cell", collectionId: "c1", rowIndex: 0, fieldId: "f1", value: "x" },
      ...
      { op: "define_endpoint", endpoint: { name: "users", method: "GET", baseUrl: "https://x", path: "/u" } },
      { op: "bind_element", elementId: "e1", collectionId: "c1", fieldMap: { value: "f1" } },
    ];
    for (const op of ops) expect(DataOpSchema.safeParse(op).success, op.op).toBe(true);
  ...
  it("알 수 없는 op · 필수 누락 · 중첩 children 검증", () => {
    ...
    expect(DataOpSchema.safeParse({ op: "set_cell", collectionId: "c1" }).success).toBe(false);
    const nested = {
      op: "add_field",
      collectionId: "c1",
      field: { key: "addr", type: "object", children: [{ key: "zip", type: "bogus" }] },
    };
...
describe("DataChangeSchema", () => {
  it("origin 4종 · ops 1개 이상 · label 선택", () => {
    ...
    expect(() => parseDataChange({ ops: [{ op: "delete_collection", collectionId: "c1" }], origin: "bot" })).toThrow();
    expect(DataChangeSchema.safeParse({ ops: [{ op: "delete_collection", collectionId: "c1" }], origin: "ai", label: "x" }).success).toBe(true);
  });
});
...
describe("dataChangeJsonSchema — 같은 소스에서 생성", () => {
  it("전체 op 를 담고 $defs 로 재귀 필드를 표현한다", () => {
    const json = dataChangeJsonSchema() as {
      properties: { ops: { items: { oneOf: { properties: { op: { const: string } } }[] } } };
      $defs?: Record<string, unknown>;
    };
    const kinds = json.properties.ops.items.oneOf.map((o) => o.properties.op.const);
