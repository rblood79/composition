...
`@composition/upload` 가 노출하는 코드와 이를 만드는 서버 응답. 컴포넌트/JSP 는 코드로 사용자 메시지를 고른다.
...
| `E_SERVER`          | `5xx` (위 항목 밖)                                                                     |  자동  | `retryDelays` 소진까지                                                             |
| `E_BAD_REQUEST`     | `400` · `412` · `415` · `411`                                                          |  없음  | 계약 위반 — 항목 `error`, 콘솔에 응답 본문                                         |
