...
| 토큰 획득           | 서버는 `GET` 응답에서 세션 토큰을 만들고 (a) JSP 는 `<meta name="csrf-token">` 으로 (b) 같은 값을 **`XSRF-TOKEN` 쿠키 (HttpOnly 아님, Spring Security `CookieCsrfTokenRepository.withHttpOnlyFalse()` 관례)** 로 내보낸다. composition preview/publish 클라이언트는 쿠키를 읽어 `X-CSRF-TOKEN` 헤더로 되돌린다 (`getHeaders`). 검증은 헤더 ↔ 세션 토큰 대조 하나뿐 — 쿠키 위조로는 통과할 수 없다. 교차 출처 배포에서는 쿠키를 못 읽으므로 same-origin 배치 또는 앱이 토큰을 `getHeaders()` 로 직접 공급 (2026-09-17 G5 추가, additive) |
