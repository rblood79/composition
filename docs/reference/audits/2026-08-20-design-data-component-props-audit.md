...
| Select / ComboBox | renderSelect/renderComboBox 가 `defaultSelectedKey`(uncontrolled) 로 소비 (아래 정정 ②) | **수리 완료 (2026-08-22)** — 다만 결손의 무게중심이 달랐다: 실제로 어긋나 있던 것은 **Skia 가 선택값을 아예 안 그리던 것**이고(캔버스는 계속 placeholder), uncontrolled 는 그 대칭이 생긴 뒤 드러난 후속 결함이다. §1-1 아래 정정 ② 참조 |
...
2. **Select / ComboBox `selectedKey`·`inputValue`** — `renderSelect` 는 `defaultSelectedKey={currentSelectedKey}` 로 **uncontrolled** 소비한다. mount 시점만 읽으므로 RAC 내부 state 를 거치지 않는 경로(undo/redo·history 복원)에서 DOM 이 이전 선택을 계속 표시한다. **2026-08-22 수리 시 전제가 한 번 더 갈렸다**: 이 항목의 실제 무게중심은 accepts 도 uncontrolled 도 아니라 **Skia 채널 부재**였다 — 선택 결과는 owner props 에 정상 저장되고 DOM 은 RAC SelectValue 가 라벨을 그려 맞는데, Skia 로 가는 유일한 채널이 owner placeholder 를 그대로 `SelectValue.children` 으로 넘기는 propagation rule 하나뿐이라 캔버스가 계속 placeholder 를 그렸다(D3 비대칭). 그 rule 에 `transform` 을 달아 표시값(shared `resolveSelectDisplayValue`)을 소비하게 하자 두 consumer 가 같은 prop 을 보게 됐고, **그때서야** uncontrolled 결함이 사용자-가시가 되어 함께 수리했다(key 시그니처 remount — Tabs/ListBox/GridList 선례). 채널 오인이 쉬운 자리라 기록해 둔다: `applyImplicitStyles` 의 SelectValue 분기는 layout 노드만 만들고, `propagationEngine.resolvePropagatedProps` 는 직계 자식 1단계만 매칭해 중첩 경로를 건너뛴다 — Skia 가 읽는 곳은 registry rule + `buildSpecNodeData` 뿐이다. `selectedKey`/`inputValue` 의 **accepts 노출은 미도입**: 두 키는 상호작용이 쌍으로 쓰는 값이라(선택 시 `selectedKey`+`selectedValue` 동시 기록) 한쪽만 패널에서 편집하면 문서가 갈린다.
...
**축 ②는 컬렉션별로 진행 중** (2026-08-21~22): Tree·GridList `selectionStyle` 과 GridList `quiet` 수리 완료, Menu 는 `selectionStyle` prop 대신 **선행 결손이던 선택 표시**를 복구했다(§2-E Menu 행). Table·TableView 의 `selectionStyle` 은 남았지만 **prop 결손이 아니다** — 두 경로 모두 행 선택 인프라 자체가 없어 선택 기능 신규 구현이 선행한다(§2-E Table/TableView 행). Table `quiet` 도 variant 어휘가 catalog(default/striped/bordered)와 수동 CSS(primary/secondary/tertiary/error/filled)로 갈라져 있고 `renderTable` 이 `variant` 를 넘기지 않아 채널 자체가 죽어 있다 — 어휘 통일은 기존 테이블 외형을 바꾸므로 사용자 결정 사항. **축 ①은 미착수**: `contextualHelp` 는 RSP 대응 컴포넌트 자체가 없어 신규 컴포넌트 작업이고(§3-2), `labelAlign` 은 14종 각각의 렌더러·컴포넌트 배선이다.

### 1-3. 형제·패밀리 내 비대칭 (기존 채택 이력과의 불일치)

| 비대칭                                       | 내용                                                                                                                                                                                                                                                                                                                                                        |
| -------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ~~TextArea vs TextField~~ (부분) | **수리 완료 (2026-08-22)** — errorMessage / necessityIndicator / 입력 힌트 5종. 앞의 둘은 `renderTextArea` 가 **처음부터 소비**하고 있었고 binding accepts 에만 없어 값을 넣을 방법이 없던 표면 단절이었다 (선언만으로 해소). 입력 힌트 5종은 accepts·renderer 양쪽 부재라 `resolveInputHintProps` 전개를 renderer 에 함께 추가 — RAC 가 TextAreaContext 로 inputProps 를 내려주므로 컨테이너에 준 힌트가 `<textarea>` 에 붙는다. **잔여 2건**: `value` (표시 채널 별건 — 아래 관찰 ②) / `contextualHelp` (§3-2 신규 컴포넌트 축) |
...
**정정 — "선언 + 전달로 끝나는 것은 소진" 은 한 건 이르렀다 (2026-08-22)**: 위 문장은 2026-08-21 시점 판단이었는데, TextArea 의 7건이 그 뒤에 남아 있었다. 성격이 조금 다르다 — RangeCalendar 형(선언하면 컴포넌트 spread 로 RAC 에 닿음)이 아니라 **렌더러가 이미 소비 중인데 binding accepts 에만 없던** 형태다(errorMessage / necessityIndicator). 감사 초판이 이 행을 §1-3(형제 비대칭)에 넣어 §1-1(표면 단절) 의 "소비는 사는데 편집 표면이 죽은" 축과 분리해 둔 것이 늦게 발견된 이유다. **판정 기준**: 형제 비대칭 행도 착수 시 소비 지점을 열어봐야 한다 — 어느 층이 비었는지(binding / renderer / component)에 따라 작업량이 0.5배~3배로 갈린다.
...
| 축  | 항목                                                                                                  | 외부 근거                                             | composition 현행                                              | 분류            |
| --- | ----------------------------------------------------------------------------------------------------- | ----------------------------------------------------- | ------------------------------------------------------------- | --------------- |
| A   | value/defaultValue | dd + RSP | **미수리 — 표시 채널 선행** — accepts 에 value 가 없는 것은 맞으나 선언만 하면 dead 에 가깝다: renderTextArea 가 `defaultValue`(uncontrolled)로 소비해 패널 편집이 mount 뒤에는 DOM 에 안 닿고, 캔버스는 `buildCatalogShapes` 텍스트 fallback 이 `label || text || children || placeholder` 라 value 를 아예 안 읽는다 (Select 2026-08-22 와 같은 3층 결손). TextField/NumberField/SearchField 도 동일 — field 패밀리 단위 건 | 채택후보-D2 (채널 선행) |
| A   | errorMessage / necessityIndicator | dd + RSP | **수리 완료 (2026-08-22)** — 둘 다 `renderTextArea` 가 처음부터 넘기고 컴포넌트도 소비하고 있었다. accepts 선언만 결손 | 수리 완료 |
| A   | inputMode/autoComplete/autoCorrect/spellCheck/enterKeyHint | RSP (ADR-915 P1.5-b 가 TextField/SearchField 만 채택) | **수리 완료 (2026-08-22)** — accepts 선언 + `resolveInputHintProps` 전개 (renderer). 라이브: `inputmode="numeric"` 이 `<textarea>` 에 실측 | 수리 완료 |
...
#### 신규 발견 — field 패밀리의 D3 비대칭 2건 (2026-08-22 라이브 실측)

TextArea 수리를 라이브로 확인하다 드러났다. **본 변경이 만든 것이 아니고** TextField/NumberField/DateField/TimeField 가 같이 갖고 있는 선재 결손이라 별도 행으로 분리한다. 감사 초판이 못 잡은 이유는 두 축 다 "prop 이 있는가" 로는 정합이고 **두 소비자의 시각 결과**를 대조해야만 보이기 때문이다.

| #   | 축                                | DOM (preview)                                                                              | 캔버스 (Skia)                                                    | 성격                    |
| --- | --------------------------------- | ------------------------------------------------------------------------------------------ | ---------------------------------------------------------------- | ----------------------- |
| ①   | necessityIndicator 표시           | 별도 `<span class="necessity-indicator label">` — **11.9px / weight 400 / rgb(64,64,64) / margin-left 2.975px** | 라벨 텍스트에 접미사 이어붙여 한 덩어리 — **14px / 600 / rgb(23,23,23)** | D3 비대칭 (시각 결과)   |
| ②   | isInvalid 시각 + errorMessage 표시 | 빨간 테두리 + `.react-aria-FieldError` 문구                                                | **무변화** — 테두리·문구 둘 다 없음                              | D3 비대칭 (채널 부재)   |

**①의 함정**: `FieldNecessityIndicator.ts` 헬퍼 파일 주석은 "Preview/Taffy/Skia 모두 이 함수를 사용하여 동일한 텍스트를 생성" 이라고 적혀 있는데, 그 `getNecessityIndicatorSuffix`(선행 공백 포함 `" (required)"`)를 쓰는 것은 **캔버스 3경로뿐**이다. DOM 은 같은 파일의 **다른 함수** `renderNecessityIndicator` 로 span 을 만든다(공백 없음 + CSS margin). 주석을 근거로 "3경로 동일" 로 판정하면 안 된다 — 텍스트 동일성과 시각 동일성은 다르고, 여기서는 텍스트마저 다르다. 수리 방향은 판정 선행: DOM 의 축소·중성 표기를 Skia 가 별도 text run 으로 따라갈 것인가(3 호출 지점 변경), 아니면 DOM 을 라벨 인라인으로 맞출 것인가(Spectrum 표기 관례 포기).

**②의 지점**: canonical `FieldError` 자식이 factory 에서 `children: "" / style.display: "none"` 으로 생성되고(`FormComponents.ts`), owner 의 `errorMessage` 를 그 자식으로 내려주는 propagation rule 이 없다. `FIELD_VISIBLE_CHILD_TAGS` 에는 이미 들어 있어 **가시성 membership 은 통과**하는데 factory inline `display:none` 이 가린다 (메모리 `feedback-catalog-default-absent-shows-auto-and-factory-inline-masks-it` 와 동형). 수리는 propagation rule + isInvalid 게이트 display 주입 2개소.
...
| A | Table/TableView | overflowMode truncate·wrap / selectionStyle checkbox·highlight | RSP TableView | 없음 — **선행 결손 확인 (2026-08-22)**: 두 경로 모두 행 선택 인프라 자체가 없다. `Table.tsx` 는 TanStack 가상 스크롤 커스텀 테이블(RAC Table 아님)로 selectedKeys/체크박스 열이 없고, `renderTable` 은 `Table.binding.accepts.selectionMode` 를 `<Table>` 에 넘기지도 않아 그 accepts 가 **dead** 다. `renderTableView` 는 정적 div 트리라 선택 상태가 아예 없다. selectionStyle 은 prop 결손이 아니라 선택 기능 신규 구현 뒤에 오는 항목 | 관찰(선행 결손) |
...
| A   | GridList        | isQuiet / overflowMode / selectionStyle                                      | dd + RSP ListView                                            | selectionStyle **수리 완료 (2026-08-21)** — accepts 신설, 기본 checkbox(기존 동작). isQuiet/overflowMode 는 미착수. **Skia 체크박스는 미대칭** — 카드가 세로 스택이라 행 높이(+22)까지 바뀌어 layout 경로 동반 필요 | 부분 수리                             |
...
| A   | Tree            | selectionStyle / selectionBehavior                                           | dd + RSP                                                     | **수리 완료 (2026-08-21)** — `selectionStyle`(checkbox\|highlight) accepts 신설 + `resolveSelectionBehavior` 단일 변환(컴포넌트·렌더러·Skia 공유). Skia 는 `selectionCheckbox` 채널 + `selection_checkbox` primitive 로 체크박스 대칭 복구(DOM 라벨 x=52 / 구 Skia 30 발산 해소). 기본은 기존 동작 보존 위해 highlight | 수리 완료                             |
...
| A | Menu | selectionStyle checkbox·switch / item isUnavailable | dd 만 | **선택 표시 복구 (2026-08-22)** — selection 배선(renderMenu → MenuButton → RAC Menu)은 정상인데 시각 채널이 없어 선택 항목이 DOM 상 구별되지 않았다. RAC starter 형태(multiple ✓ / single ●)를 수동 보충 CSS 의 `::before` 로 복구, 게이팅은 RAC 가 emit 하는 `[data-selection-mode]`. Skia 는 트리거만 그려 대칭 대상 없음. `selectionStyle` prop 표면(checkbox·switch)은 dd 단독 근거라 미도입 — switch 형태는 별도 결정 필요 | 부분 수리 |
