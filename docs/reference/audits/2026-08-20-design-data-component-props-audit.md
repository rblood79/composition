...
| A   | Tag avatar/icon 슬롯 | dd hasAvatar + RSP Item children | **수리 완료 (2026-08-21)** — itemSchema `icon` 채택. Skia 는 `leadingIcon.nameProp` 채널 신설(rule 정적 이름 + 행 데이터 합성, `showProp` 동형 게이팅) + `leading_icon` append primitive, DOM 은 chip `.tag-leading-icon`(14px+4px). 폭은 catalog/layout 상수/CSS 3곳 동일값 + 계약 테스트. avatar 슬롯은 별개(이미지 렌더 필요) — 미착수 | 수리 완료 |
