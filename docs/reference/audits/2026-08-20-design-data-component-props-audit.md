...
| Tag itemSchema vs Select/ComboBox itemSchema | icon 채널이 Tag 에만 없음. **재판정 (2026-08-21)**: 선언+전달 아님 — Select 선례 자체가 비대칭이었다. **선례 수리 완료 (2026-08-21)**: Select/ComboBox 팝오버 행이 `{label}` 문자열만 emit 해 itemSchema 의 icon/description 이 편집만 되고 화면에 안 나오던 것을 ListBox 행과 같은 단일 소스 마크업으로 배선. 팝오버 컨텍스트 충돌 2건 동반 수리(좌측 ✓ gutter 와 icon 겹침 / 선택 행 체크 중복). Tag 채택은 여전히 별도 슬라이스 — chip icon glyph(DOM)+per-chip leading icon(Skia)+fit-content 폭 메트릭+maxRows 미러 측정 보정 |
