...
| Tag itemSchema vs Select/ComboBox itemSchema | icon 채널이 Tag 에만 없음. **재판정 (2026-08-21)**: 선언+전달 아님 — Select 선례 자체가 비대칭 (Skia `listbox_item` 은 props.icon 소비, DOM popover render function 은 label 만 렌더 — 코드 실측). Tag 채택은 chip icon glyph(DOM)+per-chip leading icon(Skia)+fit-content 폭 메트릭+maxRows 미러 측정 보정을 묶는 icon slot 대칭 설계 슬라이스로 별도 진행 |
