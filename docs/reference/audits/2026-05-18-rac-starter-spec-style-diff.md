...
**범위:** Table 패밀리 + Tree·TagGroup·ColorPicker·GridList·ColorArea·ColorSlider 제외 — generated CSS 미보유 skipCSSGeneration 컨테이너로, ADR-059·ADR-106-a~d 가 CSSGenerator 구조적 미지원으로 인한 G2 정당 Tier 3 예외로 이미 분류함. ProgressCircle 등 starter CSS 미존재 컴포넌트 제외. 제외 컴포넌트 관련 감사 항목(H11 Tag pill, §2 MED 의 Tree selected divider·GridListItem 카드 shadow·ColorArea aspect-ratio·ColorSlider track height)은 ADR-141 범위 외.
