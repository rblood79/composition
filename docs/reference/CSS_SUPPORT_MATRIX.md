...
> **엔진**: `packages/composition-engine` (자체 Rust 엔진, wasm). 엔진 경계로 값을 넘기는 TS 층은
> `layout/engines/` — `fullTreeLayout.ts` (경계 record 조립) · `utils.ts`
> (`applyCommonEngineStyle` · `applyFlexItemProperties` · `enrichWithIntrinsicSize`) ·
> `{flexStyleAdapter,blockStyleAdapter,gridStyleAdapter,displayAdapter}.ts` (값 변환만)
> **렌더러**: CanvasKit/Skia WASM (`canvas/skia/` — `nodeRenderers.ts` 는 배럴 재export 이고 실물은
> `nodeRenderer{Text,Borders,Clip,Shapes,Image,Mask}.ts` · `build{Box,Image,Skia}NodeData.ts`)
>
> **읽는 법 (최종 검증일 2026-09-09)**
>
> - **§0** 은 **코드가 생성**한다 — `node scripts/generate-engine-matrix.mjs`, 정본은
>   `layout/engines/layoutCapabilityMatrix.ts`, drift 는 `pnpm codex:engine-matrix` (preflight) 가 잡는다.
>   엔진이 CSS 의미를 그대로 구현하지 않는 자리와 Chrome 격차 실측치가 여기 있다.
> - **§1 ~ §18** 은 손 편집 본문이다. 2026-09-09 에 다음 세 범위를 코드로 재검증했다 — (a) 표에 인용된
>   **모든 파일명·심볼의 존재 여부** (없어진 것은 현재 이름으로 교체), (b) 레이아웃 속성의 **엔진 경계
>   운반 여부** (`fullTreeLayout.ts` 경계 record 조립 · `utils.ts applyCommonEngineStyle`), (c) **ADR-206
>   (Taffy 0.10 → 0.14 대조, 2026-09-07 Implemented) 로 지원 상태가 바뀐 속성** — 대조 정본은
>   `docs/explanation/research/TAFFY_UPSTREAM_DELTA_2026-09.md`. 근거는 각 행의 구현 파일 열과 비고에
>   심볼 단위로 적었다.
> - **행 번호는 전부 뗐다** (130건). 표본 8건을 대조해 6건이 어긋났다 — `cssValueParser.ts:217-220` 은
>   주석 머리말, `styleConverter.ts:421-426` 은 `color-mix`, `fills.ts:44-51` 은 mesh RuntimeEffect,
>   `flexStyleAdapter.ts-83` 의 `flexDirection` 은 실제 92행. 파일명 + 심볼명으로 찾는다.
> - **미재검증**: 위 (a)(b)(c) 밖의 개별 판정 — 특히 렌더 품질 수준 (색 변환 정확도 · 필터 색상 행렬 ·
>   그라디언트 보간 · transform 행렬) 은 **2026-04-06 스냅샷 · 미재검증**이다. §8 ~ §16 대부분이 여기
>   해당하며, 해당 절 머리말에 표시했다.
> - 요약 절 아래 (변경 내역 · 실행 계획 · Phase 기록) 는 2026-02 ~ 2026-04 의 **역사 기록**이라 당시
>   파일명 (PixiJS 시대 `*Sprite.tsx` · ADR-142 이전 컴포넌트별 `*.spec.ts`) 과 행 번호를 그대로 둔다 —
>   현재 코드에 없는 이름이다.
> - 충돌 시 §0 과 코드가 우선한다.
...
> **경계 계약 (ADR-923 Phase 5 cutover)**: TS 는 CSS display 값을 **그대로** 보낸다 —
> `displayAdapter.ts normalizeCssDisplay` 가 인식하는 8 값 (block · inline · inline-block · flex ·
> inline-flex · grid · inline-grid · flow-root), 미인식은 `block` 폴백. outer/inner 분해와 blockify 는
> 엔진 `display.rs parse_display` 가 한다 (`display.rs` 머리말 §경계 계약).

| 속성값         | 상태 | 엔진 경로 | 구현 파일                                                             | 비고                                                                                                                                                                         |
| -------------- | ---- | --------- | --------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `block`        | ✅   | Block     | `displayAdapter.ts normalizeCssDisplay`, 엔진 `display.rs`            |                                                                                                                                                                              |
| `inline`       | ⚠️   | Block     | `displayAdapter.ts normalizeCssDisplay`, 엔진 `display.rs`, `tree.rs` | **§0 S4 — declared-substitution**. 값은 경계에 실리지만 `display.rs is_atomic_inline_level` 이 inner=flow 를 제외해 block-level box 로 격상. 순수 IFC 없음 (2026-09-09 정정) |
| `inline-block` | ✅   | Block     | `displayAdapter.ts toEngineDisplay`, 엔진 `display.rs`                | `{outer inline, inner flow-root}` → block 부모의 line item (`is_atomic_inline_level`)                                                                                        |
| `flex`         | ✅   | Flex      | `flexStyleAdapter.ts`, 엔진 `flex.rs`                                 |                                                                                                                                                                              |
| `inline-flex`  | ✅   | Flex      | `displayAdapter.ts toEngineDisplay`, 엔진 `display.rs`, `tree.rs`     | **2026-09-09 갱신 (⚠️ → ✅)**: `flex` 로 접지 않고 `{outer inline, inner flex}` 로 전달 — `is_atomic_inline_level` 이 line item 으로 인정                                    |
| `grid`         | ✅   | Grid      | `gridStyleAdapter.ts parseGridTemplate`, 엔진 `grid.rs`               |                                                                                                                                                                              |
| `inline-grid`  | ✅   | Grid      | `displayAdapter.ts toEngineDisplay`, 엔진 `display.rs`, `tree.rs`     | **2026-09-09 갱신 (⚠️ → ✅)**: inline-flex 와 동상                                                                                                                           |
| `flow-root`    | ✅   | Block     | `displayAdapter.ts`, 엔진 `tree.rs display_creates_bfc`               | **2026-09-07 갱신 (ADR-206 §4 ⑧)**: 종전엔 `block` 으로 접혀 BFC 를 만들지 않아 자식 margin 이 새어 나갔다. `EngineDisplay` 확장 + `display_creates_bfc` 로 실제 BFC 생성    |
| `none`         | ✅   | 공통      | `flexStyleAdapter.ts`, `buildSkiaNodeData.ts`, `buildBoxNodeData.ts`  | 레이아웃 제외 + 렌더 스킵                                                                                                                                                    |
...
| 속성         | 상태 | 구현 파일                                                                                 | 비고                                                                                                                                                                                                                                                               |
| ------------ | ---- | ----------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `width`      | ✅   | `fullTreeLayout.ts buildNodeStyle()`, `utils.ts applyCommonEngineStyle()`                 | px, %, em, rem, vh, vw, calc(). `%` 는 파서가 문자열 그대로 넘겨 엔진이 네이티브 해석                                                                                                                                                                              |
| `height`     | ✅   | `fullTreeLayout.ts buildNodeStyle()`, `utils.ts applyCommonEngineStyle()`, 엔진 `tree.rs` | 동상. **2026-09-07 갱신 (ADR-206 Phase 1)**: `%` 높이의 base 가 `explicit_h` 하나였다 — stretch 된 flex item · grid area · aspect 파생 높이가 base 로 전달되지 않아 자손 `height:50%` 가 0 이었다. `TreeNode.definite_h` per-solve 채널 + `pct_base_h` 신설로 해소 |
...
| 속성                            | 상태 | 구현 파일                            | 비고            |
| ------------------------------- | ---- | ------------------------------------ | --------------- |
| `margin` (shorthand)            | ✅   | `utils.ts`                           | 1값/2값/3값/4값 |
| `margin-top/right/bottom/left`  | ✅   | `fullTreeLayout.ts buildNodeStyle()` |                 |
| `padding` (shorthand)           | ✅   | `utils.ts`                           | 1값/2값/3값/4값 |
...
| 속성                      | 상태 | 구현 파일  | 비고                           |
| ------------------------- | ---- | ---------- | ------------------------------ |
| `box-sizing: border-box`  | ✅   | `utils.ts` |                                |
| `box-sizing: content-box` | ⚠️   | `utils.ts` | 일부 폼 요소에서만 명시적 처리 |
...
| `fit-content`  | ⚠️   | `utils.ts applyCommonEngineStyle()`, 엔진 `style.rs FIT_CONTENT` | **2026-09-09 정정 (사유 교체, 상태 유지)**: "엔진 미전달" 은 낡았다 — ADR-165 이후 **폭 축**은 키워드를 센티넬 (`FIT_CONTENT = -2`) 로 그대로 넘기고 엔진이 CSS-SIZING-3 §5 로 해소한다 (`applyCommonEngineStyle` 의 문자열 복원 분기 + `contentMinWidth`/`contentMaxWidth` 스칼라 동반 공급). ⚠️ 인 이유는 두 가지 — ① **높이 축**은 복원 분기가 없어 여전히 `enrichWithIntrinsicSize` 선해석 · ② 컨테이너는 enrichment 가 px 로 선해석해 키워드가 엔진까지 남지 않는다 |
| `min-content`  | ⚠️   | `utils.ts applyCommonEngineStyle()`, 엔진 `style.rs MIN_CONTENT` | 동상 (`MIN_CONTENT = -3`)                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `max-content`  | ⚠️   | `utils.ts applyCommonEngineStyle()`, 엔진 `style.rs MAX_CONTENT` | 동상 (`MAX_CONTENT = -4`)                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `aspect-ratio` | ✅   | `engines/utils.ts applyCommonEngineStyle()`, 엔진 `tree.rs`      | Flex/Grid/Block 3경로 모두 지원. **2026-09-07 갱신 (ADR-206 §4 ⑤)**: 자식 없는 block leaf 가 `aspect_needs_w` 분기 밖이라 높이 0 이었다 (`aspectRatio: 2` + width auto). 분기 조건을 `!children.is_empty() \|\| aspect_needs_w` 로 넓혀 해소                                                                                                                                                                                                                             |
...
| 속성                             | 상태 | 구현 파일                                                  | 비고                                                                                                                                                                                                                                                   |
| -------------------------------- | ---- | ---------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `flex-direction`                 | ✅   | `flexStyleAdapter.ts`                                      | row, column, row-reverse, column-reverse                                                                                                                                                                                                               |
| `flex-wrap`                      | ✅   | `flexStyleAdapter.ts`                                      | nowrap, wrap, wrap-reverse                                                                                                                                                                                                                             |
| `flex-flow`                      | ✅   | `flexStyleAdapter.ts`                                      | shorthand 파싱 → flex-direction + flex-wrap 분리                                                                                                                                                                                                       |
| `justify-content`                | ✅   | `flexStyleAdapter.ts`                                      | flex-start, flex-end, center, space-between, space-around, space-evenly                                                                                                                                                                                |
| `align-items`                    | ✅   | `flexStyleAdapter.ts`, 엔진 `flex.rs`, `tree.rs`           | stretch, flex-start, flex-end, center, baseline. **2026-09-07 갱신 (ADR-206 §4 ⑦)**: `baseline` 은 종전엔 키워드 자체가 없어 start 로 떨어졌다 — `flex.rs` 슬롯 21 `baseline_plus_one` + `baseline_group` + `ALIGN_BASELINE` 신설로 실제 baseline 정렬 |
| `align-content`                  | ✅   | `flexStyleAdapter.ts`                                      | LOW deferred: flex `align-content: space-evenly` 는 기존 gap (ADR-206 §4-1 기록)                                                                                                                                                                       |
| `gap` / `row-gap` / `column-gap` | ✅   | `flexStyleAdapter.ts`, `utils.ts applyCommonEngineStyle()` |                                                                                                                                                                                                                                                        |
...
| `flex-shrink`      | ✅   | `flexStyleAdapter.ts`                                                              | `overflow !== visible` 부모: 명시적 flexShrink 없으면 `flexShrink: 0` 자동 주입 (fullTreeLayout Step 5.7 + 엔진 `flex.rs`) |
| `flex-basis`       | ✅   | `flexStyleAdapter.ts`                                                              |                                                                                                                            |
| `align-self`       | ✅   | `flexStyleAdapter.ts`                                                              | `baseline` 은 §4.3 참조                                                                                                    |
| `order`            | ✅   | `flexStyleAdapter.ts`, `compositionEngine.ts`                                      | 엔진 WASM order 전달                                                                                                       |

### 4.3 정렬 키워드 (Box Alignment Level 3)

> **2026-09-07 신설 (ADR-206 §4 ⑦⑧)**. 파서 4개 (`align-items` · `align-self` · `justify-content` ·
> `align-content`) 가 `tree.rs strip_overflow_position` 을 공유한다. 대조 정본:
> `docs/explanation/research/TAFFY_UPSTREAM_DELTA_2026-09.md` §2 F4 · F8b · B5.

| 키워드                           | 상태 | 구현 파일                                               | 비고                                                                                                                                      |
| -------------------------------- | ---- | ------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| `baseline` (flex)                | ✅   | 엔진 `flex.rs baseline_group`, `tree.rs`                | 2026-09-07 신설. LOW deferred: first/last baseline 구분 · grid baseline 은 미구현                                                         |
| `safe` / `unsafe` 접두           | ✅   | 엔진 `tree.rs strip_overflow_position`                  | 2026-09-07 신설 (종전 정확 문자열 매치라 `safe center` 가 start 로 떨어졌다). 여유 `max(0)` 가 기본, `unsafe` 만 음수 허용                |
| `self-start` / `self-end`        | ⚠️   | 엔진 `tree.rs`                                          | start/end 와 동치로 접는다 — writing-mode 미지원이라 LTR 에서 결과가 같다 (`tree.rs` 주석 "writing-mode 미지원이라 start/end 와 동치")    |
| block 컨테이너의 `align-content` | ✅   | 엔진 `tree.rs parse_block_align_content`, `solve_block` | 2026-09-07 신설 (Chrome 123+ 기능). `normal` 이 아니면 (`start` 포함) 자식 margin collapse 가 닫힌다 — 정렬 대상 묶음이 margin box 이므로 |
| `justify-items` / `justify-self` | ✅   | `fullTreeLayout.ts buildNodeStyle()`                    | 경계 record 에 그대로 실린다 (grid 축)                                                                                                    |
| `place-items` / `place-content`  | ❌   | —                                                       | **2026-09-09 정정 (✅ → ❌)**: `placeItems` / `placeContent` / `placeSelf` 는 저장소 전체에 구현이 0건이다 — shorthand 분해가 없다        |
...
> **역할 분담 정정 (2026-09-09)**: `gridStyleAdapter.ts` 는 61행이고 그 안의 함수는
> `parseGridTemplate` (최상위 토큰 분리) 하나뿐이다. 경계 record 조립은 `fullTreeLayout.ts` 의
> `coerceGridTrack` 계열이, 트랙 크기·배치 계산은 엔진 `grid.rs` / `tree.rs` 가 한다. 종전 표의
> `gridStyleAdapter.ts` 행 번호 인용은 전부 존재하지 않는 위치였다.
>
> 엔진 `NodeStyle` (tree.rs) 의 grid 필드는 8개다 — `grid_template_columns` · `grid_template_rows` ·
> `grid_auto_flow` · `grid_auto_columns` · `grid_auto_rows` · `grid_column_start` / `_end` ·
> `grid_row_start` / `_end`. **여기 없는 속성은 엔진에 도달하지 않는다.**

| 속성                             | 상태 | 구현 파일                                                     | 비고                                                                                                                                                                                                                                                                                            |
| -------------------------------- | ---- | ------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `grid-template-columns`          | ✅   | `gridStyleAdapter.ts parseGridTemplate`, 엔진 `grid.rs`       | px, fr, auto, minmax(), repeat(). **2026-09-07 갱신 (ADR-206 §4 ⑥)**: 대괄호 라인 이름 (`[a] 1fr [b]`) 이 종전엔 `auto` 트랙으로 파싱돼 트랙 수가 늘었다 — `tokenize_template_with_line_names` + `resolve_line_name` 신설. 미지원: `subgrid` (§0 S8) · `fit-content()`                          |
| `grid-template-rows`             | ✅   | `gridStyleAdapter.ts parseGridTemplate`, 엔진 `grid.rs`       | 동상. **2026-09-07 갱신**: auto 축에서 `repeat()` 토큰이 content 행 하나로 접히던 결함 해소 (`expand_repeat_tokens`)                                                                                                                                                                            |
| `grid-template-areas`            | ⚠️   | `implicitStyles.ts`                                           | **2026-09-09 정정 (✅ → ⚠️)**: 엔진 `NodeStyle` 에 areas 필드가 없어 **문자열 이름 영역은 경계에 실리지 않는다**. catalog implicitStyles 가 명명 영역을 숫자 grid line 으로 병기하는 자리에서만 반영된다 (`implicitStyles.ts` 주석 "Skia grid 경로는 gridArea 이름 해석 미지원 → 숫자 line")    |
| `grid-auto-flow`                 | ⚠️   | `fullTreeLayout.ts`, 엔진 `grid.rs`                           | **2026-09-09 정정 (✅ → ⚠️)**: row · column 만. `dense` 는 값이 실리지만 auto-placement 가 `contains("column")` 만 읽어 sparse 커서로만 놓는다 — 빈칸 역채움 없음 (**§0 S8**, `grid.rs` 머리말 미지원 목록)                                                                                     |
| `grid-auto-columns`              | ✅   | `fullTreeLayout.ts coerceGridTrack`, 엔진 `grid.rs`           | **2026-09-07 갱신 (ADR-206 Phase 2)**: 목록의 첫 px 토큰만 읽던 결함 (`1fr 2fr` → 100/100) 을 `with_implicit_tracks` 토큰 순환으로 해소                                                                                                                                                         |
| `grid-auto-rows`                 | ✅   | `fullTreeLayout.ts coerceGridTrack`, 엔진 `grid.rs`           | 동상                                                                                                                                                                                                                                                                                            |
| 암묵 트랙 (template 없는 grid)   | ✅   | 엔진 `grid.rs`, `tree.rs`                                     | **2026-09-07 신설 (ADR-206 Phase 2)**: 정폭 grid 의 암묵 열이 100px 고정 폴백이었다 (Chrome 은 `auto` = 컨테이너 폭). 셀 폴백 100 제거 3곳 + 암묵 열 합성을 `inline_intrinsic` 밖으로. catalog 의 `MeterTrack` · `ProgressBarTrack` · `SliderTrack` 등 template 없는 `display:grid` 규칙에 도달 |
| `justify-items`                  | ✅   | `fullTreeLayout.ts buildNodeStyle()`                          |                                                                                                                                                                                                                                                                                                 |
| `align-items`                    | ✅   | `fullTreeLayout.ts buildNodeStyle()`                          | grid 는 `grid_align_items_code` 로 0~3 접음 (baseline 미지원 — §4.3)                                                                                                                                                                                                                            |
| `gap` / `row-gap` / `column-gap` | ✅   | `utils.ts applyCommonEngineStyle()`                           |                                                                                                                                                                                                                                                                                                 |
| `place-items` / `place-content`  | ❌   | —                                                             | **2026-09-09 정정 (✅ → ❌)**: 저장소 전체에 구현 0건 — §4.3 참조                                                                                                                                                                                                                               |
| `repeat(auto-fill)`              | ✅   | 엔진 `grid.rs auto_repeat_count`, `expand_auto_repeat_tokens` | **2026-09-07 갱신 (ADR-206 §4 ⑥)**: 반복 수를 `minmax.min` px 만 보고 세 1 반복으로 접히던 결함 해소 — §7.2.3.2 (max definite 면 max, 아니면 min) + 반복 밖 트랙·gutter 선차감. "containerSize 기반 TS 동적 계산" 서술은 낡았다 (계산은 엔진)                                                   |
| `repeat(auto-fit)`               | ✅   | 엔진 `grid.rs`                                                | **2026-09-07 갱신**: 종전엔 `_is_auto_fit` 이 폐기돼 빈 트랙이 축소되지 않았다 — 배치 뒤 미점유 트랙을 gutter 포함 토큰 삭제 + 인덱스 당김 + `placement_spec` 숫자 재작성                                                                                                                       |
| `minmax()`                       | ✅   | 엔진 `grid.rs`, `tree.rs`                                     | `minmax(60px, auto)` 는 Chrome 과 일치 (ADR-206 §2 P0725)                                                                                                                                                                                                                                       |

### 5.2 아이템 속성

| 속성                      | 상태 | 구현 파일                                            | 비고                                                                                                                                                                                                                                                                        |
| ------------------------- | ---- | ---------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `grid-column` (shorthand) | ❌   | —                                                    | **2026-09-09 정정 (✅ → ❌)**: 경계 record 조립이 `gridColumnStart` / `gridColumnEnd` **longhand 만** 싣는다 (`fullTreeLayout.ts`). `gridColumn: "1 / 3"` 은 조용히 버려진다 — ADR-923 round 33 의 "dense Δ0" 오판정 원인이 이것이었다 (`layoutCapabilityMatrix.ts` 머리말) |
| `grid-row` (shorthand)    | ❌   | —                                                    | 동상                                                                                                                                                                                                                                                                        |
| `grid-column-start/end`   | ✅   | `fullTreeLayout.ts buildNodeStyle()`, 엔진 `grid.rs` | `"1"`, `"span 2"`. **2026-09-07 갱신 (ADR-206 Phase 2)**: 명시 배치가 explicit grid 를 넘으면 10,000 가드가 "행 10001" 로 텔레포트시켰다 — `block_fits` 한계 해제 + `implicit_minor_count` 로 암묵 트랙 생성, 가드는 `MAX_GRID_LINE` clamp 로 교체                          |
| `grid-row-start/end`      | ✅   | `fullTreeLayout.ts buildNodeStyle()`, 엔진 `grid.rs` | 동상                                                                                                                                                                                                                                                                        |
| `grid-area`               | ❌   | —                                                    | **2026-09-09 정정 (✅ → ❌)**: shorthand 도 이름 기반도 경계에 실리지 않는다. catalog implicitStyles 는 `gridArea` 를 DOM 용으로 남기고 엔진용 숫자 line 을 **따로** 주입한다                                                                                               |
| `justify-self`            | ✅   | `fullTreeLayout.ts buildNodeStyle()`                 |                                                                                                                                                                                                                                                                             |
| `align-self`              | ✅   | `fullTreeLayout.ts buildNodeStyle()`                 |                                                                                                                                                                                                                                                                             |
...
| ----------------------------------- | ---- | ------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `position: static`                  | ✅   | (기본값)                                                                       |                                                                                                                                                                                                                                                                                                                   |
| `position: relative`                | ✅   | `cssStackingContext.ts`                                                        | stacking context 판정                                                                                                                                                                                                                                                                                             |
| `position: absolute`                | ✅   | `flexStyleAdapter.ts`, 엔진 `tree.rs`                                          | **2026-09-07 갱신 (ADR-206 §4 ③)**: abs-pos used size 가 min/max clamp 를 무시했다 (`resolve_dimension` 이 clamp 결과를 덮어쓰고, stretch 경로에는 clamp 자체가 없었다) — `resolve_abs_axis` 에 `(min, max)` clamp 추가. 남은 격차: 부모 justify/align 을 반영한 static position 미구현 (inset 없는 abs 는 (0,0)) |
| `position: fixed`                   | ⚠️   | `flexStyleAdapter.ts`                                                          | `absolute`로 정규화 — 뷰포트 기준 고정 동작 없음 (`flexStyleAdapter.ts` 의 `position === "absolute" \|\| "fixed"` 분기)                                                                                                                                                                                           |
| `position: sticky`                  | ⚠️   | `cssStackingContext.ts`, `layout/stickyResolver.ts`, `skia/renderCommands.ts`  | **2026-09-09 정정 (사유 교체, 상태 유지)**: "fullTreeLayout 연동 대기" 는 낡았다 — `renderCommands.ts` 가 `resolveStickyY` / `resolveStickyX` 를 호출해 렌더 시 3-state 보정 (normal→stuck→limit) 을 적용한다. ⚠️ 인 이유는 레이아웃 층 (`NodeStyle`) 에 sticky 개념이 없어 **보정이 렌더 좌표에만** 걸리기 때문  |
| `top` / `right` / `bottom` / `left` | ✅   | `flexStyleAdapter.ts` (Inset 블록)                                             | absolute/relative 요소에 적용                                                                                                                                                                                                                                                                                     |
| `z-index`                           | ✅   | `cssStackingContext.ts parseZIndex`, `nodeRendererClip.ts sortByStackingOrder` | auto/숫자, stacking context 렌더 정렬                                                                                                                                                                                                                                                                             |
...
| `overflow: hidden`                | ✅   | `buildBoxNodeData.ts resolveEffectiveOverflow()`, `nodeRendererBorders.ts`                          | CanvasKit `clipRect` + flex 자식 `flexShrink: 0` 자동 주입. catalog `containerStyles` 에만 overflow 를 둔 컨테이너도 `resolveEffectiveOverflow` 로 동작                                        |
| `overflow: scroll`                | ✅   | `buildBoxNodeData.ts`, `stores/scrollState.ts`, `skia/renderCommands.ts`                            | 클리핑 + 스크롤바 UI (`nodeRendererShapes.ts renderScrollbar`) + 마우스 휠 스크롤 + flex 자식 shrink 방지 + hover guideline scroll offset 동기화 (2026-04-06)                                  |
| `overflow: auto`                  | ✅   | `buildBoxNodeData.ts`, `stores/scrollState.ts`                                                      | scroll과 동일 동작 (콘텐츠 초과 시 스크롤바 표시)                                                                                                                                              |
| `overflow: clip`                  | ✅   | `buildBoxNodeData.ts`, 엔진 `tree.rs overflow_creates_bfc`                                          | **2026-09-09 정정 (경로 교체)**: BFC 생성은 `blockStyleAdapter.ts` 가 아니라 엔진 `overflow_creates_bfc` (= `is_scroll_container`) 다. hidden과 동일한 clipRect + flex 자식 shrink 방지        |
| `overflow-x` / `overflow-y`       | ✅   | `engines/utils.ts applyCommonEngineStyle()`                                                         | Flex/Grid/Block 3경로 모두 지원. 축별 독립 제어 — `style.overflow` 를 두 축에 펴고 개별 축 값이 이를 덮는다                                                                                    |
...
| **hover guideline scroll 동기화** | ✅   | `skiaFrameHelpers.ts getCachedOverflowInfoMap()`, `skia/renderCommands.ts`, `stores/scrollState.ts` | treeBoundsMap 에 scrollOffset 반영. **2026-09-09 정정 (심볼 교체)**: `scrollVersion` 이라는 심볼은 없다 — 캐시 무효화 키는 `registryVersion` + `pagePosVersion` + **treeBoundsMap 참조** 3개다 |
...
| 속성                           | 상태 | 구현 파일                                                          | 비고                                                                               |
| ------------------------------ | ---- | ------------------------------------------------------------------ | ---------------------------------------------------------------------------------- |
| `object-fit: fill`             | ✅   | `buildImageNodeData.ts computeObjectFit()`, `nodeRendererImage.ts` | 기본 동작, 컨테이너 전체에 늘리기                                                  |
| `object-fit: cover`            | ✅   | `buildImageNodeData.ts computeObjectFit()`, `nodeRendererImage.ts` | 비율 유지 + 컨테이너 채움 + overflow 클리핑. 미지정 시 기본값                      |
| `object-fit: contain`          | ✅   | `buildImageNodeData.ts computeObjectFit()`, `nodeRendererImage.ts` | 비율 유지 + 컨테이너 안에 맞춤                                                     |
| `object-fit: none`             | ✅   | `buildImageNodeData.ts computeObjectFit()`, `nodeRendererImage.ts` | 원본 크기 + 중앙 정렬 + overflow 클리핑                                            |
| `object-fit: scale-down`       | ❌   | —                                                                  | `buildImageNodeData.ts` 의 허용 값 4개 (contain/cover/fill/none) 밖 → `cover` 폴백 |
...
> - cover/none 시 `nodeRendererImage.ts`에서 `canvas.clipRect()` overflow 클리핑 적용
...
> **재검증 범위 (2026-09-09)**: 이 절은 파일명·심볼 존재 여부만 대조했다. 값 범위와 렌더 결과
> (색 변환 정확도 · 필터 색상 행렬 · 그라디언트 보간 · transform 행렬 · easing 곡선) 은
> **2026-04-06 스냅샷 · 미재검증**이다.

### 8.1 배경

| 속성                                  | 상태 | 구현 파일                   | 비고                                                                        |
| ------------------------------------- | ---- | --------------------------- | --------------------------------------------------------------------------- |
| `background-color`                    | ✅   | `fills.ts`                  | hex, rgb, rgba, hsl, hsla, named 색상                                       |
| `background-image: linear-gradient()` | ✅   | `fills.ts`                  | `MakeLinearGradient`. G5: repeating TileMode 지원. G7: oklab 보간 모듈 준비 |
| `background-image: radial-gradient()` | ✅   | `fills.ts`                  | `MakeTwoPointConicalGradient`. G6: closest-side/farthest-corner 키워드 변환 |
| `background-image: conic-gradient()`  | ✅   | `fills.ts`                  | `MakeSweepGradient` (−90° 보정)                                             |
| `background-image: url()`             | ✅   | `fills.ts`                  | `Image.makeShaderOptions`                                                   |
...
| mesh-gradient (비표준)                | ✅   | `fills.ts`                  | SkSL RuntimeEffect                                                          |

### 8.2 테두리

| 속성                                      | 상태 | 구현 파일                                              | 비고                                                                        |
| ----------------------------------------- | ---- | ------------------------------------------------------ | --------------------------------------------------------------------------- |
| `border-width` (4방향)                    | ✅   | `utils.ts`, `fullTreeLayout.ts buildNodeStyle()`       | shorthand + 개별                                                            |
| `border-color` (4방향)                    | ✅   | `fullTreeLayout.ts buildNodeStyle()`                   |                                                                             |
| `border-style: solid`                     | ✅   | `nodeRendererBorders.ts`                               |                                                                             |
| `border-style: dashed`                    | ✅   | `nodeRendererBorders.ts`                               |                                                                             |
| `border-style: dotted`                    | ✅   | `nodeRendererBorders.ts`                               |                                                                             |
| `border-style: double`                    | ✅   | `nodeRendererBorders.ts` renderDoubleBorder            | 3등분 outer/inner 선, sw<3px 시 solid 폴백                                  |
| `border-style: groove/ridge/inset/outset` | ✅   | `nodeRendererBorders.ts` renderGrooveRidge/InsetOutset | colord darken/lighten 명암 계산                                             |
| `border` (shorthand)                      | ✅   | `utils.ts` parseBorder → parseBorderShorthand          | `border: "1px solid red"` → borderWidth 추출; `cssValueParser.ts` 파서 연동 |

### 8.3 모서리

| 속성                              | 상태 | 구현 파일                                     | 비고                          |
| --------------------------------- | ---- | --------------------------------------------- | ----------------------------- |
| `border-radius`                   | ✅   | `styleConverter.ts`, `nodeRendererBorders.ts` | 단일값, 4방향, 각 모서리 path |
| `border-top-left-radius` 등 (4개) | ✅   | `styleConverter.ts`                           |                               |

### 8.4 그림자

| 속성          | 상태 | 구현 파일                                     | 비고                                                                              |
| ------------- | ---- | --------------------------------------------- | --------------------------------------------------------------------------------- |
| `box-shadow`  | ✅   | `styleConverter.ts`, `nodeRendererBorders.ts` | 다중 shadow, inset 지원. G1: border-radius RRect 반영. G2: spread RRect 확대 처리 |
...
| 속성                                      | 상태 | 구현 파일                               | 비고                                                |
| ----------------------------------------- | ---- | --------------------------------------- | --------------------------------------------------- |
| `color`                                   | ✅   | `cssResolver.ts`, `nodeRendererText.ts` | 상속 가능                                           |
| `opacity`                                 | ✅   | `styleConverter.ts`                     | CanvasKit `saveLayer` (OpacityEffect)               |
| hex 색상 (`#rgb`, `#rrggbb`, `#rrggbbaa`) | ✅   | `styleConverter.ts`                     | colord 라이브러리                                   |
| `rgb()` / `rgba()`                        | ✅   | `styleConverter.ts`                     |                                                     |
| `hsl()` / `hsla()`                        | ✅   | `styleConverter.ts`                     |                                                     |
| Named colors                              | ✅   | `styleConverter.ts`                     | CSS named colors 전체                               |
...
| 속성               | 상태 | 구현 파일                                                | 비고                                                                       |
| ------------------ | ---- | -------------------------------------------------------- | -------------------------------------------------------------------------- |
| `font-family`      | ✅   | `cssResolver.ts`, `nodeRendererText.ts`                  | 상속 가능, fallback 목록                                                   |
| `font-size`        | ✅   | `cssResolver.ts`                                         | em/rem 상속 기반 해석                                                      |
| `font-weight`      | ✅   | `cssResolver.ts`, `nodeRendererText.ts`                  | 100-900, normal, bold — CanvasKit FontWeight 매핑                          |
| `font-style`       | ✅   | `cssResolver.ts`, `nodeRendererText.ts`                  | normal, italic, oblique                                                    |
| `font` (shorthand) | ✅   | `cssValueParser.ts` parseFontShorthand, `cssResolver.ts` | style/weight/size/line-height/family 분리, 개별 속성 우선                  |
| `font-variant`     | ✅   | `cssResolver.ts`, `nodeRendererText.ts`                  | small-caps, oldstyle-nums 등 → CanvasKit fontFeatures                      |
| `font-stretch`     | ✅   | `cssResolver.ts`, `nodeRendererText.ts`                  | condensed~expanded → CanvasKit FontWidth 매핑                              |
| `line-height`      | ✅   | `utils.ts`, `nodeRendererText.ts`                        | 배수, px, normal: fontBoundingBox 기반 (`measureFontMetrics().lineHeight`) |
...
| 속성                          | 상태 | 구현 파일                               | 비고                                                                                                    |
| ----------------------------- | ---- | --------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| `text-align`                  | ✅   | `cssResolver.ts`, `nodeRendererText.ts` | left, center, right, justify (상속)                                                                     |
| `text-decoration`             | ✅   | `nodeRendererText.ts`                   | underline, overline, line-through (비트마스크)                                                          |
| `text-decoration-style`       | ✅   | `nodeRendererText.ts`                   | solid, dashed, dotted, double, wavy — CanvasKit DecorationStyle 매핑                                    |
| `text-decoration-color`       | ✅   | `nodeRendererText.ts`                   | colord 파싱 → Float32Array RGBA                                                                         |
| `text-transform`              | ✅   | `cssResolver.ts`, `styleConverter.ts`   | uppercase, lowercase, capitalize (상속)                                                                 |
| `text-overflow`               | ✅   | `nodeRendererText.ts`                   | ParagraphStyle maxLines:1 + ellipsis:'...'                                                              |
| `white-space`                 | ✅   | `cssResolver.ts`, `utils.ts`            | normal, nowrap, pre, pre-wrap, pre-line (상속)                                                          |
| `word-break`                  | ✅   | `cssResolver.ts`                        | normal, break-all, keep-all (상속)                                                                      |
| `overflow-wrap` / `word-wrap` | ✅   | `cssResolver.ts`                        | 상속 가능, CanvasKit breakStrategy API 대기                                                             |
| `letter-spacing`              | ✅   | `cssResolver.ts`, `nodeRendererText.ts` | 상속 가능                                                                                               |
| `word-spacing`                | ✅   | `cssResolver.ts`, `nodeRendererText.ts` | 상속 가능, ParagraphStyle wordSpacing                                                                   |
| `text-indent`                 | ✅   | `cssResolver.ts`, `nodeRendererText.ts` | 상속 가능, canvas.drawParagraph x 오프셋                                                                |
| `vertical-align`              | ⚠️   | `utils.ts`                              | baseline(FontMetrics ascent 기반), top, bottom, middle — text-top/text-bottom/super/sub은 baseline 폴백 |
...
| 함수 / 속성                                           | 상태 | 구현 파일           | 비고                                                   |
| ----------------------------------------------------- | ---- | ------------------- | ------------------------------------------------------ |
| `translate(x, y)`                                     | ✅   | `styleConverter.ts` |                                                        |
| `translateX()` / `translateY()`                       | ✅   | `styleConverter.ts` |                                                        |
| `rotate()`                                            | ✅   | `styleConverter.ts` | deg, rad, turn, grad                                   |
| `scale()`                                             | ✅   | `styleConverter.ts` |                                                        |
| `scaleX()` / `scaleY()`                               | ✅   | `styleConverter.ts` |                                                        |
| `skew()`                                              | ✅   | `styleConverter.ts` |                                                        |
| `skewX()` / `skewY()`                                 | ✅   | `styleConverter.ts` |                                                        |
| `matrix()`                                            | ✅   | `styleConverter.ts` | CSS matrix(a,b,c,d,e,f) → CanvasKit row-major 3x3 변환 |
| `transform-origin`                                    | ✅   | `styleConverter.ts` | px, %, 키워드(left/center/right/top/bottom)            |
| 다중 함수 조합                                        | ✅   | `styleConverter.ts` | 3x3 행렬 곱셈 (좌→우)                                  |
...
| 속성                      | 상태 | 구현 파일                          | 비고                                                                  |
| ------------------------- | ---- | ---------------------------------- | --------------------------------------------------------------------- |
| `filter: blur()`          | ✅   | `styleConverter.ts`                | LayerBlurEffect (전경 블러). G3: sigma=radius/2.355 (W3C Gaussian)    |
| `filter: brightness()`    | ✅   | `styleConverter.ts`                | SVG 사양 4x5 색상 행렬, CanvasKit ColorFilter                         |
| `filter: contrast()`      | ✅   | `styleConverter.ts`                | SVG 사양 4x5 색상 행렬, CanvasKit ColorFilter                         |
| `filter: grayscale()`     | ✅   | `styleConverter.ts`                | SVG Filter Effects Level 1 사양 4x5 색상 행렬, CanvasKit ColorFilter  |
| `filter: saturate()`      | ✅   | `styleConverter.ts`                | SVG 사양 feColorMatrix saturate, CanvasKit ColorFilter                |
| `filter: sepia()`         | ✅   | `styleConverter.ts`                | SVG Filter Effects Level 1 사양 4x5 색상 행렬, CanvasKit ColorFilter  |
| `filter: invert()`        | ✅   | `styleConverter.ts`                | 4x5 색상 행렬, CanvasKit ColorFilter                                  |
| `filter: hue-rotate()`    | ✅   | `styleConverter.ts`                | SVG 사양 feColorMatrix hueRotate, CanvasKit ColorFilter               |
| `filter: drop-shadow()`   | ✅   | `styleConverter.ts` parseCSSFilter | CanvasKit DropShadowImageFilter                                       |
| `backdrop-filter: blur()` | ✅   | `styleConverter.ts`, `effects.ts`  | BackgroundBlurEffect + BackdropFilterEffect (SaveLayer + blur behind) |
...
| `visibility: hidden`   | ✅   | `buildBaseNodeProps.ts`, `buildSkiaNodeData.ts`, `buildBoxNodeData.ts`, `cssResolver.ts` | 상속 가능 (`INHERITABLE_PROPERTIES`), 렌더 스킵                                                                                                                                                                                                                                                                      |
| `visibility: collapse` | ✅   | `buildBaseNodeProps.ts`, `buildSkiaNodeData.ts`, `buildBoxNodeData.ts`                   | hidden과 동일 처리 (렌더 스킵)                                                                                                                                                                                                                                                                                       |
| `mix-blend-mode`       | ✅   | `blendModes.ts`                                                                          | 18종 (multiply, screen, overlay, darken, lighten, color-dodge, color-burn, hard-light, soft-light, difference, exclusion, hue, saturation, color, luminosity 등). 매핑표 존재만 확인 · 결과 화소는 2026-04-06 스냅샷 · 미재검증                                                                                      |
| `cursor`               | ❌   | —                                                                                        | **2026-09-09 정정 (✅ → ❌)**: 근거였던 "PixiJS Container cursor 매핑" 은 PixiJS 제거와 함께 사라졌다. Skia 캔버스에는 `style.cursor` → 렌더 매핑이 없다 (`CursorManager.ts` 는 편집 도구용으로 canvas **엘리먼트** 커서를 바꾸는 것이라 별개). 요소 커서는 Preview/Publish DOM 이 브라우저 네이티브로 처리한다      |
| `pointer-events`       | ❌   | —                                                                                        | **2026-09-09 정정 (✅ → ❌)**: 동상 (PixiJS `eventMode` 매핑 소멸). `editorMutationEffectRegistry.ts` 의 `pointerEvents` 항목은 무효화 축 등록이지 렌더 구현이 아니다. 캔버스는 상호작용 대상이 아니다 (Preview D1 소관)                                                                                             |
| `clip-path`            | ✅   | `styleConverter.ts` parseClipPath, `nodeRendererClip.ts` buildClipPath                   | inset, circle, ellipse, polygon — CanvasKit clipPath                                                                                                                                                                                                                                                                 |
| `mask` / `mask-image`  | ⚠️   | `nodeRendererMask.ts`, `skia/renderCommands.ts`                                          | **2026-09-09 정정 (❌ → ⚠️)**: CanvasKit `DstIn` 기반 `mask-image` 가 구현돼 있다 — alpha 는 native DstIn, luminance 는 SkSL RuntimeEffect (`MASK_SKSL`, ITU-R BT.709 계수). `renderCommands.ts` 가 요소를 offscreen layer 로 캡처해 합성한다. ⚠️ 인 이유: layer 캡처 분기가 `maskImage.type === "gradient"` 만 탄다 |
...
| 단위                             | 상태 | 구현 파일                            | 비고                                        |
| -------------------------------- | ---- | ------------------------------------ | ------------------------------------------- |
| `px`                             | ✅   | `cssValueParser.ts`                  |                                             |
| `%`                              | ✅   | `cssValueParser.ts`                  | containerSize 기준                          |
| `em`                             | ✅   | `cssValueParser.ts`                  | 부모 fontSize 상속 기반                     |
| `rem`                            | ✅   | `cssValueParser.ts`                  | rootFontSize 기반                           |
| `vw` / `vh`                      | ✅   | `cssValueParser.ts`                  |                                             |
...
| 함수                          | 상태 | 구현 파일                      | 비고                                    |
| ----------------------------- | ---- | ------------------------------ | --------------------------------------- |
| `calc()`                      | ✅   | `cssValueParser.ts`            | +, −, ×, ÷, 괄호 중첩, 혼합 단위        |
| `var()`                       | ✅   | `cssValueParser.ts`            | 중첩, fallback, 순환 참조 방지          |
| `min()` / `max()` / `clamp()` | ✅   | `cssValueParser.ts`            | CSS Values Level 4 준수, 혼합 단위 지원 |
...
| 기능             | 상태 | 구현 파일                               | 비고                                                                                                                                                                                                                                                                                                                                                              |
| ---------------- | ---- | --------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 속성 상속        | ✅   | `cssResolver.ts INHERITABLE_PROPERTIES` | **2026-09-09 정정 (목록 실측)**: **17종** — color, font-size, font-family, font-weight, font-style, font-variant, font-stretch, line-height, letter-spacing, word-spacing, text-align, text-transform, text-indent, visibility, word-break, overflow-wrap, white-space. 종전 표의 "13종" 과 `text-decoration` 포함은 틀렸다 (text-decoration 은 상속 목록에 없다) |
| `inherit` 키워드 | ✅   | `cssResolver.ts`                        |                                                                                                                                                                                                                                                                                                                                                                   |
...
> **집계 기준 (2026-09-09 재산출)**: 각 절 표의 **행 수**를 그대로 센다 (지원율 = (✅ + ⚠️×0.5) / 행 수).
> 2026-04-06 집계 (✅167 ⚠️12 ❌8 · 90%) 와 총계가 다른 이유는 지원율 변동이 아니라 **행 구성 변경**이다 —
> §4.3 정렬 키워드 절 신설 (6 행) · §5 grid 행 분리 (place-\* 병합 · 암묵 트랙 행 추가) · §7 의 보조 행
> (flex shrink 보정 · min-width:auto 에뮬레이션 · 에디터 UI · 해칭) 이 이번엔 집계에 포함됐다. 절대 수치보다
> **절별 ⚠️/❌ 행**을 보라.

| #   | CSS Spec Module             | ✅      | ⚠️     | ❌     | 지원율  |
| --- | --------------------------- | ------- | ------ | ------ | ------- |
| 1   | Display Level 3             | 9       | 1      | 0      | 95%     |
| 2   | Box Model Level 3           | 11      | 1      | 0      | 96%     |
| 3   | Box Sizing Level 3          | 2       | 3      | 0      | 70%     |
| 4   | Flexbox Level 1             | 17      | 1      | 1      | 92%     |
| 5   | Grid Layout Level 1         | 15      | 2      | 4      | 76%     |
| 6   | Positioning Level 3         | 5       | 2      | 0      | 86%     |
| 7   | Overflow Level 3            | 17      | 0      | 2      | 89%     |
| 8   | Backgrounds/Borders Level 3 | 20      | 1      | 1      | 93%     |
...
| 15  | Visual Effects              | 5       | 1      | 2      | 69%     |
| 16  | Values/Units Level 3        | 12      | 0      | 0      | 100%    |
| 17  | Cascade Level 4             | 6       | 0      | 1      | 86%     |
| 18  | Logical Properties Level 1  | 7       | 0      | 0      | 100%    |
|     | **합계**                    | **176** | **15** | **14** | **89%** |

### 2026-09-09 판정 변경 (근거는 각 행 비고)

| 속성                              | 옛 판정 | 새 판정 | 사유                                                        |
| --------------------------------- | ------- | ------- | ----------------------------------------------------------- |
| `display: inline`                 | ✅      | ⚠️      | §0 S4 declared-substitution — block-level 격상              |
| `display: inline-flex`            | ⚠️      | ✅      | `{outer inline, inner flex}` 로 전달, line item 인정        |
| `display: inline-grid`            | ⚠️      | ✅      | 동상                                                        |
| `grid-template-areas`             | ✅      | ⚠️      | 엔진 `NodeStyle` 에 areas 필드 없음 — 숫자 line 병기 경로만 |
| `grid-auto-flow`                  | ✅      | ⚠️      | `dense` 미구현 (§0 S8)                                      |
| `place-items` / `place-content`   | ✅      | ❌      | 저장소 전체 구현 0건                                        |
| `grid-column` / `grid-row` (약식) | ✅      | ❌      | 경계는 longhand 만 운반                                     |
| `grid-area`                       | ✅      | ❌      | 동상                                                        |
| `cursor`                          | ✅      | ❌      | PixiJS 제거로 매핑 소멸                                     |
| `pointer-events`                  | ✅      | ❌      | 동상                                                        |
| `mask` / `mask-image`             | ❌      | ⚠️      | `nodeRendererMask.ts` DstIn 구현 (gradient 분기 한정)       |
...
> **2026-09-09 재확인**: 아래 목록은 2026-04-06 시점 우선순위다. `overflow: scroll/auto` 는 §7 에서 ✅ 로
> 확인됐다 (`buildBoxNodeData.ts` + `scrollState.ts` + `renderCommands.ts`). 남은 항목은 `position: fixed`
> 하나이며, ADR-206 이후의 엔진 잔여 격차는 `docs/explanation/research/TAFFY_UPSTREAM_DELTA_2026-09.md`
> §3 (`display: inline` IFC · flex abs static position · grid `subgrid` · `dense` 역채움) 이 정본이다.

| 우선순위 | 항목                          | 이유                                                                                        |
| -------- | ----------------------------- | ------------------------------------------------------------------------------------------- |
| ~~P0~~   | ~~`overflow: scroll/auto`~~   | ✅ 구현 완료 (2026-09-09 §7 재확인 — 스크롤바 UI · 휠 이벤트 · hover guideline 동기화 포함) |
| ~~P0~~   | ~~`text-overflow: ellipsis`~~ | ✅ v1.3에서 구현 완료                                                                       |
| P0       | `position: fixed`             | 뷰포트 고정 UI 미동작 — `absolute` 로 정규화 (2026-09-09 재확인, §6)                        |
...
| 우선순위 | 항목                                                                | 이유                                                                                                                                                                                    |
| -------- | ------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| P1       | `fit-content` / `min-content` / `max-content` 의 **높이 축**        | **2026-09-09 갱신**: 폭 축은 ADR-165 로 엔진 센티넬 전달이 완료됐다 (§3). 남은 건 높이 축 복원 분기 부재 + 컨테이너 enrichment 선해석                                                   |
| ~~P1~~   | ~~`background-size` / `background-position`~~                       | ✅ v1.3에서 구현 완료                                                                                                                                                                   |
| P1       | `cursor` / `pointer-events`                                         | **2026-09-09 회귀 확인**: v1.3 의 "구현 완료" 는 PixiJS 매핑이었고 PixiJS 제거와 함께 사라졌다 (§15 ✅ → ❌). 캔버스에 필요한지 자체가 재판정 대상 — 요소 커서/포인터는 Preview D1 소관 |
| ~~P1~~   | ~~`filter` 함수 확장 (brightness, contrast, saturate, hue-rotate)~~ | ✅ 구현 완료 (v1.1에서 누락 확인)                                                                                                                                                       |
| ~~P1~~   | ~~`currentColor`~~                                                  | ✅ v1.3에서 구현 완료                                                                                                                                                                   |

---

> **아래는 역사 기록이다 (2026-02 ~ 2026-04)**. 당시의 파일명 (PixiJS 시대 `*Sprite.tsx` ·
> ADR-142 이전 컴포넌트별 `*.spec.ts` · 배럴 이전 `nodeRenderers.ts:NNN`) 과 행 번호를 **그대로**
> 둔다 — 현재 코드에 없는 이름이며, 무엇을 언제 했는지의 기록으로만 읽는다. 현재 상태는 §0 ~ §18 이
> 정본이다.
