...
- **RangeCalendar active primitive binding 추가**:
  - `componentCatalog` 에 `RangeCalendar` 를 `cutover:"catalog"` active `date-color` primitive 로 등록했다.
  - canonical range start/end/variant/size/month paging props 를 RAC RangeCalendar surface 로 투영하는 `toRangeCalendarRacProps()` 와 `rangeCalendarPrimitiveBinding` 을 추가했다.
  - `RangeCalendar.tsx` shared wrapper 가 catalog projection 을 소비하고 `data-variant`/`data-size`/`data-max-visible-months` 상태를 active preview 경로에 반영한다.
- **RangeCalendar Preview / Skia generic 경로 연결**:
  - `CanonicalNodeRenderer` 가 RangeCalendar resolved node 를 legacy `rendererMap` 보다 primitive branch 에서 먼저 렌더한다.
  - generic Skia path 는 Calendar month grid 를 재사용하면서 range band 를 추가하고 `RangeCalendarSpec.render.shapes()` 를 호출하지 않는 fixture 를 추가했다.
...
- date/color family pilot 은 ColorSwatch, ColorSlider, ColorArea, ColorWheel, ColorPicker, Calendar, RangeCalendar 까지 진행했다. DatePicker/DateRangePicker 계열은 후속 slice 로 남아 있다.

## [ADR-142 Phase 3/5/6 — Calendar date-color primitive catalog pilot] - 2026-05-21

### Architecture

- **Calendar active primitive binding 추가**:
  - `componentCatalog` 에 `Calendar` 를 `cutover:"catalog"` active `date-color` primitive 로 등록했다.
  - canonical variant/size/month paging/date state props 를 RAC Calendar surface 로 투영하는 `toCalendarRacProps()` 와 `calendarPrimitiveBinding` 을 추가했다.
  - `Calendar.tsx` shared wrapper 가 catalog projection 을 소비하고 `data-variant`/`data-size`/`data-max-visible-months` 상태를 active preview 경로에 반영한다.
- **Calendar Preview / Skia generic 경로 연결**:
  - `CanonicalNodeRenderer` 가 Calendar resolved node 를 legacy `rendererMap` 보다 primitive branch 에서 먼저 렌더한다.
  - generic Skia path 는 month heading, weekday row, day cells 를 생성하고 `CalendarSpec.render.shapes()` 를 호출하지 않는 fixture 를 추가했다.
  - 현재 ListBox binding 의 확장된 default items 와 catalog placement fixture 기대값을 동기화했다.
...
- date/color family pilot 은 ColorSwatch, ColorSlider, ColorArea, ColorWheel, ColorPicker, Calendar 까지 진행했다. RangeCalendar/DatePicker/DateRangePicker 계열은 후속 slice 로 남아 있다.
...
- **ColorWheel active primitive binding 추가**:
  - `componentCatalog` 에 `ColorWheel` 을 `cutover:"catalog"` active `date-color` primitive 로 등록했다.
  - canonical color/hue/radius/size/disabled props 를 RAC ColorWheel surface 로 투영하는 `toColorWheelRacProps()` 와 `colorWheelPrimitiveBinding` 을 추가했다.
  - `ColorWheel.tsx` shared wrapper 가 catalog projection 을 소비하고 `data-hue`/`data-outer-radius`/`data-inner-radius`/`data-size` 상태를 active preview 경로에 반영한다.
- **ColorWheel Preview / Skia generic 경로 연결**:
  - `CanonicalNodeRenderer` 가 ColorWheel resolved node 를 legacy `rendererMap` 보다 primitive branch 에서 먼저 렌더한다.
  - generic Skia path 가 hue arc ring + thumb 를 렌더하고 `ColorWheelSpec.render.shapes()` 를 호출하지 않는 fixture 를 추가했다.
...
- date/color family pilot 은 ColorSwatch, ColorSlider, ColorArea, ColorWheel 까지 진행했다. Calendar/RangeCalendar/DatePicker/DateRangePicker/ColorPicker 계열은 후속 slice 로 남아 있다.

## [ADR-142 Phase 3/5/6 — ColorArea date-color primitive catalog pilot] - 2026-05-21

### Architecture

- **ColorArea active primitive binding 추가**:
  - `componentCatalog` 에 `ColorArea` 를 `cutover:"catalog"` active `date-color` primitive 로 등록했다.
  - canonical color/xChannel/yChannel/colorSpace/size/xValue/yValue/disabled props 를 RAC ColorArea surface 로 투영하는 `toColorAreaRacProps()` 와 `colorAreaPrimitiveBinding` 을 추가했다.
  - `ColorArea.tsx` shared wrapper 가 catalog projection 을 소비하고 `data-x-channel`/`data-y-channel`/`data-color-space`/`data-x-value`/`data-y-value` 상태를 active preview 경로에 반영한다.
- **ColorArea Preview / Skia generic 경로 연결**:
  - `CanonicalNodeRenderer` 가 ColorArea resolved node 를 legacy `rendererMap` 보다 primitive branch 에서 먼저 렌더한다.
  - generic Skia path 가 color area plane + thumb 를 렌더하고 `ColorAreaSpec.render.shapes()` 를 호출하지 않는 fixture 를 추가했다.
...
- date/color family pilot 은 ColorSwatch, ColorSlider, ColorArea 까지 진행했다. Calendar/RangeCalendar/DatePicker/DateRangePicker/ColorPicker/ColorWheel 계열은 후속 slice 로 남아 있다.

## [ADR-142 Phase 3/5/6 — ColorSlider date-color primitive catalog pilot] - 2026-05-21

### Architecture

- **ColorSlider active primitive binding 추가**:
  - `componentCatalog` 에 `ColorSlider` 를 `cutover:"catalog"` active `date-color` primitive 로 등록했다.
  - canonical color/channel/colorSpace/orientation/size/value/disabled props 를 RAC ColorSlider surface 로 투영하는 `toColorSliderRacProps()` 와 `colorSliderPrimitiveBinding` 을 추가했다.
  - `ColorSlider.tsx` shared wrapper 가 catalog projection 을 소비하고 `data-channel`/`data-color-space`/`data-orientation`/`data-value` 상태를 active preview 경로에 반영한다.
- **ColorSlider Preview / Skia generic 경로 연결**:
  - `CanonicalNodeRenderer` 가 ColorSlider resolved node 를 legacy `rendererMap` 보다 primitive branch 에서 먼저 렌더한다.
  - generic Skia path 가 color slider track/thumb 를 렌더하고 `ColorSliderSpec.render.shapes()` 를 호출하지 않는 fixture 를 추가했다.
...
- date/color family pilot 은 ColorSwatch 에 이어 ColorSlider 까지 진행했다. ColorArea 는 2026-05-21 후속 slice 로 진행했고, Calendar/RangeCalendar/DatePicker/DateRangePicker/ColorPicker/ColorWheel 계열은 후속 slice 로 남아 있다.
...
- date/color family pilot 을 ColorSwatch 로 착수했다. ColorSlider 는 2026-05-21 후속 slice 로 진행했고, ColorArea/ColorWheel 계열 gradient/arc draw 는 후속 slice 로 남아 있다.
...
- **Modal active primitive binding 추가**:
  - `componentCatalog` 에 `Modal` 을 `cutover:"catalog"` active `overlays` primitive 로 등록했다.
  - canonical text/size/focus/open/dismiss props 를 RAC Modal surface 로 투영하는 `toModalRacProps()` 와 `modalPrimitiveBinding` 을 추가했다.
  - `Modal.tsx` shared wrapper 가 catalog projection 을 소비하고 단독 catalog surface 에서는 controlled `DialogTrigger` anchor 로 실제 Modal DOM 을 생성한다.
- **Modal Preview / Skia generic 경로 연결**:
  - `CanonicalNodeRenderer` 가 Modal resolved node 를 legacy `rendererMap` 보다 primitive branch 에서 먼저 렌더한다.
  - generic Skia path 가 modal panel + text 를 렌더하고 `ModalSpec.render.shapes()` 를 호출하지 않는 fixture 를 추가했다.

### Documentation

- overlays family 는 DropZone/Tooltip/Dialog/Popover/Modal pilot 까지 진행했다. Toast overlays slice 는 아직 잔여다.

## [ADR-142 Phase 3/5/6 — Popover overlays primitive catalog pilot] - 2026-05-20

### Architecture

- **Popover active primitive binding 추가**:
  - `componentCatalog` 에 `Popover` 를 `cutover:"catalog"` active `overlays` primitive 로 등록했다.
  - canonical text/variant/size/placement/offset/flip/arrow/open props 를 RAC Popover surface 로 투영하는 `toPopoverRacProps()` 와 `popoverPrimitiveBinding` 을 추가했다.
  - `Popover.tsx` shared wrapper 가 catalog projection 을 소비하고 단독 catalog surface 에서는 controlled `DialogTrigger` anchor 로 실제 Popover DOM 을 생성한다.
- **Popover Preview / Skia generic 경로 연결**:
  - `CanonicalNodeRenderer` 가 Popover resolved node 를 legacy `rendererMap` 보다 primitive branch 에서 먼저 렌더한다.
  - generic Skia path 가 popover panel + text + optional arrow 를 렌더하고 `PopoverSpec.render.shapes()` 를 호출하지 않는 fixture 를 추가했다.

### Documentation

- overlays family 는 DropZone/Tooltip/Dialog/Popover pilot 까지 진행했다. Modal/Toast overlays slice 는 아직 잔여다.

## [ADR-142 Phase 3/5/6 — Dialog overlays primitive catalog pilot] - 2026-05-20

### Architecture

- **Dialog active primitive binding 추가**:
  - `componentCatalog` 에 `Dialog` 를 `cutover:"catalog"` active `overlays` primitive 로 등록했다.
  - canonical text/size/role/dismissable props 를 RAC Dialog surface 로 투영하는 `toDialogRacProps()` 와 `dialogPrimitiveBinding` 을 추가했다.
  - `Dialog.tsx` shared wrapper 가 catalog projection 을 소비하고 `role`, `data-size`, `data-dismissible` 를 active preview 경로에 반영한다.
- **Dialog Preview / Skia generic 경로 연결**:
  - `CanonicalNodeRenderer` 가 Dialog resolved node 를 legacy `rendererMap` 보다 primitive branch 에서 먼저 렌더한다.
  - generic Skia path 가 dialog panel + text 를 렌더하고 `DialogSpec.render.shapes()` 를 호출하지 않는 fixture 를 추가했다.

### Documentation

- overlays family 는 DropZone/Tooltip/Dialog pilot 까지 진행했다. Popover/Modal/Toast overlays slice 는 아직 잔여다.
...
- **Tree/Table active primitive binding 추가**:
  - `componentCatalog` 에 `Tree`, `Table`, `TableView` 를 `cutover:"catalog"` active `tree-table` primitive 로 등록했다.
  - canonical tree `items[]`, table `columns[]`/`rows[]`, selection/density/sort props 를 RAC surface 로 투영하는 `toTreeRacProps()` / `toTableRacProps()` 와 primitive bindings 를 추가했다.
  - `Tree.tsx` shared wrapper 는 static hierarchical items 를 RAC Tree 로 렌더하고, `Table.tsx` 는 active catalog columns/rows 에 대해 RAC Table projection 을 사용한다.
  - `TableView` 는 별도 RAC primitive 가 없으므로 canonical tag 는 유지하되 runtime exportName 은 `Table` binding 을 사용한다.
- **Tree/Table Preview / Skia generic 경로 연결**:
  - `CanonicalNodeRenderer` 가 Tree/Table/TableView resolved node 를 legacy `rendererMap` 보다 primitive branch 에서 먼저 렌더한다.
  - generic Skia path 가 Tree row/disclosure/text 와 Table/TableView header/row/cell 을 렌더하고 `TreeSpec.render.shapes()` / `TableSpec.render.shapes()` / `TableViewSpec.render.shapes()` 를 호출하지 않는 fixture 를 추가했다.

### Documentation

- Tree·Table HIGH family primitive pilot 은 완료했다. ADR-132 collection 데이터 binding 전체 전환은 아직 닫지 않았고, 다음 entrypoint 는 overlays 또는 date/color family 다.
...
- **ComboBox active primitive binding 추가**:
  - `componentCatalog` 에 `ComboBox` 를 `cutover:"catalog"` active collections primitive 로 등록했다. 사용자 패널 위치는 기존과 동일하게 `forms` category 로 유지했다.
  - canonical `items[]` / label / placeholder / input value / selected key 를 RAC ComboBox surface 로 투영하는 `toComboBoxRacProps()` 와 `comboBoxPrimitiveBinding` 을 추가했다.
  - `ComboBox.tsx` shared wrapper 가 catalog projection, static `items[]`, size/icon/state props 를 소비하도록 전환했다.
- **ComboBox Preview / Skia generic 경로 연결**:
  - `CanonicalNodeRenderer` 가 ComboBox resolved node 를 legacy `rendererMap` 보다 primitive branch 에서 먼저 렌더한다.
  - generic Skia path 가 ComboBox label + input + list item row/text/icon 을 렌더하고 `ComboBoxSpec.render.shapes()` 를 호출하지 않는 fixture 를 추가했다.

### Documentation

- collections family 는 ListBox/GridList/TagGroup/Menu/ComboBox pilot 까지 완료했다. collection 데이터 binding(ADR-132) 전체 전환은 아직 닫지 않았고, 다음 entrypoint 는 Select/Tabs 잔여 collections slice 다.

## [ADR-142 Phase 3/5/6 — Menu collections primitive catalog pilot] - 2026-05-20

### Architecture

- **Menu active primitive binding 추가**:
  - `componentCatalog` 에 `Menu` 를 `cutover:"catalog"` active collections primitive 로 등록했다. 사용자 패널 위치는 기존과 동일하게 `buttons` category 로 유지했다.
  - canonical `items[]` → RAC Menu item projection 인 `toMenuRacProps()` 와 `menuPrimitiveBinding` 을 추가했다.
  - `Menu.tsx` shared wrapper 가 catalog projection, static `items[]`, trigger label/variant/size 를 소비하도록 전환했다.
- **Menu Preview / Skia generic 경로 연결**:
  - `CanonicalNodeRenderer` 가 Menu resolved node 를 legacy `rendererMap` 보다 primitive branch 에서 먼저 렌더한다.
  - generic Skia path 가 Menu trigger + popover item row/text 를 렌더하고 `MenuSpec.render.shapes()` 를 호출하지 않는 fixture 를 추가했다.

### Documentation

- collections family 는 ListBox/GridList/TagGroup/Menu pilot 까지 완료했다. collection 데이터 binding(ADR-132) 전체 전환은 아직 닫지 않았고, 다음 entrypoint 는 ComboBox/Select/Tabs 잔여 collections slice 다.

## [ADR-142 Phase 3/5/6 — TagGroup collections primitive catalog pilot] - 2026-05-20

### Architecture

- **TagGroup active primitive binding 추가**:
  - `componentCatalog` 에 `TagGroup` 을 `cutover:"catalog"` active collections primitive 로 등록했다.
  - canonical `items[]` → RAC TagGroup chip projection 인 `toTagGroupRacProps()` 와 `tagGroupPrimitiveBinding` 을 추가했다.
  - `TagGroup.tsx` shared wrapper 가 catalog projection, static `items[]`, `data-type-variant`, `data-type-size` 를 소비하도록 전환했다.
- **TagGroup Preview / Skia generic 경로 연결**:
  - `CanonicalNodeRenderer` 가 TagGroup resolved node 를 legacy `rendererMap` 보다 primitive branch 에서 먼저 렌더한다.
  - generic Skia path 가 TagGroup label + chip box/text 를 렌더하고 `TagGroupSpec.render.shapes()` 를 호출하지 않는 fixture 를 추가했다.

### Documentation

- collections family 는 ListBox/GridList/TagGroup pilot 까지 완료했다. collection 데이터 binding(ADR-132) 전체 전환은 아직 닫지 않았고, 다음 entrypoint 는 Menu/ComboBox/Select/Tabs 잔여 collections slice 다.

## [ADR-142 Phase 3/5/6 — GridList collections primitive catalog pilot] - 2026-05-20

### Architecture

- **GridList active primitive binding 추가**:
  - `componentCatalog` 에 `GridList` 를 `cutover:"catalog"` active collections primitive 로 등록했다.
  - canonical `items[]` → RAC GridList item/card projection 인 `toGridListRacProps()` 와 `gridListPrimitiveBinding` 을 추가했다.
  - `GridList.tsx` shared wrapper 가 catalog projection, static `items[]`, `data-variant`, `data-layout` 을 소비하도록 전환했다.
- **GridList Preview / Skia generic 경로 연결**:
  - `CanonicalNodeRenderer` 가 GridList resolved node 를 legacy `rendererMap` 보다 primitive branch 에서 먼저 렌더한다.
  - generic Skia path 가 GridList container + card label/description 을 렌더하고 `GridListSpec.render.shapes()` 를 호출하지 않는 fixture 를 추가했다.

### Documentation

- collections family 는 ListBox/GridList pilot 까지 완료했다. collection 데이터 binding(ADR-132) 전체 전환은 아직 닫지 않았고, 다음 entrypoint 는 Menu/TagGroup/ComboBox/Select/Tabs 잔여 collections slice 다.
...
- **CheckboxGroup active primitive binding 추가**:
  - `componentCatalog` 에 `CheckboxGroup` 을 `cutover:"catalog"` active selection primitive 로 등록했다.
  - canonical props → RAC props projection 인 `toCheckboxGroupRacProps()` 와 `checkboxGroupPrimitiveBinding` 을 추가했다.
  - `CheckboxGroup.tsx` shared wrapper 가 label/orientation/size/selection catalog projection 을 소비하도록 전환했다.
- **CheckboxGroup Preview / Skia generic 경로 연결**:
  - `CanonicalNodeRenderer` 가 CheckboxGroup resolved node 와 Checkbox children 을 legacy `rendererMap` 보다 primitive branch 에서 먼저 렌더한다.
  - generic Skia path 가 CheckboxGroup label + resolved Checkbox children 을 렌더하고 `CheckboxGroupSpec.render.shapes()` 를 호출하지 않는 fixture 를 추가했다.

### Documentation

- 이 변경은 `selection` family 의 네 번째 active primitive pilot 이다. Radio/RadioGroup 은 아직 잔여다.
...
- **Checkbox active primitive binding 추가**:
  - `componentCatalog` 에 `Checkbox` 를 `cutover:"catalog"` active selection primitive 로 등록했다.
  - canonical props → RAC props projection 인 `toCheckboxRacProps()` 와 `checkboxPrimitiveBinding` 을 추가했다.
  - `Checkbox.tsx` shared wrapper 가 TreeItem slot 예외를 유지하면서 catalog projection 을 소비하도록 전환했다.
- **Checkbox Preview / Skia generic 경로 연결**:
  - `CanonicalNodeRenderer` 가 Checkbox resolved node 를 legacy `rendererMap` 보다 primitive branch 에서 먼저 렌더한다.
  - generic Skia path 가 Checkbox box/indicator/label 을 container + box/text node 로 렌더하고 `CheckboxSpec.render.shapes()` 를 호출하지 않는 fixture 를 추가했다.

### Documentation

- 이 변경은 `selection` family 의 두 번째 active primitive pilot 이다. Radio/Slider 계열과 CheckboxGroup/RadioGroup 은 아직 잔여다.
...
- **ColorField active primitive binding 추가**:
  - `componentCatalog` 에 `ColorField` 를 `cutover:"catalog"` active fields primitive 로 등록했다.
  - canonical props → RAC props projection 인 `toColorFieldRacProps()` 와 `colorFieldPrimitiveBinding` 을 추가했다.
  - generic Inspector section 은 Content / Appearance / Color / State 순서로 ColorField canonical props 를 노출한다.
- **ColorField shared wrapper / Preview / Skia generic 경로 연결**:
  - `ColorField.tsx` 가 catalog projection 을 소비하도록 전환했다.
  - `CanonicalNodeRenderer` 가 ColorField resolved node 를 legacy `rendererMap` 보다 primitive branch 에서 먼저 렌더한다.
  - generic Skia path 가 ColorField label/input/value/swatch 를 container/text/box node 로 렌더하고 `ColorFieldSpec.render.shapes()` 를 호출하지 않는 fixture 를 추가했다.

### Documentation

- 이 변경은 `fields` family 의 여섯 번째 active primitive pilot 이다. fields family 전체 cutover 완료 판정은 아니다.
...
- **DateField active primitive binding 추가**:
  - `componentCatalog` 에 `DateField` 를 `cutover:"catalog"` active fields primitive 로 등록했다.
  - canonical props → RAC props projection 인 `toDateFieldRacProps()` 와 `dateFieldPrimitiveBinding` 을 추가했다.
  - generic Inspector section 은 Content / Appearance / Locale / State 순서로 DateField canonical props 를 노출한다.
- **DateField shared wrapper / Preview / Skia generic 경로 연결**:
  - `DateField.tsx` 가 catalog projection 을 소비하도록 전환했다.
  - `CanonicalNodeRenderer` 가 DateField resolved node 를 legacy `rendererMap` 보다 primitive branch 에서 먼저 렌더한다.
  - generic Skia path 가 DateField label/input/value 를 container/text node 로 렌더하고 `DateFieldSpec.render.shapes()` 를 호출하지 않는 fixture 를 추가했다.

### Documentation

- 이 변경은 `fields` family 의 네 번째 active primitive pilot 이다. fields family 전체 cutover 완료 판정은 아니다.

## [ADR-142 Phase 3/5/6 — SearchField primitive catalog pilot] - 2026-05-20

### Architecture

- **SearchField active primitive binding 추가**:
  - `componentCatalog` 에 `SearchField` 를 `cutover:"catalog"` active fields primitive 로 등록했다.
  - canonical props → RAC props projection 인 `toSearchFieldRacProps()` 와 `searchFieldPrimitiveBinding` 을 추가했다.
  - generic Inspector section 은 Content / Appearance / Input Type / State 순서로 SearchField canonical props 를 노출한다.
- **SearchField shared wrapper / Preview / Skia generic 경로 연결**:
  - `SearchField.tsx` 가 catalog projection 을 소비하도록 전환하고, 기존 `@composition/specs` icon helper 의존을 제거했다.
  - `CanonicalNodeRenderer` 가 SearchField resolved node 를 legacy `rendererMap` 보다 primitive branch 에서 먼저 렌더한다.
  - generic Skia path 가 SearchField label/input/value/search icon 을 container/text/icon_path node 로 렌더하고 `SearchFieldSpec.render.shapes()` 를 호출하지 않는 fixture 를 추가했다.

### Documentation

- 이 변경은 `fields` family 의 세 번째 active primitive pilot 이다. fields family 전체 cutover 완료 판정은 아니다.

## [ADR-142 Phase 3/5/6 — NumberField primitive catalog pilot] - 2026-05-20

### Architecture

- **NumberField active primitive binding 추가**:
  - `componentCatalog` 에 `NumberField` 를 `cutover:"catalog"` active fields primitive 로 등록했다.
  - canonical props → RAC props projection 인 `toNumberFieldRacProps()` 와 `numberFieldPrimitiveBinding` 을 추가했다.
  - generic Inspector section 은 Content / Appearance / Constraints / Locale / Format / State 순서로 NumberField canonical props 를 노출한다.
- **NumberField shared wrapper / Preview / Skia generic 경로 연결**:
  - `NumberField.tsx` 가 catalog projection 을 소비하도록 전환했다.
  - `CanonicalNodeRenderer` 가 NumberField resolved node 를 legacy `rendererMap` 보다 primitive branch 에서 먼저 렌더한다.
  - generic Skia path 가 NumberField label/input/value 를 container/text node 로 렌더하고 `NumberFieldSpec.render.shapes()` 를 호출하지 않는 fixture 를 추가했다.

### Documentation

- 이 변경은 `fields` family 의 두 번째 active primitive pilot 이다. fields family 전체 cutover 완료 판정은 아니다.

## [ADR-142 Phase 3/5/6 — TextField primitive catalog pilot] - 2026-05-20

### Architecture

- **TextField active primitive binding 추가**:
  - `componentCatalog` 에 `TextField` 를 `cutover:"catalog"` active fields primitive 로 등록했다.
  - canonical props → RAC props projection 인 `toTextFieldRacProps()` 와 `textFieldPrimitiveBinding` 을 추가했다.
  - TextField의 `type` prop 이 component tag 로 오인되지 않도록 `CanonicalNodeRenderer` 의 type 복원 순서를 node type 기준으로 고정했다.
- **TextField shared wrapper / Preview / Skia generic 경로 연결**:
  - `TextField.tsx` 가 catalog projection 을 소비하도록 전환했다.
  - `CanonicalNodeRenderer` 가 TextField resolved node 를 legacy `rendererMap` 보다 primitive branch 에서 먼저 렌더한다.
  - generic Skia path 가 TextField label/input/value 를 container/text node 로 렌더하고 `TextFieldSpec.render.shapes()` 를 호출하지 않는 fixture 를 추가했다.

### Documentation

- 이 변경은 `fields` family 의 첫 active primitive pilot 이다. fields family 전체 cutover 완료 판정은 아니다.
...
- **ToggleButtonGroup active primitive binding 추가**:
  - `componentCatalog` 에 `ToggleButtonGroup` 을 `cutover:"catalog"` active primitive 로 등록했다.
  - canonical props → RAC props projection 인 `toToggleButtonGroupRacProps()` 와 `toggleButtonGroupPrimitiveBinding` 을 추가했다.
  - group placement 는 `PrimitiveBinding.placement` child template 으로 고정해 active catalog 생성 시 기본 `ToggleButton` 자식 2개를 함께 만든다.
- **ToggleButtonGroup shared wrapper / Preview / Skia generic 경로 연결**:
  - `ToggleButtonGroup.tsx` 가 catalog projection 을 소비하도록 전환했다.
  - `CanonicalNodeRenderer` 가 `ToggleButtonGroup` resolved node 를 legacy `rendererMap` 보다 primitive branch 에서 먼저 렌더하고 child `ToggleButton` 선택 상태를 RAC `selectedKeys` 로 투영한다.
  - generic Skia path 는 resolved children 을 재귀 렌더하고 `ToggleButtonGroupSpec.render.shapes()` 를 호출하지 않는 fixture 를 추가했다.

### Documentation

- 이 변경은 `primitives/actions` family 의 다섯 번째 active primitive pilot 이다. Icon, Badge 등 family 잔여는 아직 완료 판정하지 않는다.

## [ADR-142 Phase 3/5/6 — ToggleButton primitive catalog pilot] - 2026-05-20

### Architecture

- **ToggleButton active primitive binding 추가**:
  - `componentCatalog` 에 `ToggleButton` 을 `cutover:"catalog"` active primitive 로 등록했다.
  - canonical props → RAC props projection 인 `toToggleButtonRacProps()` 와 `toggleButtonPrimitiveBinding` 을 추가했다.
- **ToggleButton shared wrapper / Preview / Skia generic 경로 연결**:
  - `ToggleButton.tsx` 가 catalog projection 을 소비하도록 전환했다.
  - `CanonicalNodeRenderer` 가 `ToggleButton` resolved node 를 legacy `rendererMap` 보다 primitive branch 에서 먼저 렌더한다.
  - generic Skia path 가 `ToggleButton` 을 selected/emphasized 상태를 반영한 button-like container/text node 로 렌더하고 `ToggleButtonSpec.render.shapes()` 를 호출하지 않는 fixture 를 추가했다.

### Documentation

- 이 변경은 `primitives/actions` family 의 네 번째 active primitive pilot 이다. ToggleButtonGroup, Icon, Badge 등 family 잔여는 아직 완료 판정하지 않는다.

## [ADR-142 Phase 3/5/6 — Link primitive catalog pilot] - 2026-05-20

### Architecture

- **Link active primitive binding 추가**:
  - `componentCatalog` 에 `Link` 를 `cutover:"catalog"` active primitive 로 등록했다.
  - canonical props → RAC props projection 인 `toLinkRacProps()` 와 `linkPrimitiveBinding` 을 추가했다.
- **Link shared wrapper / Preview / Skia generic 경로 연결**:
  - `Link.tsx` 가 catalog projection 을 소비하도록 전환했다.
  - `CanonicalNodeRenderer` 가 `Link` resolved node 를 legacy `rendererMap` 보다 primitive branch 에서 먼저 렌더한다.
  - generic Skia path 가 `Link` 를 underline text node 로 렌더하고 `LinkSpec.render.shapes()` 를 호출하지 않는 fixture 를 추가했다.

### Documentation

- 이 변경은 `primitives/actions` family 의 세 번째 active primitive pilot 이다. family 전체 cutover 는 아직 완료 판정하지 않는다.
