...
## [ADR-210 P1–P5 — 차트 다중 수치 컬럼 · 시리즈 표시 · 숫자 형식 (In Progress — 예산 재승인 대기)] - 2026-09-10
...
- (P5) 종결 검증: preflight FAIL 0 · builder unit 5,689 · shared 1,174 · specs 1,160 · chart Chromium 212+149 · live P2 6/6 · P3 7/7 · production network 9/9. 엔진 parity 4건 FAIL 은 기준선 `baf535258` 과 동일 (ADR-210 무관). **호환 제한**: 새 설정 (값 컬럼·시리즈 설정·숫자 형식) 문서를 구버전에 넣으면 빈 차트 또는 legacy 값 필드 단일 시리즈로 보인다 (데이터 손실 0) — 지원되는 rollback 은 업데이트 전 백업 복원. Implemented 승격은 Builder/Preview 초기 번들 예외 재승인 뒤.
