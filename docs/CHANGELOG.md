...
- **ComboBox/Select labelPosition=side 가 Skia 에만 반영되고 preview CSS 에 미반영** (CSS↔Skia 비대칭):
  ...
  - **기존 element 잔존**: factory fix 는 신규 생성에만 적용. 기존 page 의 Select/ComboBox element 에 남은 stale inline `flexDirection:column` 은 데이터로 잔존 → 해당 element 는 Direction 토글/dropdown 으로 side 전환해도 preview 미반영(개별 inline 제거 필요). hydration migration 은 별도 판단.
