...
Proposed — 2026-09-12. 사용자가 범위 (시간축 + 지시자 형식/파싱 + 가변 창; ReferenceLine · Scatter 는 후속, zoom 제외) 와 제목을 AskUserQuestion 으로 선택 (분리 4질문 lock-in: [breakdown §1](design/216-chart-time-axis-format-window-breakdown.md#1-범위--선행-관계-분리-4질문-lock-in)). 이론 원천: [CHART_TIME_AXIS_BRUSH_PATTERNS_2026-09](../explanation/research/CHART_TIME_AXIS_BRUSH_PATTERNS_2026-09.md) (d3-time · d3-scale · d3-array · d3-brush · d3-time-format 소스 + RSC `vega-spec-builder`).

## Context

차트의 x 축은 항상 **범주 등간격**이다 — 날짜 문자열도 문자열 범주로 취급되어 `2026-01-10` 과 `2026-1-9` 가 별개 범주로 문자열 순 정렬되고, 빠진 날은 자리가 없으며, 눈금은 범주 라벨 그대로다. 대시보드 데이터의 가장 흔한 형태 (시계열) 에서 간격·정렬·눈금이 전부 틀린다. ADR-211 의 창 (window) 은 넘치는 범주를 고정 길이 창으로 넘기지만 사용자가 구간을 넓히거나 좁힐 수 없다.
...
1. **기존 문서 byte 동일**: `dimensionScale` 미설정 문서는 scene byte 동일 — 스냅샷 4 · ADR-210 T10 32 · ADR-211 오라클 18 회귀 0. 자동 시간축 전환 금지 (opt-in 만).
...
4. **최소 창 = fitEff**: 창 `[start, end]` 는 `end − start ≥ fitEff` — ADR-211 예산 (마크 ≤ M · 점 ≤ P) 이 창 크기와 무관하게 유지. 넓힌 창은 극값 재추출.
...
6. **성능**: 창 드래그 중 모델 재계산 (극값 재추출) p95 ≤ 20 ms (R = 20,000 · S = 4, ADR-211 G4 "모델 ≤ 20 ms" 와 같은 기준).
7. **UTC 단일**: 파싱·눈금·라벨 전부 UTC. 로컬 시간대 없음.
...
| ID  | 위험                                                                                                                                                                                                                                | 심각도 | 대응                                                                                                                                                                                     |
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :----: | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| R1  | ADR-211 예산 모델이 범주 index 기준 — 시간 스케일에서 창 index · 극값 bucket · fit 의 뜻이 어긋나 두 leg 가 다른 visible 을 낼 수 있다 (`budget.ts:209 resolveFitEff` · `:624 clampWindowStart` · `model.ts:122 resolveChartModel`) |  HIGH  | 창 index 는 **transformed 점 index** 로 고정 (시간 단위 창은 범위 밖), 극값 입력을 `[start, end)` 구간으로만 바꾸고 bucket 규칙은 211 그대로 — G2 에서 두 leg visible byte 동일 오라클   |
...
| G2   | Phase 2 종료   | `dimensionScale:"time"` line/area: 같은 입력 두 leg visible · 눈금 · 라벨 byte 동일 (R1) · bar/pie 거부 진단 · 파싱 실패 진단 · 미설정 문서 byte 동일 (HC1)                                                                       | 시간 창 index 규약 재검토 (breakdown §2.3), byte 동일 깨지면 opt-in 경계 수리                       |
...
| G4   | Phase 5 종료   | 창 `[start, end]`: 최소 창 불변식 (원복 RED) · 본체 드래그 길이 보존 · 넓힌 창 극값 재추출 마크 ≤ M · 점 ≤ P · Canvas 초기 창 = Preview 초기 창 byte 동일 · reset/clamp 규약                                                      | 최소 창 위반이 나오면 `end` 갱신을 fitEff 로 강제; 재추출 초과면 211 적응 B 로 축소                 |
| G5   | Phase 6 종료   | Builder initial 순증 ≤ 7 KiB · Preview ≤ 3 KiB gzip (clean worktree 2개) · 드래그 재계산 p95 ≤ 20 ms · live 하니스 PASS · 절대 상한 (215) 대비 보고 → 초과분은 **사용자 재승인**                                                  | 순증 초과: parse 를 lazy (Data 패널 형식 검증) 로 이동 · 로케일 ko 표 lazy; 절대 상한은 사용자 결정 |
...
- ADR-211 의 "창 길이 불변" 결정이 개정된다 — 211 본문은 그대로 두고 본 ADR 이 대체 지점을 명시하므로 두 문서를 같이 읽어야 한다 (breakdown §2.4).
