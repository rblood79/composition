...
| T-PROJECT | (ADR-920 흡수, HC#7) collection projected tree(breakdown §4.12)의 render-space `projectionId` 가 canonical mutation/history/IndexedDB 에 유입되면 데이터 corruption. selection/hover/edit/mutation 4 경로마다 projected id guard 필요 — projected tree 도입의 구조적 동반 위험                                                                                                                                                                                                                                                               | **HIGH** | Gate G8. projected id → canonical API 직접 유입 negative fixture PASS + refresh 후 `elementsMap` synthetic projectionId 0건. ADR-135/136 Render-Space Boundary 의 collection 적용 — §5.11 edit route 가 canonical write target 으로 명시 변환. 실패 시 해당 collection family `cutover:"cutting-over"` 보류(발효 금지) |
...
잔존 HIGH 위험: T-1 / T-3 / T-ADAPT / T-4 / T-7 / T-PARITY / **T-PROJECT / T-DEEP** (8건). 모두 Gate 와 1:1 대응하며 family 단위 격리 가능(T-1→G1 / T-3→G2·G5 / T-ADAPT→G2 / T-4→G7 / **T-7→G-state** / T-PARITY→G-parity / T-PROJECT→G8 / T-DEEP→G9). **T-PARITY 는 과거 재설계 폐기의 직접 원인(정합성 통과 ≠ 기능 동등)을 설계 단계에서 차단하는 위험축** — 정합성 Gate(G2/G5)와 별개로 G-parity 가 기능 동등을 독립 검증한다. **T-7 은 Skia state 모델 미완(hover/pressed 시각 부재)** — G-state 가 selection family cutover 마다 상태 시각 parity 를 독립 검증한다(상태 시각 미달 시 cutover 발효 금지). **T-PROJECT/T-DEEP 는 ADR-920 흡수로 추가된 collection Interactive Projected Tree(HC#7) 의 구조적 위험** — G8(canonical boundary)/G9(깊은 노드 편집)가 각각 1:1 대응한다. behavior/page frame/data 영역은 ADR-131/132/135/136 이 관할하므로 본 ADR Risks 에 추가하지 않는다(breakdown ⑪ 흡수 경계).

## Gates

공통 기반 Gate(G1/G2)는 family cutover 착수 전 1회. family Gate(G4~G6 + G-parity + G-state)는 family 마다 반복(G-state 는 hover/pressed 상태 시각이 있는 selection/interactive family 에 적용). G7 은 전 family `cutover:"catalog"` 도달 시 1회. **G8/G9(ADR-920 흡수)는 collection/tree-table family cutover 의 발효 조건** — 그 family 의 `cutover:"catalog"` 도달이 projected tree(legacy 경유 아님)를 요구하므로, 별도 후순위 단계가 아니라 collection family cutover 와 동시점이다(projected-first). (breakdown ⑩ roadmap 과 동기.)
...
| G-state  | family Skia state 모델 (T-7)                    | family cutover 시 Skia editor surface 가 `default`/`disabled` 뿐 아니라 **hover/pressed/selected 상태 시각**을 derive: `racStateAttrs`(RAC `data-*` 상태 → Skia state) wiring + `ComponentState` enum 으로부터 base⊕override 시각 분기. selection family fixture 가 Builder Skia 에서 hover/pressed data-attribute parity 를 Preview DOM 과 동일하게 통과(`/cross-check` 상태 시각 대칭). **상태 시각 부재로 cutover 발효 금지**                                                                                                                                                                     | 해당 family `cutover:"cutting-over"` 보류 — 상태 시각 미달 family 격리 |
