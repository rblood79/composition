...
잔존 HIGH 위험: T-1 / T-3 / T-ADAPT / T-4 / T-7 / T-PARITY / **T-PROJECT / T-DEEP** (8건). 모두 Gate 와 1:1 대응하며 family 단위 격리 가능. **T-PARITY 는 과거 재설계 폐기의 직접 원인(정합성 통과 ≠ 기능 동등)을 설계 단계에서 차단하는 위험축** — 정합성 Gate(G2/G5)와 별개로 G-parity 가 기능 동등을 독립 검증한다. **T-PROJECT/T-DEEP 는 ADR-920 흡수로 추가된 collection Interactive Projected Tree(HC#7) 의 구조적 위험** — G8(canonical boundary)/G9(깊은 노드 편집)가 각각 1:1 대응한다. behavior/page frame/data 영역은 ADR-131/132/135/136 이 관할하므로 본 ADR Risks 에 추가하지 않는다(breakdown ⑪ 흡수 경계).

## Gates

공통 기반 Gate(G1/G2)는 family cutover 착수 전 1회. family Gate(G4~G6 + G-parity)는 family 마다 반복. G7 은 전 family `cutover:"catalog"` 도달 시 1회. **G8/G9(ADR-920 흡수)는 collection/tree-table family cutover 의 발효 조건** — 그 family 의 `cutover:"catalog"` 도달이 projected tree(legacy 경유 아님)를 요구하므로, 별도 후순위 단계가 아니라 collection family cutover 와 동시점이다(projected-first). (breakdown ⑩ roadmap 과 동기.)
