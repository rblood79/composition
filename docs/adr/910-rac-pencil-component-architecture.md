...
> **ADR-920 흡수 (supersede)**: 본 ADR 은 ADR-920(RAC Format Interactive Projected Tree, 같은 외부 입력의 Codex 독립 설계)을 흡수해 supersede 한다. 두 설계의 base/override 원리는 동일하다(HC#3 ≡ ADR-920 HC#5 — 독립 도달한 수렴 증거). 910 을 상위 컴포넌트 아키텍처로 두고, 920 이 정면 설계한 **collection Interactive Projected Tree**(910 미설계 영역)를 HC#7 + Risks(T-PROJECT/T-DEEP/T-TPL) + Gates(G8/G9) + breakdown §4.12/§5.11/§7-4/⑪ 로 흡수했다. 920 의 behavior/page frame/data 축은 이미 ADR-131/132/135/136 관할(Implemented)이므로 **bridge 참조만** 두어 관할 중복을 피했다(breakdown ⑪ 흡수 경계).
