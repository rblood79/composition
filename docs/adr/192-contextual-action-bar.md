...
- ADR-182 이후 선택 요소에 적용 가능한 액션 14종 (복제 / 그룹·해제 / 정렬·분배 8 / 컴포넌트 토글 / 원본 이동 / 인스턴스 분리 …) 은 `canvasActions` + store 액션으로 단일화됐지만, **마우스 진입점은 우클릭 메뉴 하나**다. 정렬·분배 8종은 우측 패널 노출도 없어 메뉴 서브메뉴가 유일한 경로 (182 breakdown §2 각주).
...
- 182 항목 `id` 가 두 표면의 계약이 되므로 안정 문자열이어야 한다 (Phase 0 실측, 필요 시 상수화 1회).
...
- **포커스**: 버튼 `preventFocusOnPress` — 캔버스 단축키 유지 (HC3).
...
| R2  | 바 버튼이 포커스를 가져가 캔버스 단축키 회귀                                         |  MED   | `preventFocusOnPress` + live 3종 (⌘D/⌫/⌘G) 게이트 (G2). 키보드 진입은 Tab 한정 + Escape 복귀                                                      |
...
| G2   | Phase 2 종료 | **live**: 4종 선택에서 항목이 breakdown §2 와 일치 / 빈 선택 미마운트 / 버튼 실행 / 클릭 직후 ⌘D·⌫·⌘G 캔버스 적용 / 드래그·텍스트 편집 중 비표시 | 포커스 회귀 시 `preventFocusOnPress` 대신 클릭 후 캔버스 `focus()` 복귀 |
...
- 선택 의존 액션의 **두 번째 마우스 경로** (우클릭 외) 가 생기고, 정렬·분배 8종에 처음으로 상시 노출 진입점이 생긴다.
