...
필요한 것은 registry 를 agent 에게 여는 것이 아니라 (Codex P4: 아직 이르다), **명령의 정적 사실 (agent 호출 가능 여부 · 변경 범위 · 되돌림 · 승인) 을 UI handler 에서 분리**하고, agent 가 부를 수 있는 것은 **store-level 액션으로 별도 adapter** 를 두는 것이다. 액션 층 (`canvasActions.ts` 11 함수 · `historyManager` · root store `panelWorkspaceLayout` · viewport `setZoom`) 은 이미 공유돼 있어 재구현이 아니라 **바인딩** 만 추가한다 — 195 와 같은 형태다.
...
5. **되돌림 단위** — `undo: "history"` 명령은 agent 호출 1건이 history **1 entry** (사용자 ⌘Z 1회로 복원). 1 entry 를 보장할 수 없는 명령은 `irreversible` 로 표기하고 `confirm` 필수.
...
- 승인 흐름은 `detachInstance` 의 `requestEditingSemanticsDetachConfirmation` 선례를 재사용하되 agent 가 결과를 기다릴 수 있는 Promise 형태가 필요하다 (Phase 0 실측).
- history 배치 group 표면 (`historyManager`) 유무는 Phase 0 실측 — 없으면 HC5 의 `irreversible` 분기로 흡수.
...
- 설명: 정의 옆에 `COMMAND_META[id] = { agentCallable, mutation, undo, confirm, precondition?, args? }` 표를 두고, allowlist 명령마다 `canvasActions.*` / `historyManager` / root store / viewport 를 부르는 adapter 를 둔다. `executeAgentCommand(id, args, ctx)` 하나가 모든 호스트 (AI 패널 tool · Chrome MCP · 향후 MCP) 의 진입점. UI handler 와 registry 는 무변경. breakdown §3.
...
- 위험:
  - 기술: **M** — adapter 와 UI handler 의 의미 drift (정렬·분배·그룹은 같은 `canvasActions` 라 0, z-order/selection/zoom 은 store 함수 실측 필요) · history group 표면 유무 · 승인 Promise 화. 전부 jsdom parity 로 잠긴다.
...
5. **잔존 위험이 jsdom + live 15건으로 닫힌다** — parity (allowlist 각 id) · 승인 분기 · history 1 entry · 기록 1:1 은 단위 테스트, 승인 다이얼로그와 undo 복원은 Chrome MCP 가 실측 (§Gates).
...
| ID  | 위험                                                                                                                                                                                                | 심각도 | 대응                                                                                                                                                                                       |
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :----: | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| R1  | adapter 가 UI handler 와 다른 일을 한다 — z-order · selection · zoom · 패널은 `canvasActions` 가 아니라 store 함수를 따로 부르므로 handler 가 하는 부가 동작 (포커스 이동 · 스크롤) 이 빠질 수 있다 |  MED   | Phase 0 이 store 함수명과 handler 본문을 대조해 부가 동작을 표에 적는다. jsdom parity = "store 상태 동일" 로 정의하고, 부가 동작이 필요한 명령은 노출 금지로 내린다 (allowlist 상한 40 안) |
...
| R3  | history 단위 — `historyManager` 에 group 표면이 없으면 agent 호출 1건이 entry N 개가 돼 사용자 undo 가 부분 복원                                                                                    |  MED   | Phase 0 실측. 없으면 `undo: "irreversible"` + `confirm` 로 표기 (HC5 분기) 하고 group 표면 추가는 별도 결정 (ADR-180 history 와의 관계) — 본 ADR 은 표기만 한다                            |
...
| ---- | ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| G0   | Phase 0 | breakdown §2 재grep 일치 · 71 id 분류표 확정 (allowlist ≤ 40, external 전부 금지, 연속키·tree·패널 로컬 금지) · z-order/selection/zoom store 함수명 + handler 부가 동작 대조 · `historyManager` group 표면 판정 · `detachInstance` 확인 흐름 Promise 화 판정 · AI 도구 7종 승인 0·기록 0 재확인                                                     | 표 갱신 commit 으로 흡수 (fork 아님). group 표면 없음 → HC5 `irreversible` 분기 적용                       |
| G1   | Phase 1 | `COMMAND_META` 71 전부 (누락 type error) · 정적 게이트 5조항 PASS + 민감도 (allowlist id 의 adapter 제거 → RED · external id `agentCallable: true` → RED · irreversible document 명령 `confirm: false` → RED) · jsdom parity allowlist 전부 PASS · 195 키보드 oracle 26/26 · 입력창 7 · 정적 게이트 기존 조항 PASS · type-check · HC1 diff 0        | 실패 조항만 수정. parity 실패 명령은 노출 금지로 내린다 (allowlist 축소는 scope 변경 아님)                 |
| G2   | Phase 2 | jsdom: denied (allowlist 밖) · precondition-failed · declined (승인 거부 시 store 무변경) · ok · 배치 원소별 승인 · 기록 1:1 (5 status 전부) · history 1 entry (group 표면 있을 때) · 승인 전 store 변경 0                                                                                                                                          | 실패 분기만 수정. 승인 Promise 가 기존 다이얼로그와 결합 불가면 agent 전용 confirm 컴포넌트 (최소) 로 대체 |
| G3   | Phase 3 | live (Chrome MCP · 사용자 confirm 구분 기재): agent 호출 ≥ 15 (정렬 6 · 분배 2 · 그룹/해제 · 복제 · z-order 2 · undo/redo · 줌 · 패널 토글 2) 전부 사람 경로와 같은 결과 · `delete` 승인 다이얼로그 실측 (거부 → 무변경, 승인 → 삭제 + 기록) · undo 1회 복원 1건 · 팔레트 G3 23건 재실행 동일 · 번들 Δ ≤ +3KB gz · `pnpm agent:work -- verify` 통과 | 다른 결과가 나온 명령은 R1 — parity 정의 보강 또는 노출 금지. 3건 이상이면 Phase 1 분류표 재검토           |
