...
**Flow→Absolute 복합 명령 (2026-09-18, §6.1)**: Position 토글 활성화는 store 명령 `applyAbsoluteFromSelection` 하나로 간다 — position/left/top (패널이 scene bounds 로 계산, 기존 write 라우팅) + 절대 위치에서 무효가 되는 Fill 축 (marker 또는 legacy CSS grow) 만 변경 전 used px 로 CSS Fixed · marker null · Fill 파생 CSS 정리 + 형제 맨 앞, 한 transaction. tier 규칙은 Ratio 와 같다 (자기 상태가 있는 tier 만 쓰고 geometry 를 요구, 나머지는 base 상속). Absolute→Flow 는 기존 `position` 해제만 — Fixed 유지, marker 가 null 이라 이전 Fill 이 되살아나지 않는다 (자동 복원 0). live 가 드러낸 엔진 결함 1: absolute leaf 의 auto 축은 leaf 가 content-box 로 보고하는데 abs 배치가 pad/border 를 더하지 않아 Button 높이 Canvas 20 / Preview 30 — leaf 는 자기 layout (border-box) 을 읽도록 수리 (`absolute_leaf_auto_axes_are_border_box`, RED 20 → 30). 검증: 실제 Builder `adr224-size-live.mjs absolute --absolute` **16/16** (Fill 13 + Absolute 3: on → `width 591.33px` · `sizing.width null` · `left/top 0px` · Canvas/Preview 591.33×30 양 leg · history +1 → off → Fixed 유지 · position 없음 · Fill 복원 0 → Undo 2회 = `sizing.width {factor 2}` · width 없음), 단위 `absoluteSizingEdit` 6 · TransformSection 16 (activation 4건을 명령 계약으로) · Rust 426 · 전체 parity 회귀 0 (기존 4파일 10건 동일). 남은 범위: 실제 resize pointer 경로 · 다중 선택 Ratio/Absolute live · G5 · G6.

**캔버스 핸들 resize (2026-09-18, breakdown §4.3 · §5)**: 2026-03 정리 이후 중앙 pointer 핸들러가 핸들 히트에서 조작 없이 return 하던 자리에 resize 세션을 붙였다 (단일 요소 선택 — 다중 선택의 combined 핸들은 종전대로 표시만). gesture owner `resize` (spacing 과 같은 원자 승격) + `ResizePresentationSession`: 드래그 중에는 프레임당 `style.patch` 하나 — 요청 크기 (숫자 px) 와 **잡은 축의 Fill 파생 CSS 제거 키** (`flexGrow/flexShrink/flexBasis` · `alignSelf` · projection 이 넣은 `minWidth/minHeight`, 값 `""`) — 를 ADR-187 layout lane 으로 publish 해 엔진이 Fixed 로 미리 놓는다 (release 키를 continuous 로 등록하고 bridge·targeted compute 가 `""` 를 제거로 읽는다). pointerup 은 runtime commit 한 번 (`RESIZE_COMMIT_INTENT` → store 명령 `applyCanvasResize`): 요청 축마다 Size 메뉴 Fixed 와 같은 경로 (`buildSizingEdit` css mode) 로 그 축의 marker null + CSS px (정수) + Fill 파생 CSS 정리, 다른 축 Fill 보존, active tier 라우팅, history 1개. 핸들이 닿는 축만 요청하고 (엣지 = 한 축, 코너 = 두 축), Ratio 잠금이면 driver 축 하나만 — 종속 축에만 닿으면 목표 종속 크기를 ratio 로 driver 로 환산한다 (세로 resize → 목표 H × ratio → Width). 임계값 (3 screen px) 미만은 클릭 = 저장 0, 시작 크기와 같으면 no-op, layout receipt rejected (grid 조상 등) 면 미리보기가 없었으므로 commit 하지 않는다. 위치는 바꾸지 않는다 (흐름은 부모가 놓고, absolute 도 이 phase 는 크기만 — left/top 핸들 이동은 후속). live 가 드러낸 결함 1: resize 뒤 `sizing {width:null,height:null}` + Ratio 잠금 요소가 Canvas 240 / Preview 210 — Canvas Fill projection 의 skip 가드가 truthiness 라 명시 null 축을 "Fill 시스템 밖" 으로 걸러 Ratio 종속 축의 stretch 보호 (`alignSelf: start`) 를 빼먹었다 (Preview 의 `createFillTreeResolver` 는 skip 없음). `=== undefined` 로 맞췄다 (`projectFillLayout.test.ts`, 원복 RED). 검증: 실제 Builder `adr224-size-live.mjs resize --resize` **18/18** (Fill 13 + resize 5: 실제 마우스로 우측 엣지 −100px → 드래그 중 scene bounds 491 (presentation) → `width 491px` · `sizing.width null` · `sizing.height {factor 1}` 보존 · 형제 b 308.67→409 · Canvas/Preview 491/409 양 leg · history +1 → Undo 1회 = Width Fill 2 → Ratio lock → 아래 엣지 −30px → `width 517px` (210 × 2.4639) · `height auto` · aspectRatio 보존 · Canvas 517×209.83 = Preview → Undo 1회 = 잠금 상태 → 핸들 1px 클릭 저장 0), 재실행 `absolute` 16/16 · `ratio-ui` 22/22 · ADR-222 spacing live 27/27 (코너 핸들 우선 유지), 단위 `canvasResizeEdit` 11 · `resizeGeometry` 5 · `projectFillLayout` 2 · builder 스위트 503 파일 · Chrome fixture 20/20 · type-check · preflight. 남은 범위: 다중 선택 Ratio/Absolute live · absolute 요소의 left/top 핸들 위치 이동 · 전체 adapter/출력/Undo · G5 · G6.

**다중 선택 · absolute left/top 핸들 (2026-09-18)**: ① 다중 선택 live (a+b 동시 선택, 실제 패널): Ratio lock 은 요소마다 own used 비율 (`591.33 / 240` · `308.67 / 240`) · history +1 · Undo 1 로 이미 맞았다. Absolute 는 **리더의 left/top 을 전부에 써서 b 가 a 위로 겹쳤다** (b `left 0px`) — 패널이 요소마다 자기 부모·자기 scene bounds 로 inset 을 계산하는 콜백 (`positionStylesFor(id)`) 을 넘기고 store 명령이 요소별로 부른다 (b `left 591.333px`). ② absolute 요소의 left/top 핸들: 잡은 변만 `left/top = 시작 + (시작 크기 − 결과 크기)` (Ratio 로 파생된 종속 축 포함) 로 반대 변 고정. presentation `style.patch` 의 `left/top` 은 CSS px 이고 bridge 의 absolute-leaf 경로가 **현재 CSS left/top 과의 차로 layout x/y 를 옮긴다** (종전은 값을 layout x 로 그대로 써 containing block 원점 ≠ 0 이면 어긋났다 — 소비처 0 이던 lane). 현재 값이 px 가 아니면 (auto) 원점을 모르므로 크기만. 저장은 기존 write 라우팅 (`shouldWriteBreakpointOverride`). 검증: 실제 Builder `multi --multi` **17/17** · `resize-abs --resize --resize-abs` **21/21** (Absolute on → 좌측 엣지 +50 → `width 541px · left 50.33px`, 우측 변 고정, 드래그 중 x 도 이동, Preview 541 → 좌상 코너 −30/−20 → `571×260 · left 20.33 · top −20`, 우하 변 고정 → Undo ×2), 재실행 `absolute` 16/16, 단위 `resizeGeometry` 7 · `canvasResizeEdit` 12 · bridge 6 (fixture 를 CSS left/top 의미로) · TransformSection 16. 남은 범위: 전체 adapter/출력/Undo · G5 · G6. 알려진 한계: absolute **컨테이너** (자식 있음) 의 핸들 resize 는 layout lane 이 targeted-unsupported 로 거부해 미리보기·commit 모두 없다 (ADR-187 lane 의 절대 위치 노드 제외 — 후속).

**G2 왕복 · G6 A/B (2026-09-18)**: G2 — 정적 export 가 Preview 와 같은 `collectResponsiveCss` 로 Fill CSS 를 낸다 (`generateStaticHtml.fillSizing.test.ts`: base Fill 2 → `flex-grow:2 · flex-basis:0px · width:auto`, tablet null + 320px → `flex-grow:0 · width:320px`, mobile 미지정은 tablet 상속, marker 없는 노드 Fill 규칙 0). 실제 Builder `roundtrip --roundtrip` **18/18**: 실제 ⌘C/⌘V 복제본이 `sizing`·`responsive` 를 그대로 (history +1) · tablet 전환에 a·복제본 320 · b Fill · mobile 상속 · refresh 뒤 factor·tier null·tier width·복제본 보존 · 선택/breakpoint 왕복에 history·노드 무변경 (무편집 저장 0). Pencil/JSON/ref descendant 는 기존 `sizingRoundTrip.test.ts` 3. G6 — `adr224-size-perf-ab.mjs`: before = ADR-224 이전 commit `e8987c394` 의 별도 worktree (그 lockfile · wasm 재빌드, 5174) / after = HEAD (5173), 같은 머신 · headless · 1440×900 · DPR 2 · CPU throttle 1, Row 900 + Fill 자식 N=100/1,000, 워밍업 5 + 30회, 부모 resize (`updateElementProps` width ±) 와 가중치 편집 (after `applySizingFromSelection` fill factor / before `updateSelectedStyle("flexGrow")` — 각 빌드의 실제 패널 경로) 의 commit + layout (공유 layout map 갱신까지) p95: parentResize N100 16.9 → 16.1 · N1000 52.4 → 52.6 · factorEdit N100 34.5 → 32.4 · N1000 68.5 → 58.6 — 전부 허용 (max(10%, 1ms)) 안 **PASS** (`docs/adr/evidence/224-g6/` before/after/verdict.json, 로컬). 신규 강제 DOM 측정 0 (resize·projection·geometry 발행은 엔진 layout 만 읽는다). 측정 함정: `updateElement({sizing})` (full-node replace) 로 재면 after 가 +12ms 로 보인다 — 사용자 경로가 아니다 (패널은 `updateAndSave`) · HEAD 에서 marker 자식의 `flexGrow` style 편집은 projection 이 덮어 layout 이 안 바뀐다 (dead 채널 = 대조군 불가). **G5 (blocking)**: 소유자가 foreground Builder 에서 "남은 공간 채우기 / 배분 2:1 / Ratio 잠금·해제 / MinMax 제한" 을 직접 관찰해야 한다 — 자동 결과 (위 live 들) 와 혼동하지 않는다. 확인되면 Implemented 승격 + `### Live Exercise` 절.

**hug 부모의 Fill (2026-09-18, 사용자 보고 "display:flex 에서 height: fill 이 왜 안되지")**: 실제 Builder 재현 — 팔레트 Frame 은 `flex-direction: column` · 높이 없음이라 자식 Height Fill 이 projection `flex-basis: 0px` 로 **pad/border 만 남기고 무너졌다** (Button 30 → 10, 부모 60 → 40). Chrome 도 같은 CSS 에서 같은 값 (10) — 엔진 결함이 아니라 "나눌 공간이 없는 부모에 basis 0" 이라는 투영 규칙의 결함. 수리: `FillParentContext.definite` — 부모가 그 축에 크기를 갖는지 (명시 길이·% · 자기 Fill marker · legacy grow/stretch · body, `hasDefiniteAxisSize`) 를 두 consumer (`createFillTreeResolver` · `projectFillLayoutNodes`) 가 같은 판정으로 넣고, fraction Fill 은 hug 부모에서 `flex-basis: auto` (콘텐츠 크기 유지 = Figma 의 hug 부모 안 fill), 정해진 부모에서 `0px` (남은 공간을 factor 로 분배). 교차축 stretch 는 무관. 검증: 실제 Builder 팔레트 Frame + Button → Height 메뉴 Fill → 30 유지 · 부모 60 유지 (종전 10/40), column 240px 부모는 210 그대로, row 240px 부모 stretch 240 그대로; 단위 `fillSizing` 9 (규칙 · 판정 · emitter) · `projectFillLayout` 3 · shared 131 파일 · Chrome fixture 20/20 · Fill 13 + resize live 18/18. 마커 없는 노드 무변경.

**wrap·Min/Max 독립 재검증과 closure 수리 (2026-09-18)**: 기록된 실행 결과를 근거로 삼지 않고 현재 설계와 코드 경로를 다시 대조했다. fresh Builder에서 `row + wrap + width Fill 1 ×2 + min-width:200px`를 300px 부모에 놓자 Preview는 300×20 두 줄인데 Canvas는 200×20 한 줄이었다. 원인은 Rust flex line collection이 CSS Flexbox §9.3의 outer hypothetical main size가 아니라 unclamped `flex_basis`를 사용한 것; 같은 오류가 §9.7 grow/shrink 방향·inflexible 판정에도 있었다. `FlexItem::hypothetical_main` 하나로 세 경로를 통일했다. 실제 Builder 결과는 Canvas/Preview 모두 300×20, y=0/20이고 Chrome parity fixture 21/21 · 전체 인접 parity 69/69 · Rust lib 427/427이다. Size 패널은 같은 단위의 신규 min>max commit을 현재 canonical active-breakpoint 값과 비교해 차단하고, 서로 다른 단위·수식·reset은 비교하지 않는다. 실제 UI에서 Min W 200 → Max W 100은 authored write/history 0 + alert 1, 이어 Max W 300은 정상 commit했다. 통합 headed Builder run은 Fill/Ratio/Absolute/resize/다중 선택/복제·tier·refresh를 한 프로젝트에서 연속 실행해 41/41, pageerror 0; reload 뒤 Compare/engine probe 복구, history 50-entry 상한, 다중→단일 선택 격리도 harness에 고정했다. G6도 이전 evidence를 재사용하지 않고 `e8987c394` baseline worktree와 현재 작업본을 새로 측정했다: p95 parent resize N100 16.5→17.3ms, factor edit N100 20.5→22.0ms, N1000 53.2→52.5ms / 62.7→63.2ms로 4/4 허용 범위 안이다. 제품·자동 gate의 잔여는 없고 G5 소유자 관찰·확인만 남는다.

**G5 handoff 크리티컬 정합 수리 (2026-09-18)**: 사용자 지정 기존 프로젝트를 foreground Compare로 직접 열어 기록과 독립 검증했다. CSS Preview는 현재 Home 기준인데 Canvas는 Components+Home 전체 scene의 document 원점을 그대로 사용해 현재 페이지가 서로 정렬되지 않았고, body Width 390→400은 canonical/Preview만 400, Canvas는 390이었다. Canvas의 기존 다중 페이지 표시는 기본값으로 보존하고 current page의 document position만 camera에서 상쇄했으며, `현재 페이지만 비교`를 명시적 헤더 필터(기본 off)로 분리했다. `fullTreeLayout`은 authored body width/height를 viewport로 덮지 않고 부재한 축에만 fallback을 주입한다. Preview에서는 canonical Page `FrameNode`/frame-bound `RefNode`가 generic renderer를 타며 `.react-aria-frame|ref` 실제 박스로 누출되던 경로도 닫았다. Page shell은 canonical 문서와 state scope에 남기되 DOM에서는 자식 body를 직접 렌더하고, 사용자·reusable Frame만 layout box로 유지한다. 이어 드러난 ButtonGroup 68/96px 높이 차이는 auto-height 부모의 `height:100%` leaf가 Canvas 측정 scalar를 받지 못한 것과 Preview prop 기본 gap 8이 Canvas projection에 없던 독립 결함이었다. percentage authored 값은 유지하되 높이 basis가 indefinite일 때 intrinsic content로 fallback하고, ButtonGroup의 size/orientation/align 기본 layout을 Preview와 같은 우선순위로 투영했다. 실제 결과는 기본 Canvas에 Components+Home이 함께 보이고 current Home이 CSS leg와 정렬되며, 필터 on에서는 Home만, off에서는 즉시 두 페이지가 복원된다. Preview DOM은 `document.body > .react-aria-body`이고 page shell box는 0개다. body 390×844, ButtonGroup 양쪽 342×96 (`x=24`, `y=374`), Button 30·gap 8이다. Width 390→400은 canonical/Preview/Canvas가 동시에 400이 되었고 reload 뒤 보존됐다. 검증 후 390으로 복원하고 재reload도 확인했다. 자동 수리는 닫혔지만 소유자 확인 전 G5와 Status는 그대로 Accepted다.
...
2026-09-18 headed Chromium의 foreground Builder에서 새 프로젝트를 만들고 실제 팔레트·Size 패널·Canvas handle·Preview iframe을 연속 조작했다. Fill 선택→같은 입력에서 factor2→refresh, Row/Column, Ratio lock/preset/unlock/Undo/refresh, Absolute 왕복, resize/Undo, 다중 Ratio·Absolute, 복제·tier null·breakpoint·refresh·무편집 저장, wrap+Min, Min/Max 오류·복구가 41/41 통과했고 pageerror는 0이다. wrap fixture는 Canvas/Preview가 300×20 두 줄 (y=0/20)로 일치한다. Min W 200 뒤 Max W 100은 canonical/history를 바꾸지 않고 오류를 표시하며 Max W 300은 저장된다. 자동화 결과는 소유자 관찰을 대신하지 않으므로 G5는 UNVERIFIED, Status는 Accepted로 유지한다.
