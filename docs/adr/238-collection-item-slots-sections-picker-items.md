...
- **선택 계약 불변**: 이관 뒤 Select · ComboBox `selectedKey` 가 이관 전과 같은 항목을 가리킨다 (행 `id`/`value` → 항목 `props.id`). Menu per-section selection 필드 (F6) 는 section 노드 props 로 그대로.
...
2. **Section 층**: 새 type `ListBoxSection` · `MenuSection` · `GridListSection` (+ `Header`, Menu 는 `Separator`). section origin 3 을 Components 페이지에 두고 owner origin slot 후보에 더한다 (owner slot 이 없을 때만 seed). section 도 slot host (후보 = 그 목록의 항목 origin). 두 leg 의 정적 항목 key 는 `props.id` 또는 해석 경로를 포함한 유일값 (R4 · F8).
3. **Select · ComboBox**: owner 자신이 목록 틀 (Menu 선례) — 정적 항목 = ListBoxItem origin instance 자식 · section = `ListBoxSection` instance. Canvas 는 트리거만, Preview 는 Popover > ListBox 합성. 이관이 행 `id`/`value` 를 항목 `props.id` 로 옮겨 `selectedKey` 를 보존. 바인딩은 `items` 유지.
...
| R3  | 같은 section origin 의 instance 형제에서 상속 항목 key 가 겹쳐 RAC 선택 · 포커스가 두 항목에 걸린다 (F8 — 44ec413ad 와 같은 뿌리) |  HIGH  | G2 진단 RED (c) → 경로 포함 key 로 GREEN · Preview 객체 동일성 조회 확장                           |
...
| G3   | Phase 3 | unit (원복 RED): Select · ComboBox 항목 instance 자식 · ListBoxItem origin 편집이 popover 항목에 닿음 (Preview renderer unit) · **`selectedKey` 이관 전후 같은 항목 (SelectValue 글자 · Preview 초기 선택 · 클릭 writeback)** · ComboBox 입력 필터가 label 글자로 · 바인딩 무변경 · live (Skia): 트리거 SelectValue 글자 | Select/ComboBox 이관 보류         |
