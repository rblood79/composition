...
| -------------------------- | ----------------- | ----------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------- |
| G0 Inventory freeze        | Phase 0           | `rendererMap`, `DEFAULT_PROPS_MAP`, factory creators, propagation registrations, child-runtime sets의 count와 owner classification 문서화 | count 불명확 시 구현 착수 금지                              |
...
| G7 Contract swap           | Phase 7           | `componentRegistrationContract`가 막던 missing drift를 `entryUniverseContract`가 전부 대체, baseline append 금지                          | 두 contract 병행 유지                                       |
