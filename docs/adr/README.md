...
#### [152](152-data-panel-collection-binding-integration.md) — Data 패널 ↔ Collections ↔ 컴포넌트 Collection 바인딩 통합 (ADR-131/132 후속)

- **상태**: **Accepted 2026-09-11** (`/execute-adr 152` 착수) — Phase 0 (G0 = 0건) · Phase 1 (계약 v2 · `DataField.id` · Map id 키, G1 live 10/10) · Phase 1b (`{#id}` 템플릿 저장형 · 차트 `#id`, G4 live 14/14) · Phase 1c (`DataChange` 적용기 + History `data` entry, G5 live 14/14) · Phase 2 (Inspector fieldMap value/icon Select, live 10/10) · Phase 3 (fieldMap 소비 Skia·DOM 단일 resolver, G2 live 9/9 — Table wrapper 정렬 deferred) · Phase 4 (7종 sweep 12/12, Tabs·Tree 정렬 + TagGroup key 결함 수리) · Phase 5 (legacy 흡수 live 6/6 — 파일 삭제 3건 승인 대기) · Phase 6 (publish snapshot + export fieldId 통과, G3 5/5) Implemented 2026-09-11 — 다음 Phase 7 closure
- **규모**: 실측 격차 6건 (name 참조 파손 / 읽기 경로 3중화 / column mapping 부재 / publish 소비 0건 / store 이중화 / binding 이중 저장 위치) — binding 계약 v2 (`collectionId` + `fieldMap`, lazy upgrade) + `resolveBoundCollection` 단일 resolve + `resolveCollectionItems` fieldMap 소비 (Skia↔DOM 대칭) + legacy 경로 조건부 흡수 + publish data snapshot 직렬화. Phase 0~~7, HIGH 0 / R1~~R6 / G0~G3. ~~착수 금지 (사용자 지시 2026-07-16)~~ **해제 + `fieldId` scope 확장 (2026-09-11 사용자 결정, 리서치 DATA_PANEL_REDESIGN_RESEARCH_2026-09 §5 Track 1) — Risk-First 개정 완료 · 리뷰 round 3 (HIGH 1 · MED 1 · LOW 1) → round 4 수리 검증 이슈 0, 승인 가능 (2026-09-11). Accepted 승격은 사용자 착수 승인 시**. **개정 2026-07-21 (ADR-159 경계 재획정, 사용자 confirm)**: 텍스트 표시(label/description)는 159 `{field}` 템플릿으로 이관 — fieldMap 은 icon/value 비텍스트 한정 축소, API source 전제는 159 dataTable 단일 방향 종속. scope 변경으로 착수 전 재리뷰 대상. design breakdown `design/152-data-panel-collection-binding-integration-breakdown.md`
- **우선순위**: **P1** (착수 중 — 다음 Phase 1c)
...
| 순위 | ADR                                                                                                                                                  | 착수 준비도                                                                                                                           | 차단 · 선행                                                                          |
| :--: | ---------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------ |
|  1   | [152](152-data-panel-collection-binding-integration.md)                                                                                              | **Accepted 2026-09-11 · 착수 중** — Phase 0 ~ 6 Implemented (G1 10/10 · G4 14/14 · G5 14/14 · P2 10/10 · G2 9/9 · P4 12/12 · P5 6/6 · G3 5/5), 다음 Phase 7 closure (+ P5 legacy 파일 삭제 승인) (`DataChange` 적용기 + History) | 승인 시 3건 동시 해소 — 013 착수 조건 G0 / 157 G1 Preview 대조 residual / 194 R7     |
|  2   | [150](150-rac-pencil-residual-interaction-execution.md) (A3)                                                                                         | Accepted · A1 철회(2026-07-20 D1/D3 경계 재판정) · A2 delivered(시각 확인 대기) · A3 미착수                                           | A2 live 확인 + ADR-148 Phase 4 `canvasSceneNode` 표면 재실측 후 A3                   |
|  3   | [162](162-gridlist-template-subtree-projection.md)                                                                                                   | round 1 이슈 2건 fixed. 선행 의존 ADR-159 P1/P4 는 **Implemented 로 해소**                                                            | R1 HIGH(카드 높이 formula→실측 전환) 잔존 — Phase 0 재실측 필요                      |
|  4   | [921](921-render-scene-backend-integration.md)                                                                                                       | round 1 LOW 3건 fixed이나 **Phase 0 baseline 재freeze 선행 필요** (187~190 이후 §6-2 파일 대량 변경)                                  | 재freeze 전 착수 금지                                                                |
|  5   | [013](013-quick-connect-data-binding.md)                                                                                                             | Risk-First 재작성 완료(round 2 전부 fixed)                                                                                            | 착수 조건 G0 = **152 Implemented** + Phase 0 재-inventory                            |
|  6   | [212](212-data-panel-editor-redesign.md) · [213](213-data-tool-contract-propose-review-apply.md) · [214](214-variables-owner-model-runtime-state.md) | Proposed 2026-09-11 (리서치 DATA_PANEL_REDESIGN Track 2 · 3 · Variables) — 리뷰 전                                                    | 212 Phase 1 · 213 Phase 1/3 · 214 는 152 와 독립 착수 가능, 나머지는 152 Phase 1c 뒤 |
...
<details>
<summary>2. ADR-152 (사용자 착수 승인 필요)</summary>

```
ADR-152 착수를 승인한다. Accepted 승격 후 /execute-adr 152

- Phase 0 실측 먼저 (binding 이중 저장 위치 분포 격차 6 포함) — legacy 경로 흡수는 실측 게이트 조건부
...
- 완료 기준: 동일 collections 스냅샷에서 Skia↔DOM 동일 row/label (/cross-check) +
  publish 앱 실렌더 1회 exercise + 격차 7(provider 미마운트) 수리로 157 G1 · 194 R7 동시 해소
```

</details>

<details>
<summary>3. ADR-013 (152 완료 후)</summary>
