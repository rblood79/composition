...
`toNumberFieldRacProps()` 를 사용해 catalog binding projection 을 shared wrapper 의
...
문서화했다. 이 slice 는 placeable eight primitive plus Breadcrumb subpart 의 G6
...
Status: In Progress — 2026-05-20 (Separator line + Link/Breadcrumb/TextField/NumberField text +
...
`breadcrumb` / `text-field` / `number-field` / `toggle-button` 를 추가하고, generic Skia path 가
Separator resolved node 를 `line` node 로, Link resolved node 를 underline text node
로, Breadcrumb subpart 를 text node 로, TextField/NumberField resolved node 를
...
`TextFieldSpec.render.shapes()`, `NumberFieldSpec.render.shapes()`, `ToggleButtonSpec.render.shapes()`,
`ToggleButtonGroupSpec.render.shapes()`,
`ToolbarSpec.render.shapes()` 미호출을 검증한다.
Button/Separator/Link/Breadcrumbs/TextField/NumberField/ToggleButton/ToggleButtonGroup/Toolbar 외
