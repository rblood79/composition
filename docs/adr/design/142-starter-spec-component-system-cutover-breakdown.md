...
> **2026-05-30 실측 recalibration #2 — #3/#4 의 진입점 부분은 ADR-116 으로 이미 충족 (fork 없음, M3 흡수)**: 본 breakdown #3/#4 는 "Preview canonical = `?canonical` opt-in, 기본 legacy" 전제로 작성됐으나, **실측상 ADR-116 이 canonical 을 기본 ON 으로 이미 전환**했다 (`apps/builder/src/preview/App.tsx:58-60` `?canonical !== "0"` → opt-out, `:850` canonical 우선 resolve → `CanonicalNodeRenderer`, `:873+`/`:901+` legacy 는 fallback 안전망). 즉 **canonical 데이터 파이프라인(document→resolve→ResolvedNode 트리→단일 렌더 진입점)은 ADR-116 이 이미 제공** — #3 의 "단일 진입점" / #4 의 "resolved-tree 소비" 는 충족됨. **그러나** `CanonicalNodeRenderer` 는 아직 legacy per-component `rendererMap` 으로 위임한다 (`CanonicalNodeRenderer.tsx:~106`). ADR-142 의 근본 목적은 "canonical 을 진입점으로" 가 아니라 **per-component 렌더링 코드(DOM `rendererMap` / Skia `render.shapes`)를 binding+theme generic 경로로 교체**하는 것이므로, #3 의 ADR-142 고유 잔여분 = **`CanonicalNodeRenderer` 가 catalog primitive 를 `toRacProps`→RAC 로 렌더** (= #5 Skia 의 거울, 같은 generic-renderer 핵심의 양면). gap 원인은 ADR-903/116-era framing 자동 승계(ADR-111/112 패턴) — 새 ADR fork 사유 아님, 본 노트로 흡수. **파일럿 land**: `packages/shared/src/catalog/bindings`(getPrimitiveBinding) + `CanonicalNodeRenderer` 의 `cutoverPrimitives`-게이트 generic Button 렌더 (기본 미지정 → live 회귀 0). family cutover(Phase 6) 가 type 을 catalog 로 옮길 때 caller 가 `cutoverPrimitives` 에 추가한다.
