...
| 5   | Overlay                  | Dialog / Popover / Tooltip / Modal                                               | 중               | ✅ **slice 5 완료(2026-06-19)**: 정찰(13 agent) → fix 2(radius xs/2xl 토큰 + Popover arrow 2px) + defer 4. 다층 box-shadow elevation 평탄화는 **defer**(shadow element-ownership=별도 slice / Modal cascade=별도 ADR — 본 slice 미수정). 실제 fix=공유 토큰 결함 + arrow 한 줄(검증-우선 좁은 slice). 진행 로그 본문 참조 |
...
| Sheet            | 신규 등록 (미흡수) | 후속 (slice 5 정찰 scope 외)          | RAC Modal + ModalOverlay (슬라이드 변형). slice 5 는 검증-우선 좁은 slice(radius 토큰+arrow fix)로 진행 → Sheet 신규 등록은 흡수 안 함, 후속 작업 |
