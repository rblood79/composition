...
### Phase 2 — G2 자유 내용 채우기 · 채운 노드 편집 · 팔레트 영역 삽입 · Canvas drop (2026-09-24)

- **새 코드 사실** (G2 실측 — Phase 2 설계 입력 · 결함):

| ID  | 사실                                                                                                                                                                                                                                                                 | 위치                                                                                                   | 처리                                                                                                      |
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------- |
| F21 | 팔레트는 클릭 삽입만 있다 (canvas 로 끄는 drag 없음) — "Canvas drop" 은 canvas 위 요소 이동 (`useDragBridge`) 이다                                                                                                                                                   | `panels/components/ComponentList.tsx` · `setData` 소비처 0                                             | drop = 기존 요소를 영역으로 옮기기 (mode C 로 소유 이전)                                                  |
| F22 | `ref-descendants` 이동 대상 · 커널 (`moveCanonicalChildToDescendants`) 은 페이지 frame slot 용으로 이미 있다                                                                                                                                                         | `resolveCanonicalMutationTarget.ts:30-38` · `compositionDocumentOrder.ts:606`                          | synthetic 영역 → `ref-descendants` 로 해석해 재사용                                                       |
| F23 | move history 의 `to` 는 이동 뒤 문서에서 찾는다 — mode C 안 노드는 부모가 instance 로 잡혀 **Redo 가 노드를 instance 자기 자식으로 넣는다** (live ④ 실측 · 주석이 "ref override 내부 노드 금지" 로 적은 경계)                                                        | `canonicalHistoryEvents.ts:560-600`                                                                    | `ref-descendants` 이동은 remove + instance replace 로 기록 (`buildCanonicalMoveIntoRefDescendantsEvents`) |
| F24 | 채운 영역 host 자체를 편집하면 항목이 `{ children, style }` — **Canvas 는 style 을 싣고 Preview mode C 분기는 버린다** (D3 비대칭) · Slot 채우기는 항목을 `{ children }` 로 덮어 영역 style 을 지우고 · style 만 있는 영역 (mode A) 에는 drop 삽입이 조용히 무시된다 | `resolvers/canonical/index.ts:481-494` · `slotFillPath.writeSlotFill` · `insertIntoDescendantChildren` | Preview 는 같은 항목 props patch 적용 · 채우기/비우기는 영역 편집 보존 · 삽입 커널은 mode B 만 거부       |
| F25 | drag bridge 는 지나온 drop 대상을 hysteresis 로 붙든다 — 아래에서 끌어 올리면 경로의 첫 영역 (Card Footer) 에 들어간다                                                                                                                                               | `useDragBridge.ts` (prevTarget 유지 분기)                                                              | 기존 UX (변경 없음) — live 하니스는 경로상 첫 영역을 목표로                                               |
| F26 | presentation lane (Styles pilot · 캔버스 spacing/resize 핸들) 의 대상 해석은 master 경로만 걷는다 — 채운 노드는 null → Styles 는 inspector 경로 (`updateAndSave`) 로 떨어지고, 캔버스 spacing/resize 핸들은 채운 노드에 없다                                         | `editorPresentationCommitAdapter.ts:174-267`                                                           | LOW deferred (production 결함 아님 — 채운 노드는 Properties/Styles 로 편집)                               |
| F27 | Preview resolver cache key 는 ref id + descendants 만 본다 (props 제외) — 두 instance 의 같은 id 채움 ref 가 공유 cache 에서 충돌할 수 있다. production Preview (`preview/App.tsx:247`) 는 cache 없이 해석                                                           | `resolvers/canonical/index.ts:114-141`                                                                 | LOW deferred (production 재현 없음)                                                                       |

- **구현**:
  - `components/slotFillEdit.ts` (`applyEditToSlotFill`) — synthetic 편집 경로에 mode C 조상 (가장 바깥) 이 있으면 **배열 안 그 노드** 를 고친다 (segment = Canvas `getOverrideNodeSegment` 규칙 · props 는 style 심층 병합 · `null` 은 지움 · `fills` 는 1차 필드 · 채운 ref 안쪽은 그 ref 의 자기 descendants 로 재귀). `inspectorActions.buildInstanceDescendantPatches` 한 곳에 연결 → Properties · Styles · fills (`updateSelectedFills`) · 자식 갱신 전부 같은 경로. history 는 기존 instance replace event.
  - Canvas mode C synthetic 노드에 `fills` 탑재 (F19 — `materializeOverrideChildren`).
  - `components/slotFillNodes.ts` — 채움 노드 생성 하나: reusable origin → ref · 자유 내용 (`Text` · `Image` · `Icon` · `Separator` · `frame` — 팔레트 항목 중 reusable origin 없는 leaf, 팔레트 생성과 같은 props 합성) → plain · 복합 factory type → null.
  - Slot 채우기 절: 이름 영역 host 는 후보에 자유 내용 5종 (reusable 뒤). Popover · Tooltip instance (`FrameSlotSection`): 자유 내용 Insert (instance 자기 자식 plain 노드 — 상속 Heading · Description 뒤) · Clear (자기 자식만 삭제).
  - `components/slotRegionInsert.ts` — synthetic id → 가장 가까운 **자유 내용 slot host** (`isFreeContentSlotHost` = `slot` 배열 + 목록 틀 · 그룹 규칙 아님 → 이름 영역 · frame 가족 slot). 팔레트 클릭 (F18 · `ComponentsPanel`) 은 그 host 의 mode C 끝에 추가하고 새 노드를 선택.
  - Canvas drop (F20): `acceptsDraggedElement` — synthetic 후보는 자유 내용 slot host 만 (목록 틀 · inherited · 고정 부품 거부). `resolveCanonicalMoveTarget` — synthetic host → `ref-descendants` (F22). history 는 F23 수리 (drag bridge 3 지점: 단일 flow · 절대배치 · 다중).
  - F24 수리 3 곳 (Preview mode C 항목 props patch · `writeSlotFill` 영역 편집 보존 · shared 삽입 커널 mode A 보존).
- **G2 unit** (`adr240Phase2.freeContent.test.tsx` 22 · 진단 (e)(e2) `it` 전환 4 · `ComponentSlotFillSection` +2 · `FrameSlotSection` +2 — 240 3 파일 56 PASS · 1 expected fail = (g) LOW):
  - 실제 store 쓰기 (`createInspectorActionsSlice` + canonical store): primitive Text · ref 자식 (234 Slot "+" 모양) 편집 → 배열 노드 · 바깥 `Content/<id>` 키 0 · Preview · Canvas 둘 다 B · style 심층 병합 · `null` 삭제 · 채운 ref 안쪽 · Undo/Redo (기록된 replace event 재생 · 두 leg A ↔ B) · `updateSelectedFills` → 배열 노드 `fills` · 두 leg.
  - 자유 내용 5종 × 두 leg 같은 노드 · 채움 노드 규칙 · 자유 내용 type = 팔레트 항목 중 origin 없는 것 (drift 가드) · 팔레트 삽입 대상 (영역 · 영역 안 노드 = 그 영역 · Dialog 제목 · Close · 본문 · instance root = null) · 삽입 계획 두 leg.
  - drop 판정 (영역 host 받음 · inherited · 고정 부품 · DialogFooter · TabList 거부) · `ref-descendants` 이동 두 leg · 이동 history (반례: 종전 move event 는 Redo 뒤 영역이 빈다 · 수리: Undo = 원래 자리 (instance 앞 형제 순서 포함) · Redo = 영역) · 영역 style 두 leg · 채우기/비우기 보존 · mode A 영역 drop.
- **원복 RED 15/15** (편집 역적용 → 5 파일 실행 → 복원): R1 inspector mode C 판정 off 5 · R2 배열 fills off 1 · R3 Canvas fills off 1 · R4 채운 ref 안쪽 off 1 · R5 자유 내용 후보 off 1 · R6 Popover 자유 내용 · Clear off 1 · R7 영역 해석 off 3 · R8 synthetic drop 게이트 off 1 · R9 `ref-descendants` 대상 off 1 · R10 history instance index 보정 off 1 · R11 목록 틀 제외 off 1 · R12 primitive 생성 off 8 · R13 Preview 항목 props off 1 · R14 채우기 영역 편집 보존 off 1 · R15 mode A drop 삽입 off 1.
- **live** (`apps/builder/scripts/adr240-phase2-live.mjs`, headed Playwright · worktree dev 5181 · Skia layout · store, Compare Mode · Preview 미개방) **9/9**:
  - ① Card Content 영역 선택 + 팔레트 Text 클릭 → `descendants.Content.children = [text]` · Skia rect `live-card/Content/text` · 새 노드 선택.
  - ② 채운 Text 편집 (`updateSelectedProperties` — Properties 가 부르는 action) → 배열 노드 `children` 갱신 · 바깥 키 0.
  - ③ 페이지 Text 를 pointer drag (마우스 down · 16 step · up) 로 Card Footer 영역에 → `descendants.Footer.children = [live-text]` · body 에서 빠짐 · Skia rect.
  - ④ Undo → body 로 · 영역 빔 / Redo → 다시 영역 (Skia rect) — 수리 전 Redo 는 instance 자기 자식으로 넣었다 (F23 발견 지점).
  - ⑤ 상속 Title 위로 drag → `Header/Title…` 키 0 · 노드는 경로상 첫 영역 (Footer) mode C.
  - ⑥ Dialog Content 영역 + 팔레트 Text → mode C · Skia rect · Close rect 불변 (x 252 · 68×30).
  - ⑦ Popover instance 자유 내용 Insert Text → 자기 자식 plain Text (y 76 ≥ Description 48+16) · Clear → 자기 자식 0 · 상속 Description 유지.
  - ⑧ reload → Content · Footer · Dialog Content 채움 보존 · Skia rect. page error 0.
- **LOW deferred**: F26 (채운 노드의 캔버스 spacing/resize 핸들 · Styles pilot) · F27 (Preview 공유 cache props 키) · (g) (Phase 1 이월). Preview 채널은 unit 고정 (Compare Mode 미개방 — 사용자 확인 대상).
- **회귀**: builder 918 파일 — 실패 6 = Phase 1 과 같은 6 (base 에서도 실패 5 + textAxisGate · adr113 grep gate 44 건 = base 와 같음, 새 경계 파일 2 는 allowlist) · shared 138 파일 — 실패 2 (catalog placeable 단일성 · generated CSS 인벤토리, 기존) · type-check PASS · Phase 1 live 재실행 6/6.

### Phase 3 — G3 성능 A/B · G4 BC (2026-09-24)

- **대조 arm**: base `61d29f98a` 별도 worktree (scratchpad `adr240-base`, 그 lockfile 의존성 · 엔진 소스 동일이라 wasm 복사) dev 5182 · 240 arm = 이 worktree dev 5181. 같은 headed Playwright 세션 · arm 교대 · DPR 1 · visibilityState `visible` · Compare Mode · Preview 미개방.
- **새 코드 사실** (G4 실측):

| ID  | 사실                                                                                                                                                                                                                                                             | 위치                                                                                  | 처리                                                                                                    |
| --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| F28 | origin 편집 history 는 Components body 전체 (또는 origin 하나) 의 remove + insert 스냅샷이다 — 이관 뒤 Undo 하면 240 전 스냅샷이 그대로 들어가 Dialog Content · Actions 영역과 Card 영역 `slot` 이 사라진다 (이미 전치된 instance 경로와 어긋남 · reload 전까지) | `canonicalHistoryEvents.ts` `applyCanonicalHistoryEventsToDocument` · 사용자 IDB 실측 | 결함 — 재생 시점에 origin 스냅샷도 같은 영역 이관 (`ensureRegionSlotsInSnapshot`) · 원복 RED 2/2 · live |
| F29 | Components body 스냅샷 Undo/Redo 뒤 저장 → reload 에서 ListBoxItem origin 의 선택 배경 (`var(--accent-subtle)`) 이 빠진다                                                                                                                                        | base → base 같은 조작에서도 재현                                                      | 범위 밖 (240 전부터 — 대조 arm 으로 확인) · 사용자 보고                                                 |
| F30 | 저장 history 는 `pageId` 키만 가진다 (`projectId` 없음) — `page-components` 는 모든 프로젝트가 같은 id                                                                                                                                                           | `historyIndexedDB.ts` 항목 모양 · 사용자 IDB 실측                                     | 범위 밖 (관찰 — 프로젝트 사이 Components history 공유 가능성, live 미확인) · 사용자 보고                |

- **G4 사용자 문서 계수** (이관 빌드를 열기 전 · 사용자 Chrome 의 `localhost:5173` origin · 앱 코드를 싣지 않는 정적 파일 탭에서 IndexedDB 를 현재 버전 지정 · 읽기 전용): 프로젝트 9 · Dialog · Card · Popover · Tooltip instance **0** → 경로 전치 대상 0 · Δbyte 0. 저장 history 1,346 항목 · 22 페이지 — Dialog instance 스냅샷 0 · Dialog origin 을 담은 항목 46 (전부 `page-components`: body 전체 remove + insert 42 · Dialog origin 3 · 본문 update 1) → F28 발견.
- **G4 live** (`apps/builder/scripts/adr240-g4-bc-live.mjs`) — before arm 에서 Card · Dialog (`defaultOpen`) · Popover instance 를 두고 **실제 Properties 쓰기 action** (`updateSelectedProperties`) 으로 Card Description · Dialog Close "Done" · Dialog Description "Edited 1" (S1) → "Edited 2" (S2) · Components 페이지 Dialog 본문 origin padding 편집. 저장 층 문서 + 두 페이지 저장 history 를 after arm 에 써 넣고 reload (이관). Components history 에 사용자 IDB 모양 (body 전체 remove + insert, 240 전 문서) 한 건을 더한다.
  - 픽셀 (Skia, 채널 차 > 16): 이관 뒤 vs S2 · Undo vs S1 · Redo vs S2 — Card 320×322 · Dialog 본문 400×172 · Popover 300×80 **전부 0**.
  - Δbyte: Dialog instance +8 (Description 키 1 × `Content/`) · Card 0 · Popover 0 — Phase 0 수식 그대로.
  - 저장 history 이관 → Undo → Redo: 키 = `component-dialog__2/Content/component-dialog__2_2` (Close 키 불변) · Undo "Edited 1" · Redo "Edited 2" · 옛 경로 키 0.
  - Components 페이지 Undo (body 스냅샷) → Dialog 본문 `[title, content-region, footer]` · footer `[actions-region, Close]` · Card content `slot` 유지 · Redo 같음 (F28 수리 전 모양은 unit 반례로 RED).
  - 재hydration Δ0: 이관 직후 저장 → reload **true**. Undo/Redo 뒤 reload 는 F29 한 건 (base → base 도 같음).
  - page error 0 (두 arm).
- **G3** (`apps/builder/scripts/adr240-g3-perf-ab.mjs`, 3 pair · warm-up 3 · 표본 7 · 판정 = `scene.build` p95 median Δ (240 − base) ≤ +1 ms). fixture = Card 50 · Dialog 20 instance, 영역 채움 3 노드씩 (Text 2 + Button ref) — 같은 문서를 두 arm 에. base 는 Card 채움은 그리고 Dialog 영역 키는 경로가 없어 무시 (240 전에는 Dialog 에 채울 자리가 없다) → 그린 노드 640 vs 720.

  | 조작 (불리)                           | base p95 median | 240 p95 median |        Δ | 편집→화면 총시간 Δ | 판정 |
  | ------------------------------------- | --------------: | -------------: | -------: | -----------------: | ---- |
  | fillEdit (Card 0 채운 Text)           |             4.6 |            5.3 |     +0.7 |              +11.4 | PASS |
  | cardOriginEdit (instance 50 무효화)   |             8.5 |            8.9 |     +0.4 |               −0.1 | PASS |
  | dialogOriginEdit (instance 20 무효화) |             8.4 |           10.5 | **+2.1** |               −1.2 | FAIL |
