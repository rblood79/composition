...
## 2. Current Baseline (Phase 0 freeze — 2026-09-08 재실측, HEAD `795f405b5`)

> 초안은 2026-08-27 Explore 결과 (HEAD `e319a95af`). 아래는 Phase 0 (G0) 에서 전 항목을 재실측해
> 갱신한 표다. **변동 4건** (CanvasKit 0.40→0.42 + ADR-117 Implemented · 라인 이동 5 · PALETTE_ORDER
> 의 TableView source 가 `overlay` · R10 은 심볼 이관으로 이미 등재) 은 비고에 표시했다.

| 항목           | 실측 (2026-09-08)                                                                                                                                                                                                                                                                                                        |
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 차트 의존성    | 0건 — d3/vega/recharts/echarts/chart.js/visx/nivo/victory 전부 없음 (lockfile transitive 포함). **변동 없음**                                                                                                                                                                                                            |
| CanvasKit      | `apps/builder/package.json:41` `^0.42.0`. **변동**: ADR-117 이 2026-08-28 Implemented (PathBuilder 단일 생성 경로) → 0.42.0 전환 완료. `Path.MakeFromSVGString` 은 `nodeRendererShapes.ts:238` 에 그대로 존재 (0.42.0 유지 API) → 본 ADR 의 path 생성 경로는 117 완료 후에도 무변경. publish 앱은 CanvasKit 없음 |
| Shape 유니온   | `packages/specs/src/types/shape.types.ts:34-46` 12종 — rect/roundRect/circle/arc/text/shadow/border/container/gradient/image/line/icon_font. **임의 path·polyline·회전 텍스트 없음**. 변동 없음                                                                                                                        |
| SVG path→Skia  | `skia/nodeRendererShapes.ts:238` `ck.Path.MakeFromSVGString(d)` 1곳 (가드 `:220 if (!node.iconPath) return`) — 입력이 `IconFontShape.iconName` → lucide 레지스트리로 잠김 (`specShapeConverter.ts:484-540`, `iconPath` 조립 `:525`). 노드 레벨 `iconPath.paths: string[]` (`nodeRendererTypes.ts:134`) 은 임의 문자열 수용 |
| converter case | `specShapeConverter.ts` 12 case — `:257 roundRect` `:311 rect` `:364 circle` `:405 arc` `:443 line` `:484 icon_font` `:541 border` `:736 text` `:930 shadow` `:957 container` `:976 gradient` `:1083 image`. **`case "path"` 없음** (Phase 1 추가 지점)                                                                 |
| 노드 transform | `nodeRendererTypes.ts` `transform?: Float32Array` → `renderCommands.ts` `canvas.concat`. Shape 레벨 진입로 없음 (텍스트 회전은 v2)                                                                                                                                                                                     |
| Skia primitive | `packages/specs/src/renderers/skiaPrimitives.ts` `SkiaPrimitiveDrawFn = (ctx:{props,size,visual,paint,style}) => Shape[] \| null` (**rows 채널 없음 — R8 확정**); 등록 `SKIA_PRIMITIVES` `:3278`, 모드 `SKIA_PRIMITIVE_MODES`; dispatch `buildSpecNodeData.ts:1195-1320` `buildCatalogShapesOrPrimitive` (binding read `:1212`, replace 루프 `:1247-1259`) |
| DOM 3rd-party  | `PrimitiveSource.kind:"internal"` + `renderer` (`catalog/types.ts:47-61`) → `packages/shared/src/renderers/index.ts:19` `rendererMap` → `renderFacetDeclaration.ts` delegating-internal. 선례 TableView(@tanstack/react-table)                                                                                          |
| 등록 지점      | catalog entry `componentCatalog.ts` `primitiveEntry` (Icon `:129` · TableView `:443`) · binding `catalog/bindings/*.binding.ts` · rule `catalog/generated/componentRulesTable.ts` · `panels/components/paletteItems.ts:171` `PALETTE_ORDER` (**변동**: TableView 는 `:240 source:"overlay"` — Chart 는 `source:"catalog"` 로 신규) + oracle `panels/components/__tests__/paletteOracle.ts:340` · factory · defaults · rendererMap · publish `apps/publish/src/registry/ComponentRegistry.tsx` (**경로 정정** — 초안의 `apps/publish/src/components/` 아님) |
| CI ratchet     | `factories/__tests__/componentRegistrationContract.test.ts:131` `BASELINE_RATCHET = {rendererMap:0, TAG_SPEC_MAP:0, getDefaultProps:0}`. **R6 판정**: `RegistryName` 은 이 3개뿐 (`:96`) — publish `ComponentRegistry.tsx` 는 계약 테스트 범위 **밖** → Phase 3 에서 publish registry oracle 추가 필요 (확정)               |
| 데이터 계약    | `readDataBindingRows(dataBinding: unknown, collections: readonly CollectionDataSource[] = []): unknown[]` — `packages/shared/src/collections/resolveCollectionItems.ts:211`, **동기 함수** (async 아님 → primitive/renderer 양쪽에서 그대로 호출 가능). `DataBinding {type, source, config}` (`shared/src/types/element.types.ts:36`) · 샘플 N행 정책 ADR-157 |
...
| 폰트/문단      | `skia/fontManager.ts` 공유 FontCollection · `nodeRendererText.ts` `MakeFromFontCollection` (per-call `ParagraphBuilder.Make` 금지 — 5.78MB/paragraph 복제)                                                                                                                                                              |
| 자체 SVG 선례  | `panels/monitor/components/RealtimeChart.tsx` (227줄, SVG polyline + threshold, zero-dep)                                                                                                                                                                                                                               |
| 번들 규칙      | `CLAUDE.md` 초기 번들 <500KB · `component-design/SKILL.md:81` 외부 라이브러리 추가 금지                                                                                                                                                                                                                                 |

### G0 판정 (Phase 0 종료 조건)

| 항목                                       | 판정                                                                                                                                                                                                                                                                                                                                                                                                     |
| ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **R1 — generate-css 가 chart 채널 emit?**  | **미지원 → Phase 3 에 emit 확장 선행 commit 추가** (ADR G0 실패 대안 그대로). 근거: CSS custom prop emit 은 `CSSGenerator.ts:1003/1008` 이 `SizeSpec.iconSize/iconGap` 을 **size 블록 안에서** 내는 1패턴뿐이고, 각 채널은 3단 수동 확장 (`ComponentRuleSize` 타입 → `generate-css.ts` `ruleSizeToSizeSpec` 변환 → `CSSGenerator` emit 줄) 이다. 시리즈 색 **배열** 은 size 축도 `ComponentRuleVariant.colors`(typed key) 축도 아니라 현재 어느 경로로도 나가지 못한다. → `ComponentRule.chart?: ComponentRuleChart` **top-level 채널 신설** + root 블록 emit (`ComponentRule` 정본 `shared/src/types/composition-document.types.ts:480-529` — 기존 top-level 채널 `textDecoration`/`containerStyles`/`densities` 와 같은 층) |
| **R6 — publish registry 검출 테스트 존재?** | **없음 → Phase 3 에서 oracle 추가** (확정). `componentRegistrationContract.test.ts:96` `RegistryName = "rendererMap" \| "TAG_SPEC_MAP" \| "getDefaultProps"` — publish 는 세 축 어디에도 없다                                                                                                                                                                                                          |
| **R8 — primitive ctx 에 rows 채널?**       | **없음 (확정)** — `SkiaPrimitiveDrawFn` ctx 5필드에 데이터 채널 0. 주입 지점은 `buildSpecNodeData.ts:1760-1766` `CONTAINER_DIMENSION_TAGS` 블록 (`_containerWidth`/`_containerHeight`) 과 동형 자리 — 여기에 `_chartRows` 를 얹는다                                                                                                                                                                       |
| **R9 — PathShape bbox 필요?**              | **필요 (확정)** — 노드 width/height 가 컬링·캐시 키 축이므로 `PathShape.width/height` 필수 필드 유지                                                                                                                                                                                                                                                                                                    |
| **R10 — `size` → layout 무효화 등재?**     | **이미 등재됨 → 추가 작업 0 (해소)**. 초안의 "grep 0건" 은 심볼 이관 후의 stale grep 이었다 — 정본은 `presentation/invalidation/editorMutationEffectRegistry.ts` 로 이관됐고 `"size"` 가 `:31 LAYOUT_AFFECTING_PROP_SOURCE` · `:96` (cacheSignature 집합) · `:362 SELF_BOX_PROP_KEYS` 3곳에 등재돼 있다. `LAYOUT_AFFECTING_PROP_KEYS`(`:583`)/`LAYOUT_PROP_KEYS`(`:595`) 는 이 registry 파생 뷰다 |
| **ADR-117 진행 상태**                      | **Implemented 2026-08-28** — CanvasKit 0.42.0 + PathBuilder 단일 생성 경로. `MakeFromSVGString` 유지 확인 → 직교 전제 성립 (본 ADR 무영향)                                                                                                                                                                                                                                                              |
| **추정 vs 실측 gap**                       | 라인 이동 5 · 경로 정정 1 · source 값 정정 1 · R10 해소 1. scope 배수 **1.0x** (신규 작업 없음, R1/R6 은 ADR 이 이미 조건부로 예고한 대안) → fork 사유 없음 (M3)                                                                                                                                                                                                                                        |
