...
### Phase 2 — 펼침 두 leg 대칭 (G2, 2026-09-25)

- **정본 = 소속 Tree `expandedKeys`** — Canvas 판정 `isTreeItemExpanded` (`workspace/canvas/treeItemRow.ts`): Preview `renderTree` 와 같은 읽기 (배열 아니면 `[]` — controlled, 부재 = 전부 접힘) · 항목 key = `resolveCanvasTreeItemKey`. Tree 밖 단독 TreeItem (Components 페이지 origin · `--collapsed` 변형) 은 자기 `isExpanded` (부재 = 펼침, 237 Disclosure 선례). layout 평탄화 (`flattenTreeRows`) 가 접힌 항목의 자식 행을 싣지 않고 (Skia 는 layout 자식표 밖 노드를 그리지 않는다 · 히트도 없다), Skia 는 `isExpanded` 를 TreeItem 에 주입해 chevron 방향 (`leading_icon` — 펼침 ↓ · 접힘 →).
- **무효화**: `expandedKeys` 를 계층 A (`LAYOUT_AFFECTING_PROP_KEYS`) · B (`LAYOUT_PROP_KEYS`) 양쪽 등재 (ADR-187 baseline JSON 같은 자리) · Skia 증분 재빌드 확장 — Tree 가 바뀌면 TreeItem 자손 전부 (synthetic-merge 규칙은 손주까지라 3 단계 이상 chevron 이 stale), TreeItem 이 바뀌면 부모 TreeItem.
- **접힘 층** (`--collapsed`): Canvas 해석 끝 위치 상태 pass (`applyPositionalStateLayers`) 가 Tree · TreeItem 부모의 TreeItem 자식에 RAC render prop 과 같은 조건 (`isExpanded` = 자식 항목 있음 ∧ key ∈ `expandedKeys` — 잎은 false, `aria-expanded` 없음) 으로 얹는다 · Preview 는 `stateStyle` 이 RAC `isExpanded` 로. 기본 `--collapsed` (props `isExpanded:false` 만) 는 해석 결과를 바꾸지 않아 재합성을 건너뛴다 (`isInertCollapsedLayer` — 잎 항목마다 매 build 재합성 방지).
- **저장 배선**: `BUILDER_SYNCED_PREVIEW_PROPS` += `expandedKeys` — Preview 토글이 `ELEMENT_PROPS_CHANGED` → builder `updateElementProps` (history · canonical · persist). 선택 `selectedKeys` 역전파는 범위 밖 (기록만).
- **이관 (펼침 채우기)**: `migrateTreeHost` 가 plain TreeItem → ref 변환과 **같은 pass 에서만** `expandedKeys` 부재 · `[]` + 중첩 항목이면 부모 항목 새 key 전부로 채운다 — 변환이 끝난 Tree 의 `[]` 는 사용자가 전부 접은 값이라 다시 채우지 않는다 (별도 표식 없이 1회 · "plain TreeItem 이 있는 Tree = 239 전 문서"). 비어 있지 않은 배열은 보존 (key 대응만). Tree instance (2단계): 자기 plain 자식도 변환 · 자기 값이 `[]` 이거나 자기 중첩 항목이 새로 생기면 origin 의 채운 값 + 자기 부모 항목 · key 필드는 origin 대응표 + 자기 대응표 (instance 는 대응 없는 key 를 그대로 둔다 — origin 쪽이 보류 판정).
- **Slot "+" 가 접힌 항목에 넣을 때**: 펼침 정본대로면 넣은 항목이 두 leg 에서 안 보인다 → 항목 host 에 넣으면 host key 를 소속 Tree `expandedKeys` 에 더한다 (`treeHostExpansion` — ref host 는 owner Tree props, instance 가 상속한 host 는 origin 안 key 로 instance props). live 에서 발견 (Phase 1 하니스의 중첩 삽입이 접힌 항목 아래라 행이 사라짐).
- unit: `adr239Phase2.expansion.test.tsx` 9/9 (부재 · `[]` 채우기 · 비어 있지 않은 보존 · 1회 · 중첩 없음 · instance 병합 · 새 문서 origin 상속 · 역전파 allowlist · `--collapsed` 층 두 leg 같은 행 · Preview chevron 클릭 → `expandedKeys: []`) · 진단 (b) `it` 전환 (Canvas 행 = layout 평탄화 + 펼침 판정, `[]`/`["ti-c"]`/`["ti-a"]` 세 경우 Preview 행 집합과 같음) · layout unit 접힘 케이스.
