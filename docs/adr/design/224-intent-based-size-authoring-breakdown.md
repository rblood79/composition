...
최신 실행: 기본 `fit-content` 보존 + 콘텐츠 측정 분리로 기존 Fill 차단을 수리했고 실제 Builder 13/13을 통과했다. Ratio 후보의 Canvas 높이 30px / Preview 154.328~295.664px 발산으로 다시 중단했다. Ratio 후보는 로컬 evidence 패치로 분리, Fill 수리는 유지, 전체 Phase는 미완료다. 이후 그 발산을 엔진 경계 (flex 커널 aspect 슬롯 22 + leaf 전송값 content-box 보고) 로 확정·수리했다 — 실제 Builder 17/17, Chrome oracle fixture 20/20. §4 Ratio 계약의 UI·복합 명령도 반영했다 (live 22/22) — 잠금·해제의 tier 쓰기는 **자기 상태가 있는 tier 만** (없는 tier 는 base 상속; 후보의 전 tier 쓰기는 해제를 항상 막았다). §6.1 Flow→Absolute 복합 명령도 반영했다 (live 16/16, absolute leaf 높이 엔진 결함 1 동반 수리). §4.3 · §5 의 캔버스 핸들 resize 도 반영했다 (live 18/18): 중앙 pointer handler 의 핸들 히트 → gesture owner `resize` → `ResizePresentationSession` (드래그 중 layout lane 에 크기 + 잡은 축의 Fill 파생 CSS 제거 키 publish) → commit 1 (`applyCanvasResize` = 축별 `buildSizingEdit` css mode). 잡은 축만 Fill 해제 + 정수 px, Ratio 는 driver 하나 (종속 축 드래그는 ratio 환산), 위치 무변경. 동반 수리 = Canvas Fill projection 의 null 축 skip (Ratio 종속 축 stretch 보호 누락, 240/210). 다중 선택 live (Ratio 는 요소별 own 비율로 이미 정합, Absolute 는 리더 inset 복사 결함 → 요소별 `positionStylesFor(id)`) 17/17 · absolute left/top 핸들 (반대 변 고정, bridge 가 CSS left/top 과 현재 값의 차로 layout 이동) 21/21 도 반영. G2 왕복 (정적 export Fill CSS 단위 3 · 실제 ⌘C/⌘V 복제·tier null·breakpoint·refresh·무편집 저장 0 live 18/18) 과 G6 A/B (`adr224-size-perf-ab.mjs`, before worktree e8987c394 vs HEAD, N=100/1,000 × 부모 resize·가중치 편집 p95 전부 허용 안) 도 통과. 남은 것은 G5 소유자 확인 (blocking). 알려진 한계: 자식 있는 absolute 컨테이너의 핸들 resize (layout lane targeted-unsupported).
