...
| F7  | 배치 루프 4곳이 `+ 10_000` 가드 후 **실패 위치를 그대로 반환** (definite-col 은 `r = cur_row + 10000`)                                                           | `grid.rs:1008` · `:1014` · `:1038` · `:1044`                                                            | `grep -n "10_000" grid.rs`                                                     |
| F8  | 트랙 부재 시 셀 폭 폴백 `tracks_x.first().unwrap_or(100.0)` — 정폭 grid 에 template 이 없으면 item 100px                                                         | `grid.rs:682`                                                                                           | `grep -n "unwrap_or(100.0)" grid.rs`                                           |
...
### Phase 0 산출 (착수 승인 전 반드시)

- [ ] F1~F16 전수 대조 (리뷰어가 명령 실행).
- [ ] **F14 live 판정**: 실제 프로젝트에 Meter · ProgressBar · Slider 를 심고 `__composition_LAYOUT_DEBUG__.getSharedLayoutMap()` 으로 Track 폭이 컨테이너 폭인지 100 인지 읽는다. 컨테이너 폭이면 implicitStyles 가 가리는 것 → ④ 는 잠복 (우선순위 하향, Phase 2 후반). 100 이면 지금 어긋나는 중 → Phase 2 선두.
- [ ] **BC 인벤토리 (R1 수식화)**: 로컬 프로젝트 전수에서 `style.height` 가 `%` 이고 부모가 (a) definite cross 를 가진 flex 컨테이너 안의 stretch item, (b) grid area, (c) aspect 상자 인 요소 수 N 과 전체 요소 M 을 센다 — 영향 = N/M, 변화 = "캔버스에서 0 높이 → 부모 채움". 스크립트는 페이지 콘솔 (`__composition_STORE__.getState().elements`) 로 1회.
- [ ] `cargo bench tree_solve` baseline 수치 기록 (같은 머신 · 같은 조건 — `.claude/rules/measurement-validity.md`).
...
- 게이트 fixture: `percentSize.browser.test.ts` 에 F5 · B1d · B6c + 파생 (column 컨테이너 stretch 폭의 `width:%` 손자는 이미 정합 — 대조군 · `align-self: flex-start` 면 **비확정** 대조군 · auto margin 있으면 비확정 대조군 · grid `align-self: start` 대조군 · 3단 중첩 1건).
- 원복 RED: 채널 제거 시 F5·B1d·B6c 3건 RED, 대조군 GREEN 유지 (대조군이 RED 면 가짜 확정 — ledger §백분율 금지 패턴 1).
...
| 10,000 반복 가드 → **라인 clamp** (`start.min(10_000)`, `span` 은 `10_000 - start` 로) 후 정상 배치. 실패 위치 반환 삭제                    | `grid.rs:1008/1014/1038/1044`                                                                          | Chrome 10,000 트랙 clamp (Taffy #986) |
...
| 셀 폭 폴백 100 제거 — 트랙이 없으면 위 합성이 항상 선행하므로 도달 불가; 남기면 assert 로                                                   | `grid.rs:682`                                                                                          | —                                     |
...
| `apps/builder/tests/parity/percentSize.browser.test.ts`                                                         | 1     | F5 · B1d · B6c + 대조군                                                                                                   |
