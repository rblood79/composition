...
| F19 | 빌더 factory · preset 은 `height:%` 를 생성하지 않는다 (grep 0건). catalog 의 `height: "100%"` 는 Meter · ProgressBar **Value** 2곳뿐 — F14 의 template 없는 Track grid area 안. ①의 catalog 도달은 이 2곳, 나머지는 사용자 Styles 패널 입력 (리뷰 round 1) | `componentRulesTable.ts:7058` · `:8339`                                                                 | `grep -rn 'height: "100%"' apps/builder/src/builder/factories packages/shared/src/catalog/generated/componentRulesTable.ts` |

### Phase 0 산출 (착수 승인 전 반드시)

- [x] F1~F19 전수 대조 — 리뷰 round 1 (2026-09-07) 에서 실행: F7 5곳 · F8 3곳 · F19 신규, 나머지 일치.
- [ ] **F14 live 판정**: 실제 프로젝트에 Meter · ProgressBar · Slider 를 심고 `__composition_LAYOUT_DEBUG__.getSharedLayoutMap()` 으로 Track 폭이 컨테이너 폭인지 100 인지, 그리고 **Value 높이가 Track 높이인지 0 인지** (F19 — ① 의 catalog 도달) 를 같이 읽는다. 컨테이너 폭이면 implicitStyles 가 가리는 것 → ④ 는 잠복 (우선순위 하향, Phase 2 후반). 100 이면 지금 어긋나는 중 → Phase 2 선두.
