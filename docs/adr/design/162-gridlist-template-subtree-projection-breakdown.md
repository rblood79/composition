...
- ✅ 2026-09-27 — **반영**: `scene/expandedCardHeights.ts` (신규) — window 카드의 layout 실측 (카드 projection 상자 높이) 을 owner 캐시에 넣고 (키 = 템플릿 서명 · owner 폭 · 행 key + 행 데이터 참조), 아직 안 본 카드는 이 owner 의 첫 실측 (없으면 150 공급자의 공식) 으로 추정, 시각 행 = 그 행 카드 최대 → 150 `resolveGridListRowPlan` 이 이 목록을 `resolveCollectionRowOffsets` 에 넣는다 (plan 캐시 조건에 owner 실측 version). 수확은 `onLayoutPublished` 뒤 (BuilderCanvas) — 값이 바뀌면 anchoring (끝이면 새 끝, 아니면 화면 첫 시각 행 top 변화만큼 scrollTop 이동 · `updateMaxScroll` 먼저) 후 version → resolver 재계산. window 경계 서명에 spacer · 행 영역을 넣어 window 는 같고 높이만 바뀐 경우도 spacer 를 다시 투영 (스크롤 중에는 불변이라 rebuild 증가 없음). 추정 = 첫 실측 고정 (react-virtuoso 기본 항목 크기와 같은 방식) — 평균 추정은 측정이 늘 때마다 미방문 offset 이 흔들려 기각.
  - **live 가 잡은 결함 (같은 phase 에서 수리)**: 팔레트 GridList (ref instance) 의 master slot[0] 은 상태 변형 origin `component-gridlist-item-default--unselected` (ADR-234, 자기 자식 없는 ref) 인데, Canvas GridList 카드 context 만 raw 문서 노드를 읽어 (ListBox · Tag · Tab 은 `resolveTemplateOriginNode`) 펼침 판정이 "자식 없음" → 접기, slot 구성 · origin style 도 비었다. P2 live 는 plain GridList (기본 origin 직행) 라 이 모양을 못 봤다. → `resolveGridListCardContext` 가 `resolveTemplateOriginNode` 로 체인을 펼친다 (id 는 변형 id — 행 `ref`). scene unit (팔레트 모양, 수리 전 RED) · Preview 해석기는 해석된 트리를 읽어 원래 정상 (unit).
  - **G3 DOM oracle** `tests/parity/adr162VariableRowsDom.browser.test.ts`: 2 열 · 100 카드 (시각 행 50, 짧은 label 행 · 3 줄 label 행 교대) · 높이 400 overflow auto — 카드 126 / 174, gap 12, 행 y = 누적합, scrollHeight 8088. 규모는 100 카드 (DOM 은 가상화하지 않을 때 100 항목에서 자른다 — ADR-150 §3-3). src 없는 Image 의 alt 는 비운다 (DOM 이 alt 글자를 48px 상자 밖으로 넘겨 scrollHeight 를 +53 늘린다 — Phase 3 곁가지와 같은 차이, 행 높이 대조와 섞지 않음).
  - **G3 live** `apps/builder/scripts/adr162-p4-variable-rows-live.mjs` (headed, 팔레트 GridList ref instance · static collection 바인딩, page error 0) **9/9 PASS**: (a) 20 카드 · 높이 2000 전 행 실체화 = oracle ±1 · (b) 첫 이동을 끝으로 — 마지막 행 하단 = viewport 하단 (400), scrollTop = maxScrollTop, 보이는 행 상대 y · 높이 = oracle; 기록: 그 시점 총 높이 7368 vs DOM 8088 (차 −720, 미방문 3 줄 행을 첫 실측 126 으로 추정 — 게이트 아님) · (d) 미방문 중간 (3000) 이동 — 기준 행 20 화면 y 0 → 0 (scrollTop 3000 → 3144), 1.5 초 뒤 재읽기 동일 · 추가 anchoring 0 · (c) 맨 위부터 200px 순차 39 걸음 — 매 위치 실체화 카드 = oracle, 끝에서 총 높이 8088 = DOM.
  - **원복 RED** (백업 복사 원복): resolver 연결 끊기 3 · plan 캐시 version 조건 제거 3 (테스트가 production 처럼 같은 collections 배열을 넘겨야 캐시가 적중 — 처음엔 새 배열이라 반응 없음, 테스트 수정) · 폭 무효화 제거 1 · anchoring 제거 1 · 진동 가드 제거 1 · 변형 origin 해석 되돌리기 → 팔레트 scene unit 1. 회귀: builder · preview unit 820 파일 통과 (실패 1 = `catalogOrigins.test.ts` origin 개수 — ADR-150 §3-3 에 기록된 다른 세션 변경, 무관) · type-check 0.
  - **판독 1회 (reviewer) → 수리 1건**: 펼친 카드가 접힘 (origin 에서 Image 제거 · Undo) 또는 sample 모드로 바뀌어도 owner 는 문서에 남아 옛 plan · window entry 가 남았다 — 접힌 카드도 같은 projection id 라 다음 layout publish 에서 수확이 공식 높이를 실측으로 읽고 옛 plan 의 maxScrollTop 으로 scroll state 를 덮었다 (probe: resolver 1448 vs scroll state 1948, 마지막 행 아래 500px 빈 스크롤). → `pruneExpandedCardOwners` 가 이번 resolve 에서 수확 구간을 받지 못한 owner 의 window 를 비운다. unit (수리 전 RED) · scene unit 390 · type-check 0 · live G3 9/9 재실행. 수리 검증에서 HIGH 0 — 닫힘. LOW deferred: 카드 폭만 바꾸는 입력 (columns · columnGap · owner padding) · origin root `responsive` · theme 변경은 템플릿 서명 밖 (window 카드는 다음 publish 에서 다시 재고 anchoring, window 밖은 추정 수준) · `resolveGridListRowStride` 의 raw origin 읽기 (legacy `rowHeight` · spacer gating 에만 쓰임, 보이는 차이 미확인) · 수확의 변화 없는 publish 에서도 O(n) oldVisual 계산.
  - 범위 밖 기록: 팔레트 GridList 에 legacy `{type:"collection"}` 바인딩을 걸면 Preview 는 상속 정적 카드 (Path 3) 를, Canvas 는 데이터 행을 그린다 — production 바인딩 UI 는 `{source:"dataTable"}` (Path 1) 라 하니스 모양 한정.
