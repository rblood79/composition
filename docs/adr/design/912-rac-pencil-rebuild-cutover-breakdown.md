...
| RAC 공식 starter CSS (시각 원천) + design structured source                                                                       | `packages/react-aria-starter/src/*.css` (56) + `packages/design-system/*.ts`(신규) | theme rule base 의 **입력 source** — spec(부채) 대체. `design.md` 는 audit 문서 (②-6)                       |
...
| `generate-rules.ts`(spec→table) + `pnpm generate:rules` step               | `packages/specs/scripts/generate-rules.ts` + `package.json:34,40`                                             | spec→rule 변환기. spec 삭제 시 입력 소멸 → ②-6 대로 **starter→table 생성기로 교체**(spec 입력 제거, starter CSS+design structured source 입력). 기존 spec 변환 로직 삭제 |
...
**결정 (사용자 방향 2026-06-02)**: theme rule base 의 입력 source 를 spec(전환기 부채)에서 **starter CSS + design structured source → generated** 로 교체한다. `componentRulesTable.ts`(149KB)를 사람이 손으로 저작하는 것은 비현실적이므로, **starter style inventory → structured generated source → 분배** 흐름으로 둔다.

```
입력:  packages/react-aria-starter/src/*.css (RAC 공식 starter 시각 원천, 56 CSS)
       + packages/design-system/*.ts | design.generated.json (structured source — 정본 입력)
            │ (design.md 의 §Mapping/§Source Inventory 를 구조화 — markdown 파싱 아님)
            ▼ generate (starter→table 생성기, 기존 generate-rules.ts 대체)
생성물: componentRulesTable.ts  (theme rule base — DOM/Skia 공통 source)
       + theme tokens (tokens.generated.ts | theme.css)
       + reactAriaGenerated.css  (DOM base CSS + data-attr 매칭)
       + (필요 시) skiaVisualRecipes.generated.ts
            │
소비자: DOM/Preview/Publish → react-aria-{Type} + data-* + generated CSS (RAC 정통)
       Skia                 → 같은 componentRulesTable/recipe runtime resolve
       Properties/Style     → 같은 rule 에서 baseValue read, props.style override 만 저장
audit:  packages/design.md → 정본 아님, structured source 의 human audit/검증 문서
```

| 항목                   | 현재 (전환기)                                    | ADR-912 (직행)                                                                 |
| ---------------------- | ------------------------------------------------ | ------------------------------------------------------------------------------ |
| base 층 입력 source    | `*.spec.ts` 124 (variants/sizes/fill)            | **starter CSS + design structured source**(`design-system/*.ts`)               |
| 생성기                 | `generate-rules.ts` (spec→table)                 | **starter→table 생성기로 교체** (spec 미참조)                                  |
| base 층 산출물         | `generated/componentRulesTable.ts`               | `componentRulesTable.ts` + theme tokens + `reactAriaGenerated.css` (generated) |
| `design.md` 위상       | human audit (starter 측은 css-tokens.md 가 정본) | 무변경 — structured source 의 audit/검증 문서 (정본 아님)                      |
| `resolveComponentRule` | 이 테이블 read                                   | 위치·시그니처 무변경, 생성된 테이블 read                                       |

**왜 starter→structured→generated 인가** (대안 비교):

- 대안 ②-6-D (starter inventory → structured generated source, **채택**): base 입력 source 를 spec 이 아니라 RAC 공식 starter CSS + design structured source 로 둔다. DOM 은 RAC 정통(CSS+data-attr) 유지, Skia 는 같은 generated rule runtime resolve → 비대칭이 "출력 형태 차이"일 뿐 "source 차이" 아님(대칭 보장). spec 124 완전 삭제 가능(뿌리가 starter 로 이동). ADR-911 대안 E 의 theme/tokens SSOT 와 정합.
- 대안 ②-6-A (componentRulesTable 직접 SSOT, 손 저작): 149KB 를 사람이 직접 저작/편집 — 비현실적(저작·유지 부담 과대). 채택 후 본 ②-6-D 로 재정정.
...
> **취약성 회피 (사용자 지적)**: `design.md` 를 markdown parser 로 긁어 생성하면 취약하다. `design.md` 는 human audit 문서로 유지하고, **structured source(`packages/design-system/*.ts` 또는 `design.generated.json`)를 정본 입력으로 둔다**. design.md 는 그 structured source 에서 같이 생성되거나 검증 대상이다.

**삭제·전환 순서 (선행 의존, base 단절 0 구간)**: (1) starter CSS + design structured source 정의 → starter→table 생성기 작성 → `componentRulesTable.ts`(+theme tokens+`reactAriaGenerated.css`) 생성·검증 → (2) `resolveComponentRule` / CSSGenerator / DOM 소비자가 generated 산출물을 read 하도록 전환 (spec 미참조) → (3) `generate-rules.ts`(spec→table) + `pnpm generate:rules` step 삭제 → (4) `*.spec.ts` 124 + `render.shapes()` + `resolveComponentVisual`(spec seam) 삭제. base 층이 한 순간도 정의되지 않는 구간이 없도록 starter-based 생성을 먼저 land 한다.
...
**핵심 (1A 실측 정합)**: DOM 과 Skia 는 base 시각 적용 **메커니즘이 다르다**(DOM=build-time generated CSS+data-attr / Skia=runtime rule resolve). "단일 진실"은 **같은 런타임 inline object** 가 아니라 **둘 다 같은 generated source(starter→structured→table)의 같은 theme rule 을 base 로 쓴다**는 것이다. `resolveMergedStyle` 은 이 공통 base 위에 `props.style` override 를 얹는 병합 코어이고, 각 backend 어댑터는 그 결과를 자기 출력 형태로 투영한다.

```ts
// ★ base = 같은 generated theme rule, override = props.style. 두 backend 가 같은 base source.
resolveMergedStyle(node, doc):
  rule = resolveComponentRule(node.type, doc)            // generated theme rule (starter→table), VisualRule seam 없음
...
1. **공통 기반 (family 무관) + Button slice 첫 증명** — 3 sub-step 으로 분해, **early-kill 시점은 1A 직후(7~10h)**. (전체 단계 1 예상 20~28h, 보수적 3일 / 복잡도 터지면 4일)
   - **1A — common spine (7~10h)**: `ComponentNode` schema + `resolveMergedStyle`(theme rule 직접) + `toReactStyle`(override 전용) + `toSkiaStyle`(보편속성 직접) + `resolveEditContract`(variant·size 의미 props 분리) — Button proof 최소.
     - 권장 sub-순서 (R-2 격리, DOM-first):
       - **(a) base source 선행 (②-6-D)**: Button 한정 starter→table 경로 — `react-aria-starter/src/Button.css` + design structured source(Button entry)를 `componentRulesTable.ts` Button rule 로 generate. DOM 은 generated CSS(`reactAriaGenerated.css` Button 분) + `data-variant`/`data-size`, Skia 는 같은 table rule runtime resolve. **DOM/Skia 가 같은 generated source 파생인지** 먼저 확인(같은 inline object 아님).
     ...
     - **🔴 1차 kill 판단 (1A 끝, 7~10h)**: Button 하나에서 (1) starter→table 생성기가 깨끗하게 Button rule 을 내는가, (2) DOM generated CSS 와 Skia runtime rule 이 **같은 source 파생**인가, (3) `resolveMergedStyle`/`toSkiaStyle`/`resolveEditContract` 가 지저분하거나 **legacy seam(spec) 호출이 필요한가** — 셋 중 하나라도 막히면 → 1B/1C 로 가지 말고 **③ 영역 A + ②-6 source 설계 재보정**. 20~28h 끝이 아니라 여기서 1차 판단 — 깨끗하지 않은 spine 위에 1B(5 consumer wiring) + 1C(검증)를 쌓는 헛수고 차단. R-1 전역 회귀 방지.
...
| base 층        | `resolvers/resolveComponentRule.ts`(위치 무변경, 생성된 테이블 read) + `catalog/generated/componentRulesTable.ts`·`reactAriaGenerated.css`·theme tokens(②-6-D: **starter CSS + design structured source 에서 generate**, variant·size → 시각값 직접 resolve). 신규 입력: `packages/react-aria-starter/src/*.css` + `packages/design-system/*.ts`(structured source) / `packages/design.md`(audit 무변경). 교체: `generate-rules.ts`(spec→table) → starter→table 생성기. 삭제: `pnpm generate:rules`(spec step) |
...
1. **G-slice**: Button `size="md"→"sm"` → DOM/Skia/Properties/Style/Publish 5곳 `fontSize:14` 동일 + `/cross-check`. **성공 = (a) Button 경로 legacy seam(resolveComponentVisual/render.shapes/buildCatalogShapes·CSSGenerator spec 읽기) 제거하고도 5곳 작동 + (b) DOM generated CSS 와 Skia runtime rule 이 같은 generated source(starter→table) 파생**. "작동하지만 seam(spec) fallback 유지" = 실패. **kill: 깨끗하게 안 나오면 단계 2~5 진입 금지, ③ 영역 A + ②-6 source 설계 재보정**.
