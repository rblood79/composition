...
>
> - **(A) parent projector** [items 소스, 선례 직접] — projected-tree 8(Tag/Tab/TabList/TreeItem/Breadcrumb†/SelectIcon/SelectTrigger/SelectValue) + **List 1**(ListBox 동형, items SSOT 전환 + ListItem=row) + **CalendarGrid 1**(Table 2D grid 변형 6주×7일). C1 collectionProjector 직접 재사용, 신규 `kind:"{owner}-row"` metadata 만.
>   - **† Breadcrumb 정찰 정정(2026-06-05) — (A) 직접 복제 부적합**: Tag 와 구조 비대칭. (1) 데이터 모델: Tag=items SSOT(TagGroup.props.items propagation) vs Breadcrumb=정적 factory-child(`children-manager`, items 미정의). (2) separator: Tag X 는 chip **내부**(단일 노드) vs Breadcrumb separator(›)는 crumb **사이**(DOM `Breadcrumbs.css:20-27` `::after` / Skia `Breadcrumb.spec.ts:175-191` 별도 shape). (3) Skia 현황: `Breadcrumbs.spec.ts:156-163` shell-only(`render.shapes:()=>[]`) + 자식 `Breadcrumb.spec` self-render(text+separator). (4) click: crumb=href navigation(`<Link>`) — owner select redirect 무관. → Tag 패턴 직접 적용 아니라 **선행 축 ① 되감기(items SSOT 신설)부터 필요**. (A) projection 진입은 items SSOT 확정 후 재평가. 다음 (A) slice 후보는 Tab/TabList(items 기반 collection, Tag 선례 적합도 ↑) 우선 정찰 권장.
...
> | **step 4 — spec 파일 삭제**         | import graph 끊긴 뒤 `*.spec.ts` 124+ 제거 (마지막). **no-go 사유 = catalog 미등록 type 의 render.shapes fallback 대체 필요**: step 2 가 fallback 을 "catalog 미등록 type 전용 임시 경로"로 격하했으나 미등록 type(실측 69 = 111 spec − 42 catalog: color 7 scope 외 / sub-part 29 / 미발효 leaf 22 / legacy~native 11)은 아직 spec.render.shapes fallback 으로 그려진다. spec 통째 삭제 시 시각 source 소멸. 선행 = (A) **미등록 type inventory + 경로 대체**(별도 후속, [912-step4-unregistered-types-inventory.md](912-step4-unregistered-types-inventory.md) — 선행-1 leaf 등록 9 / 선행-2 value-part escape 6 / 선행-3 흡수 검증 20 / 선행-4 legacy·native 정책 11 / 선행-5 color 보존 + dead 2) + (B) import graph 0 확인 + (C) **사용자 명시 삭제 승인**(원본 124+ 파일, CLAUDE.md 마이그레이션 원칙) + (D) type-check/cross-check.                                                                       | 🚫 **no-go** (선행: 71 type 대체 + 명시 삭제 승인) |
> | **step 5 — 문서/status 정합**       | 각 step breakdown 진행표 갱신(step 동반) + ADR Status 정책: **Proposed 유지 근거 현재 상태 기준 정리**(step 4 핵심 결정 no-go + 미등록 71 type spec fallback 잔존 = 부분적 dual-SSOT + adr-writing Implemented 전이 규칙 미충족) + **execute-adr `--auto` 자동 land 금지**(step 4 는 71 type 선행 + 명시 삭제 승인 요구 → 수동 게이트 경유). step 1~3 완료 반영 + step 4 no-go 사유 명시. 반영 위치: ADR 본문 §Status + `docs/adr/README.md` + 본 표.                                                                                                                                                                                                                                                                                                                                                                                                                                                            | ✅ **완료** — step 1~3 정합 반영                   |
