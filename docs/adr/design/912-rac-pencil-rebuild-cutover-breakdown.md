...
- **palette O / catalog X = 9** (`AvatarGroup`, `Image`, `Card`, `Accordion`, `Disclosure`, `CardView`, `ButtonGroup`, `TailSwatch`, `TableView`): palette 에 노출되지만 catalog 미발효. ComponentList 가 `getCatalogEntries()` 파생으로 바뀌면 이 9 가 palette 에서 **사라진다**. → **각 type 을 catalog 발효(leaf/reusable) or `panel.layoutOnly`/palette-only 분류**로 선행 해소해야 회귀 0.
- **catalog O / palette X = 20** (`Heading`/`Paragraph`/`Code`/`Kbd`/`Label`/`Description`/`FieldError`/`Input`/`body`/`DisclosureHeader`/`CalendarHeader`/`CalendarGrid`/`DateInput`/`TextArea`/`ColorField`/`Radio`/`Meter`/`ProgressBarTrack`/`MeterTrack`/`SliderTrack`): catalog 발효됐으나 palette 비노출. 대부분 sub-part/leaf(정상 비노출 — Label/Input/FieldError/track 류) + 일부 노출 후보(TextArea/ColorField/Meter). 파생 전환 시 **palette 노출 여부를 entry 가 결정**(예 `panel.placeable` 또는 신규 노출 플래그)해야 의도치 않은 노출 0.
...
> **영역 B 의 두 직교 진행 축 (혼동 방지)**: 영역 B 는 서로 독립된 두 축으로 진행된다. ① **DOM source acquisition 단일 계약**(`useResolvedCollectionItems` 되감기) — collection wrapper 의 items 데이터 획득을 단일 hook 으로 통일(Preview/Publish DOM 경로). ② **Skia generic 발효**(단계 4, skiaLegacy 제거) — Builder Skia 컨테이너 렌더를 generic 경로로 전환. 두 축은 같은 collection 컴포넌트 집합을 다루지만 backend(DOM vs Skia)가 달라 직교 — 한 축의 완료가 다른 축을 함의하지 않는다.
...
> | 4 (collection 발효 C1/C2/C3) | ✅ **완료** — collection 7종 전부(ListBox proof + 비-data-bound 4종 + GridList C1 + Table 2D C1 + Menu) + 영역 B (A) projection 발효 3(Tag/Tab/TabList) | `d5da74c72`/`173765201`/`831079766`/`0e9c501d4`/`2818c6bf0`/`d7cfd470c`/`595805242` · (A) `a49e63541`(Tag)/`a39ad98fd`/`dc9617da0`/`c3b5c268f`(Tab/TabList) |
