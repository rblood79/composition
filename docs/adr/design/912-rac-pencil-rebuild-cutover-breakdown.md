...
> | **step 4 — element type 폐기 phase** (구 "spec 파일 삭제"; 명칭 정정 2026-06-08, 사용자 옵션 B) | **단위 = "spec 파일 물리 삭제"가 아니라 "element type 폐기 decision"** — 4축 gate(G-render/G-factory/G-registry/G-consumer, inventory §"step4 element 폐기 정책")를 통과한 type 만 폐기 가능. 폐기 = spec 삭제 + factory/migration + registry + layout/text consumer 이관의 동반 작업 묶음(해당 축만). **4 gate 전부 PASS(true-dead) = 0건**. **no-go 사유 = catalog 미등록 type 의 render.shapes fallback 대체 필요**: 미등록 type(2026-06-08 set-difference 재실측 43 = 111 spec map − 68 catalog[primitiveEntry 65 + native 3]: color 7 scope 외 / sub-part / 미발효 leaf / legacy~native — 순 차단 15 = ③ 영역 B 14 + ④ value-fill 잔여 1[SliderTrack])은 아직 spec.render.shapes fallback 으로 그려진다. spec 통째 삭제 시 시각 source 소멸. **✅ 첫 proof land (2026-06-09)** = Autocomplete element type 폐기 완료(사용자 명시 삭제 승인). 9 참조처 atomic(spec/tagToElement/specRegistry/barrel×2/ComponentTag union+카운트 117/componentRulesTable rule/metadata/styles @import) + 파일 3 + 생성 CSS. 검증 5종 PASS(type-check 신규 0 / build:specs 124 CSS·orphan 0 / dist fresh[stale-dist masking 회피] / registration-contract 10/10 불변식 A / live builder element 11·canonical element 0·palette 미노출·콘솔 에러 0). **전제 반전**: scope 검증으로 inventory "registry-only(3곳)" 과소집계 → 9 참조처(vocabulary/rule/metadata/DOM 컴포넌트 포함). 잔여 = (A) **미등록 type inventory + 경로 대체**([912-step4-unregistered-types-inventory.md](912-step4-unregistered-types-inventory.md)) + (C) **type별 명시 폐기 승인**(원본 파일, CLAUDE.md 마이그레이션 원칙). 다음 후보 = DateSegment < Field(P3 비용 오름차순). | 🟢 **첫 proof land** (Autocomplete 폐기 완료; 잔여 type 각각 명시 승인) |
