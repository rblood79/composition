...
> **⚠️ 이 표는 cutover 진입 전 스냅샷(2026-06-03, historical)이다.** 아래 수치(spec.ts 124 / Factory 60 / rendererMap 94 / DEFAULT_PROPS 96 / BASE 108)는 ADR-912 작업 시작 시점의 진단값으로, **현재 정본 아님**. 2026-06-20 실측 종결 카운트 = spec.ts **3**(Frame/Group/Slot 영구) / Factory creator(COMPLEX_COMPONENT_TAGS) **45** / rendererMap **94** / DEFAULT_PROPS_MAP 정의 **93**(파생 6/92) / BASE_TAG_SPEC_MAP native **3** / BUILDER_ALIAS_MAP **0**. 종결 카운트 정본은 §2-5 "P2 종결" 표(아래) 참조.
...
| 6 레지스트리                                       | Factory creators 60 / rendererMap 94 / getDefaultProps(DEFAULT_PROPS_MAP 96) / BASE_TAG_SPEC_MAP 108 / builder TAG_SPEC_MAP(108+9 alias) / Component Panel 61 | 다수 (카운트 2026-06-09 recon 실측: rendererMap 95→94 / BASE 111→108)                                                  |
...
| 6 레지스트리 중 5개                                                        | Factory 60 / rendererMap 94 / getDefaultProps(DEFAULT_PROPS_MAP 96) / TAG_SPEC_MAP(BASE_TAG_SPEC_MAP 108) 2종 | 단일 등록 collapse (카운트 2026-06-09 recon 실측 동기화: rendererMap 95→94 / BASE 111→108)                                                                                        |
...
| 2   | `rendererMap`                         | `shared/renderers/index.ts:19`      | 94            |
| 3   | `DEFAULT_PROPS_MAP` (getDefaultProps) | `unified.types.ts:2278`             | 96            |
| 4   | `BASE_TAG_SPEC_MAP`                   | `specs/runtime/tagToElement.ts:134` | 108           |
| 5   | builder `TAG_SPEC_MAP`                | `sprites/tagSpecMap.ts:31`          | 108 + 9 alias |
| 6   | Component Panel list                  | `ComponentList.tsx:70-150`          | 61            |

> **카운트 2026-06-09 recon(wws64u1dj) 실측 동기화**: rendererMap 95→94 / BASE_TAG_SPEC_MAP 111→108 / builder ALIAS 8→9 (body 포함, JSDoc "8 진짜 alias" 는 ADR-902 후속 body 추가 전 표기 = stale) / Component Panel 61 (정적 const 배열 7 합산). Factory creators 60 / DEFAULT_PROPS_MAP 96 은 표기 유지(실측 일치).
...
| --- | --------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Component Panel (`ComponentList.tsx:70-150`, 61 entry)                                        | ❌ **정적 하드코딩 배열 7개** (`contentComp` 13/`layoutComp` 11/`buttonsComp` 6/`formsComp` 14/`collectionsComp` 7/`dateTimeComp` 6/`overlaysComp` 4 = 61). 항목 구조 `{ type, label(i18n 소문자 key), icon(lucide 컴포넌트 참조), layoutOnly? }` — catalog 비파생 평행 list | **`panel:PanelMeta` 필드는 catalog entry 에 이미 존재**(types.ts:179-184 `category/label/icon/placeable`, 3 variant 전부) — recon 2026-06-09 실측. 따라서 "필드 추가"는 **`layoutOnly?:boolean` 1개 신규 + icon string→lucide 매핑** 만. ComponentList 가 catalog 파생으로 전환. **단 선행: palette↔catalog 불일치 9 해소**(아래 §2-5-1a) |
| 2   | builder `TAG_SPEC_MAP` (`tagSpecMap.ts:31`)                                                   | ⚠️ `{...BUILDER_ALIAS_MAP, ...BASE_TAG_SPEC_MAP}` — BASE 는 이미 파생, ALIAS **9개**(ComboBox 3 + SearchField 4 + TabBar + body, JSDoc "8" 은 body 추가 전 stale)는 별도                                                                                                     | BASE 파생부는 catalog 흡수. ALIAS 9 는 leaf entry 정규 등록 or parent sub-part 유지 판정 후 별도 map 제거 (§2-5-2)                                                                                                                                                                                                                        |
| 3   | `BASE_TAG_SPEC_MAP`(108) / `rendererMap`(94) / `DEFAULT_PROPS_MAP`(96) / Factory creators(60) | 4 registry 가 spec/수동 list 병치                                                                                                                                                                                                                                            | catalog entry 파생 view 로 collapse (③ schema + ②-6 theme rule)                                                                                                                                                                                                                                                                           |
...
#### §2-5-1a — palette↔catalog set 불일치 (recon 2026-06-09 실측, #1 파생 전환의 필수 선행)

ComponentList 를 catalog 파생으로 단순 전환하면 **palette 노출 컴포넌트가 회귀**한다. palette set(61) 과 catalog entry set(72) 의 교집합이 부분적이기 때문 — set-difference 실측:

- **palette O / catalog X = 9** (`AvatarGroup`, `Image`, `Card`, `Accordion`, `Disclosure`, `CardView`, `ButtonGroup`, `TailSwatch`, `TableView`): palette 에 노출되지만 catalog 미전환. ComponentList 가 `getCatalogEntries()` 파생으로 바뀌면 이 9 가 palette 에서 **사라진다**. → **각 type 을 catalog 전환(leaf/reusable) or `panel.layoutOnly`/palette-only 분류**로 선행 해소해야 회귀 0.
- **catalog O / palette X = 20** (`Heading`/`Paragraph`/`Code`/`Kbd`/`Label`/`Description`/`FieldError`/`Input`/`body`/`DisclosureHeader`/`CalendarHeader`/`CalendarGrid`/`DateInput`/`TextArea`/`ColorField`/`Radio`/`Meter`/`ProgressBarTrack`/`MeterTrack`/`SliderTrack`): catalog 전환됐으나 palette 비노출. 대부분 sub-part/leaf(정상 비노출 — Label/Input/FieldError/track 류) + 일부 노출 후보(TextArea/ColorField/Meter). 파생 전환 시 **palette 노출 여부를 entry 가 결정**(예 `panel.placeable` 또는 신규 노출 플래그)해야 의도치 않은 노출 0.
...
**불일치 9 전환 판정 완료 (recon wv4xu67t9 + 사용자 결정 2026-06-09)** — 상세 [step4-inventory §2-5-1a](912-step4-unregistered-types-inventory.md):
...
| **E-palette-only 1** | `TailSwatch`                                                                       | catalog 전환 안 함, palette 유지 (color scope 외, 사용자 "패스" — color scope 작업 시 재판단)                                                       |
...
| **2-5-1a** ✅     | palette↔catalog 불일치 9 전환 판정 (recon wv4xu67t9) — C-shell 7 + Image 별도 + TailSwatch palette-only                                                  | 9 type 판정 (분석, 코드 0)                              | 2-5-0         |
| **2-5-1a-proof**  | Disclosure 1개 shell entry 전환 proof (kill criteria: palette 유지 + Skia shell box + DOM 대칭 + spec fallback 0)                                        | componentCatalog Disclosure entry + binding             | 2-5-1a        |
...
| #4  | rendererMap       | 94                          | ⚠️ catalog generic 흡수 진행 + `DELEGATING_*`(~24) 영구 잔존 (아래)                                                                                                                                                           |
| #6  | Factory creators  | 45(=COMPLEX_COMPONENT_TAGS) | ⚠️ 자식 element-tree 생성 함수 (아래)                                                                                                                                                                                         |
...
- **#6 Factory creators = R-5 reusable composite tree schema 에 종속**. `COMPLEX_COMPONENT_TAGS` 45 (2026-06-20 실측) 는 자식 트리를 생성하는 함수인데 catalog `binding.props.accepts` 는 flat props 만 담고 자식 트리 schema 가 없다. leaf(catalog cutover)는 이미 creators 미경유(`useElementCreator` else 분기), Form 류는 R-5 로 `type:"ref"` 전환 완료. 나머지 COMPLEX 를 파생화하려면 child template schema 를 catalog 에 선축해야 하는데, 소비처 없는 schema 선행 = dormant foundation(`feedback-no-dormant-foundation-ahead-of-flip` 차단 영역).
...
따라서 P2 는 "파생 가능한 registry(#1·#2·#3·#5) 전부 파생" 으로 완료다. `componentRegistrationContract.test.ts` 는 placeable(`ComponentFactory.getRegisteredTypes()`, 2026-06-20 실측 45) ⟹ rendererMap/TAG_SPEC_MAP·catalog/getDefaultProps 정합(baseline ratchet 전부 0, 누락 0)을 계속 강제 — `entryUniverseContract.test.ts` 교체는 #4·#6 의 종속 축(generic 흡수 완성 / R-5 후속)이 정리될 때 함께 졸업. **현 상태(2026-06-20): `componentRegistrationContract.test.ts` 활성(10 it PASS), `entryUniverseContract.test.ts` 미생성** — "게이트 졸업" 은 미래 작업이지 완료가 아니다.

> **Δ11 — structure 축 내부 영구 잔존 (검증 Workflow `wtuk64q6m`, 2026-06-20)**: 위 P2 collapse(등록 entry) 와 직교하는 별도 잔여. field-trigger 계열(Select/ComboBox/SearchField/NumberField)의 자식 SelectTrigger + selecttrigger-자체-container 분기가 base-axis(`flexDirection ?? "row"`)를 `implicitStyles.ts:1336/1396/1439` 에 inline 보유(3곳). catalog 로 모으는 유일 채널(`SelectTrigger` rule `structure.composition.layout`)이 generate-css CSS emit 게이트(`buildVirtualSpecs: if(!rule.structure) continue`)와 같은 필드라 추가 시 `.react-aria-SelectTrigger` dead CSS 신규 emit → byte-diff 0 위반. DOM 은 `.react-aria-Select .react-aria-Button` 으로 base-axis 보유(SelectTrigger className 미사용)라 신규 CSS dead. **byte-diff 0 보존 위해 inline 영구 유지(사용자 confirm)** — dual-SSOT 아니라 single-source 가 layout 코드에 위치(DOM/Skia 둘 다 row, 시각 정합 0). grep gate `adr912CollapseGrepGate.test.ts` Δ11 baseline 3 추적. field-trigger 항상 row + 계열 고정이라 drift 0, 0 종결 대상 아님(structure-분리 schema 채널 도입 = dormant 인프라 회피).
...
> **영역 B 의 두 직교 진행 축 (혼동 방지)**: 영역 B 는 서로 독립된 두 축으로 진행된다. ① **DOM source acquisition 단일 계약**(`useResolvedCollectionItems` 되감기) — collection wrapper 의 items 데이터 획득을 단일 hook 으로 통일(Preview/Publish DOM 경로). ② **Skia generic 전환**(단계 4, skiaLegacy 제거) — Builder Skia 컨테이너 렌더를 generic 경로로 전환. 두 축은 같은 collection 컴포넌트 집합을 다루지만 backend(DOM vs Skia)가 달라 직교 — 한 축의 완료가 다른 축을 함의하지 않는다.

> **진행 상태 (2026-06-05) — 축 ① DOM source acquisition 단일 계약 = 5+1군 완료, Breadcrumbs 잔여 발견(2026-06-05 정정)**: `resolveCollectionItems` 순수 계약(`packages/shared/src/collections/`) + `useResolvedCollectionItems` DOM adapter 로 collection wrapper 의 이중 source(`useCollectionData` 직접 호출 + wrapper별 `if(!hasDataBinding && items)` 분기 복제)를 단일 진입점으로 통일. ADR-142 no-classification 원칙(wrapper별 분기 제거) 정합. **5군 + ComboBox 완료**:
>
> | 대상     | Task | seam 성격                                          | binding items | commit        |
> | -------- | ---- | -------------------------------------------------- | ------------- | ------------- |
> | TagGroup | 2-B  | 정적 items chip 누락 복구                          | 추가          | (영역 B 연장) |
> | Menu     | 3    | dataBinding/정적 items 경로 통합                   | 기존          | —             |
> | ListBox  | 4    | 정적 items Preview DOM 렌더 복구                   | 추가          | —             |
> | GridList | 5    | 정적 items 카드 렌더 복구                          | 추가          | —             |
...
>   4. **Breadcrumbs catalog 미등록** — `componentCatalog.ts` grep 0건. 축 ① 되감기 전 catalog 전환 여부부터 갈림.
>   5. separator(`Breadcrumbs.css:20-22` `::after content:"\203A"`)는 DOM 전용 — source 단일화와 **직교**(미관여).
>      → **판정: 설계 보류**(code fix 아님). 되감기 = `StoredBreadcrumbItem` 타입 + `Breadcrumbs.binding.ts` items accepts + 정적 items 기반 children 렌더 경로(factory-child→items SSOT 전환) **신설 3종** 선행. 단일 patch 미충족. **다음 (A) 후보 = Tab/TabList(items 기반, Tag 선례 적합) 우선** → ✅ **Tab/TabList land 완료**(Tab slice 2026-06-06, `a39ad98fd`/`dc9617da0`/`c3b5c268f`). **그 다음 (A) 후보 = TreeItem / Select\*** — Breadcrumbs 는 items SSOT 신설 결정이 별도로 날 때 재개.
...
> - **GridList 한계 note**: section entry 가 정적 items 에 섞이면 hook source 로 안 넘기고 legacy `SelectionRenderers.tsx::renderGridList` Path 2(`isGridListSectionEntry`) + Skia `getGridListProjectionRows`(kind:"section" emit)에 격리 — 본 flat slice 와 직교(미수정).
> - **ComboBox live 검증 한계 note**: publish fixture 가 `inputValue=""` controlled + `onInputChange` 미연결(read-only preview 특성, fiber 확인)이라 필터링("ca"→"Cat")/custom value 키 입력 직접 검증 불가 — 본 변경(`{...props}` 무수정 forward + items 정상 전달)과 무관, RAC 위임으로 무회귀. 직접 확증 = 정적 items popover `role=option` 4 + option id 원본 보존 + catalog cutover child 0(vs legacy 2) + Skia trigger shell-only + projection id 영속 0.
> - 핵심 파일: `packages/shared/src/collections/resolveCollectionItems.ts`(순수 계약) + `packages/shared/src/hooks/useResolvedCollectionItems.tsx`(DOM adapter) + 각 `{Component}.tsx` wrapper + `{Component}.binding.ts` items accepts.

> **진행 상태 (2026-06-03) — 축 ② Skia generic 전환**: 단계 4 를 전환 난이도별로 묶음 분리 실행(사용자 결정). **묶음 1 — 비-data-bound 4종 전환 완료(`0e9c501d4`)**: Select/ComboBox(trigger-overlay) + Tabs/TagGroup(factory-child)는 row 순회가 render.shapes 에 없어 **C1 generic projector 불필요**, C2(4-2.5)+C3(4-2.6)만으로 전환 안전. skiaLegacy 제거 → buildCatalogShapes 경로. 검증: vitest 회귀 0(+7)·drift 102/102·generated CSS diff 0·live console 0. **묶음 2 — GridList C1 전환 완료(`b958cc006`/`f984f6462`/`3d933df54`/`2818c6bf0`)**: data-bound row 순회를 projected GridListItem tree 로 전환(C1 4-2). step0 공통 모델 추출 → step1 GridListItem.spec.render.shapes 카드 렌더 이전(ListBoxItem 동형) → step2 projection metadata union(gridlist-row/rows + 단일 진입점 helper `isCollectionRowProjectionKind`) → step3-5 appendGridListRowProjection + GridList.render.shapes shell-only + skiaLegacy 제거. **origin 인프라 불필요**(verify CRITICAL #1 실측 정정 — originStyle `?? {}` graceful + GridList factory children:[], origin = optional enhancement). **projected node 렌더 경로 = Item spec.render.shapes**(GridListItem catalog 미등록 → buildCatalogShapes 아님, verify dataLossRisks 경로 정정). 검증: vitest 회귀 0(+3)·canvasSceneNode 16 test(GridList projection + window 100)·live builder GridList 카드(bg+border+label+description) 정상 렌더+데이터 소실 0+console 0. **묶음 3 — Table 2D C1 전환 완료(`3efce7a0d`/`5bde0cbd8`/`d7cfd470c`)**: data-bound 2D grid(columns×rows)를 projected RowsGroup→Row[i]→Cell[i][j] tree 로 전환(사용자 결정 "행 단위 셀 노드", 2026-06-03). step T0 collectionRowProjectionModel 2D 확장(TableColumnDef/TableProjectionRow/getTableProjectionRows, flat 모델과 직교) → T1 TableCell.spec(TableRow bg/divider + TableCell text-only, Table.spec.childSpecs 등록 → TAG_SPEC_MAP 자동 확장) → T2 projection metadata union(table-rows/row/cell + isCollectionCellProjectionKind + columnId write-target route) → T3 appendTableRowProjection(window 100) + Table.render.shapes shell-only + skiaLegacy 제거 + **C2 정렬**(Table.css bg `--bg-raised`→`--bg`, rule fill `{color.base}` 정합 — 전환 전부터 존재하던 drift 해소, 전환 자체는 Skia-neutral). **TableCell/TableRow catalog 미등록 → spec.render.shapes 경로**(GridListItem 동형). Table factory 의 빈 TableHeader/TableBody(spec 없음 → Skia 미렌더)는 suppression 불필요. 검증: type-check 0 new(baseline 110)·specs 544·builder 36 failed(baseline 동일, 회귀 0)/1688 passed(+8)·canvasSceneNode 18 test(Table projection 2 + window/header)·registration contract 10/10·generated CSS diff 0·**live builder 2D 테이블(header fw600 + data fw400 + 컬럼 배치 + 행 구분선) 정상 렌더 + cell 클릭→owner Table 선택 + console 0**. **묶음 4 — Menu 전환 완료(2026-06-04, skiaLegacy 제거 단순 1줄)**: 직전 "popup↔trigger 본질 미확정" 보류 framing 정정 — 사용자 정정("skia Menu = trigger Button 동일") + RAC 공식(MenuTrigger>Button+Popover>Menu, 초기 화면 = trigger Button, popup 리스트는 Popover 안 숨김). Skia 정적 캔버스는 popover 미개방 → trigger Button 만 그리면 Preview 초기와 일치. **설계 가정(아래 4-2.5/4-2.6 표의 "Menu.css trigger 버튼 정렬" + `triggerText:true` 예외) 실측 정정**: ① Menu.css 의 popup CSS(`.react-aria-Menu` `--bg-raised`/max-height)는 Popover 전용 → 전환 무관(수정 불필요, generated CSS diff 0). ② Menu 는 `SYNTHETIC_CHILD_PROP_MERGE_TAGS` 비멤버(items SSOT, factory children:[]) → C3 차단이 애초에 미적용 → buildCatalogShapes 가 text "Menu" 자연 렌더(`triggerText:true` 별도 메타 불필요). ③ rule fill base `{color.neutral}` == Button → drift test 116 passed(Menu cutoverType 추가, spec↔table 일치). ④ 유일 차이 text align(legacy left → catalog center)은 Button center 정합(정렬 정정, render.shapes 미수정 — drift test 대상 아님). 검증: drift 116 / type-check 0 new / specs 544 / builder 회귀 0(+7, 1695 passed) / registration 10/10 / generated CSS diff 0 / **live builder: Menu = 62×30 trigger 버튼(popup 아님) = Button 동일 위치·크기·색, Button text 선명(Menu 동일 경로) + console 0**. → **단계 4 = collection 7종 전부 전환 완료**.
>
> **영역 B 3 메커니즘 분리 (2026-06-05 — step 4 잔여 13+List 설계 정합, 사용자 confirm "3 메커니즘 분리")**: 단계 4 가 전환한 것은 **(A) collection/items parent projector** 메커니즘(C1 generic collectionProjector)뿐이다 — collection 7종 컨테이너 자체. step 4(spec 삭제) 차단으로 남은 영역 B 보류 **13 + List** 는 데이터 소스가 3가지로 갈려 **단일 projector 로 억지 흡수하지 않는다**(선례 `getListBoxProjectionRows`/`getTableProjectionRows` 가 collection items 에 닫힘). 고수준 골격(append\*RowProjection + `projection:` id + 3-route write-target + boundary guard + `isCollectionRowProjectionKind` 단일 handler)은 3 메커니즘 공유, **데이터 소스 어댑터만 분리**:
>
> - **(A) parent projector** [items 소스, 선례 직접] — ✅ **전환 3**(Tag `a49e63541` / Tab·TabList Tab slice `a39ad98fd`/`dc9617da0`/`c3b5c268f`) + 잔여 projected-tree 5(TreeItem/Breadcrumb†/SelectIcon/SelectTrigger/SelectValue) + **List 1**(ListBox 동형, items SSOT 전환 + ListItem=row) + **CalendarGrid 1**(Table 2D grid 변형 6주×7일). C1 collectionProjector 직접 재사용, 신규 `kind:"{owner}-row"` metadata 만.
>   - **† Breadcrumb 정찰 정정(2026-06-05) — (A) 직접 복제 부적합**: Tag 와 구조 비대칭. (1) 데이터 모델: Tag=items SSOT(TagGroup.props.items propagation) vs Breadcrumb=정적 factory-child(`children-manager`, items 미정의). (2) separator: Tag X 는 chip **내부**(단일 노드) vs Breadcrumb separator(›)는 crumb **사이**(DOM `Breadcrumbs.css:20-27` `::after` / Skia `Breadcrumb.spec.ts:175-191` 별도 shape). (3) Skia 현황: `Breadcrumbs.spec.ts:156-163` shell-only(`render.shapes:()=>[]`) + 자식 `Breadcrumb.spec` self-render(text+separator). (4) click: crumb=href navigation(`<Link>`) — owner select redirect 무관. → Tag 패턴 직접 적용 아니라 **선행 축 ① 되감기(items SSOT 신설)부터 필요**. (A) projection 진입은 items SSOT 확정 후 재평가. 다음 (A) slice 후보는 Tab/TabList(items 기반 collection, Tag 선례 적합도 ↑) 우선 정찰 권장 → ✅ **Tab/TabList land 완료(Tab slice 2026-06-06)** — Breadcrumb 은 잔여 (A) 5 중 하나로 축 ① 되감기 선행 대기.
> - **(C) RAC controller-bound projection** [RAC render-prop 어댑터 신규] — DateSegment(=TimeSegment)/DateInput 2. collection items 아님 — RAC `<DateInput>{(segment)=>...}` controller(react-aria-starter DatePicker/TimeField/DateRangePicker)가 source. (A) 골격 위에 `getDateSegmentProjectionRows` controller 어댑터만 신규(granularity/locale/value → segment row 파생).
> - **(B+icon) leadingIcon generic 채널** [rule 채널, projector 아님] — DisclosureHeader/CalendarHeader 2. chevron = ADR-142 정본상 컴포넌트별 if 금지 → buildCatalogShapes 에 `leadingIcon`(icon_font) generic 채널 추가(rule `leadingIcon: "chevron-*"` 선언, box+icon+text). DOM=부모 흡수(Disclosure 내부 ChevronRight / Calendar `<header>` 자동) 비대칭은 (A)/(C) 와 동일.
>
> 상세 설계 비교 + 메커니즘별 proof slice + 차단 메모리 인용: [912-step4-unregistered-types-inventory.md](912-step4-unregistered-types-inventory.md) §"영역 B 3 메커니즘 설계 비교". **진입은 메커니즘별 proof slice 단위 사용자 별도 승인**(설계서 lock-in 만으로 confirm 자동 처리 금지 — `feedback-no-derived-adr-mid-execution`).
>
> **(A) proof slice #1 — Tag chip projection 설계 비교 완료(2026-06-05, 코드 변경 0)**: 사용자 confirm "개별 chip projection 노드화 (선례 직접 동형)". 실측상 Tag 는 ListBox 와 **데이터 모델(items SSOT)·container shell·boundary guard 동형**이나 **item 렌더 구조 비대칭** — ListBox 는 처음부터 projection 노드, TagList 는 spec 내부 self-render(`TagList.spec.ts:230-455` items 순회 chip 배열 emit, 이미 작동 중). proof slice 목표 = TagList self-render seam 제거 → 각 chip 을 `appendTagRowProjection` render-space 노드화. **wrap/maxRows 좌표 = Taffy flex-wrap 위임**(rowsGroup `flexWrap:"wrap" flexDirection:"row"` + chip `width:fit-content`)으로 수동 wrap 시뮬(`:299-333`) 제거. 재사용 = `getFlatProjectionRows`/boundary guard/`resolveCollectionWriteTarget` 3-route/selection handler. 신규 = `appendTagRowProjection`(rowsGroup style 차이만) + `resolveDataBoundTagProjection` 게이트 + remove route(content 변형, itemKey 삭제) + chip leaf = `Tag.spec.render.shapes` 재사용. kill criteria = self-render seam 제거 + DOM/Skia `/cross-check` 대칭 + selection/remove 상태 보존 + deep hit per-chip. **코드 진입(`appendTagRowProjection` 구현) → ✅ land 완료**(`a49e63541`). 이후 동형 패턴으로 **Tab/TabList land 완료**(Tab slice 2026-06-06, `appendTabRowProjection`). 상세: inventory §"(A) parent projector proof slice #1 — Tag chip projection 설계 비교".
>
> **단계 4 = 3계약 묶음 (2026-06-03 재설계)**. collection family 의 Skia generic 전환(skiaLegacy 제거)는 ListBox 만 안전했고, 나머지 6 collection(Menu/Select/ComboBox/Tabs/TagGroup/GridList)+Table 은 전환 시 전부 시각 변화가 났다(kill criteria 미통과 — `feedback-proof-gate-seam-removal-kill-criteria`). 단순 skiaLegacy 제거로는 대칭이 깨지므로, 단계 4 는 다음 **3계약을 한 묶음**으로 전환한다.
>
> | #   | 계약                                                                              | 절          | 해결 대상                                                                 |
> | --- | --------------------------------------------------------------------------------- | ----------- | ------------------------------------------------------------------------- |
...
>
> **전환 안전 3조건(재사용 판정 규칙)** = (1) legacy render.shapes 결과 ≡ 전환 후 buildCatalogShapes 결과(rule fill base ↔ legacy shell fill 일치) AND (2) trigger/text 를 buildCatalogShapes 가 그리는데 자식 element 도 같은 시각 안 그림(중복 회피) AND (3) items 순회가 render.shapes 안에 없음. **ListBox 만 셋 다 충족**(전환 완료, `ListBox.binding.ts`). 단계 4 의 C2(조건 1)+C3(조건 2)+C1(조건 3)이 나머지 family 에 셋을 동시 성립시킨다.
...
전환 후 컨테이너 노드는 `buildCatalogShapes` 가 rule fill base + border(`buildCatalogShapes.ts:106,116,153`)로 shell 을 그린다. 이 shell 이 **Preview DOM 의 컨테이너 CSS 와 시각 일치**(D3 대칭)해야 전환이 안전하다. 컴포넌트별 rule(`componentRulesTable.ts`) ↔ Preview CSS 실측 대조 결과, 일부 type 의 rule 이 어긋나 있다.
...
- Select/ComboBox 의 `variants:{}` 는 결함이 아니라 **의도**(컨테이너 투명, 시각은 자식 trigger 가 담당). 전환 시 컨테이너 shell 이 빈 box 가 되는 것이 Preview 대칭.
...
| -------------------------------- | -------------------------- | ---------------------------------------------------------------------------------------------------- | :----------: |
| **(I) data-bound row 순회**      | ListBox / GridList / Table | items/rows 데이터 → 행별 independent cell tree. ListBox=전환 완료(ADR-146), GridList/Table=동형 확장 |   **필요**   |
...
> | 2 (편집 계약 전환)           | ✅ **완료**                                                                                                                                             | `5b89e707e` (`getEditor`→`resolveEditContract`)                                                                                                             |
> | 3 (Skia state)               | ✅ **완료** (disabled 실효, hover/pressed threading 후속)                                                                                               | `5eebef96a` (`racStateAttrs`)                                                                                                                               |
> | 4 (collection 전환 C1/C2/C3) | ✅ **완료** — collection 7종 전부(ListBox proof + 비-data-bound 4종 + GridList C1 + Table 2D C1 + Menu) + 영역 B (A) projection 전환 3(Tag/Tab/TabList) | `d5da74c72`/`173765201`/`831079766`/`0e9c501d4`/`2818c6bf0`/`d7cfd470c`/`595805242` · (A) `a49e63541`(Tag)/`a39ad98fd`/`dc9617da0`/`c3b5c268f`(Tab/TabList) |
> | 5 (구 정본 제거)             | ◐ **분해된 삭제 진행** — (1b) escape 해소 + step 1 dead gate 제거 완료. step 2~5 진행 예정 (아래 분해표)                                                | (1b) `4b9ecb13e` · step 1 본 작업                                                                                                                           |
>
> 따라서 아래 1A "1차 kill 판단" / "단계 1 전체 통과해야 단계 2 진입" 류 미래시제는 단계 1~3 한정으로는 **이미 통과·진행**된 상태의 사후 기록으로 읽는다. 미완은 단계 4(collection 전체)/5.
>
> **단계 5 본체 — 분해된 삭제 (사용자 결정 2026-06-04 "전면 삭제 아니라 분해된 삭제")**: 일괄 삭제는 render.shapes dispatch / text measurement / spec import graph / generate-rules.ts 삭제가 한 번에 터져 실패 시 원인 분리가 안 된다. 회귀 표면을 step 별로 닫는다.
>
> > **⚠️ 아래 표의 step 4/5 셀 일부 서술은 승격 이전(2026-06-09) 진행 스냅샷이다.** ADR-912 본문 Status 는 **Implemented 2026-06-18** 로 승격됐다(catalog cutover 대상 spec 전수 물리 삭제 + dual-SSOT 소멸 land 완료). 표 안의 "Proposed 유지" / step 4 "잔여 type 마이그레이션 선행" 류는 그 시점 판정으로, 미등록 type spec fallback 잔존은 ADR-913/914 후속 영역으로 분리됐다. 정본 Status 는 ADR 본문 §Status header.
>
> | step                                                                                            | 내용                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | 상태                                                                                                                                                                                      |
> | ----------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
> | **step 1 — dead gate 제거**                                                                     | `skiaLegacy` 필드(type union + primitiveEntry opts + getter 필터) + `isCatalogSkiaCutover` 의미 소멸 정리(→ `isCatalogCutover` 위임 collapse) + invariant 테스트 잠금(모든 non-native entry catalog cutover + skiaLegacy 0건 + 두 게이트 동치)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | ✅ **완료** — 본 작업                                                                                                                                                                     |
> | **step 2 — runtime spec 의존 끊기**                                                             | **catalog registered runtime 에서 spec.render.shapes 도달 0건**(전역 0건 아님 — 사용자 정정 2026-06-04). 3 호출처(buildSpecNodeData:1211 / specTextStyle:227 / specTextStyleForOverlay:97) 모두 `isCatalogSkiaCutover` 가드 뒤 else 로 격하 → catalog 전환 type 은 generic(buildCatalogShapesOrPrimitive / rule 기반 buildCatalogShapes) 경로, fallback 은 **catalog 미등록 type 전용 임시 경로**로 명시 + dev 가드(catalog 누수 검출). measurement 의 `resolveComponentVisual(spec)` → `resolveSkiaVisualRule`(rule) 전환. replace-primitive(checkbox/radio/switch) measurement fallback 제거: rule+spec variant 에 textWeight:400 명시(drift 0). 미등록 type inventory(2026-06-05 재계산 51) 는 별도 후속 backlog.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | ✅ **완료** — 본 작업                                                                                                                                                                     |
> | **step 3 — source generator 제거**                                                              | `generate-rules.ts` 물리 삭제(사용자 명시 승인 2026-06-04) + `package.json:40` `generate:rules` script 제거 + 현재 상태 주석 정정("삭제 예정"→"삭제됨": `componentRulesTable.ts` 헤더 / `generate-css.ts:17` / `composition-document.types.ts:193`). **ADR 역사 기록(왜 table 이 정본이 됐는지)은 유지** — "실행 경로 + 현재 상태 주석"만 정리(사용자 결정 "모든 generate-rules 언급 삭제 아님"). `componentRulesTable.ts` 직접 정본 독립 확인: import = type-only 1개(`ComponentRulesTable`), 런타임 spec/generate-rules 참조 0(잔존은 전부 drift 분석 주석), `build:specs` 가 `generate:rules` 없이 generate:css 만으로 125 CSS 정상 생성 + generated CSS diff 0(DOM 불변). build chain(`package.json:34`)은 이미 generate:css 중심(generate:rules 미포함이었음). specs 소스(비-테스트) type-check 에러 0(15 에러는 `CSSGenerator.rootSelectors.test.ts` pre-existing, generate-rules 무관 — stash 비교 검증).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | ✅ **완료** — 본 작업                                                                                                                                                                     |
> | **step 4 — element type 폐기 phase** (구 "spec 파일 삭제"; 명칭 정정 2026-06-08, 사용자 옵션 B) | **단위 = "spec 파일 물리 삭제"가 아니라 "element type 폐기 decision"** — 4축 gate(G-render/G-factory/G-registry/G-consumer, inventory §"step4 element 폐기 정책")를 통과한 type 만 폐기 가능. 폐기 = spec 삭제 + factory/migration + registry + layout/text consumer 이관의 동반 작업 묶음(해당 축만). **4 gate 전부 PASS(true-dead) = 0건**. **no-go 사유 = catalog 미등록 type 의 render.shapes fallback 대체 필요**: 미등록 type(2026-06-08 set-difference 재실측 43 = 111 spec map − 68 catalog[primitiveEntry 65 + native 3]: color 7 scope 외 / sub-part / 미전환 leaf / legacy~native — 순 차단 15 = ③ 영역 B 14 + ④ value-fill 잔여 1[SliderTrack])은 아직 spec.render.shapes fallback 으로 그려진다. spec 통째 삭제 시 시각 source 소멸. **✅ proof land 2건 (2026-06-09, 사용자 명시 삭제 승인 각각)**: ① **Autocomplete** 폐기 — 9 참조처 atomic(spec/tagToElement/specRegistry/barrel×2/ComponentTag union+카운트 117/componentRulesTable rule/metadata/styles @import) + 파일 3 + 생성 CSS. ② **DateSegment + TimeSegment** 폐기(`c77ff8619`) — atomic 9곳(DateSegment.spec.ts / tagToElement import+2 entry[TimeSegment undeclared alias 동시 청산] / specs+components barrel 2 / ComponentTag union+카운트 117→116 / componentRulesTable rule / preview case 2 / buildSpecNodeData CONTAINER_DIMENSION_TAGS / DateField+TimeField propagation rule 2). 보존 = shared/components 4 RAC self-compose + 4 spec childSelector(D1). 두 proof 모두 검증 5종 PASS(type-check 신규 0 / build:specs orphan 0 / dist fresh[stale-dist masking 회피] / registration-contract 10/10 불변식 A / **live behavior** — Autocomplete: palette 미노출·canonical element 0·콘솔 0 / DateSegment: factory 경로 DateField/TimeField 정상 렌더·자식 Label+DateInput+FieldError·segment element 0·DateInput escape 가 MM/DD/YYYY·HH:MM box 자체 렌더·CSS↔Skia 대칭·콘솔 0). **전제 반전 양방향**: Autocomplete = scope 확대(registry-only 3 과소집계 → 9 참조처), DateSegment = scope 축소(render.shapes 실체 있으나 canonical element 미생성 = 수요 측 공백 → 4 gate 전부 PASS true-dead). 잔여 = (A) **미등록 type inventory + 경로 대체**([912-step4-unregistered-types-inventory.md](912-step4-unregistered-types-inventory.md)) + (C) **type별 명시 폐기 승인**(원본 파일, CLAUDE.md 마이그레이션 원칙). **🔒 즉시-폐기 proof 라인 종결 (2026-06-09, 사용자 order-2/4)**: ① 다른 true-dead 후보 전수 recon = **0건**(잠재 후보 8 cross-check → 전부 보존), ② Field proof = **no-go 유지**(G-factory+G-consumer FAIL = 활성 데이터 sub-element, TagEditor slot+5 renderer DataField+hydration migration 선행 = 별도 설계축, 즉시-폐기 라인 미적격). 잔여 폐기는 마이그레이션 선행 + 별도 명시 승인(무손실 7 = parent factory canonical element 생성 G-factory FAIL / 영역 B 14+List / container shell 9). | 🟢 **proof land 2건 + 즉시-폐기 라인 종결** (Autocomplete + DateSegment/TimeSegment 폐기 완료; true-dead 후보 0 전수 recon; Field no-go 확정; 잔여 type 각각 마이그레이션 선행+명시 승인) |
> | **step 5 — 문서/status 정합**                                                                   | ~~Proposed 유지 근거~~ → **Status=Implemented 2026-06-18 승격 완료** (catalog cutover 대상 spec 전수 물리 삭제 + dual-SSOT 소멸 land — 잔존 spec 3 = Frame/Group/Slot 영구 native/D1, cutover 대상 0). 각 step breakdown 진행표 갱신(step 동반) + Tab/TabList projection land 반영(2026-06-06). 미등록 type spec fallback 잔존(step 4 잔여)은 ADR-913/914 후속 영역으로 분리 — 본 ADR-912 핵심 결정(catalog SSOT collapse)은 종결. 반영 위치: ADR 본문 §Status + `docs/adr/README.md` + 본 표.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | ✅ **완료** — Implemented 승격 + step 1~3 정합 반영                                                                                                                                       |
...
2. **편집 계약 전환**: `useEditContract` + 패널 section 필터 + generic field renderer. `getEditor`/4 섹션 삭제. _(단계 1 전체 통과 후에만 진입)_
3. **Skia state**: `racStateAttrs` + interaction threading + scene invalidation. → **G-state**.
4. **collection 전환 (3계약 묶음 — C1/C2/C3, ④ 영역 B)**: 6 collection + Table 의 Skia generic 전환은 단순 skiaLegacy 제거가 아니라 3계약 동시 성립이 필요(④ 4-1 표).
   ...
   - **C1 generic collectionProjector (4-2/4-2.7)**: `isRenderProjectionId` 일반화 → `CollectionProjectorConfig`(family 추상화) → window → `resolveCollectionWriteTarget` 3-route + negative fixture. 적용 범위 = data-bound (I) ListBox(proof 완료) + GridList. (II) Tabs/TagGroup, (III) Menu/Select/ComboBox 는 C2+C3 만으로 전환(projector 불필요).
...
5. **구 정본 + 전체 seam 제거**: 124 spec(123 `shapes:` 정의) + 3 `render.shapes()` call site(그리기 1 + 측정 2) + cutover/skiaLegacy + `buildCatalogShapes`/`resolveComponentVisual` seam(전 family) + 5 레지스트리 + buildSpecNodeData 30+ 분기 삭제. 조합 family Components page reusable 저작(Table 2D column culling 분기).
   ...
   > | 의존                                                                            | 실측                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | 차단 이유                                                                                                            | 해소 선행                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
   > | ------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
   > | **(1a) Skia 그리기 — collection 7**                                             | 현재 `componentCatalog.ts` `{ skiaLegacy: true }` 호출 **12개**(실측 2026-06-03) 중 collection 7 (Menu/Select/ComboBox/Tabs/TagGroup/GridList[④ `:219-254`] + Table[⑤ `:286`]). `isCatalogSkiaCutover = cutover==="catalog" && !skiaLegacy` (`componentCatalog.ts:467`) → `buildSpecNodeData.ts:1180` 에서 `spec.render.shapes(...)` 로만 그려짐                                                                                                                                                                                                                                                                                                                                                                                        | render.shapes 제거 시 collection Skia 렌더 붕괴 (전환 시 5-type 시각 변화 — 4-1 표)                                  | **본 ADR 단계 4 (C1/C2/C3, ④ 영역 B)** 가 해소 — generic projector(ListBox proof→GridList/Table) + rule fill 정렬(Tabs/TagGroup/Menu) + shell-only(Select/ComboBox text 중복). 단계 4 전환 후 collection 7 의 skiaLegacy 제거 → 단계 5 의 `render.shapes` 제거 대상에서 collection 은 이미 빠짐. (메모리 [[project-collection-skia-flip-not-listbox-isomorphic]] + [[project-adr142-family1-flipped]] "Skia 잔여 → ADR-146 + ADR-912 단계 4")                                                                                                                                                                   |
   ...
   > | **(2) 텍스트 측정 — catalog 전환 TEXT_BEARING** ✅ **해소(step 2, 2026-06-04)** | `extractSpecTextStyle`(`specTextStyle.ts`) + `extractFullSpecTextStyle`(`specTextStyleForOverlay.ts`)의 catalog 전환 type(button/badge/togglebutton/link + checkbox/radio/switch)이 측정 source 를 `resolveComponentVisual(spec)` → `resolveSkiaVisualRule`(rule) 기반 buildCatalogShapes 로 전환 → 측정에서 spec.render.shapes 미참조. replace-primitive(checkbox/radio/switch) 는 rule+spec variant 에 textWeight:400 명시로 generic 측정 drift 0(과거 spec 미emit→400 vs generic fallback 500 차이 해소). **잔여 = catalog 미등록 TEXT_LEAF/sub-part**(text/heading/paragraph/code/kbd/input + breadcrumb/menu/selectvalue 등) — catalogType 미설정/비-전환 → render.shapes 측정 유지(미등록 전용 임시 경로, 단계 5 후속 inventory). | (해소됨) render.shapes 제거 시 텍스트 줄바꿈 어긋남 — 전환 type 은 rule 측정으로 정합(byte-level oracle parity test) | **measurement spec 의존 끊기 land**: specTextStyle/specTextStyleForOverlay 둘 다 `useCatalog ? resolveSkiaVisualRule+buildCatalogShapes : spec.render.shapes(fallback)`. drift 0 = Checkbox/Radio/Switch rule variant + spec variant 양쪽 textWeight:400(spec 추종 guardrail `resolveSkiaVisualRule.test` 통과). parity: specTextStyle.test 137/137(checkbox/radio/switch measurement == render.shapes oracle) + drift guardrail 559/560 + DOM generated CSS diff 0(불변).                                                                                                                                      |
   ...
   >
   > **cutover/skiaLegacy union 제거 차단 — 해소(2026-06-04)**: `skiaLegacy?` 필드는 (1a)collection 7(단계 4) + (1b)잔여 5(Tooltip + date 4, 본 작업) 전환으로 **사용처 0건** 도달 → `skiaLegacy?` 필드 자체 제거 가능(현재 helper 정의 + getCatalogSkiaCutoverTypes 필터에만 잔존, 안전망으로 유지). `CutoverState = "legacy"|"cutting-over"|"catalog"` 의 `"legacy"`/`"cutting-over"` 분기도 모든 entry 가 `cutover:"catalog"` 이라 dead — union 단순화는 단계 5 본체에서 spec/측정 경로와 함께 정리(표면 정리, 회귀 무관).
   >
   > **결론 (2026-06-04 step 2 갱신)**: 단계 5 본체(spec/render.shapes 물리 삭제) 진입 선행 의존 중 **(1a) collection 전환 + (1b) 잔여 5 escape + (2) catalog 전환 TEXT_BEARING 측정 전환 = 해소 완료**. step 1(dead gate) + step 2(runtime spec 의존 끊기) 로 **catalog registered runtime 에서 spec.render.shapes 도달 0건** 달성. **잔여 = catalog 미등록 type(2026-06-05 재계산 51) 의 render.shapes**(color scope 외 / sub-part / 미전환 leaf / legacy — fallback 임시 경로) **+ (3) Layer D 정리**(대부분 `resolveContainerSpacing` 경유). 이들은 step 3(generate-rules 삭제) → step 4(spec 파일 삭제) 진행 시 함께 정리 — 단, **catalog 미등록 type 의 generic 전환/등록은 별도 후속 inventory**(spec 삭제 전 해당 type 의 fallback 경로 대체 필요, 사용자 결정 2026-06-04 "미등록 type inventory 는 별도 후속"). collection 7 의 Skia 그리기 의존은 **본 ADR 단계 4**, (1b) 는 **본 ADR 단계 5 step 1b**, (2) 는 **본 ADR 단계 5 step 2** 가 직접 해소. 단계 1~4 + 단계 5 step 1/2 성과 유지. [[feedback-no-fallback-thinking]] + [[feedback-proof-gate-seam-removal-kill-criteria]] 정합.
...
| 영역                    | 파일                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| schema/어댑터           | `packages/shared/src/catalog/types.ts` (cutover/skiaLegacy 제거 + ComponentNode + variant·size accepts 분리 + PropContract.section 확장 + **entry `panel:PanelMeta` 는 `category/label/icon/placeable` 이미 존재(types.ts:179-184) → `layoutOnly?:boolean` 1개만 신규**, ②-5 #1), 신규 `resolvers/resolveMergedStyle.ts`(theme rule 직접)/`outputs/toReactStyle.ts`/`toSkiaStyle.ts`(보편속성 직접)                                                                                                                                                                                                                                                                                                                                               |
...
| 등록                    | `componentCatalog.ts`(6 레지스트리 collapse, cutover/skiaLegacy 제거), `cutover.ts` 삭제, **`panels/components/ComponentList.tsx:70-150` 정적 배열 7(61 entry) → catalog 파생 전환 + palette↔catalog 불일치 9 선행 해소**(②-5-1a/1b), **`sprites/builderAliasMap.ts` ALIAS 9(ComboBox 3 + SearchField 4 + TabBar + body) → leaf entry 정규 등록 or parent sub-part 유지 판정**(②-5-2)                                                                                                                                                                                                                                                                                                                                                             |
| base 층                 | `resolvers/resolveComponentRule.ts`(위치·시그니처 무변경, 정본 테이블 read) + `catalog/generated/componentRulesTable.ts`(②-6-A: **직접 정본 승격** — 헤더 "AUTO-GENERATED"→정본 표시, 손 편집). DOM swap: `renderers/CSSGenerator.ts`(`generateCSS` optional variant source 파라미터) + `scripts/generate-css.ts`(shared table import 주입) + `scripts/validate-sync.ts`(동일 주입). audit: `packages/design.md` + `react-aria-starter/src/*.css`(토큰 팔레트, variant→token 매핑 없음 — 무변경). **삭제됨(단계 5 step 3, 2026-06-04)**: `generate-rules.ts` 물리 삭제 + `package.json:40` `generate:rules` script 제거(build chain `package.json:34` 는 이미 generate:css 중심이라 generate:rules 미포함이었음 — script 정의만 잔존했던 것 제거) |
