...
> | **step 2 — runtime spec 의존 끊기** | `buildSpecNodeData` `spec.render.shapes` fallback 제거 + replace-primitive/text measurement 가 spec 없이 동작 고정. runtime `spec.render.shapes` 0건                                                                                           | ⬜ 예정 (선행: step 1)                         |
