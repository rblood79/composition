...
> | **step 4 — spec 파일 삭제**         | import graph 끊긴 뒤 `*.spec.ts` 124+ 제거 (마지막). **no-go 사유 = catalog 미등록 type 의 render.shapes fallback 대체 필요**: step 2 가 fallback 을 "catalog 미등록 type 전용 임시 경로"로 격하했으나 미등록 type(2026-06-08 코드 set-difference 재실측 43 = 111 spec map − 68 catalog[primitiveEntry 65 + native 3]: color 7 scope 외 / sub-part / 미발효 leaf / legacy~native — 상세 분해는 inventory 소계, 순 차단 15 = ③ 영역 B 14 + ④ value-fill 잔여 1[SliderTrack])은 아직 spec.render.shapes fallback 으로 그려진다. spec 통째 삭제 시 시각 source 소멸. 선행 = (A) **미등록 type inventory + 경로 대체**(별도 후속, [912-step4-unregistered-types-inventory.md](912-step4-unregistered-types-inventory.md) — 선행-1~6 leaf/escape/흡수/legacy·native/color 분류 완료, 잔여 = 영역 B (A) projector 5 + List + value-fill 잔여 1[SliderTrack] + internal 3) + (B) import graph 0 확인 + (C) **사용자 명시 삭제 승인**(원본 124+ 파일, CLAUDE.md 마이그레이션 원칙) + (D) type-check/cross-check. | 🚫 **no-go** (선행: 미등록 type 대체 + 명시 삭제 승인) |
