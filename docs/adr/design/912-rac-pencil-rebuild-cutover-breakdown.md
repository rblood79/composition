...
- **Table 2D**: row + column cell grid(셀 단위 독립 + column 스타일 전파)는 row projection 만으로 불충분 → ListBox/GridList proof 후 column culling 분기 별도 정밀화(단계 4 내 후행 sub-item, 미해결 표 참조).
...
5. **구 정본 + 전체 seam 제거**: 124 spec(123 `shapes:` 정의) + 3 `render.shapes()` call site(그리기 1 + 측정 2) + cutover/skiaLegacy + `buildCatalogShapes`/`resolveComponentVisual` seam(전 family) + 5 레지스트리 + buildSpecNodeData 30+ 분기 삭제. 조합 family Components page reusable 저작(Table 2D column culling 분기).
   ...
   > **선행 의존 — 단계 5 진입 차단 조건 (실측 2026-06-03, 단계 4 종료 후 측정).** plan 작성 시점 추정("render.shapes 일괄 삭제")은 `render.shapes` 가 **단일 역할(Skia 그리기)** 이라는 가정이었으나, 실측은 **3개 SSOT 역할 겸직**이라 현 상태에서 안전 제거 가능한 spec 교집합이 거의 비어있다. 세 의존이 각각 해소돼야 단계 5 진입 가능:
   >
   > | 의존                                        | 실측                                                                                                                                                                                                                                                                                                                                             | 차단 이유                                                                                                                         | 해소 선행                                                                                                                                                                                                                                                                                                                                                                                                                                     |
   > | ------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
   ...
   > | **(1b) Skia 그리기 — 잔여 5 skiaLegacy**    | 나머지 `{ skiaLegacy: true }` 5 entry — **Tooltip 1**(⑥ overlays `:329`; Dialog/Modal/Popover/DropZone 은 이미 Skia 발효·skiaLegacy 미설정) + **date 4**(⑦ Calendar/RangeCalendar/DatePicker/DateRangePicker `:355-376`). 동일하게 `spec.render.shapes(...)` 의존. (Slider/color 는 애초 skiaLegacy 없음 — 잔여 아님)                            | render.shapes 제거 시 Tooltip/date Skia 렌더 붕괴                                                                                 | Tooltip portal/overlay + date 날짜 grid 의 skiaPrimitive escape hatch (node-type별, 컴포넌트별 아님 — ④ 영역 B 와 별개 backend). 단계 4 범위 밖 — 단계 5 진입 전 별도 정밀화(미해결 표)                                                                                                                                                                                                                                                       |
   ...
   >
   > **cutover/skiaLegacy union 제거 차단**: `CutoverState = "legacy"|"cutting-over"|"catalog"` + `skiaLegacy?` 필드는 현재 12 skiaLegacy entry 가 실제 사용 중 → (1a)collection 7 은 단계 4 발효로 skiaLegacy 제거되나, (1b)잔여 5(Tooltip + date 4) 해소 전까지 `skiaLegacy?` 필드 자체는 제거 불가. (단, 모든 entry 가 `cutover:"catalog"` 이므로 `CutoverState` 의 `"legacy"`/`"cutting-over"` 분기는 사실상 dead — union 단순화의 부분 여지는 있으나 spec/측정 경로 무관한 표면 정리.)
   >
   > **결론**: 단계 5(render.shapes 일괄 제거) 진입 = **단계 4 collection 발효(1a 해소) + 잔여 5 skiaPrimitive escape(1b) + 텍스트 측정 generic 전환(2) + Layer D 정리(3)** 가 모두 통과한 이후. collection 7 의 Skia 그리기 의존은 ADR-920(Superseded by ADR-910, 비착수)이 아니라 **본 ADR 단계 4 가 직접 해소**한다(사용자 결정 2026-06-03 — collection family 발효 = ADR-912 단계 4 일괄). 단계 1~4 성과(Button slice / Properties 편집 계약 / disabled state / ListBox·collection projected 인프라)는 유지. 현 상태에서 "render.shapes 의존 0" 인 안전 제거 spec 교집합이 거의 비어있어 강행 시 collection/Tooltip/date Skia + 전 leaf 텍스트 측정 동시 회귀. [[feedback-no-fallback-thinking]] (skiaLegacy 는 fallback 아닌 미발효 정상 경로) + [[feedback-proof-gate-seam-removal-kill-criteria]] (generic 발효 검증 후 seam 제거) 정합.
