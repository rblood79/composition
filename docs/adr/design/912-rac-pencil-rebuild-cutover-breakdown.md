...
> | **step 4 — spec 파일 삭제**         | import graph 끊긴 뒤 `*.spec.ts` 124+ 제거 (마지막). **no-go 사유 = catalog 미등록 71 type 의 render.shapes fallback 대체 필요**: step 2 가 fallback 을 "catalog 미등록 type 전용 임시 경로"로 격하했으나 그 71 type(color 7 scope 외 / sub-part ~30 / 미발효 leaf ~20 / legacy~native ~14)은 아직 spec.render.shapes fallback 으로 그려진다. spec 통째 삭제 시 71 type 시각 source 소멸. 선행 = (A) **71 type inventory + 경로 대체**(별도 후속 — color 유지/별도, sub-part 부모·primitive 흡수 분류, 미발효 leaf catalog 등록/generic 전환, legacy/native 유지·제거 정책) + (B) import graph 0 확인 + (C) **사용자 명시 삭제 승인**(원본 124+ 파일, CLAUDE.md 마이그레이션 원칙) + (D) type-check/cross-check.                                                                                                                                                                                                 | 🚫 **no-go** (선행: 71 type 대체 + 명시 삭제 승인) |
