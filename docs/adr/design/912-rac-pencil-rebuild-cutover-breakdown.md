...
5. **구 정본 + 전체 seam 제거**: 124 spec(123 `shapes:` 정의) + 3 `render.shapes()` call site(그리기 1 + 측정 2) + cutover/skiaLegacy + `buildCatalogShapes`/`resolveComponentVisual` seam(전 family) + 5 레지스트리 + buildSpecNodeData 30+ 분기 삭제. 조합 family Components page reusable 저작(Table 2D column culling 분기).
   ...
   > | 의존                                        | 실측                                                                                                                                                                                                                                                                                                                  | 차단 이유                                                                                                                         | 해소 선행                                                                                                                                                                                                      |
   > | ------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
   > | **(1) Skia 그리기 — 18 skiaLegacy**         | `componentCatalog.ts` 에 `skiaLegacy:true` 18 entry (collection 7 + Table + date 4 + overlay Dialog/Modal/Popover/DropZone + Tooltip/Slider). `isCatalogSkiaCutover = cutover==="catalog" && !skiaLegacy` (`componentCatalog.ts:445`) → 이들은 `buildSpecNodeData.ts:1180` 에서 `spec.render.shapes(...)` 로만 그려짐 | render.shapes 제거 시 collection/overlay/date 전체 Skia 렌더 붕괴                                                                 | **ADR-920(Projected Tree)** 로 collection family generic 발효 + overlay/date skiaPrimitive escape hatch (메모리 [[project-adr142-family1-flipped]] "Skia 잔여 → ADR-146 + ADR-920 명시 이관, 142 는 여기까지") |
   ...
   >
   > **cutover/skiaLegacy union 제거 차단**: `CutoverState = "legacy"|"cutting-over"|"catalog"` + `skiaLegacy?` 필드는 18 skiaLegacy entry 가 실제 사용 중 → (1) 해소 전 union 필드 제거 불가. (단, 7 family 모두 `cutover:"catalog"` 이므로 `CutoverState` 의 `"legacy"`/`"cutting-over"` 분기는 사실상 dead — union 단순화의 부분 여지는 있으나 spec/측정 경로 무관한 표면 정리.)
   >
   > **결론**: 단계 5 = **ADR-920(Projected Tree, 18 skiaLegacy generic 발효) + 텍스트 측정 generic 전환** 이후로 순서 재정의. 단계 1~4 성과(Button slice / Properties 편집 계약 / disabled state / ListBox 변환 인프라)는 유지. 현 상태에서 "render.shapes 의존 0" 인 안전 제거 spec 교집합이 거의 비어있어 강행 시 collection/overlay/date Skia + 전 leaf 텍스트 측정 동시 회귀. [[feedback-no-fallback-thinking]] (skiaLegacy 는 fallback 아닌 미발효 정상 경로) + [[feedback-proof-gate-seam-removal-kill-criteria]] (generic 발효 검증 후 seam 제거) 정합.
