...
- 잔여: `historyActions` / `inspectorActions`. `elements.ts` page removal local mutation map 은 store-local mutation cleanup 로 별도 처리. `canonicalSceneModelLegacy` 는 bootstrap boundary 로 Phase 5/6 allowlist 정렬 필요.
