...
### Phase 3 — 정적 행 origin · 열 ↔ 셀 동기화 · TableView 이관 (G3, 2026-09-25)

- **origin · slot · 이관** (`components/tableOrigins.ts` — Phase 2 `tableColumnOrigins.ts` 를 넓힘): Row origin `component-table-row` (행 모양만) · TableView TableBody `slot: [Row origin]` · **TableView 이관** = plain Column → Column origin ref · plain Row → Row origin ref, **id 유지** · 셀은 그 Row ref 의 자기 자식 (노드째 그대로) · 이관한 사용자 TableView 도 TableHeader · TableBody slot 을 연다. 행 하나라도 셀 수 ≠ 열 수면 이관하지 않는다 (같은 노드 · slot 없음). Components origin TableView 도 같은 이관 (열 3 · 행 1 × 셀 3). history 스냅샷 (body · origin · 사용자 TableView subtree) 도 같은 보정.
- **바깥 instance 경로 전치 불필요**: id 를 유지하므로 `descendants["component-tableview__2/component-tableview__2_1/component-tableview__2_1_3"]` 가 이관 뒤에도 같은 셀에 닿는다 (선행 수리 — 두 해석기가 중첩 ref 자기 자식에 바깥 patch 적용). 진단 (f) 기준선이 이관 뒤 그대로 GREEN.
- **셀 동기화**: 추가 = `planTableColumnInsert` 가 TableView 정적 행마다 셀 (plain 은 `cells` · instance 는 TableBody 도 mode C — origin 행 복제 시 셀 patch 를 복제본에 접는다) · 행 "+" = `planTableRowInsert` (Row instance + 열 수만큼 셀, instance 는 TableBody mode C) · 삭제 = `elementRemoval` 의 Table 전용 분기를 구조 기준 (TableHeader 자식 · TableBody 자식 · 행 자식 — plain · ref 무관) 으로 넓혀 TableView 포함 · 순서 변경 = `moveCanonicalChild` 가 같은 TableHeader 안 열 이동이면 모든 행 셀을 같은 자리로 (Layer drop · 키보드 · Canvas drag · undo/redo 재생이 모두 이 함수를 지나 되돌리기까지 대칭). 전부 셀 수 = 열 수인 TableView 만.
- **결함 2 (수리)**: ① Canvas 해석기가 mode C 항목 ref 의 자기 자식 (instance TableBody 복제 행의 셀) 을 버렸다 (`materializeOverrideChildren` 이 origin 자식만 펼치고 return) → 선행 수리와 같은 순서 규칙 (origin 자식 뒤) 으로 실체화. ② 삭제 history 가 요청 root 만 remove event 로 실어 동기화로 같이 지운 다른 행의 셀 (root 자손 아님) 이 undo 에서 사라졌다 (종전 Table 분기도 같은 공백) → 삭제 집합의 모든 subtree 최상단을 싣는다.
