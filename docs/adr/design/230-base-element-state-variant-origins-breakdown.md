...
> 본문: [230](../completed/230-base-element-state-variant-origins.md). 구현 상세 (Phase · 파일 변경표 · 게이트 실행 기록) 는 이 파일에만 둔다.
...
### Phase 3 — G4 PASS (2026-09-22) · Implemented

- BC unit `adr230BackwardCompat.test.ts` 6 (229 패턴): pre = 229 hydration − 상태 변형 + 사용자 저작 (Button origin 을 body 맨 뒤로 · 사용자 페이지 plain Button / `isSelected` ToggleButton instance) → 230 hydration: 기존 노드 직렬화 불변 · **필드 추가 0** · 변형은 default 바로 뒤 (사용자가 옮긴 default 를 따라감 — body 끝이 아니다) · 최초 보충 **Δnode 33 (root 23 + Checkbox/Switch Label 10) · Δbyte 9,793** (= 변형 직렬화 정확히) · 재hydration 같은 객체 · 롤백 (변형 0 문서 = plain 경로 projection 없음 · 변형 있는 저장 문서 그대로 읽힘).
- BC live `adr230-bc-live.mjs` **6/6** (Compare Mode 없음): 저장 층 (IndexedDB `document_parts` · `document_heads.count`) 에서 interaction 변형 21 레코드를 지워 Phase 1 저장 모양을 만들고 reload → 편집 보존 · 15 보충 위치 · Skia A 빨강 · 재reload Δ0 (251 노드 · 53,021 B).
- Properties 읽기 전용 배지: `ComponentSemanticsSection` 정체 칩에 상태 pill (`SELECTED` · `DISABLED` · `HOVER` · `PRESSED` · `FOCUS`, i18n ko/en · 툴팁 "X 의 상태 변형 — 이 상태일 때의 배경 · 색 · 테두리 · 투명도만 편집한다") — `.component-semantics-identity-state` (10 mono · 얇은 테두리). unit 1. 편집 자체는 막지 않는다 (변형 origin 은 편집 대상 — "읽기 전용" 은 정체 표식이라는 뜻).
- 보류 (사용자 판정 대상): **Navigator 그룹 접기** — 변형은 body 의 형제 노드라 접기는 합성 그룹 UI 가 필요하다 (이름 prefix `ToggleButton/` 가 이미 묶음을 읽어 준다) → LOW 후속 후보. **Components 페이지 배치 confirm** (G4) — default 오른쪽 같은 행 (`variantOf` 순) — 사용자 "확인했다" (2026-09-22) → Implemented, `completed/` 이동.
- 후속 후보 (범위 밖 기록): Radio/Tab base origin (228 축) · item template `--disabled` (229 slot 정렬 뒤) · 캔버스 hover 재현 없음 (ADR-150).
