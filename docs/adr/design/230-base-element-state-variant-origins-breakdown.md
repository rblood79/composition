...
### Phase 3 — G4 BC PASS (2026-09-22) · 배치 confirm 대기
...
- 보류 (사용자 판정 대상): **Navigator 그룹 접기** — 변형은 body 의 형제 노드라 접기는 합성 그룹 UI 가 필요하다 (이름 prefix `ToggleButton/` 가 이미 묶음을 읽어 준다) → LOW 후속 후보. **Components 페이지 배치 confirm 1회** (G4) — default 오른쪽 같은 행 (`variantOf` 순) — 사용자 confirm 뒤 Implemented.
