...
- 이번 absolute leaf 실측은 Preview geometry와 분할 surface 존재를 증명한다.
  absolute leaf의 Skia 내부 bounds/terminal 계측은 일반 flow allowlist와 별도로
  계속 commit-only 범위에 둔다.
- 2026-08-24 실행 중인 Builder의 `Compare Mode`에서 `Badge` spacing spot-check도
  ...
  generic non-grid flex sibling의 Skia `bounds`/`hitBounds` parity는 별도 runner
  live gate로 종결했다.

## 일반 flow allowlist 종결

non-grid flow의 `width`, `height`, `padding`, `gap`은
[Phase 5 flow live parity harness](187-phase-5-layout-flow-live-parity.md)의
generic multi-sibling Builder run에서 Skia bounds/hitBounds, Preview sibling,
비영향 identity, Canvas 변경·복귀, terminal canonical handoff를 모두 통과했다.
이 네 항목은 Phase 5 allowlist 승격 조건을 충족한다.
...
| in-flow `width/height`       | `computePresentationLayoutTargeted` + bridge            |             parent + sibling + descendant `N >= 2` | grid/unsupported ancestry는 reject                 | **통과** — generic live runner의 bounds/hitBounds·identity·terminal·Preview parity                             |
| `padding`                    | `computePresentationLayoutTargeted` + bridge            | parent content box + affected descendants `N >= 2` | grid track cache가 필요한 target/ancestry는 reject | **통과** — non-grid flow live draw/hit/Preview parity와 terminal handoff                                       |
| `gap`                        | `computePresentationLayoutTargeted` + bridge            |            flex parent + in-flow siblings `N >= 2` | grid 및 persistent cache가 필요한 구조는 reject    | **통과** — flex sibling children-map/hit-test parity와 atomic restore                                          |

코드 경로·회귀 테스트·populated Builder live evidence가 위 최소 범위를 닫았다.
전역 `layoutVersion++` 또는 full rebuild fallback은 재개 조건이 아니다. absolute
leaf의 내부 bounds 계측과 grid/intrinsic/structure는 별도 fail-closed 범위다.
