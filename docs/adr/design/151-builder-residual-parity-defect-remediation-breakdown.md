...
| preview prop 편집 stale (신규 관측 2026-07-17)     | 요소 prop 편집 (Inspector size/heading) 이 builder canonical·Skia 에는 즉시 반영되나 preview DOM 은 미반영 — reload (canonical 재송신) 후에만 반영 | 미규명 — canonical 렌더 경로는 UPDATE_CANONICAL_DOCUMENT 재송신에만 반응하는데 prop 편집 시 재송신이 발화하지 않는 것으로 관측 (UPDATE_ELEMENT_PROPS 는 legacy elements state 만 갱신) | 후속 확인 (preview 동기화 계열)                                                                                                                                                                  |
