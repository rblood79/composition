...
>
> **3차 갱신 (2026-07-18, 잔여 ②)**: preview prop 편집 stale 해소 — 근본 원인은 **rAF hidden-탭 throttling** (canonical 재송신이 `requestAnimationFrame` 예약이라 background 탭 미동작). visible→rAF / hidden→setTimeout 하이브리드 스케줄러 `scheduleFrameOrTimeout` 도입으로 해결 (표 하단 행 참조). **남은 잔존은 엔진 percent-in-intrinsic 1건뿐** (Disclosure/DisclosureGroup flex 부모).
