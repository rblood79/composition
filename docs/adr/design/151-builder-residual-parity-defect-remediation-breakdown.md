...
> **2026-07-17 후속 갱신**: 아래 4건 중 3건 해소 (원인 조사 → 수정, 커밋 4953fc478 / marker 커밋 / 0f5743273). **2차 갱신 (같은 날)**: IllustratedMessage 시각 발산도 해소 (06f1319b3 — metric SSOT 3경로 공유). 잔존 = 엔진 percent-in-intrinsic (유지) + preview prop 편집 stale (신규 관측, 표 하단).
>
> **3차 갱신 (2026-07-18, 잔여 ②)**: preview prop 편집 stale 해소 — 근본 원인은 **rAF hidden-탭 throttling** (canonical 재송신이 `requestAnimationFrame` 예약이라 background 탭 미발화). visible→rAF / hidden→setTimeout 하이브리드 스케줄러 `scheduleFrameOrTimeout` 도입으로 해결 (표 하단 행 참조). **남은 잔존은 엔진 percent-in-intrinsic 1건뿐** (Disclosure/DisclosureGroup flex 부모).

| 항목                                               | 실측                                                                                                                                               | 원인                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | 처리                                                                                                                                                                                                                                                                                                                                                    |
| -------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Disclosure / DisclosureGroup (flex 부모 한정)      | Skia 0×56 vs CSS 168.2×56 (fit-content)                                                                                                            | **2-part (2026-07-18 Rust repro 로 규명, column-flex-start cross=width)**: ① `width:100%` 를 indefinite 부모(`container_size = INDEFINITE_AVAIL = -1`)에 곱해 `-1` 로 붕괴 (`style.rs` percent 해소) → `cs<0 → auto(None)` 강등으로 수정 가능(CSS 정확·엔진 테스트 회귀 0 확인). **단 이것만으로는 미해소** — ② indefinite(shrink-to-fit) 부모 안 auto/percent width 가 content 로 shrink 되지 않음(엔진 shrink-to-fit/intrinsic width 계산 gap). repro: leaf 150 / header −1 / **Disclosure 0** (CSS 3자 모두 150). part 2 는 레이아웃 core(block/flex width) 변경이라 bounded 아님 | **보류 (2026-07-18, 사용자 결정)** — 실측상 bounded(2-5줄)가 아니라 moderate 엔진 계약 변경(part 2 = shrink-to-fit width 계산, 회귀 위험). 증상 범위 좁음(Disclosure/DisclosureGroup 가 flex 부모 안일 때 한정, edge). 정확한 2-part root cause 기록 후 후속 세션/트리거 시 착수. ADR-156 §Residual 에 E18-class 로 등재(engine↔CSS 파리티 domain 귀속) |
...
| IllustratedMessage                                 | preview iframe 미렌더 (CSS 측정 불가)                                                                                                              | cutover marker props (data-element-id) 를 컴포넌트가 rest 미전개로 소실 — 시각 렌더는 정상, 측정/클릭 선택만 불가. Skeleton 동일 결함                                                                                                                                                                                                                                                                                                                                                                                                                                                | **해소 (2026-07-17)** — rest 전개 + 7종 marker contract lock                                                                                                                                                                                                                                                                                            |
| IllustratedMessage 시각 발산 (marker 해소 후 노출) | Skia 48 vs CSS 240 (md)                                                                                                                            | 3겹: layout 높이 분기 부재 (generic 0 + style padding 48 = 48) + escape top-left 고정 (padding/factory alignItems:flex-start 미소비) + DOM size prop 미소비 (propPassthrough 부재)                                                                                                                                                                                                                                                                                                                                                                                                   | **해소 (2026-07-17, 06f1319b3)** — resolveIllustratedMessageMetric 3경로 공유 (content-box 계약 — caller 가 style padding 가산, 포함 시 288 이중 계상). 라이브 md 350×240 / sm 350×197 양측 일치                                                                                                                                                        |
...
| preview prop 편집 stale (신규 관측 2026-07-17)     | 요소 prop 편집 (Inspector size/heading) 이 builder canonical·Skia 에는 즉시 반영되나 preview DOM 은 미반영 — reload (canonical 재송신) 후에만 반영 | **rAF hidden-탭 throttling** — 재송신 유일 경로(useIframeMessenger canonical effect)가 `requestAnimationFrame`(scheduleNextFrame) 예약인데 rAF 는 background(hidden) 탭에서 미발화. layouts 송신은 동기라 정상, canonical 만 정체 → reload(PREVIEW_READY 비-gated 초기 송신)로만 회복. 관측이 나온 Chrome MCP sweep 이 정확히 hidden 탭 컨텍스트 — focused 실사용자 무영향. (초기 가설 "updateElementProps 가 canonical 미갱신" 은 오진 — 라이브 실측상 doc 참조는 정상 변경됨)                                                                                                      | **해소 (2026-07-18)** — visible→rAF / hidden→setTimeout 하이브리드 스케줄러 `scheduleFrameOrTimeout`(scheduleTask.ts) 도입 + canonical effect 배선. hidden 탭 라이브 확증: prop 편집 → UPDATE_CANONICAL_DOCUMENT 송신 + preview DOM 반영 (reload 불필요). 회귀 가드: scheduleTask.test.ts 5 + canonical effect source-guard                             |
