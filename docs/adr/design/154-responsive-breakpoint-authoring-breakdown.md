...
>
> **잔여 (후속)**: ~~(a) render-visual props 대칭~~ → **해소 (2026-07-19 후속 fix)**: `fontSize`/`textAlign`(2/14) Skia glyph 를 `StoreRenderBridge.buildNodeForElement`(렌더 단일 choke point)에서 layout 과 동일 `resolveResponsiveLayoutNode` 적용 → glyph 가 activeBreakpoint override 값으로 렌더. layout props 12/14 + render-visual 2/14 = 14/14 Skia↔DOM 대칭. ~~(b) **breakpoint 배지 UI** + `ResponsiveVisibilityEditor` PropertySection 배선(항목 4 후반)~~ → **해소 (2026-07-19 후속 fix)**: 스타일 패널 헤더에 항상-표시 breakpoint 배지(`BreakpointHeaderBadge`) + `ResponsiveSection`(활성 breakpoint override chip 목록 + `ResponsiveVisibilityEditor` desktop-lock 배선). override 존재 판정은 raw `element.responsive` 를 읽는 `useResponsiveOverrides` 훅(병합 map 재판정 회피). visibility 는 `updateSelectedResponsiveVisibility(bp, visible)`(tablet/mobile 만, desktop=base lock) 신규 액션. live(Chrome MCP): Tablet↔Mobile 배지 전환 + Width chip 노출/✕clear + Mobile hidden(eye-off) 반영 + base(width 100%) 격리. ~~(c) **updateSelectedStylePreview** 는 비-desktop 에서 live preview skip~~ → **해소 (2026-07-19 후속 fix)**: 비-desktop preview 가 early-return 대신 `buildResponsiveStyleOverride`(commit 과 동일 helper)로 responsive override 를 elementsMap 에 반영(히스토리/DB 없음, base 무변경) → `resolveResponsiveLayoutNode` 가 merge → 드래그/타이핑 중 캔버스 즉시 반영(desktop preview 와 동일 구조·공통 tail). live(Chrome MCP): mobile mid-drag width preview → 카드 layout 390→50 즉시 + Skia 축소 + ResponsiveSection Width chip, commit 전 + base 격리. 참고: preview 의 canonical in-memory sync 는 documents subscriber autosave 로 intermediate 값이 persist 될 수 있으나 이는 desktop preview 와 동일 기존 구조(실사용은 release 시 commit 으로 최종값 확정).
>
...
>
> **잔여 (후속)**: ~~(a) render-visual props 대칭~~ → **해소 (2026-07-19 후속 fix)**: `StoreRenderBridge.buildNodeForElement` 에서 layout 과 동일 `resolveResponsiveLayoutNode` 적용 → Skia glyph 가 activeBreakpoint override 값 렌더(live window probe: fontSize desktop 14 ↔ mobile 40). DOM @media(Phase 3) 와 동일 resolve helper 라 mobile 에서 DOM computed == Skia glyph == 40 대칭. ~~(b) **apps/publish SSG(ElementRenderer)**~~ → **해소 (2026-07-19 후속 fix)**: apps/publish 리액트 SSG 도 `@media` emit. shared `collectResponsiveCssFromElements`(flat `Element[]` 대응 신규, `collectResponsiveCss` nested 판의 flat 형제) + `deriveProjectRenderModelFromDocument` 가 canonical `responsive` 를 runtime `Element` 로 전달 + shared `Element` 타입에 `responsive?` 추가. `PageRenderer` 가 `pageElements` 로 CSS 수집 후 `<style data-adr154-responsive>` 주입(`ElementRenderer` 는 이미 `data-element-id` 부여). live(Chrome MCP, publish dev 3001 + sessionStorage): tablet `display:none`/mobile `width:50px`+`display:none`(cascade) `@media` + base inline 100% 무변경. `generateStaticHtml` 과 동일 `buildResponsiveElementCss` SSOT → 3-publish-경로(generateStaticHtml/preview/SSG) 발산 0. (c) **BreakpointContext Provider(forcedBreakpoint 테스트용, 항목 3)** — 실제 iframe 리사이즈로 @media 가 동작하므로 강제 breakpoint provider 는 미착수(선택). (d) generateStaticHtml 의 base 숫자 스타일 inline 미적용은 pre-existing 한계(ADR-154 무관).
