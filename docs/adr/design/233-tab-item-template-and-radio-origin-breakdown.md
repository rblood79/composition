...
### Phase 3 — 두 leg · 성능 (G2 · G3, 2026-09-23)

- **G2 Skia live `adr233-tab-radio-live.mjs` 10/10** ([evidence](../evidence/233-g2-tab-radio-live.json)) — L1 seed · L2 기준 · L3 Tab/Default padding 24/10 → Tab 행 폭 +24 · 높이 40 · TabList 40 · 선택 TabPanel +11 (겹침 0) · L4 label fontWeight 700 · L5 Tab/Selected fills → 선택 Tab 만 픽셀 · L6 `component-radio` opacity → instance Radio 전부 픽셀 · L6b `component-radio` paddingLeft 8 → instance Radio rect +8 · L7 Radio/Selected → 선택 Radio 만 픽셀 · L8 reload 보존 · Components body 순서 Δ0 · page error 0. Compare Mode / Preview iframe 미개방 (사용자 지시) — Preview 는 unit (`tabsItemTemplate` · `standaloneRadio`) + 사용자 확인 대상.
- **live 가 잡은 결함 2 (unit 통과)**:
  1. **Tab 높이 비대칭** — 생성 CSS 가 Tab 에 `height: 29px` 를 고정해 Tag chip 선례의 `height: auto` 만으로는 Skia 만 40 으로 자랐다 (DOM 29). shared `resolveItemTemplateRowBoxStyle` (auto + rule 높이 하한) 를 두 leg 가 같이 싣는다.
  2. **TabList 고정 높이 + Tab 테두리 기본값** — TabList 가 탭 바 29 로 고정돼 행 40 이 위로 5.5 삐져나왔다 → projection 이 TabList scene props 에 `_tabTemplateStyle` 를 싣고 implicitStyles tabs/tablist 분기는 고정 대신 하한 (`minHeight`). 풀자 Tab size config 의 테두리 기본값 (rule 에 `borderWidth` 없음 → thin 1px) 이 높이 추정에 끼어 42 — DOM 생성 CSS 는 `border: none` 이라 0 으로 (`adr923IntrinsicMeasureBaseline` 의 tab 4 항목 갱신: 폭 61 → 59 = DOM).
- **관찰 (게이트 아님, 233 이전부터)**: Radio `fills` 는 Skia 에서 선택 점 색 (`skiaPrimitives` radio — 미선택 Radio 에 안 보임) · DOM 은 Radio 행 배경. plain Radio 와 ADR-230 Checkbox/Selected 도 같은 비대칭 — 후속 후보 (선택 컨트롤 fills 의 D3 뜻 정의). G2 Radio 전파는 두 leg 가 같은 뜻인 opacity · padding 으로 검증.
...
- 후속 후보 (범위 밖): Tab 추가 UI (items + TabPanel 쌍) · 선택 컨트롤 fills 의 D3 뜻 (Radio 는 round 3 에서 정함 — Checkbox 등 나머지) · 기존 문서 RadioGroup origin plain 자식 ref 이관 · Tab 항목 disabled/interaction 변형 · `calculateContentHeight` tabs 분기의 template 탭 바 높이 (engine 은 이미 행을 따라 잰다 — live Tabs 53 → 64).

### round 3 수리

2026-09-23 · 구현 검증 round 3 ([리뷰](../reviews/233.md)) 의 h1 · m2 · l3. 두 결함 모두 production 재현 (Codex 진단 2 RED) 이 있어 수리했다. 바뀐 쪽은 Preview (DOM) 만 — Skia 코드 변경 0.

- **h1 Radio 채움 영역**: 판정 — catalog 정본 (`Radio.variants.*.fill.default.selected` = 선택 표시 색) 과 Skia `radio` primitive (root 배경색 → 선택 점 · ring alpha 0 · 행 배경 없음) 가 이미 같은 뜻이고 DOM 만 행 배경에 칠했다. DOM 을 정본에 맞춘다: shared `INDICATOR_FILL_CSS_VAR` (`packages/shared/src/catalog/indicatorFill.ts`, `Radio → --radio-color`) 하나를 instance inline (`CanonicalNodeRenderer` catalog 경로 `routeIndicatorFillStyle`) 과 상태 변형 CSS (`collectStateVariantCss` — default 가 소유하지 않은 `backgroundColor` 선언) 가 같이 읽는다. default 가 소유한 키는 이미 `--co-*` 변수 경유라 inline 쪽이 `--radio-color: var(--co-…, baseline)` 로 옮긴다. Radio.css 의 `--radio-color` (layer components) 는 inline · 비-layer 상태 규칙이 이긴다. Codex 진단의 "Canvas root 도 빨강" 기대는 반대 방향 (Skia 가 행을 칠하게) 이라 채택하지 않았다 — 정본이 선택 표시를 이미 정의한다.
- **m2 Preview Tabs slot**: 종전 `App.tsx` 는 `component-tabs.slot` 하나로 문서 전역 template 1개를 만들어 모든 Tabs 에 줬다. template 해석을 `preview/utils/itemTemplates.ts` (`createTabTemplateResolver` · `indexTemplateOriginRecords` · `resolveTemplateOriginRootStyle`) 로 옮기고, `CanonicalNodeRenderer` 위임 경로가 Tabs 노드마다 `renderContext.resolveTabTemplate(node)` 로 `tabTemplate` 을 바꿔 넘긴다 — 규칙은 Canvas `resolveTabTemplateOriginIds` 와 같다 (ref instance = master `_resolvedFrom` 의 slot · 문서 Tabs = 자기 slot · 없으면 상수). template 은 origin 쌍마다 한 번 만든다.
- **l3 문서**: 본문 Hard constraints · Decision · G1 의 `placeable:false` 문구를 실행 판단 (reusable true · 같은 이름 primitive false · 노출 정본 `PALETTE_ORDER`) 에 맞췄다.
- unit `adr233Round3.fillAndTabSlot.test.tsx` 4: Radio/Selected fills (Skia 선택 점 1 · root 아님 · 상태 CSS `--radio-color` · `background` 0 · DOM inline background 0) · Radio 자기 fills (inline `--radio-color`) · 표 경계 (Checkbox · Button 그대로) · 한 문서 Tabs 3개 (사용자 slot 47 · slot 없음 20 · instance 20) Canvas scene ↔ production catalog DOM 일치. 원복 RED 3 (상태 CSS 경로 · inline 경로 · Tabs 마다 slot) 모두 반응 · 복원 GREEN.
- 회귀: builder 7,155 pass · 5 fail (모두 이전부터 — `adr113DescendantsGrepGate` · `factoryInlineDirtyBaseline` Dialog · `historyActions.static` · AI `componentCatalog` Modal · `propertyFieldIcons.static` `isOpen`, 이 diff 는 해당 파일 무변경) · shared 1,398 pass · 2 fail (Modal placeable · `generatedCssLoadInventory`, 이전부터) · type-check 0.
- live: 바뀐 쪽이 Preview 뿐이라 사용자 지시 (Compare Mode / Preview iframe 미개방) 에 따라 열지 않았다 — **사용자 확인 대상**. G2 L7 은 fills 대신 opacity 로 돌았다 (Codex 지적 맞음) — fills 계약은 위 unit 이 맡는다.
