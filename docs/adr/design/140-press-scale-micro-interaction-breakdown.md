...
| C1-b     | Calendar 셀          | 0.9              | 수동 `Calendar.css`                             |
...
범위 외: C3(Checkbox 체크마크 — 채택 기각), C4(Separator min-height), C5(Toast View Transitions). C3 dead CSS / Separator generated 상태 emit 정리는 composition 내부 작업으로 별도.
...
- 현 CSSGenerator snapshot 326 test PASS baseline 기록.
...
- `Calendar.css` `.react-aria-CalendarCell` — `&[data-pressed] { scale: 0.9 }` + base `transition`에 scale.
...
- CSSGenerator snapshot 갱신 — `@composition/specs` 326 test (의도된 diff: Button/ToggleButton/DisclosureHeader 3개 CSS의 `[data-pressed]` 변경분만).
...
| `packages/shared/src/components/styles/Calendar.css`      | CalendarCell `[data-pressed]` scale                         | 3     |
