...
| 핸들러 소재 (정의 경유)  | `bindHandlersToDefinitions` 호출 **7곳** — `hooks/useGlobalKeyboardShortcuts.ts:622-673` (44 id, `BuilderCore.tsx:377` 에서 마운트) · `panels/properties/CanvasSelectionShortcuts.tsx:269-300` (16 id, `BuilderCore.tsx:1155` host — ADR-155 Activity gating 중에도 등록 유지) · `workspace/canvas/BuilderCanvas.tsx:1032-1041` (zoomToSelection — frameAreas 컨텍스트 필요) · `panels/styles/StylesPanel.tsx:150-165` (toggleFocusMode·toggleSections — `collapsedSections` 로컬 state) · `panels/properties/PropertiesPanel.tsx:684` (copy/pasteProperties) · `components/overlay/CommandPalette.tsx:140-149` (commandPalette) · `main/BuilderHeader.tsx:101-108` (openProject) |
| 손수 선언 등록 (id 없음) | 4건 — `useViewportControl.ts:476-511` Space keydown/keyup (pan cursor) · `dashboard/index.tsx:431` ⌘K (다른 화면) · `CanvasSelectionShortcuts.tsx:255-268` ⌘C/⌘V `panel:properties` (canvas 쪽 `copy`/`paste` 와 동작이 달라 정의 공유 불가 — 2026-08-27 주석). 전부 **키보드 전용으로 남긴다**                                                                                                                                                                                                                                                                                                                                                                                   |
| 중복 id 등록             | `escape` 2곳 (global `handleEscape` / CanvasSelectionShortcuts `handleEscapeClearSelection`). 키보드는 리스너 2개가 각자 발화 (capture:document vs bubble:window). 나머지 70개는 1곳                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| scope 판정               | `hooks/useActiveScope.ts` `determineScope` — modal(`[role=dialog][aria-modal]`) → text-editing → 선언 scope(`data-shortcut-scope`) → canvas → 포커스 패널 → 활성 패널(우측 우선) → global. `focusin`/DOM 변화로 갱신. **팔레트가 열리면 scope 는 `modal`** 이라 열린 뒤에는 원래 컨텍스트를 알 수 없다                                                                                                                                                                                                                                                                                                                                                                            |
| 팔레트                   | `CommandPalette.tsx` — 목록 = 정의 71 전부 (`:107-114`). `executeCommand` switch **12 case** (`:168-216`): 패널 토글 11 + openProject. `default` 는 주석만. RAC `ModalOverlay` 가 닫힐 때 트리거로 포커스 복원                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| 액션 층 공유             | `workspace/canvas/actions/canvasActions.ts` — `alignSelection`/`distributeSelection`/`groupSelection`/`ungroupSelection` 을 컨텍스트 메뉴 provider (`canvasContextMenuProviders.ts:196-208`) 와 CanvasSelectionShortcuts (`:206-211`) 가 **같이 부른다**. 중복은 액션이 아니라 **바인딩**(컨텍스트 조립 + 로컬 state) 이다                                                                                                                                                                                                                                                                                                                                                        |
...
- 형태: Zustand vanilla `createStore` (기존 slice 패턴, `stores/*.ts`) 또는 module-level `Map` +
  `useSyncExternalStore`. **Zustand vanilla 로 간다** — 팔레트가 구독해야 하고(등록/해제 시 목록 재계산),
  `useStore` 와 섞이지 않는 독립 store 가 `canvasSettings.ts` 류와 같은 관례다.
...
- `scopeAtOpen`: `handleOpenChange(true)` 에서 `activeScope` 현재값을 ref 에 저장. ⌘/ 핸들러가 도는 시점에는
  ...
  갱신). 헤더 메뉴 "Shortcuts" 경유(`open-command-palette` 이벤트) 도 같은 시점 — 메뉴 popover 는
  `aria-modal` 이 아니라 `modal` 로 잡히지 않는다 (Phase 0 에서 실측 확인 항목).
...
| `treeNav*` 6 · `treeSelect`·`treeSelectSpace` | `panel:nodes` 트리의 **포커스된 행**에 작용 — 팔레트가 닫히며 복원되는 포커스가 그 행이라는 보장이 없다 |
...
- 조항 4 "panels 정의 → 팔레트 case" 는 switch 소멸로 **대상이 사라진다**. 대체 조항: **정의 71 개 각각이
  `bindHandlersToDefinitions` 배열 리터럴 어딘가에 등장한다** (`palette:false` 여부와 무관 — 키보드도
  등록이 있어야 한다). 2026-08-27 Step C 로 미연결 11 → 0 이 된 상태를 잠근다. 소스 grep 기반 (기존 조항과
...
- [ ] Phase 0: 재grep 5종 결과가 §2 와 일치 (불일치 시 표 갱신 commit — fork 사유 아님)
- [ ] Phase 0: 헤더 메뉴 popover 열린 상태에서 `useActiveScope` 가 `modal` 이 아님을 실측 (아니면 `scopeAtOpen` 을 메뉴 열기 전에 잡는 경로 추가)
...
- [ ] Phase 2: 정적 게이트 민감도 — `bindHandlersToDefinitions` 배열에서 id 하나 제거 → RED
