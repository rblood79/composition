...
**first-nail (전제 반증 1 케이스)**: §2 의 첫 행이 뜻하는 바 — "definite 높이를 가진 flex item 은 collection 이 아니어도 floor 가 0 인가" 를 Chrome 차등 1 케이스로 판정한다.

- 케이스: `flex column height:80` > `div { height:164px; overflow:visible }` (자식 없음, 콘텐츠 없음) 과 같은 형태를 Chrome 과 production 양쪽에서 측정.
  - Chrome 이 164 (= min(specified 164, content 0) 이 아니라 specified 절 적용) 면 → **엔진의 일반 gap** 이 확인되고 Phase 1 에 §4.5 specified size suggestion 절이 포함된다.
  - Chrome 이 80 이면 → definite-size 항목의 floor 0 은 정합이고, collection 격차의 단독 원인은 **content 공급 0** 이다. Phase 1 은 세로축 스칼라만.
- 이 판정 전에는 Phase 1 의 형태를 확정하지 않는다. ADR-923 evidence 의 서술 ("automatic minimum = min(specified 164, content 0) = 0") 은 스펙 수준 재진술이고 `flex.rs:332` 의 코드 형태와 일치하지 않는다 — 어느 쪽이 실제 동작인지가 이 케이스의 답이다.
...
3. BC 수식화 — 로컬 검증 프로젝트와 fixture 전수에서 "collection + overflow visible|clip + 제약 flex item" 형태의 실제 출현 수를 센다. 재직렬화는 0 (스칼라는 read-time 파생, canonical 미변경) 임을 같이 기록.

## 4. Phase 1 — 엔진 (Rust)

- 세로축 content-min 필드 추가 (`content_min_height`) — Rust `NodeStyle` + TS `layoutTypes.ts` (활성 경로는 JSON). 커널 쪽은 슬롯 19 가 row 축에서만 실리던 것을 `is_row` 에 따라 둘 중 하나가 실리도록 확장 — **`FLEX_FIELD_COUNT` 는 늘리지 않는다** (같은 슬롯의 축별 의미 분기, ADR 본문 G3 실패 시 축소안과 같은 형태를 처음부터 택한다).
- `binaryProtocol.ts` 의 비등재 주석에 세로축 스칼라를 같이 적는다 — binary protocol 실구현 시 누락되면 silent drop 이고 그 주석이 유일한 경고다 (R3).
- (Phase 0 결과가 "Chrome 164" 일 때만) `auto_min_main_from_parts` 의 `main_size == AUTO` 가드를 §4.5 specified size suggestion 절로 대체 — definite 이면 `min(specified, content)`. 조건 정의는 계속 이 함수 하나가 소유한다 (ADR-183 R2 — 조건을 호출부에 복제하면 explain 이 옛 조건을 보고한다).
...
| Gate | 내용                                                                                                                              |
| ---- | --------------------------------------------------------------------------------------------------------------------------------- |
| G0   | Phase 0 first-nail 1 케이스 (Chrome vs production) — 판정 기록 후 Phase 1 형태 확정                                               |
| G1   | 도달 매트릭스 Chrome 차등 — 격차 행이 전부 ≤1px, scrollable 행은 종전 값 불변 (회귀 0)                                            |
| G2   | 원복 RED — (a) 세로축 스칼라 공급 제거 → 격차 행 RED, (b) (해당 시) specified 절 원복 → 일반 케이스 RED                           |
...
- HIGH 위험 (R3) 의 코드 경로 3곳 이상 인용: `layoutTypes.ts:181` · `tree.rs:260` · `binaryProtocol.ts:81-85` (+ 커널 `flex.rs:332`).
- Generator 확장 아님을 ADR 본문 Context 에 선언 (D3 시각 토큰·생성 CSS 불변).
- BC 수식화: 재직렬화 0 파일 · 팔레트 기본 상태 영향 0% · 실제 출현 수는 Phase 0 §3 인벤토리 3 에서 계수.
- HIGH 잔존 시 별도 ADR 분리 가능한가: R3 는 본 ADR 구현 내부의 배선 위험이라 분리 대상이 아니다. 분리 판단이 필요한 유일한 지점은 Phase 0 G0 가 대안 C 를 부르는 경우이고, 그때도 **본 ADR 안 흡수가 기본** — 확장분이 본 ADR 규모를 넘을 때만 결정 지점 ④ 로 질문한다.
