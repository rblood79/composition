...
| 분기 | 옵션                                                                                                                               | 채택 후보                                                                                     |
| ---- | ---------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| D1   | (a) `documents` store row 의 column 확장 (`events: SerializedEvent[]`) / (b) 별도 store 분리 (`events` / `data` / `actions` store) | (a) 우선 — round-trip 단순, ADR-116 cutover 패턴 정합. (b) 는 데이터 양 폭증 시 별도 ADR 분리 |
