...
| C1 단일 일반 요소                 | 1개, body·frame·인스턴스 아님                                | T1-3 복제 · T1-11 컴포넌트 만들기↔해제 · ⋯                                                                       | T1-8 그룹은 `groupSelection` 이 2+ 전용(`canvasActions.ts:259`)이라 단일에서 제외 (Phase 2 live 실측). E1 텍스트 편집은 Phase 0 ② 로 v1 제외 (§6) |
...
Phase 1 결과 (2026-08-27): `actionBarPolicy` + `buildActionBarItems` — vitest 18 (신규) + 182 회귀 31 PASS. C0 규칙 보강: `group` 항목이 없으면 body 만 선택 → null.

Phase 2 결과 (2026-08-27, live Chrome MCP `ZZZF` 프로젝트):

| 검증 항목                              | 결과                                                                                                                                                                                                                                                 |
| -------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| C0 body / 빈 선택 / projection id 선택 | 바 미마운트 ✅ (`selectionResolved` 게이트 — `750aabf3…/Preview` 같은 projection id 도 미마운트)                                                                                                                                                     |
| C1 ButtonGroup 단일                    | 복제 · 컴포넌트 만들기 · ⋯ ✅                                                                                                                                                                                                                        |
| C2 frame (그룹 만들기 결과)            | 그룹 해제 · 복제 · 컴포넌트 토글 · ⋯ ✅                                                                                                                                                                                                              |
| C3 Card 인스턴스                       | 원본으로 이동 · 복제 · ⋯ ✅ (인스턴스 분리는 182 조건 미충족으로 숨김)                                                                                                                                                                               |
| C4 다중 (ButtonGroup + Card)           | 정렬▾ · 그룹 만들기 · 복제 · ⋯ ✅ — 정렬 popover 4×2 ✅                                                                                                                                                                                              |
| 버튼 실행                              | 컴포넌트 토글 (라벨 즉시 반전) ✅ · 다중 그룹 만들기 (새 frame 선택 + C2 전환) ✅ · ⋯ → 182 메뉴가 버튼 위치에 열림 ✅                                                                                                                               |
| HC3 포커스                             | 바 클릭 직후 `document.activeElement` = `.canvas-container` 유지 ✅ (`preventFocusOnPress` 가 RAC Button 에서 동작 — type-check 통과로 prop 존재 확인). `⌫` 삭제 → 바 자동 숨김 → `⌘Z` 복귀 ✅. `⌘D` 는 Chrome 북마크 단축키에 잡혀 MCP 로 판정 불가 |
| 라벨 신선도                            | render 단계 산출 시 한 클릭 늦게 바뀌는 결함 실측 → `useEffect` 산출로 정정 (182 provider 가 읽는 interactive map 이 `BuilderCanvas.tsx:756` effect 에서 갱신되기 때문)                                                                              |
| 시각                                   | 공유 Button 에 ghost variant 부재 → `--button-color` 중화 (HistoryPanel.css 선례). 하단 중앙 16px, 스크롤바와 비겹침 ✅                                                                                                                              |
| M1 텍스트 편집 중 미마운트             | **live 미검증** — MCP 더블클릭이 Skia 텍스트 편집 진입을 열지 못함 (Button 은 `TEXT_EDITABLE_TAGS` 밖, Card 제목은 projection). 배선은 `useTextEdit` → `useCanvasStore.setEditing` 1줄 + 바 selector. Phase 3 live 에서 실제 마우스로 재확인         |

**Phase 2 에서 드러난 182/canvasActions 관찰 (본 ADR 범위 밖 — 사용자 판정 필요)**:
