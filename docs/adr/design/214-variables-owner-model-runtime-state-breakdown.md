...
>
> **G1 live 완료 2026-09-13** — `adr214-p1-live.mjs` 10/10 (②③④). 잡은 결함 1: History 재적용 (undo/redo) 이 projectId 를 안 넘겨 **변수 0개 프로젝트의 첫 생성 redo 가 실패** (reducer 가 projectId 를 store 첫 변수에서 추론) → `applyDataChange` 가 `currentProjectId` 를 기본값으로 (RED 회귀 `dataChange.variables.test.ts`). 하니스 관찰 지점: dev 전용 `window.__composition_SCENE_DEBUG__` (sceneVersion · 노드 props/stateDeps). ⌘D 는 `canvas-focused` scope 라 캔버스 컨테이너 포커스 필요.
>
