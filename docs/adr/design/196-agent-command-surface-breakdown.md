...
> ADR 본문: [196-agent-command-surface.md](../completed/196-agent-command-surface.md). 구현 상세는 이 파일에만 둔다.
...
- **4 질문** (adr-writing.md §Fork 게이트 형식을 신규 주제에도 적용):
  ...
  3. 선행 전제 reverse — 195 HC1 (키보드 경로 무변경) 은 consumer 가 하나 늘어도 유지된다 (adapter 는 handler 를 부르지 않는다, §3-2). 134 Phase 0~2 (provider 추상화·벤더 SDK 제거) 는 본 ADR 의 선행이 아니다 — 본 ADR 은 호스트 불문 (AI 패널 tool · Chrome MCP 로 조작하는 Claude/Codex · 향후 MCP/ACP).
...
| 현재 agent 표면       | `services/ai/tools/` 7종 — `create_element · update_element · delete_element · batch_design · get_editor_state · get_selection · search_elements` (Tool Calling). **승인 게이트 0** (`deleteElement.ts` 는 body 보호만), **실행 기록 0** (AIPanel 대화만), undo 는 개별 store 호출이 남기는 history 에 의존. 정렬/분배/그룹/복제/z-order/undo/줌/패널은 **도달 불가** — agent 가 정렬을 하려면 geometry 를 직접 계산해 `update_element` 를 N 회                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| 외부 agent 표면       | Chrome MCP (Claude/Codex 가 빌더를 실제 조작 — CLAUDE.md §완료 기준 live exercise 경로). 지금은 키보드/팔레트/클릭을 흉내낼 뿐 명령을 이름으로 부를 수 없다. ADR-134 D11 (MCP 호환 도구 표면) 은 Proposed — 134 착수 전까지 본 ADR 의 노출 경로는 AI tool 1개 (`run_command`) 와 window 진입점 (Phase 3 live 용)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
SHORTCUT_DEFINITIONS (정의 — 무엇)            195: 표기 formatShortcut / 실행 commandRegistry (UI adapter = 등록 hook 의 handler)
        ...
        └─ AGENT_COMMANDS[id] (신규 — agent adapter: store-level action. handler 를 부르지 않는다)
            ...
            executeAgentCommand(id, args, ctx)  →  allowlist → precondition → confirm 게이트 → 실행 → history 1 entry → 기록 1건
                  │
        consumer: AI 패널 tool `run_command` (클라우드 LLM, 134 이전 최소) · window.__compositionAgent (Phase 3 live) · 134 D11 MCP 도구 (후속)
```
...
- **AI tool `run_command`** (`services/ai/tools/runCommand.ts`) — 파라미터 `{ id: enum(allowlist), args? }`. 도구 정의는 `COMMAND_META` 에서 생성 (allowlist 가 enum). 134 가 클라우드 LLM 를 제거하면 같은 descriptor 를 MCP tool 로 옮긴다 (D11).
...
| 2     | **Implemented 2026-08-28 (G2 통과)** — executor + confirm 게이트 + `agentCommandLog` 슬라이스 + jsdom (denied / precondition / declined / ok / 배치 confirm 원소별)                                                                                                                                              | G2   | 3 파일 신규 + store 1 |
| 3     | **Implemented 2026-08-28 (G3 통과)** — consumer — `run_command` AI tool + `window.__compositionAgent` (DEV) + AIPanel 로그 표시 → **live** (Chrome MCP): agent 호출 ≥ 15 · confirm 실측 (delete) · undo 1회 복원 · 195 oracle 재실행 · 번들 Δ                                                                  | G3   | 2 파일 + UI 1         |
| 4     | **Implemented 2026-08-28 (종결)** — CHANGELOG · README (카운트·착수 순서) · `### Live Exercise` · 134 D11 에 "descriptor 소비" 1줄 정합 메모 · 본문 `completed/` 이동                                                                                                                                            | —    | 문서                  |
...
| `apps/builder/src/services/agent/agentCommands.history.test.ts`                                            | 2     | 신규 — Phase 0 실측 harness 상시화: canonical 경로 재현, executor 경유 entry 수 = 1 계약                                          |
| `apps/builder/src/builder/stores/agentCommandLog.ts`                                                       | 2     | 신규 — 세션 기록 **독립 store** `useAgentCommandLogStore` (Phase 2 판정: root 편입 안 함 — history/persist 경로와 분리, 상한 500) |
| `apps/builder/src/services/ai/tools/runCommand.ts` + `definitions.ts` + `index.ts` + `AgentService.ts` | 3     | tool 1개 추가 — descriptor 는 allowlist 생성, 레지스트리 등록은 **지연 로딩** (초기 번들 분리)                                    |
| `apps/builder/src/services/agent/devAgentEntry.ts`                                                         | 3     | 신규 — `window.__compositionAgent` (DEV, 동적 import)                                                                             |
| `apps/builder/src/services/agent/agentCommandConfirmation.ts`                                              | 3     | 신규 — 승인 채널 (host 없으면 **거부**, native confirm 금지)                                                                      |
| `apps/builder/src/builder/components/overlay/AgentCommandConfirmDialog.tsx` + `.css`                       | 3     | 신규 — 승인 다이얼로그 host (BuilderCore 마운트)                                                                                  |
| `apps/builder/src/builder/main/BuilderCore.tsx`                                                            | 3     | 승인 host 마운트 + DEV 진입점 설치                                                                                                |
| `apps/builder/src/builder/panels/ai/components/AgentCommandLogList.tsx` + `AIPanel.tsx`/`.css`             | 3     | 실행 기록 목록 (최근 8건, 5 status)                                                                                               |
...
- [x] Phase 2 (2026-08-28): executor 분기 jsdom 14 (denied unknown/external/not-callable · precondition-failed · declined → adapter 0 · ok · error · 배치 원소별 승인 + 첫 non-ok 중단) · 기록 1:1 (5 status, seq 단조) · `agentCommands.history.test.ts` 12 — `undo: history` 명령 executor 경유 entry 수 = 1 (align/distribute/duplicate/delete/paste batch/cut/group/ungroup/z-order 4/toggleOrigin), undo/redo 0 + 문서 복원, z-order 경계 0 · type-check 0. `agentCommandLog` 는 root store 편입 대신 독립 store (`useAgentCommandLogStore`, 상한 500) — 문서 상태와 무관한 관측 데이터
- [x] Phase 3 (2026-08-28): live 45 호출 · 키보드 대조 12쌍 전부 일치 · delete 승인/거부 실측 · undo 복원 (agent+키보드) · 팔레트 62 항목 + 팔레트 실행 정상 · AI 패널 기록 목록 표시 · `__compositionAgent` 프로덕션 0건. **번들 Δ: 초기 +1.23KB / 합계 +4.15KB — HC6 기준을 초기 번들로 확정 (2026-08-28 사용자 판정) → 충족**
- [x] Phase 4 (2026-08-28): CHANGELOG (신규 public 표면 R7) · README 완료 카운트 + 착수 순서 행 · `### Live Exercise` (G3 12쌍 + G4 종결 근거) · 134 D11 정합 1줄 (R6) · 본문 `docs/adr/completed/` 이동. **G4 는 실패 시 대안으로 종결** — 클라우드 LLM 모델 만료 + 사용자 결정 "해당 벤더 미사용" 으로 패널 end-to-end 는 134 로 이관, descriptor 실측(유효 모델에서 tool call 수신)+테스트로 대체
