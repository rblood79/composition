...
## 2. Current Baseline (2026-08-28 실측 — HEAD `37a035db2`)
...
| handler 의 정체       | 등록 hook `hooks/useKeyboardShortcutsRegistry.ts:293-352` 의 effect 클로저. 7 등록 지점 (195 breakdown §2): global 42 · CanvasSelectionShortcuts 16 · BuilderCanvas 1 (`frameAreas`) · StylesPanel 2 (`collapsedSections` 로컬 state) · PropertiesPanel 2 (속성 클립보드 로컬) · CommandPalette 1 · BuilderHeader 1. **언마운트 = 미등록** (StylesPanel 은 선택 없으면 `EmptyState`). `escape` 류는 DOM 포커스를 읽는다. → headless 호출 부적합 (Codex P4 판정과 일치)                                                        |
...
| 액션 층 (store-level) | `workspace/canvas/actions/canvasActions.ts` — `copySelection · cutSelection · paste · duplicateSelection · deleteSelection · groupSelection · ungroupSelection · alignSelection · distributeSelection` (+ `buildCanvasActionElementsMap · selectableWithoutBody`). 컨텍스트 메뉴(182)·단축키·액션 바(192) 가 공유 → **agent adapter 1순위 후보 (의미 drift 없음)**. undo/redo = `stores/history.ts` `historyManager`. 패널 토글 = root store `panelWorkspaceLayout` (`hooks/usePanelLayout.ts:22`). 줌 = `setZoom` (viewport) |
...
| 승인 다이얼로그 선례  | `requestEditingSemanticsDetachConfirmation` (detachInstance — global `:238-263` / csel `:126-142`). 사용자 확인 흐름이 이미 있다 — agent 경로는 **같은 흐름을 부르되 agent 가 결과를 기다릴 수 있어야** 한다 (Promise)                                                                                                                                                                                                                                                                                                        |
| history 단위          | ADR-180 history snapshots Implemented (08-13) — entry 단위 undo. agent 배치를 1 entry 로 묶는 API 존재 여부는 Phase 0 실측 (`historyManager` 의 group/batch 표면)                                                                                                                                                                                                                                                                                                                                                             |
...
grep -cE '^  [a-zA-Z]+: \{' src/builder/config/keyboardShortcuts.ts        # 71
...
| z-order       | `bringToFront · bringForward · sendBackward · sendToBack`                                                                                                                                                         | store z-order action (Phase 0 에서 함수명 확정)             | document                | history      | false     |
| 클립보드      | `copy · paste · cut`                                                                                                                                                                                              | `canvasActions.copySelection/paste/cutSelection`            | document (copy 는 none) | history      | cut: true |
| 삭제          | `delete · deleteAlt`                                                                                                                                                                                              | `canvasActions.deleteSelection`                             | document                | history      | **true**  |
| 인스턴스      | `detachInstance`                                                                                                                                                                                                  | 기존 `requestEditingSemanticsDetachConfirmation` Promise 화 | document                | irreversible | **true**  |
| 선택          | `selectAll`                                                                                                                                                                                                       | store selection action                                      | selection               | none         | false     |
| history       | `undo · redo`                                                                                                                                                                                                     | `historyManager`                                            | document                | history      | false     |
| 뷰            | `zoomIn/Out/ToFit/ToSelection` · `toggleRulers` · `toggleFocusMode`                                                                                                                                               | viewport `setZoom` / root store                             | view                    | none         | false     |
| 패널          | `toggleNodes … toggleAI` 11 · `openSettings`                                                                                                                                                                      | root store `panelWorkspaceLayout`                           | view                    | none         | false     |
| **노출 금지** | `openProject` (navigation) · `commandPalette` · `escape` · 방향키 8 · tree 8 · `nextElement/prevElement` · `copyProperties/pasteProperties · copyStyles/pasteStyles · toggleSections` (패널 로컬 state — UI 전용) | —                                                           | —                       | —            | —         |

예상 allowlist ≈ **35 / 71** (Phase 0 G0 이 확정 — 상한 40). `zoomToSelection` 은 BuilderCanvas 의 `frameAreas` 컨텍스트가 필요해 adapter 가 store 만으로 재현 못 하면 노출 금지로 내린다 (Phase 0 판정).
...
executeAgentCommand(id, args, ctx: { host: "ai-panel" | "chrome-mcp" | "mcp", requestConfirm })
  ...
  4. history.beginGroup(label) → adapter(args) → history.endGroup()   (group 표면 없으면 Phase 0 에서 결정: 1 entry 보장 불가 명령은 undo: "irreversible" 로 표기 + confirm)
...
| Phase | 내용                                                                                                                                                                                                                                         | Gate | 규모                  |
| ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---- | --------------------- |
| 0     | inventory freeze — §2 재grep · 71 id 분류 (§3-3 표 확정, allowlist 상한 40) · z-order/selection/zoom 의 store 함수명 실측 · `historyManager` group 표면 유무 · `detachInstance` 확인 흐름 Promise 화 가능성 · AI 도구 7종 승인/기록 0 재확인 | G0   | 문서만                |
| 1     | `COMMAND_META` 71 + 정적 게이트 5조항 + `AGENT_COMMANDS` adapter (allowlist) + jsdom parity (allowlist 각 id: adapter 실행 후 store 상태 = 같은 canvasActions 를 직접 부른 결과) — **키보드·팔레트 무변경**                                  | G1   | 3 파일 신규           |
...
- [ ] Phase 0: §2 재grep 일치 · §3-3 표 확정 (allowlist ≤ 40) · history group 표면 판정 · detachInstance Promise 화 판정
- [ ] Phase 1: 정적 게이트 5조항 PASS + 민감도 (allowlist id 의 adapter 제거 → RED · external id 를 agentCallable true → RED) · parity 전부 PASS · 195 oracle 26/26 · type-check
- [ ] Phase 2: denied/precondition/declined/ok/배치 jsdom PASS · 기록 1:1 · history 1 entry
- [ ] Phase 3: live ≥ 15 · confirm 실측 · undo 복원 · 195 oracle 재실행 · 팔레트 G3 23건 재실행 동일 · 번들 Δ ≤ +3KB gz · `pnpm agent:work -- verify` 통과 (live-exercise 기록 포함)
