...
| 뷰            | `zoomIn/Out/ToFit/ToSelection` · `toggleRulers` · `toggleFocusMode`                                                                                                                                               | `zoomViewportAtContainerCenter` (viewportSync) / root store — `canvasStore.setZoom` 금지             | view                    | none         | false     |
| 패널          | `toggleNodes … toggleAI` 11 · `openSettings`                                                                                                                                                                      | `activatePanelWorkspacePanelV3` + `setPanelWorkspaceLayout`                                          | view                    | none         | false     |
...
| Phase | 내용                                                                                                                                                                                                                                                                      | Gate | 규모                  |
| ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---- | --------------------- |
| 0     | inventory freeze — §2 재grep · 71 id 분류 (§3-3 표 확정, allowlist 상한 40) · allowlist 각 id "handler → 호출 심볼 + 부가 동작" 표 · **액션별 history entry 수 실측** (`getCurrentPageEntries` 전후 diff, async 액션은 transaction 불가) · AI 도구 7종 승인/기록 0 재확인 | G0   | 문서만                |
...
- [ ] Phase 0: §2 재grep 일치 · §3-3 표 확정 (allowlist ≤ 40) · handler→심볼 표 · 액션별 entry 수 실측
