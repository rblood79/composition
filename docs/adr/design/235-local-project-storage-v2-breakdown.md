...
| 1     | 자산 저장소 + reader — IndexedDB `assets` store (DB_VERSION 23) · `crypto.subtle` SHA-256 · 해석 함수 (`asset:` → `blob:` URL, 참조 카운트 기반 revoke) · **consumer 전부 dual-read 전환** (Canvas `imageCache` · Preview `fillAdapter` · 정적 HTML · publish 로더 · 폰트 로더 — `asset:` 과 dataURL/http 모두) · **자립 v1 내보내기** (§2) · 업로드 경로는 여전히 dataURL (writer 꺼짐)                                                                                                                                                                                                                                                                                                                                                                                                                       | G1      |
...
| 3     | GC — Phase 0 (b) root 전부에서 참조 수집 (mark) · 미참조 자산에 후보 시각 기록 · 다음 GC 에서도 미참조이고 유예 기간이 지난 후보만 삭제 (sweep). history entry 는 `cleanupOldEntries` (`historyIndexedDB.ts:508`) · `clearPageHistory` (`:360`) 로 정리될 때까지 root 로 남는다. mark 시작 후 저장된 자산은 이번 sweep 대상에서 제외                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | G3      |
...
- 원복 RED: G1 (writer 꺼짐 · 해석 누락 네트워크 0 · 자립 내보내기) · G2 (a)(b)(c) · G3 반증 6건 (백업 · 스냅샷 · 다른 프로젝트 · history entry → undo/redo · 메모리 history · mark 이후 참조) · G6 중단 주입 5지점 + manifest 손상 복구.
