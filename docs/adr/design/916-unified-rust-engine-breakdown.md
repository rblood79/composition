...
- **seam 배선 (C-1) 미포함(다음 단계 = 선결 과제)**: (1) flex.rs §9.7 main available 음수(height:auto column intrinsic 도출 — 현재 -1 sentinel 을 shrink 로 오처리) / (2) grid.rs intrinsic track(min/max-content 측정 — 현재 셀 높이를 available 로 채움). 둘 다 Phase 1(flex.rs) / Phase 1-B(grid.rs) 후속. 완료 후 (C-1) 진단 diff 0 전환 확인 → (C-2) flag 전환(자체 pkg 런타임 로드 배선 + `createLayoutEngine` flag true) 재평가. (C-2)는 여전히 live 엔진 교체라 HIGH — 별도 승인 + 넓은 실전 fixture dual-run diff 0 재확인 필수.
