...
- **seam 배선 (B2) 미포함(다음 단계 = (C))**: (C) `createLayoutEngine` flag 전환 — self 엔진을 live builder 레이아웃 엔진으로 교체. **live 엔진 교체**라 HIGH. 전제: (B2) self-diff 가 flex row fixed 에서 통과했으나, 자체 엔진 미해결 영역(flex column height:auto -1 sentinel / grid intrinsic track / 증분 available 캐시)이 실제 catalog 프로젝트 배치에서 diff 를 낼 수 있음 → (C) 착수 시 **넓은 실전 fixture(실제 catalog 컨테이너 트리)로 dual-run 확장** 후 diff 0 확인이 flag 전환 전 선결. diff 발생 시 그 영역이 flag 전환 전 flex.rs/grid.rs 후속 구현 대상으로 확정.
