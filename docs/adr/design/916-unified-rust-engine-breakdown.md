...
| **단위 3-b** | **grid dispatch** — `grid_layout`. grid 는 계약이 근본적으로 다름(자식 flat 없음, template/placement 문자열). NodeStyle `grid_template_columns: Vec<String>` → space-join, 자식 gridArea/gridColumn/gridRow → `parse_placements` 파이프 형식 직렬화 어댑터                  | 다음               |
