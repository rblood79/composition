...
- **flag 전환 선결 #1 미포함(다음 = 선결 #2)**: grid.rs intrinsic track(min/max-content 측정) — 현재 셀 높이를 available 로 채움 → C-1 grid height:auto diff 잔존(셀 h +50). Phase 1-B grid.rs 후속. 완료 후 C-1 grid diff 0 전환 → (C-2) flag 전환(자체 pkg 런타임 로드 배선 + `createLayoutEngine` flag true) 재평가. (C-2)는 여전히 live 엔진 교체라 HIGH — 별도 승인 + 넓은 실전 fixture dual-run diff 0 재확인 필수. block 은 이미 정합.
