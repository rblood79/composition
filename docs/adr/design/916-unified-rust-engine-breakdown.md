...
- **seam 배선 (B1) 미포함(다음 단계)**: (B2) JS↔WASM 통합 테스트 인프라 — vitest/node 에서 자체 pkg + 실제 Taffy(`wasm-bindings/pkg`) 동시 로드하는 경로 신규 구축(현재 dualRunHarness.test.ts 는 mock 만) + 실제 batch 로 self-diff 측정. taffy 는 `--target bundler`(Vite 전제)라 vitest 직접 로드 경로 미확립 → (B2) 가 이 인프라를 확립. self-diff 결과(flex column height:auto 등 미해결 영역이 실제 diff 를 낼 수 있음)가 (C) flag 전환 가능 여부 결정.
