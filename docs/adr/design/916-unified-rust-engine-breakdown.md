...
- **seam 배선 (A) 미포함(다음 단계)**: (B) wasm-pack 산출물 생성(`pkg/` .wasm + 바인딩 .js/.d.ts) → `dualRunHarness.runDualLayout`(candidate=본 wrapper vs reference=Taffy) self-diff 측정. 자체 엔진의 flex column height:auto 등 미해결 영역이 실제 프로젝트 배치에서 diff 를 낼 수 있어 (B) 결과가 (C) flag 전환 가능 여부를 결정. (C) seam flag 전환은 (B) self-diff 0 통과 후에만(no-dormant-foundation-ahead-of-flip).
