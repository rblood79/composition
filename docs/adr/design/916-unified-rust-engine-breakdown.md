...
- **남은 2-A 단위 (다음)**: `resolveStyle` 본체(cascade 진입점 조립) + `resolveFontStretchWidth`(spec SSOT 참조 계약 확정 후) → `taffyDisplayAdapter.ts`(display 매핑) → `implicitStyles.ts`(tag→style 데이터 주도 매핑 테이블, 2,440줄 최대 단위). 각 단위 착수 시 동일하게 최소 검증 단위 확인.
