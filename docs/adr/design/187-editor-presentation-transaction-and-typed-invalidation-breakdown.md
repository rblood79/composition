...
**진행 상태: slices 진행 — 2026-08-24 (single-fill gradient stop + fill opacity + border color + box-shadow paint + continuous shadow editor + Text/Button color slices).**
...
[Button text-bearing color live parity evidence](187-phase-5-text-bearing-color-live-parity.md)에 실제 Builder
...
Text/Button root color gate는 종결됐지만 multi-child inherited/component color, 기타
component root, 다른 opacity/paint slider, layout allowlist 확대, text metrics/resource와
...
3. opacity/paint 계열 Property slider (Text/Button root color 완료; inherited/기타 대상 잔여)
