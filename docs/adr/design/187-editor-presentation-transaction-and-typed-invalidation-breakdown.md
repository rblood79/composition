...
구현 범위와 focused gate, 실제 Builder 상단 Compare Mode split 결과를 기록했다.
첫 slice는 하나의 enabled
...
`drop-shadow` effect slot의 shadow paint fields를 `EditorPresentationTransactionRuntime`으로
...
이어 Button root도 명시적 text-bearing capability로 승격했다. Button 자체 Skia
node의 text slot만 갱신하며, multi-child inherited subtree와 기타 component root는
descendant projection이 없으면 fail-closed한다. 실제 Builder에서 Button rect 불변,
action/control RAF `0/0`, legacy write `0`, console error/warning `0/0`과
Preview/Skia 색상 수렴을 확인했다.

`Modified` 필터의 색상 editor도 같은 owner 계약으로 수렴했다. `borderColor`는
border owner, `color`는 검증된 Text/Button root owner를 사용하고, owner를 획득하지
못한 background/다중 child/미검증 root는 기존 fallback을 유지한다. 실제 Builder에서
Modify 필터의 Button color drag가 canonical write 1회, legacy write 0, action/control
RAF `0/0`, terminal event 1로 동작하고 geometry와 Preview 색상이 수렴했다.

`Modified` 필터의 명시적 opacity 입력도 같은 owner 계약으로 수렴했다. 기존 Skia
opacity effect가 있는 `0..1` unitless 값만 materialization target으로 열고, drag 중
canonical 값은 유지한 채 effect value와 Preview semantic opacity를 갱신한다. 실제
Builder에서 `0.5 → 0` 입력의 rect 불변, action/control RAF `0/0`, legacy write `0`,
console error/warning `0/0`과 Preview/canonical opacity 수렴을 확인했다.
...
재현 가능한 parity harness를 통과했고, 이번에 populated Builder generic non-grid
multi-sibling live gate까지 종결했다. `width`, `height`, `padding`, `gap` 각각에서
Skia draw/hitBounds 원자성, Preview sibling geometry, 비영향 identity, Canvas pixel
변경·복귀와 canonical terminal handoff가 모두 `true`였다. fixed Text
`fontSize`/`fontWeight`도 standalone Text paragraph slot과 targeted invalidation,
Preview CSS↔Skia metric parity, bounds/hitBounds 불변, paragraph key 변경·복귀,
Canvas pixel 변경·복귀 및 canonical terminal handoff의 populated Builder live gate를
통과했다. 상세 JSON은 [non-grid flow live parity harness](187-phase-5-layout-flow-live-parity.md)와
[text metrics/resource/structure residual slice](187-phase-5-text-metrics-resource-structure-fail-closed.md)에
기록했다.
...
추가 게이트로 남아 있다. spacing shorthand/longhand는 runtime·Preview·Skia·commit
경계에서 네 개의 padding longhand와 row/column gap으로 정규화해 React 경고의 근본
원인을 제거했고, 실행 중인 Builder의 `Badge` spot-check에서 `padding 12px 24px → 20px`,
`gap 8px → 24px`, rect `102.109375 × 52px → 94.109375 × 68px` 및
...
통합 focused gate는 12 files / 74 tests, typecheck는 baseline 43건 외 신규 오류
없음이며 초기 Chrome `SIGABRT`는 재시도로 분리됐다.
...
3. opacity/paint 계열 Property slider (Text/Button·Modified Styles color 및 명시적 Modified Styles opacity 완료; default/inherited/state/기타 대상 잔여)
