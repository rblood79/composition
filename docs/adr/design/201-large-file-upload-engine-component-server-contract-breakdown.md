...
### 2-1. `@composition/upload` 측정 (Phase 1 — G1)

| 항목                        | 측정값                 | 조건                                                 |
| --------------------------- | ---------------------- | ---------------------------------------------------- |
| core+tus (esm, minify+gzip) | (Phase 1 종료 시 기록) | `scripts/measure-size.mjs` — 상한 6,144 B            |
| IIFE (`./vanilla`, gzip)    | (Phase 1 종료 시 기록) | 상한 10,240 B                                        |
| 4GB 힙 Δ (3회 중앙값)       | (Phase 1 종료 시 기록) | Chrome 버전 · visibilityState · 처녀 힙 · parallel 3 |
| 재개 3경로 재전송 바이트    | (Phase 1 종료 시 기록) | 단절 · reload · 새 컨텍스트, 상한 = chunkSize        |
| tusd 대조군                 | (Phase 1 종료 시 기록) | darwin-arm64 바이너리                                |
