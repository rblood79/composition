...
| 스토리지           | Cloud Storage (`storage.from` / `/storage/v1`) 사용 0건. `cloud SDK ^2.112.3` 은 `apps/builder/package.json:33` 만 — publish 앱 없음                                                                                                                                                                                                  |
...
| package 선례       | `@composition/specs` — tsup esm/cjs + `exports` 서브 entry + `files: dist`, `private` 아님, react optional peer (`packages/specs/package.json`). `@composition/shared` 는 `private: true` + src export (밖에서 사용 불가). `packages/engine` 은 Rust crate → `wasm-pack` 산출 (`package.json:40`) 을 builder 가 소비 — wasm 89.7KB gz |
