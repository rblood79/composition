...
| **마크 예산 `M`**   | 차트 하나가 만드는 **데이터 마크** 총수 상한 (rule 채널, 후보 2,000). 데이터 마크 = 값에서 나오는 것 (bar rect · line/area 점 · dot · 값 라벨 · pie 조각 · radar 점/면 · radial 링). 축·눈금·격자·범례·창 트랙은 데이터 크기와 무관한 고정 비용이라 **포함하지 않는다** | 총비용 상한 — 실제로 그려지는 점 수로 센다: `데이터 마크 = S × 그려지는 슬롯 수 × k ≤ M`, `k` = 점 하나당 마크 수 (§2.2). 창·집계·others 는 `fitEff = min(fit, floor(M / (S × k)))`. 극값 선택은 합집합 index 를 모든 시리즈가 다시 그리므로 `S × U × k ≤ M` 을 **적응 B** 로 맞춘다 (§2.4) |
| **행 상한 `R`**     | 모델 계산에 넣는 원본 행 상한 (rule 채널, **유한** — 후보 20,000) — **두 leg 동일**                                                                                                                                                                                     | 모델 계산 비용 상한. 초과 시 두 leg 모두 앞 `R` 행 + 진단 `rows-truncated` (사용자-가시 안내). P0 는 **값**을 정한다 — `Infinity` 는 허용하지 않는다 (유한 상한이 R1 을 MED 로 두는 근거)                                                                                                   |
...
- `minSlot / minPointGap / minArc / minAxisGap / minRing` 은 `ChartMetrics` 확장 (rule chart 채널, D3 SSOT). 기본값은 P0 에서 light/dark 실측으로 정한다 (후보 8 / 2 / 6 / 12 / 4 px). 저장 props 가 아니다. `minRing` 은 **radial 전용** (radial 은 링 = 범주라 개수 예산이 맞다); pie 의 동심 링은 시리즈라 개수 예산 대상이 아니다 (round 2 m1).
...
- **컨트롤은 Recharts `Brush` 가 아니다** (round 1 m3: 3.10.1 `Brush` 는 손잡이 둘의 index 를 각각 움직여 창 길이가 변한다). 창 트랙 = shared 의 RAC `Slider` (`packages/shared/src/components/Slider.tsx`, D1 은 RAC 소유) 단일 thumb, `minValue 0 · maxValue n − fitEff · step 1` — 화살표 키 1 슬롯, Home/End 끝. PageUp/Down 은 RAC 기본 (범위의 1/10 을 step 에 맞춤, `useSliderState` pageSize) 을 **그대로 둔다** — 창 길이만큼 넘기는 요구는 철회 (round 2 l1: Slider 파일 변경 0 을 지키는 쪽). 별도 ARIA 작성 없음. 트랙은 플롯 **아래** 에 `windowTrackHeight` (metrics) 를 예약하고 Canvas 는 같은 높이의 비활성 트랙을 그린다 (§2.2 마지막 항).
...
- **손계산 오라클** (Q5 독립성) — 표를 먼저 적고 테스트가 표를 읽는다: 폭 500 · minSlot 8 · S 1 → bar `fit 62`; S 3 묶음 → 20; 누적 S 3 → 62 이지만 `M 2,000 / (3×1)` → 666 이라 `fitEff 62`; 폭 4 → `fit 0` (진단); 반지름 60 · minArc 6 → pie `fit 62` (링 수와 무관), 값 `[1000, 1]` 은 조각 2 (작은 호 0.377px 허용 — 최소 크기 미보장 fixture); 링 (시리즈) 3 → 데이터 마크 `3 × 62 = 186 ≤ M`; radial 반지름 60 · inner 0 · minRing 4 → `fit 15`; 극값 S 4 · B 250 · 겹치지 않는 극값 (bucket 당 범주 8 이상) → `|U| = 2,000`, 점 `8,000 > M` → 적응 B 125 → 62 → … 까지 `S × |U| × k ≤ 2,000` (손계산 표에 각 단계 `|U|` 기재).
...
| Phase | 내용                                                                                                                                                                                                                    | Gate |
| ----- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---- |
| P0    | spike: 최소 단위 5종 light/dark 실측 · `M`/`R` 필요 여부 (5,000 · 20,000행 모델 비용) · Slider 트랙 공존 (창 길이 불변 · End 로 끝까지 · PageUp/Down 은 RAC 기본) · §2.6 결선 inventory · 극값/others 반례 fixture 초안 | G0   |
| P1    | `budget.ts` + full/visible 모델 + scale 적용 (창 0 정적) · `CHART_SAMPLE_ROWS` 삭제 · 손계산 fixture · 두 leg byte 동일                                                                                                 | G1   |
| P2    | bucket 집계 (sum/mean/max/min) · bucket 극값 선택 (적응 B · gap sentinel · fallback) · others · `--chart-others` · 문자열 상수/othersLabel · transformed domain                                                         | G2   |
...
- 최소 단위 기본값 5종 · `M` · `R` (유한) · `windowTrackHeight` 는 실측 전 후보다. 적응 B 의 반감 단계 수 (최악 `log2(fitEff)` 회 재선택) 비용은 P0 5,000행 실측에 포함한다.
