...
> 정본 결정: [ADR-209](../209-chart-authoring-canvas-recharts-runtime.md). 상태 **Proposed**, 2026-09-09. 아래 경로 중 `(신규 제안)`은 현재 존재하는 구현이 아니다. 조사 기준 HEAD `f2779b7d2`; 작업 시작 시 tracked/untracked 제품 변경 0건. 기존 readiness 작업의 goal/guard/stop은 보존한다.
...
| `packages/specs/src/chart/series.ts:44`                                             | 범주×시리즈 중복 합산, 순서·결측 보존                     | 공통 의미 모델에 재사용, 양쪽 중복 집계 금지                 |
| `packages/specs/src/chart/types.ts`, `computeChartScene.ts:51,65`                   | 6종 기하·기본값, CHART_SAMPLE_ROWS=200                    | 순수 의미 계약과 Canvas 기하 구분                            |
...
각 단계는 **향후 구현 승인 후의 실행 계획**이다. 이번 문서 작업에서 완료로 표시하지 않는다. 사용자가 실행을 승인한 뒤에는 이미 승인된 범위의 단계마다 재승인을 요구하지 않는다.

| 단계                                | 책임 범위                                                                                                                         | 완료 조건/산출물                                                                       |
| ----------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| P0 사실 고정·first nail             | 실제 추가/저장/로드/import 경로, Recharts 공개 API 후보, shared provider 소비 경로와 ADR-152 선행 항목. 실제 Recharts와 6종 spike | G0, baseline HEAD·버전·원본 데이터·오차/불가능 옵션·chunk 보고서. 전체 UI 전환 전 판단 |
...
| P5 전환/성능/종결                   | 번들·실행 A/B, 기존 자체 SVG runtime의 실제 소비 종료 확인, 호환성/rollback, 문서 동기화                                          | G5/G6. 선행 ADR 부분 대체 주석·CHANGELOG·README 상태·실제 Live Exercise                |
...
| -------------- | ------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| G0 first nail  | 정적 데이터로 6종 기본형, area 100%, pie 다중 링, radar null+innerRadius, radial 각도+누적                    | 실제 pinned Recharts SVG/브라우저 좌표 vs Canvas 기하·픽셀. 라이브러리 stub이나 자체 SVG 두 개 비교 금지                                                                   |
...
| G4 정적 대칭   | 6종×초기 preset, 기본/비기본 옵션, light/dark, 320×240·640×360, DPR 1/2                                       | 외곽/plot/핵심 mark 좌표 ≤1 CSS px, tick/legend/value 일치, token 해소값 일치. 픽셀 diff는 AA를 구분하고 마크 결손·clip은 허용하지 않음                                    |
...
| G5 번들        | before/after 같은 production build·gzip 명령, dynamic transitive chunk graph                                  | 공통 초기 순증 ≤10 KiB gzip, chart 그래프 순증 ≤200 KiB gzip. 차트 없는 페이지/편집 부트에서 chunk request 0                                                               |
...
| G6 종결        | 저장/reload/export/import/ref/Undo, 구 버전 rollback, production Publish                                      | 문서 강제 rewrite 0·필수 동작 보존, focused tests/cross-check/preflight·Live Exercise 증거                                                                                 |
...
작성 시 수행 범위는 현재 코드·공식 레퍼런스 조사와 문서 구조/링크/포맷/preflight다. **제품 기능·G0~G6·성능·시각 정합은 미실행**이며 숫자는 제안 기준이다. 실행 단계의 증거는 별도 `docs/adr/evidence/209-*`에 입력·원본 결과와 함께 저장한다.
