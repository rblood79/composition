...
#### Phase 2 되돌림 (2026-07-31) — 상한을 없앤 것이 아니라 옮긴 것이었다

사용자 보고("또다시 텍스트가 정상출력되지 않는다")로 **실사용 22 페이지 문서**에서 재현했다. 도형·버튼은 그려지는데 프레임 여럿에서 **텍스트만 통째로 빠지고**, 더 그리면 캔버스가 통째로 얼어붙는다. 콘솔에 `RuntimeError: Aborted()` — `new PictureRecorder()` 할당 지점.

같은 문서·같은 뷰포트에서 런타임 플래그(`__composition_PARAGRAPH_DEBUG__.setRetained`)로 A/B:

| arm                     |    CanvasKit WASM 힙 | 결과                             |
| ----------------------- | -------------------: | -------------------------------- |
| `retained ON` (Phase 2) | **2,147 MB** (=상한) | 프레임 다수 텍스트 소실 → 프리즈 |
| `retained OFF` (구 LRU) |             1,090 MB | 전 프레임 정상, 줌·팬 내내 평탄  |

2,147 MB 는 wasm32 주소공간 상한이다. 닿는 순간 모든 할당이 실패하므로 paragraph 를 못 만들고, 그 단계에서는 **글자만 조용히 빠진다** — 원 버그와 증상이 같지만 기제는 다르다 (주소 재사용이 아니라 할당 실패). 조금 더 진행하면 `PictureRecorder` 까지 실패해 렌더 루프가 죽는다.

**진단을 흐린 것은 보유 카운터였다.** `retainedCount` 는 642 였다 — 642 × 20 KB ≈ 13 MB 라 "paragraph 탓이 아니다" 로 읽힌다. 그러나 A/B 차이는 1 GB 이상이다. 카운터는 **현재 슬롯에 달린 것만** 세므로 이 축의 지표가 못 된다. 판정은 카운터가 아니라 **힙 총량 A/B** 로 해야 한다.

**원인은 채택안을 절반만 구현한 것**이다:

- Decision 은 **대안 C** (retained + walk 스코프 정책) 인데, Phase 2 는 보유만 넣고 스코프를 Phase 3 으로 미뤘다. 그 중간 상태가 정확히 **대안 B** 이고, B 의 유일 HIGH ("무스코프 보유는 대형 문서에서 상한 없는 증가") 가 그대로 실현됐다. **스코프 없는 retained 는 착지 가능한 중간 상태가 아니다** — 플래그 기본값이 ON 인 순간 실사용이 B 를 밟는다.
- 더 근본적으로, **"노드 소유면 상한 질문이 사라진다" 가 이 캔버스에서 성립하지 않는다.** 보유 상한 = 살아 있는 텍스트 노드 수인데, composition 캔버스는 22 페이지 프레임을 **동시에** 그린다 (10% 줌에서 전 페이지가 한 화면). 그래서 그 집합이 곧 문서 전체이고, 기존 경계인 `clearSkiaRegistry`(페이지 전환)는 아무것도 회수하지 않는다. Pen 은 한 번에 한 화면을 그려 이 차이가 없었다 — 대안 C 의 "composition 의 전 페이지 전역 등록 구조 차이 보정" 이 정확히 이 지점을 가리키고 있었다.
- 즉 이 캔버스에서 per-node 소유는 상한을 **없애는** 것이 아니라 "1,000 키" 에서 "문서 전체 노드" 로 **옮기는** 것이다. 옮긴 자리가 상한보다 크면 그대로 터진다.

**부수 관측 — 여유가 원래 좁다.** `retained OFF` 의 평탄 지점이 이미 **1,090~1,194 MB** 다. 즉 구 경로로도 2 GiB 천장까지 여유가 1 GB 남짓이고, 이 문서는 필러가 아니라 실사용 문서다. 보유 모델과 무관한 별도 축이며, retained 재개를 판정할 때 이 여유가 예산의 분모가 된다.
