...
### Phase 0 — inventory freeze + core vendoring (소비자 0) — (구현됐다가 롤백, 2026-08-30)
...
### Phase 1 — `MorphIcon` + 이름 캐시 — (구현됐다가 롤백, 2026-08-30)
...
### Phase 2 — `StateIcon` + 레지스트리 + 교체 8곳 (§2-2) — (구현됐다가 롤백, 2026-08-30)
...
### Phase 3 — 확장 후보 (§2-3) — (구현됐다가 롤백, 2026-08-30)
...
**탭이 백그라운드면 rAF 가 멈춘다** — 이 세션에서 "토글해도 아이콘이 안 바뀐다" 로 보였던 것의 실제 원인이다. `document.visibilityState` 를 먼저 확인하지 않으면 driver 결함으로 오독한다. 전환 프레임 검증은 **창이 포그라운드일 때만** 유효하다.
