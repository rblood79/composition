...
>
> **2026-05-05 rollback / 2026-05-06 follow-up**: 2026-05-03 post-cutover persistence/frame scope/style write-through fix 군은 사용자 환경 회귀 누적으로 2026-05-05 에 폐기됐다. 이후 current main 기준 새 패치군으로 page-frame refresh body/Slot projection, shared frame multi-page projection, component origin/instance persistence/override, synthetic page-frame projection `order_num` DB write 제외, Nodes panel Frames tree parity 를 다시 닫았다. legacy frame/slot/component mirror 는 adapter fallback/export boundary 에만 남긴다.
...
> **Partially superseded by [ADR-131](../completed/131-events-data-actions-first-class-collections.md) (2026-05-13)** — `x-composition.events` / `actions` 채택 결정은 root collection 분리 (ADR-131 대안 A) 로 격하. 본 §3 결정 근거 ("Pencil-compatible core 와 분리") 는 Pencil 정통에 events / actions 카테고리 자체가 없다는 점을 반영하지 못한 framing 이었음. **`x-composition.dataBinding` 영역은 ADR-131 Phase 8 (2026-05-13) 에서 본 §3 결정 유지로 revert** — data SSOT 는 이미 `data_tables` / `api_endpoints` / `variables` 별 IndexedDB store + RAC/RSC 컴포넌트가 `useCollectionData` 로 통합 소비. `editor` 영역도 본 §3 결정 유지.
