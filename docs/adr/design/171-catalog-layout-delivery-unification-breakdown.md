...
| import 여부       | `index.css` 의 `generated/*.css` 참조                          | 94개 중 **62개** import                                      |
...
| catalog 경로 분포 | `componentRulesTable.ts` 블록 스캔                             | 총 121 · A 24 · B 25 · 미보유 72                             |

## 5. 파일 변경 예상

| 영역            | 파일                                                                                                                                   |
| --------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| catalog 값 정정 | `packages/shared/src/catalog/**` (9종 + Phase 2 판정분)                                                                                |
| 생성 CSS        | `packages/shared/src/components/styles/generated/*.css` (재빌드) · `index.css` (import 판정)                                           |
| 전달 경로       | `apps/builder/src/builder/workspace/canvas/layout/engines/implicitStyles.ts` · `packages/specs/src/runtime/containerStylesFallback.ts` |
| factory 인라인  | `apps/builder/src/builder/factories/definitions/*.ts`                                                                                  |
| fixture         | `apps/builder/tests/parity/catalogComponentBox.browser.test.ts` (신규)                                                                 |

## 6. 체크리스트

- [ ] Phase 0 — I1/I2/I3 인벤토리 확정
- [ ] Phase 1 — 9종 catalog 정정 + 재빌드 + 실효값 불변 재측정
- [ ] Phase 2 — 미import 30종 판정 표 확정 + 처리
- [ ] Phase 3 — resolver 단일 판정 통합 + 수기 배선 정리
- [ ] Phase 5 — parity fixture 신설 (Phase 4 앞)
- [ ] Phase 4 — factory 인라인 제거 (fixture 감시하)
- [ ] Phase 6 — origin 재저작 + components 페이지 live 확인
