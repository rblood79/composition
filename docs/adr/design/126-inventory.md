...
- Phase 4 잔여: `elements.ts` 내부 page removal local mutation map, `inspectorActions.ts`, `historyHelpers.ts`, `elementRemoval.ts`, `elementUpdate.ts` 의 `Element` map consumer 전환. `canonicalSceneModelLegacy` 는 bootstrap boundary 로 별도 allowlist 정렬.
