...
/ `PropertyEditorChildNode` 계약으로 정리했다. 잔여 생성형 editor / drop target
...
- `TableEditor`, `TableHeaderEditor`, `ChildItemManager` / `useCollectionItemManager` 등 생성형 property editor 는 add/write payload contract 와 함께 Phase 4 또는 Phase 5에서 전환한다.
