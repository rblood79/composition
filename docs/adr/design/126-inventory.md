...
아니며, 잔여 생성형 editor / drop target resolver 는 후속 slice 로 남긴다.
...
- `TableEditor`, `TableHeaderEditor`, `ListBoxItemEditor`, `TagEditor`, `ChildItemManager`, `tabsItemActions`, `TreeItemEditor` 등 생성형 property editor 는 add/write payload contract 와 함께 Phase 4 또는 Phase 5에서 전환한다.
