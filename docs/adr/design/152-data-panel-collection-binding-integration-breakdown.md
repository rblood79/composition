...
useDataStore (stores/data.ts)          (field.key === "dataBinding")   Preview DOM (iframe):
  ...
  SSOT = Cloud data_tables                                            → runtimeStore.collections
...
| 격차2 | **읽기 경로 3중화**        | `useCollectionData.ts:202-208` — ① `dataBinding`(PropertyDataBinding) ② `datatableId`(`stores/datatable.ts` legacy `useDataTableStore`) ③ legacy `DataBinding type:"collection"`(static/api/cloud, `:392-407`)                                                                             |
...
| 격차5 | **store 이중화**           | `stores/data.ts`(useDataStore, Cloud SSOT) ↔ `stores/datatable.ts`(useDataTableStore — consumers/transform/status 별도 상태 기계) 공존. 격차2 ② 경로의 원천                                                                                                                                |
...
> **Implemented 2026-09-11** (Table wrapper 정렬 1건은 같은 날 후속 커밋으로 해소 — 아래) — G2 9/9 → 후속 10/10 (`apps/builder/scripts/adr152-p3-live.mjs`, ADR 본문 §Live Exercise).
...
- [x] ~~deferred~~ **2026-09-11 해소 (후속 커밋)** — Table DOM wrapper 가 `useResolvedCollectionItems` (window limit 해제) 를 지나고 TanStack `getRowId` 가 shared 행 key (`buildTableRowIdResolver` — item 참조 → `itemKey`) 를 쓴다 → `<tr data-key>` = Skia `getTableProjectionRows` 행 key. wrapper 안 legacy `type:"collection"` 분류 제거. 이때 잡힌 결함: hook 의 rows memo 가 `reload` identity (useAsyncList 렌더별 새 객체) 에 묶여 렌더마다 새 배열 → Table 컬럼 자동 감지 setState 와 "Maximum update depth exceeded" 루프 — rows memo 를 데이터 deps 로 분리 (회귀: builder `useResolvedCollectionItems.rowsStability.test.tsx`, RED 확인). unit 6 (`tableFieldRoles.test.tsx`) · live P3 10/10 (`tr[data-key]` 항목 추가) · P4 12/12 회귀 0.
...
- [x] **원본 삭제 (사용자 승인 2026-09-11)**: `stores/datatable.ts` · `components/data/DataTable.tsx` (index export 정리, `DataTableMetadata.ts` 는 잔존) · `hooks/useDataQueries.ts` 삭제, 정적 가드 ② `ALLOW_LEGACY_STORE` 예외 제거. 후속 (사용자 지시 2026-09-11): `types/datatable.types.ts` (import 0, i18n 키 문자열 `datatable.types.*` 은 무관) 삭제 · `main.tsx` `QueryClientProvider` + `@tanstack/react-query` 의존 (builder package.json · workspace catalog · lockfile) 제거 — type-check 0 · builder main/stores/hooks 688 · live P5 6/6
