...
수리 검증 판독 (reviewer, 2026-09-26, `a1da0623e`): HIGH 0 · MEDIUM 0 → 실행자 닫힘 선언. H1 · M2 · M3 닫힘 확인 (`fills: null` 생산처 1 · 소비처 1 — 해석기는 전부 `Array.isArray` 만 적용). LOW deferred: mode C 로 채운 ref 안쪽 경로 (`appendModeAPatch`) 는 drop 을 거치지 않아 `fills: null` 키가 문서에 남음 (시각 무영향) · 무변화 비교 (JSON) 는 style 밖 prop 의 `undefined` 를 못 보나 synthetic 에 그 값을 쓰는 진입점 0 · 프리뷰 뒤 원래 값으로 커밋하면 조기 return 이 프리뷰 synthetic 항목을 남길 수 있음 (가설) · `readResolvedStyleTarget` 은 store 맵에 origin 이 없으면 자기 style 만 읽음.

## 18. 미확인 LOW 4건 병렬 조사 (2026-09-26)

사용자 지시 "ADR-236 후속 진행해". §17 의 미확인 · 새 발견 항목을 조사 에이전트 4개로 동시에 판정했다 (가설 1 + 반증 1). 넷 다 production 에서 재현됐다.

| 항목                                      | 판정                                                                                                                                                                                                                                                                                                                                                                                                             | 처리                                                                                                                                                                                                                                                                                       |
| ----------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| ColorField side 라벨 정렬                 | 재현 — Canvas 가 catalog side 변형 (`align-items: flex-start`) 을 읽지 않아 Label y 5 (DOM y 0). 공용 fallback 은 props 를 받지 않고 ColorField 는 타입 분기가 없었다. side 변형을 선언한 타입 전수 대조에서 같은 모양은 ColorField 하나                                                                                                                                                                         | `b2773798e` — catalog 변형 스타일을 그대로 옮기는 분기 (field 가족 side 보정의 wrap · Label 폭은 DOM 규칙에 없어 쓰지 않음). unit RED 2 · live side Label y 0 · top 무변화                                                                                                                 |
| instance 안쪽 요소의 tablet · mobile 편집 | 재현 (가장 해로움) — synthetic 쓰기가 `responsive` 를 버려 토글이 기록되지 않았고, 이후 tablet 편집이 base 로 가 desktop 값을 바꿨다. 해석기 `mergeFillSizing` 은 `responsive.styles` 를 얕게 병합해 patch 에 styles 가 있으면 origin tier 값 전체를 가렸다                                                                                                                                                      | `98c622d49` — 해석값과 다음 값의 차이만 (키 × tier) patch 의 `responsive` 에 쓴다 · 해석기는 키 × tier · visibility tier 단위 병합. unit RED 3 · live 토글 patch `{tablet: 0}` → 편집 `{tablet: 24}` 만 · desktop style 무변화 · 끄기 → patch 삭제 · 헤더 토글 전환 tablet 96 / desktop 56 |
| tier override 켤 때 seed                  | 재현 (가설보다 넓음) — seed 가 인라인만 보고 없으면 CSS 초기값 (row · stretch · 0 · flex) 을 넣었다. catalog column 22 타입 · display 72 · alignItems 54 · gap 49 · padding 29 건 발산. live: DisclosureGroup · Card · FileUpload 가 켜는 순간 가로로, Button padding 0                                                                                                                                          | `37469231d` — 메뉴가 Canvas 엔진과 같은 catalog 해석으로 seed 기본값을 넘기고 (`resolveTierSeedDefaults`), 액션은 자기 tier (ref 면 origin tier 포함) → 인라인 → catalog → CSS 초기값 순. store 는 canvas layout 모듈을 import 하지 않는다. live 메뉴 클릭 4 타입 켜기 전후 배치 동일      |
| side 배치 (a) FieldError · 도움말         | 재현 — Canvas 는 catalog 에 없는 규칙 (side 부모 `flex-wrap: wrap` · FieldError/Description `width 100%` + `marginLeft 192`) 을 주입한다. DOM 은 nowrap 이라 invalid · 도움말 상태에서 세 번째 flex item 으로 입력칸을 줄인다 (TextField invalid: DOM FieldError 58 폭 두 줄 · Canvas 다음 줄 x 192). Canvas 안에서도 FieldError x 192 ≠ Input x 182 (gap 16 상수 vs catalog gap 6), 합성 도움말은 들여쓰기 없음 | **사용자 판정 대기** — 아래                                                                                                                                                                                                                                                                |
| side 배치 (b) Slider + 인라인 block Label | 재현 (side 무관) — catalog Label 에 display 정본이 없어 Canvas 는 block, DOM 은 수동 `Label.css` inline-flex                                                                                                                                                                                                                                                                                                     | `ff172727e` — catalog Label `structure.containerStyles` (inline-flex · center · fit-content, CSS 생성은 계속 건너뜀). live Slider 인라인 block h48 → 34 (DOM 28) · 기본 상태 전후 동일                                                                                                     |

**(a) 판정 재료**: 2026-08-21 side 라벨 폭 (Skia 만 주입하던 176) 을 catalog nested 규칙으로 옮겨 DOM 이 읽게 한 선례와 같은 모양이다. 선택지는 (i) Canvas 가 주입을 멈춰 DOM 의 세 번째 item 배치를 따른다 (Preview · publish 무변화, 입력칸이 줄고 오류 문구가 좁은 칸에서 접힘) · (ii) catalog side 변형에 줄바꿈 + FieldError/도움말 들여쓰기 규칙을 두어 DOM 도 입력칸 아래에 둔다 (RSP side 라벨과 같은 모양, field 가족 약 10 타입의 생성 CSS · Preview 출력 변경). (ii) 의 들여쓰기는 크기별 gap (2 · 4 · 6 · 8 · 10) 을 알아야 하는데 생성 CSS 가 gap 을 크기별 고정값으로만 내보내 CSS 변수로 읽을 수 없다 — 생성기가 gap 변수를 내보내거나, side 를 grid (label · 내용 2열, 오류는 2열) 로 바꾸는 쪽이 필요하고 후자는 2026-06-19 grid → flex-row 통일 결정을 되돌린다.

**정정**: `98c622d49` 메시지의 "Canvas 가 instance tier 를 breakpoint 전환 · 새로고침 뒤 반영하지 않는 기존 결함" 은 틀렸다 — 하니스가 store `setActiveBreakpoint` 를 직접 불러 `invalidateLayout` 과 localStorage 저장을 건너뛴 탓이다 (헤더 토글로는 instance · synthetic · frame 모두 정상). 하니스는 breakpoint 를 헤더 토글로 바꾼다.

**LOW (기록)**: `setActiveBreakpoint` 가 값만 바꿔 재레이아웃을 호출측에 맡긴다 (마운트 동기화 경로 포함 — 실측상 정상) · mode C 로 채운 synthetic 노드의 tier 쓰기는 이번 경로 밖 · Slider 인라인 block 의 남은 6 px (Canvas 인라인 줄 높이) · Canvas SliderOutput 폭 0 (값 글자 미측정, 기본 상태에서도) · ColorField Input 폭 Canvas 1842 vs DOM 106 (`max-width: 12ch`) 과 Canvas 에만 있는 ColorSwatch 자식 · visual parity full 의 Preview leg 3건 실패 (PARITY-ENV, 수정 전 catalog 에서도 같음).

## 19. 재현 항목 수리 (2026-09-26)

사용자 지시 "재현된 것 수리하고 커밋 push 해". §18 의 (a) 는 선택지 (ii) — catalog 규칙 — 로 수리했다.

| 항목                              | 수리                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | 검증                                                                                                                                                                                          |
| --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| side 배치 (a) FieldError · 도움말 | `5219bf261` — catalog side 변형 (field 9 타입 + TextArea) 에 `flex-wrap: wrap` · 내용 `flex: 1 1 0%; min-width: 0` · FieldError/`[slot="description"]` `flex-basis: 100%` + `margin-inline-start: calc(var(--form-label-width, 11rem) + var(--{p}-side-gap))`. 크기별 gap 은 delegation `variables: "auto"` 에 새 `autoKeys: ["gap"]` 로 변수를 내보낸다 (auto 가 쓰이지 않는 변수까지 내보내던 것을 막음). Canvas 는 catalog 에 없던 주입 (wrap · 폭 100% · 상수 16) 을 같은 규칙으로 바꾸고, canonical FieldError 자식이 없는 타입은 합성 오류 줄 (`__synerr`) 을 둔다 | 새 parity `adr236SideFieldMessagePlacement` (10 field × invalid · 도움말 · 둘 다, DOM 과 Canvas 의 x · y · 폭 · 높이 · root 높이) · 생성 CSS 9 · live TextField · Select side + 오류 + 도움말 |
| SliderOutput 폭 0                 | `f9501fe2c` — 값 텍스트 leaf (`slideroutput` · `metervalue` · `progressbarvalue`) 를 `TEXT_LEAF_TAGS` 에 넣어 텍스트 측정을 받게 했다 (Meter · ProgressBar 도 같은 모양)                                                                                                                                                                                                                                                                                                                                                                                                 | unit RED 6 · live 값 폭 Slider 18 · Meter 29 · ProgressBar 30, side 트랙이 값 자리를 비움                                                                                                     |
| ColorField Input 폭 · swatch      | `f435623ff` — read-only subpart 투영이 delegation `max-width` (크기 변수, 예: 12ch) 를 읽는다 (`resolveDelegatedChildMaxWidth`). ch 는 Pretendard "0" 폭 (0.596em) 으로 px 환산. Canvas 에만 있던 ColorSwatch 자식은 그리지 않고 factory 도 만들지 않는다 — 기존 문서의 노드는 지우지 않는다                                                                                                                                                                                                                                                                             | unit (shared 2 · Canvas 1 RED→GREEN) · live md 100.1 · xl 171.6 (DOM 12ch = 100.08, Pretendard 로드 기준) · swatch 노드 없음                                                                  |
| Slider 인라인 block 남은 6 px     | 새 수정 없음 — `f9501fe2c` 로 해소. 원인은 SliderOutput 에 `leafBaseline` 이 공급되지 않아 엔진 인라인 흐름이 아래 가장자리 baseline 으로 폴백한 것 (`block.rs` — CSS §10.8.1 과 같은 폴백이라 엔진은 맞다, 결함은 입력 공급 쪽). 회귀 단언 추가 (값 텍스트 leaf 3종 `leafBaseline > 0`)                                                                                                                                                                                                                                                                                 | 조사 에이전트 live: `display: block` root h28.03 · Label y0.03 · Track y20.03 (수정 전 h34 · y6, DOM h28 · y0) · 새 단언은 `slideroutput` 을 목록에서 빼면 RED                                |

**알려진 차이 (기록)**: Select 의 DOM 도움말은 `.react-aria-Description` 클래스라 generated Description 의 줄 높이 16 을 받고, Canvas 합성 도움말은 root 상속 1.5 (18) 로 잰다 — 새 parity 의 `KNOWN_HEIGHT_GAP` 한 줄. 다른 field 도움말은 RAC Text 라 같다.

**남은 LOW (기록)**: Slider `display: block` 에서 Canvas SliderOutput x 38 vs DOM x 50 (위임 관측, DOM Label 폭 미측정) · Label 과 Output 의 `leafBaseline` 0.03 px 차 · 추정 — `TEXT_LEAF_TAGS` 밖의 글자 그리는 inline leaf (Button · Badge 등) 를 block 부모에서 텍스트 leaf 와 한 줄에 두면 같은 폴백이 날 수 있다 (대조군 미측정).

**측정 함정**: 하니스에서 ColorField Input DOM 폭이 105.8 로 나온 것은 system-ui 대체 폰트 기준이다. Pretendard 가 로드된 빌더에서는 100.08 이다.

**수정 전부터 실패하던 parity (이번 변경과 무관)**: Dc6 inventory 70 → 69 · Hc2 GridList grid/flex · catalogComponentBox Dialog fixed/static · visual parity full 의 Preview leg (PARITY-ENV).
