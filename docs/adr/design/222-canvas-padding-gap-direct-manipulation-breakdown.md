...
| idle → 띠 hover                                                 | 그 띠 전체 사선 + 핸들 강조 + 현재 값 배지 (resize 커서는 핸들 위에서만)                          | 저장 없음                                  |
| 핸들 pointerdown (띠 영역 press 는 요소 선택·이동 — 2026-09-26) | 사선 제거, 핸들 강조 유지                                                                         | spacing owner claim, 기준값 캡처           |
| 이동 임계값 미만 pointerup (= 클릭)                             | 인라인 숫자 입력 열림 + 패널 해당 필드 강조                                                       | RAC 입력 포커스, 값 변경 전에는 저장 없음  |
| 임계값 이상 이동                                                | dragging, 사선 없음, 핸들 숨김 (2026-09-26 Figma 어법) · 실시간 값 배지, 패널 해당 필드 동기 강조 | 프레임별 presentation publish              |
...
3. client delta를 시작 zoom으로 scene delta로 변환한다. padding 부호는 **값이 커질 때 움직이는
   ...
   주축 진행 방향이 +. 드래그 중에는 핸들을 숨긴다 (값 1:1 · 핸들 = 값/2 라
   포인터와 어긋난다 — 2026-09-26 Figma 어법, 값 배지는 유지). 범위는 0 이상, 1px/큰 단위 step.
