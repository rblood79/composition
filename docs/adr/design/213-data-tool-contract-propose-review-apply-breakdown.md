...
> 본문: [213-data-tool-contract-propose-review-apply.md](../completed/213-data-tool-contract-propose-review-apply.md) · 리서치 정본: [DATA_PANEL_REDESIGN_RESEARCH_2026-09](../../explanation/research/DATA_PANEL_REDESIGN_RESEARCH_2026-09.md) §1-6 · §3-4 · §3-5 · §4-3 · §4-4 · 시안: artifact `f7d8327e` "AgentReview" 아트보드 (스키마 diff 승인)
...
| `bind_collection`         | `services/ai/tools/bindCollection.ts:24` `SUPPORTED_SOURCES = ["static","api","cloud"]` + config 인라인                                    | 사람 UI 기록 형상 `{ source:"dataTable", name }` (ADR-159 P4b) 과 다르다 — DataTable 에 잇는 경로 없음           |
...
### Phase 6 — 사람이 부르는 AI 3종 (AI-1 · AI-2 · AI-4, 게이트 G5) — **완료 2026-09-11** (G5 PASS, evidence "Phase 6 — G5")

- [x] AI-1 "설명으로 테이블 만들기": `create_table_from_description` — 모델은 **스키마 + 샘플 행 생성 규칙** 만 (`TableSpecSchema` zod 단일 소스, tool JSON Schema 파생 · 규칙 15종: enum · sequence · name · company · email · url · image · sentence · paragraph · integer · decimal · boolean · date · reference(FK, 기존 collection 행) · constant · list) → 샘플 행은 코드가 결정적으로 생성 (`generateSampleRows`, seed = 이름) → 같은 규칙으로 검증 (`validateSampleRows` — 스키마 밖 컬럼 · enum 밖 · 깨진 FK · required · type, I3) → `dispatchDataProposal` (승인 diff = 미리보기: 스키마 표 `Fields: key: type *` + 샘플 행). **편차**: `presets/dataTablePresets.ts` 생성기를 승격하지 않고 규칙-데이터 생성기/검증기를 새로 두었다 — preset 은 규칙이 코드 안에 굳어 있어 모델 출력을 검증할 수 없다; 이름/회사 풀 (`presetData.*`) 만 공유. 객관식 clarifying 은 tool description 지시 (I8)
- [x] AI-2 붙여넣기 이해: `understand_paste` — 규칙 파서 먼저 (`buildPasteProposal`: cURL → `define_endpoint` (`utils/data/curlCommand.ts` — 212 Phase 4 가 같은 파서), JSON/표 → `create_collection`/`insert_rows`), 실패 시 `parsed:false` + guidance 로 모델 폴백 (적용 0). 미리보기는 승인 diff (212 UX-2/UX-3 은 212 착수 시 같은 proposal 을 그린다)
- [x] AI-4 반복 편집: 편집기에 열린 테이블/endpoint 를 턴 컨텍스트에 자동 첨부 (`readOpenDataEditor` → `BuilderContext.openDataEditor` → `buildTurnContext` "Data open in the editor right now" — 스키마 · fieldId · 사용처 · endpoint 는 URL redacted + 헤더 키만) → 모델이 `propose_data_change` update 계열로 제안 → Phase 4 승인 diff. 타입 변경은 "사용처 N" (Phase 4). **삭제는 제안 불가 유지** (ADR Consequences "삭제는 AI 로 못 한다" 와 `HUMAN_ONLY_DATA_OPS`) — 안내 문구가 UI 로 유도
- [x] 진입: AI 패널 프롬프트 (212 생성 패널 "AI 로" 버튼은 212 Phase 착수 시 같은 tool 을 부른다)
- [x] G5 live: "블로그 글 테이블 만들어 — 제목 · 본문 · 작성자 · 게시일 · 상태" → qwen3 `create_table_from_description` (필드 5 · enum draft/published/archived · 라벨 한글) → 승인 diff (스키마 표 + 샘플 행) → 적용 → 목록에 BlogPosts (컬럼 = 스키마 키 정확히, I3) · AI-4 "이 테이블에 tags 필드 추가" → `add_field` 제안 → 적용 · AI-2 쉼표 표 → Members 3 필드 (age number) → 적용 · provider 본문 10건 canary 0

### Phase 7 — closure — **완료 2026-09-12**

- [x] AX-6 MCP 노출 원칙 — 본 breakdown §6 (tool 별 annotation 표 추가). 노출 자체는 후순위 (대안 D 기각 유지)
- [x] ADR-202 편입 어댑터 자리 문서화 (R6) — §8
- [x] CHANGELOG (Features — AI, ADR-213 Phase 1~7 묶음 엔트리 → Implemented) · ADR README (완료 231 · Implemented 195 · 착수 순서 표) · ADR 본문 `### Live Exercise` (G1 · G4 · G3 · G2 · AX-4 · G5) · `completed/` 이관
...
| `services/ai/tools/{createTableFromDescription,understandPaste}.ts` (신규) · `services/ai/data/{tableSpec,pasteProposal}.ts` (신규) · `utils/data/curlCommand.ts` (신규) · `services/ai/data/dataToolReadModel.ts` (`readOpenDataEditor`) · `services/ai/{builderContext,systemPrompt}.ts` · `types/integrations/chat.types.ts` |   6   | AI-1 (규칙-데이터 생성기 = 검증기 — preset 승격 대신 신규) · AI-2 (규칙 파서 → 모델 폴백) · AI-4 (열린 편집기 자동 첨부)                                                                                                                                                                                                                                |
...
- ADR-202 recipe 체계에 "data recipe" 로 편입되면 AI 패널 · agent · MCP 가 같은 `DataChange` 를 쓴다 — 어댑터 1개 (R6, §8).
- tool 별 annotation (Phase 7 정리 — 노출 시 그대로 옮긴다):

  | tool                                                                              | readOnlyHint | destructiveHint | idempotentHint | openWorldHint | 비고                                                             |
  | --------------------------------------------------------------------------------- | :----------: | :-------------: | :------------: | :-----------: | ---------------------------------------------------------------- |
  | `list_collections` · `get_collection` · `list_api_endpoints` · `get_api_endpoint` |     true     |      false      |      true      |     false     | 결과는 공유 redactor 를 지난다 (HC5)                             |
  | `explain_request_failure`                                                         |     true     |      false      |      true      |     false     | 세션 실행 스냅샷만 읽는다 — 요청을 다시 보내지 않는다            |
  | `propose_data_change`                                                             |    false     |    **false**    |     false      |     false     | 사람 전용 op (delete 계열) 는 스키마에 없다 · 승인 없이는 쓰기 0 |
  | `create_table_from_description` · `understand_paste` · `bind_collection`          |    false     |      false      |     false      |     false     | 같은 dispatcher 경유 (승인 diff) — propose 의 workflow alias     |
  | agent `data.runEndpoint`                                                          |    false     |      false      |     false      |   **true**    | 정의된 요청을 바깥으로 보낸다 — GET 외 method 는 승인            |
  | agent `data.openTable` · `data.openEndpoint`                                      | true (view)  |      false      |      true      |     false     | 편집기 상태만                                                    |
  | agent `data.importPaste`                                                          |    false     |      false      |     false      |     false     | dispatcher 승인                                                  |

- MCP 서버가 생기면 `readOnlyHint:true` 그룹은 승인 없이, 나머지는 서버 안 승인 채널 (`requestAgentCommandConfirmation` — host `"mcp"`, 승인 UI 없으면 거부) 을 그대로 쓴다. 클라이언트 측 자동 승인은 신뢰하지 않는다.
...
## 8. ADR-202 편입 어댑터 자리 (R6 — 202 착수 시 결정 지점 2 confirm 1회)

202 가 `BuilderCommandProgram` (typed IR) 을 도입하면 본 ADR 의 데이터 경로는 **무변경**으로 편입된다 — 이중 IR 을 두지 않고 `DataChange` 를 statement 하나로 감싼다:

- statement 형상: `{ kind: "data", change: DataChange /* origin 없음 */, label? }` — 스키마는 `modelFacingDataChangeJsonSchema()` 를 그대로 참조 (202 의 program 스키마가 `$ref` 로 끼운다). 사람 전용 op · origin · 내부 필드는 여기서도 없다.
- 실행 seam: 202 executor 가 `data` statement 를 만나면 `dispatchDataProposal({ ops, label, host, origin })` 하나만 부른다 (`services/ai/data/dataProposalDispatcher.ts`). 승인 diff · origin stamp · History 1 · provenance 1 은 그 안이다 — 202 는 자기 승인 다이얼로그를 따로 만들지 않고 `data.propose` 요청을 program 의 다른 statement 승인과 **원소별**로 묶는다 (ADR-196 R2 와 같은 규칙: 승인 1회로 destructive 를 숨기지 않는다).
- 명령 축: `data.openTable` · `data.openEndpoint` · `data.runEndpoint` · `data.importPaste` 는 202 의 `command` statement 가 `executeAgentCommand(id, args, ctx)` 로 부른다 — 이미 인자를 받는다 (Phase 5).
- 읽기 축: 읽기 tool 4 + `explain_request_failure` 는 202 의 "관찰" 단계가 그대로 쓴다 (결과는 redactor 뒤).
- 현재 tool 3종 (`propose_data_change` · `create_table_from_description` · `understand_paste`) 은 202 에서 "data recipe" 로 이름만 옮긴다 — 각각 `DataChange` 를 만드는 순수 함수 (`buildCreateTableProposal` · `buildPasteProposal`) 가 이미 tool 밖에 있어 recipe 본체가 된다.
- 기록: 202 착수 시 이 절을 202 breakdown 이 인용하고 (`docs/adr/design/202-*-breakdown.md`), 의존 방향 (213 base ↔ 202 응용) 은 유지 — 반전 증거가 나오면 결정 지점 2.
