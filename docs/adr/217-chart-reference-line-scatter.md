...
**Generator · BC 선언**: rule `chart` 채널은 CSSGenerator 가 `--chart-axis/grid/others` 로 emit 하고 Skia 가 같은 채널을 읽는다 (`CSSGenerator.ts:1041-1046` · `skiaPrimitives.ts:3459-3461`) — 새 `chart.reference` 는 같은 3곳 확장이며 자식 selector/variant emit 은 필요 없다. BC: 새 props 미설정 = 현행이라 **기존 문서 영향 0% · 재직렬화 0 파일**. **rollback 경계 (round 1 h2)**: 현재 코드와 로컬 dist 는 미지원 `chartType` 을 진단이 아니라 **예외** 로 만난다 (`budget.ts:174-177 slotFit throw` · `:200-201 markFactor throw`, 설정 오류 검사 `computeChartScene.ts:351` 보다 모델 `:325` 가 먼저) — 두 leg 모두 catch 가 없어 (`buildSpecNodeData.ts` · `RechartsChart.tsx:182`) `chartType:"scatter"` 문서는 구버전에서 캔버스 scene 빌드 · Preview 렌더가 중단된다 (저장 데이터 삭제는 아니다). 따라서 본 ADR 의 **P1 은 "미지원 chartType 안전 경로" commit** (validator 진단 `chartType.unsupported` → 설정 오류 scene, throw 0) 이며 rollback 대상 = 그 commit 이후; 그보다 앞으로 되돌리려면 scatter 차트를 먼저 다른 종류로 바꾸는 절차가 필요하다 (breakdown §2.5). `referenceLines` 는 구버전이 무시한다 (선 없이 렌더).
...
9. **HC9 산점도 결과 = 원본 관측점** — 예산 · 창 · 넓힌 창 재추출을 거친 뒤 그리는 모든 `(x, y, series)` 가 입력 행에 있는 관측점이다 (집계 fallback 0). 점 계수는 **실제 관측점 수** (`Σ series.values.size`) 이지 `S × |U|` 가 아니다 (round 1 h1 — 600행 × 60시리즈 = 600점이 조밀 계수 7,200 으로 P 를 넘어 집계돼 `(60,2)` 가 `(47,2)` 로 옮겨졌다). bar/line/area/pie/radar/radial 의 계수 · fallback 은 바꾸지 않는다 (HC1).
...
2. 유지보수 L~M (열거 20곳) 은 HC8 컴파일 방어선 RED + P1 런타임 진단 + G2 의 6종 byte 동일 테스트가 누락을 잡는다 — 216 이 같은 열거를 이미 한 번 늘렸다 (`dimensionScale` 결선 9곳).
...
| R4  | 산점도 극값 (bucket · 시리즈별 min/max) 이 밀집 구간을 성기게 보이게 한다; `selectExtrema` 에 희소 계수 분기가 들어가 line/area 경로를 건드릴 수 있다 |  MED   | 희소 계수는 `count:"sparse"` 인자 하나 (기본 dense = 현행) — HC1 byte 동일 테스트가 line/area 회귀를 잡는다; HC9 소속 테스트 (codex 600×60 fixture + 12k×60 + 6k 시리즈) 가 집계 0 을 증명; 밀도 표현 (binning) 은 범위 밖으로 기록    |
| R8  | rollback 경계 — P1 (안전 경로) 이전 revision 은 scatter 문서에서 예외로 캔버스 · Preview 가 멈춘다 (구버전 dist 실측)                                 |  MED   | P1 을 첫 commit 으로 분리 · G3 T12 프로브 대상 = P1 dist (P1 이전은 "지원하지 않는 경계" 로 기록) · 되돌림 절차 = scatter 차트를 다른 종류로 바꾸는 패널 조작 (문서화, breakdown §2.5)                                                 |
| R9  | 산점도 점 불투명 (opacity 1) 이 겹친 점을 구분하지 못하게 한다 — 반투명은 두 leg 합성 단위가 달라 (path union vs 점별) 농도가 갈린다                  |  LOW   | RSC 기본과 같은 opacity 1 채택 (D2); 반투명 옵션은 두 leg 가 같은 합성 단위 (DOM 도 단일 path) 를 갖출 때의 후속 — line `showDots` 의 같은 차이는 기존 결함으로 별도 기록 (LOW deferred)                                               |
...
| G1   | P1·P2 | (P1) 미지원 `chartType` 문서가 두 leg 에서 설정 오류 scene ("Check chart settings") — throw 0 (unit + 구버전 형태 fixture) · 6종 문서 byte 동일 · (P2) 기준선 손계산 oracle 3 (domain 위/아래/안) · 미설정 문서 scene byte 동일 · 진단 3종 RED→GREEN                                                                       | oracle 불일치 → domain 규칙 수정 (scene 한 자리)                                                                   |
| G2   | P4    | 산점도 손계산 oracle (중복 x · 파싱 실패 · time x) · HC8 `assertNever` RED→GREEN · 6종 문서 byte 동일 (희소 계수 인자 기본값 = 현행) · **HC9 소속**: codex fixture 600행×60시리즈 → overflow 0 · 출력 ⊆ 입력, 12,000×60 → 희소 극값 ⊆ 입력 · 집계 0, 6,000 시리즈×2 → stride 솎기 ⊆ 입력 · 진단 1; 넓힌 창 재추출도 ⊆ 입력 | 소속 위반 → 계수/솎기 규칙 수정; 예산 초과면 211 모드 표 재판정                                                    |
| G3   | P3·P5 | chartParity 기준선 +4 · 산점도 +2 byte 동일 · **합성 프로브**: 같은 시리즈 겹친 점 2 의 중심 픽셀이 Skia 캡처와 Preview 캡처에서 같다 (codex m3 방법) · T12 프로브 대상 = **P1 dist** (기준선 무시 · scatter 는 설정 오류 표시 · 데이터 보존) + P1 이전 dist 는 예외 사실 기록                                             | 좌표 불일치 → DOM 이 scene 값을 그대로 쓰는지 (Recharts 재계산 경로) 추적 후 수정; 픽셀 불일치 → opacity 경로 추적 |
| G4   | P6    | live 하니스 (실제 빌더): 기준선 추가 → Skia 픽셀 (neutral 가로선) · Preview `<line>` dash/라벨 · scatter 팔레트 생성 → 점 8 · 창 트랙 · 패널 편집 canonical write 1 · 임시상태 write 0                                                                                                                                     | 실패 항목은 결함 — Implemented 승격 금지                                                                           |
| G5   | P6    | 모델 p95 scatter 20k × S4 ≤ 20 ms · 프레임 Δ ≤ 1 ms · 번들 순증 ≤ 3/3 KiB + 절대 상한 안 · lazy 청크에만 `Scatter` · 5-질문 기록                                                                                                                                                                                           | 번들 초과 → 청크 배치 조정 재측정 → 사용자 재승인 (216 선례); 성능 초과 → 격자 분기 프로파일                       |
