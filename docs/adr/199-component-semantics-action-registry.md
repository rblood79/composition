...
축별 SSOT 는 이미 하나씩 세워져 왔다 — 정의 (`SHORTCUT_DEFINITIONS` 70) · 표기 (`formatShortcut`) · 실행 (`commandRegistry`, ADR-195) · precondition (`COMMAND_META` 51, ADR-196) · 아이콘 (`ACTION_ICONS`). **비어 있는 축은 "표면 노출"** — 어떤 액션이 어느 표면에 어떤 라벨과 순서로 서는가다. 이 축만 표면마다 재작성된다 (2026-08-30 실측):
...
2. **술어 입력 사영 불일치** — `canDetachInstance` 가 `type === "ref"` 를 읽었는데, 캔버스 상호작용 map 은 Skia `interactionNodesMap` 파생 (`BuilderCanvas.tsx:777`) 이라 `type` 이 렌더 컴포넌트 (`"Button"`) 로 해소된다. 같은 함수 · 다른 입력 → **캔버스 표면에서만 "인스턴스 분리" 가 통째로 사라졌다**. 술어를 `ref`/`masterId` 기준으로 고쳐 수리 (같은 commit).
...
2. **HC2** — 기존 id 계약 불변: `ShortcutId` 70 · `COMMAND_META` 51 · ADR-182 context menu item id 19 · `ACTION_BAR_ALLOWLIST` 문자열. rename 0건.
3. **HC3** — 편집 시맨틱 술어는 **사영 불변 필드만** 읽는다 (`componentRole` / `ref` / `masterId` / `reusable`). `type` 참조 0건 — grep 게이트로 집행.
...
- 설명: `COMMAND_META` 와 같은 계층에 **표면 노출 축** (label ko/en · icon · availability · surfaces · order) 을 두고, 컴포넌트 시맨틱 4액션부터 채운다. 나머지 47 엔트리는 optional 미지정으로 남는다. 표면은 렌더/필터만 한다.
...
- 위험:
  ...
  - 마이그레이션: **MEDIUM** — 51 엔트리 타입이 넓어지고 4개만 채워진 비대칭 기간이 생긴다. agent allowlist (ADR-196) 계약과의 간섭 확인 필요
...
1. 잔존 위험 최대치가 MED (마이그레이션) 이고, 51 엔트리 중 4개만 채워지는 비대칭은 optional 필드 + 게이트로 관리 가능하다. 축이 하나로 유지되므로 나중에 z-order·정렬군을 편입할 때 새 판단이 필요 없다.
...
| R3  | `COMMAND_META` 51 엔트리 타입 확장이 ADR-196 agent allowlist 계약과 간섭                                                                                                                                                                             |  MED   | optional 필드로만 확장 + `commandMeta.static.test.ts` 기존 단언 무변경 확인 (G1)                                            |
...
| **G4** | Phase 5 종료      | 초기 번들 증가 ≤ +2KB gz, 가용성 평가가 Skia 프레임 경로에 진입 0 (호출 스택 확인)                                                                                                 | descriptor 를 lazy import 로 분리                  |
...
- `COMMAND_META` 인접 타입이 넓어지고, 51 엔트리 중 4개만 노출 축을 채운 비대칭 기간이 생긴다 (나머지는 optional 미지정).
