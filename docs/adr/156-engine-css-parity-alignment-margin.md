...
- **정합 기준선 무회귀** — 672 조합(flex 교차축 384 + main 축 288) + **2차 sweep 정합 영역 12행**(breakdown §1-2) + 기존 Rust 테스트(`cargo test --lib` **270**(Phase 4.5 relative/abs +4) + `golden` 15 + `tree_golden` 11 + doc 1 = **297**. Phase 4 는 파리티(phase4 8/8, JS)로 검증 — Rust 무증가, Phase 4.5 는 +4) + **파리티 스위트 41 무회귀**(phase2/3/4/4_5 포함)
