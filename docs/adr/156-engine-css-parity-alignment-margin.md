...
- **정합 기준선 무회귀** — 672 조합(flex 교차축 384 + main 축 288) + **2차 sweep 정합 영역 12행**(breakdown §1-2) + 기존 Rust 테스트(`cargo test --lib` **261**(Phase 2 후 — E1 align_self 5 추가, 이전 256) + `golden` 15 + `tree_golden` 11 = 288)
...
| G3   | Phase 3 착수 전 / 종료 | (착수 전) 옵션 3-a/3-b 책임 경계 확정 · (종료) **E2 + E12 + E13 + E14** 발산 0 + **live grid 컴포넌트 시각 회귀 0** + JS DFS 보정과 이중 적용 없음 + **(R6)** `justifyItems`·`gridAutoColumns`·`gridAutoRows`·`gridColumnStart`·`gridRowStart` 캐시 키 등재                                                                                                                                        | 이중 적용 해소 불가 시 옵션 3-b(정렬만, 크기 stretch 유지)로 축소 + 잔존 §Residual 기록            |
| G4   | Phase 4 / 4.5 종료     | **E3 + E7 + E8 + E17** 발산 0 (E17 = overflow BFC 상쇄 차단 — **E3 와 동시 구현 필수**, 단독 E3 는 통과 불가) + **(R6)** `overflowX`/`overflowY` 캐시 키 등재 + 상쇄 차단 케이스·672 조합 무회귀 · (4.5) **E10 + E11** 발산 0                                                                                                                                                                      | block 경로 회귀 시 E3 를 중첩 컨테이너 한정으로 축소 (root 탈출은 시각 무해)                       |
| G5   | Phase 5 종료           | **E4 + E5(결함군 전체) + E15** 발산 0 + 중첩 4형상(NST-1~4) 무회귀                                                                                                                                                                                                                                                                                                                                 | E5 결함군 중 min/max clamp 만 잔존 시 §Residual 기록 후 종결                                       |
