...
- **정합 기준선 무회귀** — 672 조합(flex 교차축 384 + main 축 288) + **2차 sweep 정합 영역 12행**(breakdown §1-2) + 기존 Rust 테스트(`cargo test --lib` **266**(Phase 3 grid 후 — 261에서 +5) + `golden` 15 + `tree_golden` 11 + doc 1 = **293**. Phase 4 는 파리티(phase4 8/8, JS)로 검증 — Rust 단위 테스트 무증가) + **파리티 스위트 36 무회귀**(phase2/3/4 포함)
