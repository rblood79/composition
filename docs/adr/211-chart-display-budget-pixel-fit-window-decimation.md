...
In Progress — 2026-09-11 (P1 · P2 반영). Proposed 2026-09-10 (코드 기준 `2a9ab37fc`, ADR-210 Implemented 직후). 사용자가 방향과 제목을 정함 (분리 4질문 lock-in: [breakdown §1](design/211-chart-display-budget-pixel-fit-window-decimation-breakdown.md#1-범위--선행-관계-분리-4질문-lock-in)). round 1 (codex, HIGH 3 · MEDIUM 6) 반영 → round 2 수리 검증 (HIGH 1 · MEDIUM 3 · LOW 1) 반영 — [reviews/211.md](reviews/211.md). 판독 루프는 **실행자 종결** (review-loop-closure §1 상한 도달, 사용자 지시 2026-09-10 "초기 설계를 흔들지 않게 한 번에 끝내자"): 극값 선택의 마크 상한을 실제 점 수 `S × |U| × k ≤ M` + 적응 B 로 고정, pie 링은 개수 예산 밖, domain 은 transformed 전체, 결측 gap sentinel, Slider Page 키 요구 철회.
...
| G3    | P3   | R6·R7 live: 실제 빌더 1,000행 범주 bar → Canvas 창 0 + 비활성 트랙 · 패널 안내 · Preview Slider 화살표/PageDown/End 로 끝까지 (창 길이 불변, canonical write 0) · 5,000행 기간 축 비누적 line → 실제 점 `S ×                                                                                                                                                                                                                                                                                                                                                                                              | U                                                      | × k ≤ M`+ gap 끊김, 누적 → 집계 + 접미 · pie 40 → others 1 · Properties`overflow`/`aggregate`/`axis`/`othersLabel` → canonical write → reload → 두 leg → Export → publish 같은 결과                                                                                                                                                                                     | 결선/창 상태 경로 수리                    |
