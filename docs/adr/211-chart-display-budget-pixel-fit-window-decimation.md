...
In Progress — 2026-09-11 (P1 반영). Proposed 2026-09-10 (코드 기준 `2a9ab37fc`, ADR-210 Implemented 직후). 사용자가 방향과 제목을 정함 (분리 4질문 lock-in: [breakdown §1](design/211-chart-display-budget-pixel-fit-window-decimation-breakdown.md#1-범위--선행-관계-분리-4질문-lock-in)). round 1 (codex, HIGH 3 · MEDIUM 6) 반영 → round 2 수리 검증 (HIGH 1 · MEDIUM 3 · LOW 1) 반영 — [reviews/211.md](reviews/211.md). 판독 루프는 **실행자 종결** (review-loop-closure §1 상한 도달, 사용자 지시 2026-09-10 "초기 설계를 흔들지 않게 한 번에 끝내자"): 극값 선택의 마크 상한을 실제 점 수 `S × |U| × k ≤ M` + 적응 B 로 고정, pie 링은 개수 예산 밖, domain 은 transformed 전체, 결측 gap sentinel, Slider Page 키 요구 철회.
