...
## 결과 — 61 케이스 전부 엔진 직결 ≤1px (round 11 수리 후)
...
바인딩 경계 3 에서 RED — Blink 모델로 교체) → 수리 19~~21 → **61/61 pass**. 어댑터 대조군은
**발산 18 / 정합 43** — 현 IFC 시뮬레이션이 Chrome 과 갈리는 차원의 실측
(Phase 5 cutover 가 닫을 대상. "정합" 은 시뮬레이션이 우연히 맞는 차원 — round 9 추가
12 케이스는 프로덕션 어댑터가 엔진 block/flex 경로를 그대로 타므로 수리 후 전부 정합).
종전 "16/23 발산" 표기는 오집계 (r8m1 — 실측 15/23; 27 케이스 18/9; 39 케이스 18/21; 42 케이스 18/24; 51 케이스 18/33; 61 케이스 18/43).
...
21. **min/max-height 바인딩 시 strut 미전파** (`tree.rs` solve_block, r11m1 후속 — Blink 모델):
    ...
    상자 안 overflow. 대조군: min-height:0 명시 · max-height:100 미바인딩 → 40.
...
- root 자신의 `height:0` (+`min-height`) 은 `fixup_root_self_size` 의 `has_h` 경로라 min-height
  clamp 를 받지 않는다 — 차등 root 는 항상 auto 높이라 관측 밖 (부모 있는 상자는 수리 20 으로
  intake clamp). Phase 5 판정.
- §8.3.1 미커버 경계 (Codex r11 과제 10 목록 — 정상 61행 밖): root margin · clearance ·
  float/clear · anonymous block box · inline text↔block 혼합 전환 · percentage height 와 bottom
  collapse 결합 · writing-mode · multicol/fragmentation · 생성 콘텐츠. float/clear · writing-mode ·
  multicol 은 명시 미대상 (`block.rs` 모듈 doc).
