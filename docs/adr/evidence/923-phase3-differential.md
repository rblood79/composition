...
## 결과 — 42 케이스 전부 엔진 직결 ≤1px (round 9 + 후속 ① 수리 후)
...
self-collapsing) 3 케이스 RED 1 → 수리 13 → **42/42 pass**. 어댑터 대조군은 **발산 18 / 정합 24** — 현 IFC 시뮬레이션이 Chrome 과 갈리는 차원의 실측
(Phase 5 cutover 가 닫을 대상. "정합" 은 시뮬레이션이 우연히 맞는 차원 — round 9 추가
12 케이스는 프로덕션 어댑터가 엔진 block/flex 경로를 그대로 타므로 수리 후 전부 정합).
종전 "16/23 발산" 표기는 오집계 (r8m1 — 실측 15/23; 27 케이스 18/9; 39 케이스 18/21; 42 케이스 18/24).
...
- ~~`height: 0` 명시 self-collapsing 미분류~~ → **후속 ① 수리 13 으로 종결** (자식 solve 의
  ...
  잔여: 텍스트 leaf 에 `height: 0` 명시 + 텍스트 내용 (line box 있음 → 비-self-collapsing)
  은 leaf 규칙이 구분 못 한다 — 빌더에서 성립 경로 없음.
