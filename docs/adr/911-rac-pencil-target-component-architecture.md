...
| R-3 | collection Interactive Projected Tree(④.2)가 60fps + 깊은 노드 편집을 동시에 성립시키는지 미증명 — projected id ↔ canonical write target 분리 정합                                                                                                                                                                                                                                                  | **HIGH** | Gate G-projected — 10k row projected draw/hit ≤ window+overscan + 깊은 노드 클릭/drill-in/edit route + projected id 의 canonical 비유입. **닫힘 2026-09-27 — [ADR-150](completed/150-rac-pencil-residual-interaction-execution.md) G1 · G2 로 재정의**: 10k 행 → window 노드 상한 + 행 위치 = DOM (G1, 실 브라우저 oracle) · 가장 깊은 선택 / drill-in → 데이터 행 더블클릭 = 템플릿 origin 자식 선택 (G2) · projected id 유입 0 (G2 c). 60fps 대용량 스크롤은 목표에서 뺐다 (150 Consequences) |
| R-4 | **(재판정 2026-07-20 — ADR-150 A1 철회 반영)** Skia 상태 모델이 **선언적 state variant(selected/disabled)** 를 RAC `data-*` 와 동일 규칙으로 derive 하는지. ~~hover/pressed/focusVisible~~ 는 pointer 실시간 재현이라 **Preview(DOM, D1 RAC 자동 소유)의 영역** — 빌더(Skia)는 화면 정의·구성 surface(Pencil app 동형)이지 프론트엔드가 아니므로 시뮬레이션하지 않는다(ADR-150 A1 철회 `5e635ebbc`) | **MED**  | Gate G-state(재정의) — selection 컴포넌트 fixture 가 Builder Skia ↔ Preview DOM 의 **selected/disabled** 상태 시각 parity. hover/pressed 는 대상 아님(DOM 소관)                                                                                                                                                                                                                                                                                                                       |
...
잔존 HIGH 위험: R-1 / R-2 / R-3 (3건 — R-4 는 2026-07-20 재판정으로 MED 격하: hover/pressed 축이 Preview 소관으로 이관돼 미증명 HIGH 영역이 아님). 모두 목표 구조의 미증명 영역이며 각각 증명 Gate 와 1:1 대응한다. **R-3 은 2026-09-27 [ADR-150](completed/150-rac-pencil-residual-interaction-execution.md) G1 · G2 로 닫혔다** (남은 미증명 HIGH: R-1 / R-2). 전환 위험(기존 마이그레이션 / 정합성 회귀 / cutover 격리)은 본 ADR 에 없다 — ADR-910 이 보유한다.
...
| G-projected | Interactive Projected Tree (R-3)               | Skia collection row 내부 Text/Icon 클릭 → deepest projected 노드 선택, 더블클릭 → drill-in/data edit, style edit → template route. 10k row draw/hit 노드 ≤ window+overscan(60fps). projected id 가 canonical mutation/영속에 0건 유입. HC#4·HC#7 증명. **닫힘 2026-09-27 — ADR-150 G1 · G2 로 재정의** (row 내부 deepest 선택 · drill stack · 3-route registry 는 폐기, 데이터 행 더블클릭 → origin 자식 선택으로 대체) | 깊은 노드 편집 미달 — 구조 재검토 |
| G-state     | Skia 선언적 상태 모델 (R-4, 재정의 2026-07-20) | selection 컴포넌트 fixture 가 Builder Skia 의 **selected/disabled** 선언적 상태 시각을 Preview DOM 과 동일하게 derive(`data-selected`/`data-disabled` parity). ~~hover/pressed/focusVisible~~ 는 pointer 실시간 재현이라 Preview(DOM) 소관 — 빌더 게이트 대상 아님(ADR-150 A1 철회)                                                                                                                                     | 상태 모델 재설계                  |
