...
7. **(r1) `INLINE_BLOCK_TAGS` 는 두 개념의 겸용이다.** 기본 display 목록이자 intrinsic width/height 측정을 켜는 escape 목록 (`utils.ts:4570-4572 needsWidth`). ProgressCircle/CalendarGrid/DateInput 같은 self-render leaf 가 들어 있는 이유. 손 목록 24 vs catalog inline-\* 선언 29건 (Phase 0 실측 2026-08-31 — r1 의 30 은 재집계로 29; [evidence/923-phase0-inventory.md](evidence/923-phase0-inventory.md) §A-4) — "catalog 파생 == 손 목록" 은 다른 개념의 비교다.
...
2. **Canvas 전용 display override 0**: catalog `containerStyles.display` 가 DOM CSS 의 display 와 다른 rule 수 0 (현재 Button·ToggleButton 확인, 전수는 Phase 0).
...
2. 새 모델이 아니라 **있는 모델의 배선**이다. `display.rs` 가 outer/inner 를 이미 구현하고, `block.rs` 가 line box 를 이미 구현한다. 비용은 tree.rs 의 문자열 매칭 4곳(`:1185/:1373/:2552/:4609`) 을 `display.rs` 호출로 바꾸는 것, baseline 출력 필드 하나, 그리고 (r2) TS 가 outer 를 지우는 3 사이트(`TaffyFlexEngine.ts:110-117` / `fullTreeLayout.ts:1060,1084` / `taffyDisplayAdapter.ts:510-512`) 를 CSS 값 통과로 바꾸는 것이다.
...
| R11 | (r2) TS flex/grid 분기가 inline-\* 의 outer 를 지운다 — `fullTreeLayout.ts:1060,1084` / `TaffyFlexEngine.ts:110-117` / `taffyDisplayAdapter.ts:510-512`. 제거 목록에서 빠지면 cutover 후에도 엔진이 `flex` 를 받아 Button 이 block item |   MED    | Phase 5 제거 목록에 3 사이트 + 운반 타입 union 2곳(`layoutTypes.ts:11` / `taffyDisplayAdapter.ts:124`) 확장 포함 (r3). G0 (a) 가 wasm 경계에서 검출, HC1 테스트가 Button 경로를 상시 대조 |
...
| G4   | Phase 4 종료 · Phase 5 종료 | (전반) 패널 Direction·Alignment 가 `inline-flex` 를 flex 로 표시하는 테스트 통과, catalog 무변경. (후반) catalog Canvas 전용 display override 0 (HC2) 이 cutover 와 **같은 commit** 에서 성립 + `blockInlineProbe` 로 block 부모 + Button 2 가 한 줄                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | 패널 테스트 실패 시 catalog 변경 보류(둘 다 revert)                                |
