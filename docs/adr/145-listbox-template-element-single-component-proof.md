...
| G1   | Phase A 완료               | ListBox factory `ListBoxItem` template element 자동 생성 + canonical document 직렬화 round-trip PASS                                                                                                                                     | factory 자식 자동 생성 회귀 시 `SHELL_ONLY_CONTAINER_TAGS` ↔ `SYNTHETIC_CHILD_PROP_MERGE_TAGS` 멤버십 재검토                               |
| G2   | Phase B 완료               | `ListBoxSpec.render.shapes` 가 `ListBoxItem` template element style 우선 소비 + Skia viewport intersection 적용 + cross-check ListBox CSS↔Skia 정합 PASS                                                                                 | `render.shapes` 가 `props.items` style 직접 hardcode 시 D3 SSOT 위반 — 정정 후 재진행. viewport intersection 회귀 시 별도 commit 으로 정정 |
