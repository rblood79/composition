...
4. layoutVersion / sceneVersion 계약 준수 — override 편집·breakpoint 전환이 재계산을 확실히 트리거 (`LAYOUT_PROP_KEYS` + projection signature 동시 등재).
...
| ID  | 위험                                                                                                                                                                                      | 심각도 | 대응                                                                                                                                                                                                        |
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :----: | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| R1  | layoutVersion 3-심볼 체인 / sceneVersion projection signature 등재 누락 → override 편집·breakpoint 전환이 캔버스에 미반영                                                                 |  HIGH  | `LAYOUT_PROP_KEYS` + `buildSceneStructureSnapshot()` signature input 동시 등재를 정적 테스트로 가드 (G1)                                                                                                    |
...
| ---- | ------------------- | -------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------- |
| G1   | Phase 2 완료 커밋   | 정적 테스트: `LAYOUT_PROP_KEYS` 에 responsive 시그니처 + signature input 등재 확인 (fullTreeLayout.static.test.ts 패턴)                | 등재 후 재실행 — 미등재 상태로 phase 종결 금지 |
