...
| G1   | Phase 1 | unit (원복 RED): 두 해석기 체인 해석 · origin 편집이 체인 끝 instance 에 전파 (캐시) · 순환/깊이 경고 · `enabled:false` 가 Canvas · Preview · 레이아웃 · 선택 · publish 에서 빠짐 · 상속된 `false` 를 체인 끝 `true` 가 풀고 조상 숨김은 못 풂 · patch `null` = 키 제거 · `fills: []` = 채움 없음 · 필드 부재 문서 Δ0                                                                                                                                                                                                  | Phase 1 롤백                                 |
