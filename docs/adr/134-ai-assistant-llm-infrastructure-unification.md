...
| 4     | **부분 반영 (08-28) — G4 미충족** | 도구 2종 신규: `bind_collection` (dataBinding 은 props 가 아니라 **extension** — `updateNodeExtension`, source 별 config 검증) · `create_interaction_rule` (ADR-158 `InteractionRule` + `capabilityRegistry` 로 trigger/capability 검증, 틀리면 사용 가능 목록 반환). 둘 다 **러너 경유** (ADR-184 — persist 포함). `systemPrompt` 에 frame/canonical/바인딩/이벤트 가이드. 은퇴 어휘 5종 0 grep gate. vitest 12 신규 + AI 영역 81 · type-check 0. **live**: AI 패널에서 두 도구 실행 → IndexedDB 문서에 규칙·바인딩 지속 확인 (검증 후 원복). **미충족**: "Preview 동작 / 데이터 렌더" — 아래 §Live Exercise 참조                                                                                                        |
| 5~9   | 미착수                            | Phase 3 (도구 canonical 어휘 확장) 부터. 원격 상용 provider 는 프록시가 생기기 전까지 차단 상태 — 프록시 도입은 별도 결정                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
**2026-08-28 (3차) · Chrome MCP** — Phase 4 G4 (부분).

- **AI 가 이벤트 규칙과 데이터 바인딩을 만든다** — 로컬 mock endpoint 를 물린 AI 패널 한 번의 대화에서 `create_interaction_rule`(Button `onPress` → toast) 과 `bind_collection`(ListBox, static 3건) 이 순서대로 실행됐고, **IndexedDB `documents` 레코드**에서 `events` 규칙과 ListBox `x-composition.dataBinding` 을 직접 확인했다 (persist 까지 도달).
- **G4 미충족 — 원인 확정**: 게이트가 요구하는 "Preview 에서 동작 / `useCollectionData` 로 데이터 렌더" 를 확인하지 못했다. `dataBinding` 은 **extension 전용 필드**라 legacy `elements` mirror 가 다시 파생돼야 빌더 캔버스·트리에 반영되는데, 그 재파생은 store 내부 `getCanonicalOrStoreElements` + `set` 조합 (러너 ② store 스테이지) 이고 **서비스 층에서 부를 수 있는 액션이 없다**. 러너 경유로 persist·인덱스 재빌드까지는 얻었지만 mirror 배열은 stale 로 남는다.
  - 필요한 것: store 층에 액션 1개 (예: `applyCanonicalExtensionPatch(elementId, patch)`) — 러너 store 스테이지에서 mirror 를 재파생. 서비스가 store 내부 헬퍼를 직접 꺼내 쓰는 것은 경계 위반이라 **사용자 판정 대상**으로 남긴다.
  - 이벤트 규칙 쪽은 legacy mirror 자체가 없어졌으므로 (ADR-158) Preview 의 canonical 구독만으로 동작해야 한다 — Preview 표면을 이 세션에서 열지 못해 미확인.
- **정리** — 만든 규칙 2건과 바인딩 1건을 문서에서 제거하고, 프로파일 설정·mock endpoint 를 정리했다 (요소 71 유지, `events: []`, 확장 필드 없음).
