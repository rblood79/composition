...
4. **rename 정합**: `data_tables` (DB snake) / `dataTables` (store camel) → `collections`. `DataTable` Pascal (Component / Editor / Panel / Type 이름 = UI/UX surface) **유지** (사용자 접근성 framing). canonical document schema / IndexedDB store / Zustand action API 모두 `collections` 단일 어휘
...
- **rename baseline** (frozen 예정): `data_tables` (snake) 13 파일 / `dataTables` (camel) 15+ 파일 / `DataTable` Pascal 30+ 파일 (Pascal 은 변경 제외 baseline)
...
| G6 (Phase 5) |           rename sweep land 직후            | `rg '\bdata_tables\b' -t ts` = 0 (theme/legacy 주석 허용 list 명시) + `rg '\bdataTables\b' -t ts` = 0 + `rg '"dataTables"' -t ts` (postMessage type literal) = 0. `DataTable` Pascal hit 수는 baseline 보존 (UI 어휘 유지) | rename 누락 site 보강 후 재검증                                                                  |
