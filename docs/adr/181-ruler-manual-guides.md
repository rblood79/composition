...
- **Phase 1 (Ruler 렌더 + 토글) Implemented 2026-08-13** — `rulerRenderer.ts` 신규 + 오버레이 패스 배선, 설정 패널 "Show Rulers" 스위치(정본, 기본 false) + 보조 단축키 Shift+R. 실측으로 드러난 결함 2건 동반 수정: 캔버스 full-bleed 로 좌측 패널이 세로 스트립을 덮던 문제(`canvasViewportInset.ts` 신규), 보이는 페이지 0개일 때 프레임 전체 skip 으로 눈금자가 사라지던 문제(`clearFrameWithChrome` additive 경로). **잔존 — HC1(a) 미달**: `render.frame` 0.40ms(off) → 1.21ms(on) = 예산 4.9% (기준 1%). 기본 상태 증가분 0, rAF cadence 불변. 처리 3안은 breakdown §Phase 1 잔존 — **Phase 7 G5 전 판정 필요**.
