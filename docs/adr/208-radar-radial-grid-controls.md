...
| R7  | 조건 판정을 **원시 `props[key]`** 로 하면 기본값이 저장 안 된 노드에서 값이 `undefined` 가 되어 `oneOf` 가 전부 거짓이 된다 — 유효한 필드가 통째로 사라진다. `createDefaultChartProps` 는 `chartType:"bar"` 를 쓰지만(`apps/builder/src/types/builder/unified.types.ts:2200`) pencil import·수기 canonical 문서는 보장되지 않는다                                                                                                                                                                                                                                                                                   |   MED    | 판정 입력을 `ResolvedField.currentValue`(= override ?? contract.default, `resolveEditContract.ts:343`) 로 고정한다. P1 의 결선 계약에 명시하고, G2 오라클에 `chartType` 미저장 노드 1건을 넣는다                                                                               |
