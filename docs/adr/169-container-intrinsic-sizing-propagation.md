...
- **대안 B 기각**: 성능이 가장 싸지만 CSS 근사를 display 마다 손으로 유지하게 되어 유지보수 H. 본 엔진의 원칙(CSS 표준 의미론은 엔진 소유)과 충돌하고, §9.9.3·grid track sizing 과의 어긋남이 잔존 발산으로 계속 남는다.
...
| G2   | Phase 2 종료 | breakdown §2-2 발산 7형태가 green, 잔존 형태가 있으면 실측 수치로 명문화                                 | 근사 범위를 좁혀 재시도, 또는 §9.9.3 완전 구현으로 승격        |
| G3   | Phase 2 중간 | max-content 단독 반영 상태에서 긴 텍스트 초과가 **확대되지 않음**을 fixture 로 확증                      | 두 채널을 단일 커밋으로 묶어 중간 상태 자체를 제거             |
| G4   | Phase 1·4    | bench — 대표 문서 layout pass 회귀가 합의된 상한 이내 (상한은 Phase 1 baseline 실측 후 확정)             | 캐시 적중률 개선, 미달 시 대안 B(집계 근사) 로 fallback 재평가 |
