...
| HC2 | initial 번들은 ADR-201 재승인 상한 (2026-09-25 — Builder ≤ 1,415,000 / Preview ≤ 622,000 B gzip, 만료 2026-10-25, 착수 시점의 유효 상한으로 재확인) 을 넘지 않는다 — zip · 해시 · v2 reader/writer 는 lazy, initial Δ 는 detached baseline 대비 실측 | `docs/adr/completed/201-*.md` §initial 상한 재승인 |
...
| R2  | 자산 GC 가 아직 참조 중인 자산 삭제 (백업 ring · 히스토리 스냅샷 · undo/redo history entry · 메모리 history · 다른 프로젝트 참조 누락, mark 이후 새 참조) |  HIGH  | GC root = 전 프로젝트 문서 + 백업 + 스냅샷 + IndexedDB history entry payload + 메모리 history (진행 중 transaction 포함) + 폰트 레지스트리 (Phase 0 에서 참조 보유처 전수 고정). 두 번 연속 GC 에서 미참조 + 유예 기간 경과한 자산만 삭제. history 가 정리돼야 참조가 풀린다 (G3) |
...
| G3   | Phase 3 종료                 | GC 반증 각 1건 RED→GREEN — 백업에만 · 스냅샷에만 · 다른 프로젝트에만 · **history entry 에만 남은 자산 → GC → undo/redo 로 이미지 복원** · 메모리 history 에만 · mark 이후 새로 참조된 자산                                                                                                                             | GC 비활성 (공간 회수만 포기)                                                |
