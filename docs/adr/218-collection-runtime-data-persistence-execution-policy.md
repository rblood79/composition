...
4. **캐시 유효성 (h1)** — 별도 store 의 runtimeData 는 collectionId 만으로 복원해선 안 된다. 캐시는 자신을 만든 **source revision** 을 함께 저장한다. 지문은 endpoint **id 만이 아니라 요청 결과를 바꾸는 정의·실행 파라미터 전부** — `baseUrl + path + method + queryParams + bodyType/bodyTemplate + header 키 집합` (헤더 **값은 secret 이라 제외**, 값 대신 `{{secret.NAME}}` 참조/키만) + `responseMapping.dataPath` + schema field key/type 집합. define_endpoint 가 같은 id 로 path/query 를 바꿔도(`dataChange.ts:440-479`, 실제 요청은 `dataActions.ts:593-648`) 지문이 갈린다. hydration·소스 변경 시 현재 revision 과 불일치하면 폐기. field rename 은 캐시를 값 보존 변환(152 G4 정합), 그 외 정의·mapping·스키마 구조 변경은 무효화. Undo/Redo 는 revision 재계산으로 판정. 진행 중 요청은 **시작 시점 source revision** 을 들고 있다가 완료 수용 시 현재 revision 과 대조 — 불일치면 그 응답으로 캐시를 채우지 않는다(소스 변경 직전 응답 차단, h2 와 연동).
...
- **캐시 유효성 (h1, HC4)**: `collection_runtime` 엔트리에 `sourceRev` — endpoint id 만이 아니라 **요청 결과를 바꾸는 정의·실행 파라미터 전부**(`baseUrl + path + method + queryParams + bodyType/bodyTemplate + header 키 집합`, 헤더 값은 secret 이라 제외 — `{{secret.NAME}}` 참조/키만) + `responseMapping.dataPath` + schema field key/type 지문. define_endpoint 가 같은 id 로 path/query 를 바꿔도(`dataChange.ts:440-479` → 실제 요청 `dataActions.ts:593-648`) 지문이 갈린다. hydration·소스 변경 시 revision 불일치면 폐기, rename 만 값 보존 변환. 진행 중 요청은 시작 시점 revision 을 완료 수용 시 대조(소스 변경 직전 응답 차단).
...
| ID  | 위험                                                                                                                                                                                                                                                                                                                            |  심각도  | 대응                                                                                                                                                                |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :------: | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| R1  | **캐시 유효성** — 지문이 endpoint id 만이면 define_endpoint 가 같은 id 로 path/query/method/body 를 바꿔도(`dataChange.ts:440-479` → 실제 요청 `dataActions.ts:593-648`) revision 이 같아 다른 소스의 옛 캐시가 유효한 것으로 offline reload 복원. field rename(`dataChange.ts:679-687`)·스키마 변경도 섞임                     | **HIGH** | HC4 — `sourceRev` 를 요청 정의·실행 파라미터 전부(secret 값 제외)로 확장 + 진행 중 요청 시작 revision 대조. Gate G1 에 같은 id path/query 수정→offline reload       |
...
| G1   | Phase 1 완료 | `executionPolicy?` + runtimeData 별도 store 영속, 쓰기 전부 `applyDataChange`, runtimeData History 밖. **캐시 유효성(R1/h1)**: API 응답 저장 → field rename → offline reload 시 현재 key 로 값 표시(옛 key 섞임 0) · **같은 endpoint id 의 path 또는 query 수정 → offline reload 시 revision 불일치 무효화**(교체뿐 아니라 동일 id 내부 변경) · `responseMapping` 교체도 무효화 · Undo 로 스키마 복원 시 재판정. 기존 레코드 로드=manual(R7) · 삭제 시 캐시 정리 고아 0(R8)                                                                                                                                                                                                                          | store 생명주기/revision/적용기 재조정 |
