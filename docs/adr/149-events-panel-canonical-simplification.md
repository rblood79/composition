...
| dual-write                 | legacy 1차 기록 + `syncEventsToRootCollection` **one-way mirror** (legacy→canonical)                                     | `stores/utils/inspectorActions.ts:290,983-1028`                                 |
...
| R4  | legacy `props.events` read site 잔존 (elementDiff / workflowEdges / canonical adapters 등) 과 legacy write 제거 시점 경합 |  MED   | Phase 0 inventory 로 boundary allowlist freeze — allowlist 외 접근 grep 게이트 (G2), adapter/export 경로는 allowlist 내 유지 |
...
| G2   | Wave 1 반영 직후 | HC1 (depth≤2 Chrome MCP) + HC2/HC4 grep 0건 + HC3 (eventHandlers legacy 읽기 0건) + HC5 round-trip test + HC6 표면 보존 grep + **live behavior 1회** (바인딩 편집 → Preview 이벤트 실행) | 회귀 축 rollback (wave 단위 revert)    |
