...
| G2   | Phase 3 종료                  | 비대칭 19종(Phase 0 확정)의 Skia box 가 실효 DOM 값과 일치 (MenuItem `280×32` 등 — 차단 3층 L1·L2·L3 를 모두 열어야 성립, design §2-0). 기존 `resolveContainerStylesFallback.test.ts` 47 케이스는 갱신 후 GREEN (R8) | 경로 B 게이트 복원 후 개별 진단                      |
