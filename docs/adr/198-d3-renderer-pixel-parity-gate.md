...
| Phase 4b — cross-leg 예산 교정 (G3 positive) | **선행 조건 해소 2026-08-31** | 유보 사유였던 Skia 발산이 수리됐다 (§7 별도 작업, 사용자 승인 `657d80467`) — `FrameSpec.render.shapes()` 가 `props.style` 미독출 + `hexStringToNumber` 의 hex8 채널 시프트, 2겹. `basic-geometry-paint` Skia variance 0 → 5178.9 (Preview 5447.7), 세 좌표 모두 fixture 색과 일치. 예산 교정 자체는 미착수 |
