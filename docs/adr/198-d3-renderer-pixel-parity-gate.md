...
6. **Layered comparison** — matching node geometry must differ by at most
   **1 CSS px**. Normalized non-text regions use `pixelmatch` threshold `0.1`
   ...
   rather than a perceptual threshold.
...
| R13 | The gate rasterizes with `MakeSurface` (software) for determinism while users see `MakeWebGLCanvasSurface` (Ganesh/GL) output, so a green gate can coexist with a visible Builder↔Preview divergence in antialiasing, gradient dithering, blur/shadow, or clip edges. Boundaries: `createSurface.ts:29-33::createGPUSurface`, `SkiaRenderer.ts:1126` (`contentNode.renderSkia` into the GL main surface), `export.ts:70::exportToImage` (`ck.MakeSurface`). | **HIGH** | Phase 0 measures the SW↔GL delta on one pilot fixture and records it in the environment manifest; G0 blocks until the delta is inside the L3 budget or the gate's claim is explicitly narrowed to software rasterization. |

The selected alternative has HIGH operational risks R1-R5, R11, and R13. Each
