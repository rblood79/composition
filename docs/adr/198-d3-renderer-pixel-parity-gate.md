...
Accepted — 2026-08-31 (Proposed 2026-08-30 · review round 1 이슈 3건 전부 `fixed` · Phase 0 완료)
...
| Phase 4b — cross-leg 예산 교정 (G3 positive) | **완료 2026-08-31 (예산 무변경)** | 두 프로덕션 leg 을 같은 fixture 로 처음 맞댔다. 실측 결론은 "예산을 얼마로 하느냐" 가 아니라 **예산이 문제가 아니다** 였다 — 발산 3건 잔존: basic 채움 maxByte 145 (요소 경계 3px 바깥에도 남아 경계 밴드만으로 설명 안 됨), catalog L1 geometry Δx 140px·Δy 55px (레이아웃 발산이라 픽셀 층 미실행), image-raster diffRatio 0.914 (한쪽이 안 그리는 쪽에 가까움). 예산을 넓히면 셋 다 통과가 되어 게이트가 vacuous 해지므로 (R5) **예산은 선언값 그대로 두고** 현재 초과 region·층 판정을 ratchet 으로 고정. G3 positive 는 여전히 미충족이며 수치로 기록됨. 원인 규명은 §7 별도 승인 대상. `9e5c7ce50` |
| Phase 5 — CI · 개발 워크플로 통합                     | **부분 완료 2026-08-31** | 로컬 절반 충족 — doctor 우선 러너 + smoke/full (실측 smoke 72 tests/7.4s·full 88/8.3s, 예산 90s/300s), 경로-스코프 pre-push 게이트 (docs/stats push 0건 통과, D3 push 실행), `/cross-check` §5.6 라우팅. CI 절반은 **작성했으나 관측 못 함** — `deploy.yml` 이 `022f43c5a`(2026-07-06) 이후 없는 스크립트 `wasm:build` 를 불러 ~2개월째 실패 중이라 의존 job 이 오늘 막는 것이 없다 (수리는 배포 재개를 뜻하므로 별도 결정). G5 부분 충족 |
| Phase 6                                               | 미착수                   | —                                                                                                                                                                                                                                                                                                                                                                                                                                         |
