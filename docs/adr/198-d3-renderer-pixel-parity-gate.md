...
4. **Controlled environment** — CanvasKit/WASM hash, Chromium major, repository
   ...
   WebGL→SW fallback fails the case instead of producing pixels.
...
7. **Oracle sensitivity** — negative probes for a `1px` geometry shift, a
   one-token color change, a `1px` border/radius change, and a font metric change
   must each fail the appropriate layer.
...
| R11 | Both legs produce a degenerate frame (blank canvas, black WebGL capture, failed WASM/CSS load) and the comparison reports a perfect match. Boundaries include `createSurface.ts::createGPUSurface` silent SW fallback and `adr190-pixel-oracle.mjs::captureCanvas`. | **HIGH** | Frame liveness layer before L0 (painted node count ≥1, per-region variance floor), doctor fixture with expected pixel value, backend assertion in the environment manifest, and G0/G2. |
