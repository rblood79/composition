...
composition 의 element.type 어휘는 ADR-116 (`CompositionDocument` canonical SSOT) + ADR-122 (canonical-only runtime) + ADR-126 (Element 타입 boundary 격리) 정착 과정에서 **데이터 경로 분기** 가 영구화됐다. 같은 layout container (display:flex) 의도가 진입 경로에 따라 두 element.type 으로 분리되어 사용자 본인도 어휘 origin 을 헷갈리는 상태에 도달.

**의존 base 3 ADR (직교 응용 분류)**:

| base ADR | 도메인                                           | 본 ADR 과의 관계                                                                                                    |
| -------- | ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------- |
| ADR-116  | canonical schema                                 | schema 위에서 literal 만 정렬 (schema 변경 0)                                                                       |
| ADR-122  | runtime mutable mirror 제거                      | runtime hot path 의 element.type 읽기 경로 정합 (mirror 변경 0)                                                     |
| ADR-126  | Element TypeScript 인터페이스 boundary 격리 (D2) | 직교 — D2 인터페이스 shape vs D3 literal 값. Element 타입을 격리해도 element.type 의 vocabulary 값 정렬은 별도 작업 |
