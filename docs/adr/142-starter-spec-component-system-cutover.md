...
로 렌더하고 `SliderSpec.render.shapes()` 를 호출하지 않는 fixture 를 가진다. selection
family 잔여는 CheckboxGroup/Radio/RadioGroup 계열이다.
