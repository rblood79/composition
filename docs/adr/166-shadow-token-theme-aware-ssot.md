...
| Phase 3 (Skia 소비 배선)                   | 🟡 구현 완료 (G3 대기) | `resolveEffectiveBoxShadow` 신설(`resolveEffectiveOverflow` 동형 + theme 분기) → `buildBoxNodeData` / `buildSpecNodeData` 배선 · 파서 수정 0 · `StoreRenderBridge` spec 폴백 분기의 `theme` 누락 동반 수정. **G3 live 확인 미완** — 캔버스에 Popover/Tooltip/Modal 이 0건이고, 요소 추가는 현재 남아 있는 redo 항목을 잘라내므로 사용자 판단 대기 |                                                                                                                                                                                                    |
