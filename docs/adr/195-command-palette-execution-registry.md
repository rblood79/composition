...
2. **팔레트 실행 가능 = 등록된 전부** — 팔레트에서 골라 아무 일도 안 나는 항목은 `palette: false` 로 명시한 것(breakdown §3-4, 9개 상한) 외 **0**. 등록이 있고 scope 가 맞는데 실행되지 않는 항목 0. 현재 12/71 → 목표는 Phase 0 inventory 가 정하는 N/71 (등록 지점 기준 62 예상).
...
- 헤더 메뉴 "Shortcuts" 경유 열기(`open-command-palette` 이벤트) 는 popover 가 닫힌 뒤 발화하는지, 그 시점 scope 가 `modal` 로 잡히지 않는지 Phase 0 실측 대상.
...
5. **같은 세션의 표기 SSOT 와 형태가 같다** — 정의(무엇을) / 표기(어떻게 보이나) / 실행(무엇이 도나) 세 축이 모두 "한 곳에서 파생" 이 된다. 정적 게이트 조항 4 의 임시 봉합(패널 정의 → 팔레트 case) 을 "전 정의 등록 존재" 로 일반화할 수 있다.
...
| ID  | 위험                                                                                                                                                                    | 심각도 | 대응                                                                                                                                                                                   |
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :----: | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| R1  | 중복 id (`escape` 2곳) 에서 팔레트가 어느 핸들러를 부르는지 — 키보드는 둘 다 발화하지만 팔레트는 하나만 부른다                                                          |  MED   | 규칙 명문화: priority 내림차순 → 등록 순번 내림차순 (`resolve`). 단위 테스트로 잠금. `escape` 는 둘 다 무해(선택 해제/모달 닫기) 하고 Phase 0 에서 `palette:false` 후보이기도 하다     |
| R2  | `scopeAtOpen` 을 잘못 잡음 — 헤더 메뉴 경유 열기에서 popover 가 `modal` 로 판정되거나, 입력창 포커스 상태에서 ⌘/ 로 열면 `text-editing` 이 잡혀 캔버스 명령이 전부 흐림 |  MED   | Phase 0 실측 항목 (breakdown 체크리스트). `text-editing`/`modal` 이면 직전 non-modal scope 로 fallback 하는 규칙을 둘지 실측 후 결정. jsdom 테스트가 scope 별 executable 집합을 잠근다 |
...
| R4  | deps 변경으로 effect 재실행 시 해제→재등록 사이 팔레트가 열려 있으면 순간적으로 "미등록" 표시                                                                           |  LOW   | 팔레트 목록은 `getSnapshot` 구독이라 다음 렌더에 복구. 실행 시점에 `resolve` 를 다시 부르므로 stale 실행 없음                                                                          |
...
| ---- | ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ |
| G0   | Phase 0 | breakdown §2 재grep 5종 일치 · 헤더 메뉴 popover 가 `modal` 로 잡히지 않음 실측 · 연속 명령 12개 1회 실행 판정으로 `palette:false` 목록 확정 (상한 9)                                                                      | 표 갱신 commit 으로 흡수 (fork 아님). popover 가 `modal` 이면 `scopeAtOpen` 을 메뉴 열기 전에 잡는 경로 추가 |
...
| G2   | Phase 2 | switch case 0 · jsdom: scope 별 executable 집합 정확 (canvas/panel:styles/global 3 케이스) · 실행이 handler 를 정확히 1회 부름 · 정적 게이트 신조항 민감도 (등록 하나 제거 → RED, `palette:false` allowlist 밖 추가 → RED) | 실패 조항만 수정. `disabledKeys` vs `onAction` 필터는 실측으로 확정                                          |
...
- 실행 축이 SSOT 를 얻는다 — 정의(`SHORTCUT_DEFINITIONS`) / 표기(`formatShortcut`) / 실행(`commandRegistry`) 세 축 모두 "한 곳에서 파생". 새 정의 + 등록이면 팔레트에 자동 등장하고, 정적 게이트가 "등록 없는 정의" 를 잡는다.
...
- 중복 id 우선순위 규칙이 새로 생긴다 (R1). 키보드는 리스너별로 각자 발화하지만 팔레트는 하나만 부르므로, 같은 id 를 두 곳에 등록하는 것이 앞으로는 "팔레트에서 어느 쪽이 도나" 를 고려해야 하는 결정이 된다.
