...
- **후속 — 드래그 중 핸들 숨김 (2026-09-26, 사용자 확정 · Figma 어법)**: 신고 "핸들 위치와 드래그 중 마우스 위치가
  일치하지 않는다" — 값은 포인터 1:1, 핸들은 값의 절반 위치 (띠 중앙) 라 핸들이 절반 속도였다 (실측 마우스 +40 →
  핸들 +20). 시도 2개 모두 사용자 거절: ① 드래그 중에만 핸들을 가장자리 쪽으로 옮김 (`31c576e80`) — 놓으면 절반만큼
...
| 드래그                                                   | 시작 즉시 사선 제거        | 핸들 숨김 (종료 시 복귀, 2026-09-26) · 실시간 값 배지, 패널 해당 필드 동기 강조 |
