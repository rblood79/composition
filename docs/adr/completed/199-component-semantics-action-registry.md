...
| R7  | **사영이 시맨틱 필드를 싣지 않는다** — `ref` 노드의 캔버스 사영에서 인스턴스 자신의 `reusable` 이 사라져, Instance·Origin 노드의 우클릭 메뉴가 `컴포넌트 만들기` (no-op) 를 띄운다. 패널은 `Detach component` — 같은 노드, 반대 라벨 (2026-08-30 live 실측, `evidence/199-surface-inventory.md` §7 D7). 술어가 아니라 **입력을 만드는 쪽**의 결함이라 HC3 만으로는 안 닫힌다 |  HIGH  | **수리 완료 2026-08-30** (Phase 4, `canonicalRefResolution.ts:235` — 원본 축은 안 넘기고 인스턴스 자신의 `reusable` 만 보존). 사영 3종 fixture 게이트에 `reusable`/`ref`/`masterId`/`componentRole` 4필드 보존을 포함하고, `renderers/rendererInput.ts:476` · `skia/StoreRenderBridge.ts:242-281` 경로에서 누락 필드를 채운다 (G3-b) |
