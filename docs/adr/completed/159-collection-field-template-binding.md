...
- **P4c residual (사용자 확정 2026-07-24)**: `useCollectionData` api/variable/route 분기 + `ApiEndpointList`/`VariableList` 관리 UI 는 read 호환 잔존 (대안 D 상태 — Decision 기각 사유에 예정된 보류 경로). 재개 조건: G4 재실측 (Supabase 저장 문서 전수 0건 확증) 후 별도 커밋. 오소링 신규 기록은 P4b 로 dataTable 단일 고정이라 잔존 경로의 신규 유입은 0.
