...
Implemented — 2026-07-24 (P0~P5 완결, execute-adr 3세션. **P4c residual**: api/variable/route 잔존 경로 물리 제거는 G4 미충족 — Cloud 저장 문서 전수 실측 불가(RLS) — 로 대안 D 상태 보류, 사용자 확정 2026-07-24 "P4c 는 residual 로 두고 P6 진행". 리뷰 round 1 2026-07-21 / round 2 2026-07-23 승인)

### Phase 진행 로그

- Phase 0 (inventory) — Implemented 2026-07-24: api/variable/route 소비처 12분류 + columnMapping 5분류 전수 grep, 로컬 저장 문서(1+백업 10) dataBinding 0건 확증. Cloud 전체 실측은 RLS 로 G4(P4c 진입 전) 이연. 발견: DataTable factory 기본값이 `source:"api"` 생성(P4b 전환 필수). 상세: breakdown §5
...
- **ADR-214 (Implemented 2026-09-14)**: `PropertyDataBinding.source:"variable"` read 호환은 그대로다 — 214 는 이 경로를 부활시키지 않고 `{{ name }}` 템플릿 (string prop) + `setState` 액션으로 변수를 소비한다. `{field}` (행 문맥) 과 `{{ }}` (상태) 는 별개 문법 — 한 문자열에 둘이면 `{{ }}` 먼저.
- **P4c residual (사용자 확정 2026-07-24)**: `useCollectionData` api/variable/route 분기 + `ApiEndpointList`/`VariableList` 관리 UI 는 read 호환 잔존 (대안 D 상태 — Decision 기각 사유에 예정된 보류 경로). 재개 조건: G4 재실측 (Cloud 저장 문서 전수 0건 확증) 후 별도 커밋. 오소링 신규 기록은 P4b 로 dataTable 단일 고정이라 잔존 경로의 신규 유입은 0.
  - **정정 (2026-07-24, 사용자 승인 삭제)**: 잔존 목록에 있던 preview `useDataSource` 4-way dispatch 는 **G4 대상이 아니었다** — `apps/builder/src/preview/hooks/` 전체(`useDataSource.ts` + 배럴 `index.ts`)가 export 8종 소비처 0건 / 배럴 importer 0건 / 별칭 import 0건인 **dead module** 로 실측됐다. 소비처가 0이면 저장 문서가 이 경로에 의존할 수 없으므로 read 호환 리스크가 성립하지 않는다. 두 파일 삭제 완료 (type-check 61 baseline 유지, builder vitest 347 files / 2600 tests PASS). G4 게이트는 나머지 항목에만 적용된다.
