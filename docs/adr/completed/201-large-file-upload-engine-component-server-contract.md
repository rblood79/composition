...
1. **의존·번들** — 신규 런타임 의존 **0**. 엔진 core+tus ≤ **6KB gz**, IIFE 전체 ≤ **10KB gz**. initial 번들은 저장소 정본 절차 (착수 시 ADR-202 재승인 **Builder ≤ 1,319,829 / Preview ≤ 592,000 B gzip, 만료 2026-10-16** → **2026-09-17 201 재승인 1,328,315 / 601,346 로 대체** → **2026-09-25 재승인 1,415,000 / 622,000, 만료 2026-10-25**, §initial 번들 상한 재승인) 를 따른다 — 등록 8지점 (catalog·binding·rule·factory·palette) 은 정적 import 라 Builder initial 에, `renderFileUpload` shell 은 shared rendererMap 이라 Preview initial 에 실리므로 **Δ 0 은 불가**. 통과 조건은 ① 엔진 chunk 가 initial closure 밖 (`initialFiles.has(uploadEngineChunk) === false`, 첫 파일 선택 시 `import()`) ② 등록·shell Δ 를 같은 디렉터리 detached checkout baseline 로 실측해 **ADR-201 initial 상한 재승인 절** (Builder/Preview 절대 상한 + 만료일, 사용자 승인) 에 기록 ③ `adr201-bundle-gate.mjs` (202 판정기 동형 + ①) PASS. publish 앱 Δ 는 gate 밖 참고치 (link-only 방침). (review round 1 h2 — 2026-09-17: 500KB 예시 인용과 SKILL.md 오인용 제거)
...
**initial 상한 재승인 (2026-09-25, 사용자 판정 "재승인 + 패널 lazy 감량")** — Builder ≤ **1,415,000** / Preview ≤ **622,000** B gzip, 만료 **2026-10-25** (09-17 값 1,328,315 / 601,346 대체). 판정기 `adr201-bundle-gate.mjs` 상수 갱신 · self-test PASS. 실측 (main `4ad12dbe3`, 별도 worktree clean 빌드 · `adr209-bundle-closure.mjs`): Builder **1,408,688** · Preview **618,865**. 09-17 (`5edba0138`) 이후 누적 증가 Builder +80,373 · Preview +47,395 를 sourcemap 으로 원본 모듈별로 분해했다 — Preview 는 builder 저작 코드 누수 (`slotHostPolicy` · `stateVariantOrigins` 가 id 상수 하나씩을 origin 빌더 모듈에서 가져와 factory 정의 · hydration 이관이 실림, +31 KB) 를 먼저 수리 (`4ad12dbe3`, −29,760 · 가드 `previewBuilderAuthoringImport.static`). 남은 증가는 ADR-222 ~ 241 의 본 기능 코드 (canvas scene/layout · origin 체계 · 패널) 이고, 초기 화면 밖 패널 (datatable · themes · history · interactions · fonts, 합 ~20 KB) 을 모두 lazy 로 돌려도 09-17 상한 아래로는 내려가지 않는다 — 패널 lazy 분리는 새 ADR 로.
