...
| Phase 1 (토큰 theme-aware + 스케일 재정의) | ✅ 2026-07-25 | SP2 3단계 + dark ×3 · `xl`/`focus-ring` 제거 · **패널 프리셋 축소를 Phase 2 → 1 로 당김** (토큰만 제거하면 패널이 리터럴 `"xl"` 을 boxShadow 로 기록하는 파손 상태가 되어 phase 경계가 성립 안 함)                                                                                                                                                                                                                                                                                                                                                                            |
| Phase 2 (값 언어 TokenRef 통일)            | ✅ 2026-07-25 | overlay 3건 → `{shadow.sm/md/lg}` · `emitContainerStyles` 토큰 해석 경유 · **§1-3 `sm` 소비처 8건 전부 `sm` 유지 판정** (예상이던 `none` 다수는 실측으로 반증 — `--bg-overlay` ≡ `--bg-inset` 이라 칩이 소실) · R1 역매핑 light+dark 인덱싱 · 패널 preset 의 TokenRef 해석 신규 파손 1건 동반 수정                                                                                                                                                                                                                                                                            |     |
| Phase 3 (Skia 소비 배선)                   | ✅ 2026-07-25 | `resolveEffectiveBoxShadow` 신설(`resolveEffectiveOverflow` 동형 + theme 분기) → `buildBoxNodeData` / `buildSpecNodeData` 배선 · 파서 수정 0 · `StoreRenderBridge` spec 폴백 분기의 `theme` 누락 동반 수정. **G3 통과** — 임시 프로젝트에서 Popover/Tooltip/Modal 3종의 Skia 노드 effects 가 light·dark 양쪽에서 토큰값과 정확히 일치(inline 없음 = 순수 catalog fallback)                                                                                                                                                                                                    |
| Phase 4 (primitive 은퇴)                   | ✅ 2026-07-25 | `popover_shadow` / `dialog_shadow` + 두 binding 항목 제거 · 절대값 단언 → **registry 부재 단언** 전환. **G4 통과** (아래) · **선행 전제 1건 정정**: 두 primitive 는 캔버스에 닿은 적이 없었다 — `target:"bg"` shadow 는 bg 가 root 로 추출되면 `nodeById` 의 spread 사본에 push 되고 root 조립부(`specShapeConverter:206-216`)는 `bgBox`/`children` 만 읽어 버린다(border 는 write-through 분기가 있으나 shadow 는 없음). 따라서 Phase 4 는 이중 그리기 해소가 아니라 **죽은 코드 제거(시각 변화 0)**, Phase 3 이전 Popover 도 Tooltip/Modal 과 같이 캔버스 그림자 공백이었다 |
| Phase 5 (검증·가드)                        | ✅ 2026-07-25 | **cross-check 8조합 전수 통과** (Preview probe computed ↔ Skia effects — Popover/Tooltip/Modal = `md`/`sm`/`lg`, Dialog = 양쪽 모두 없음, σ=blur/2.355 변환까지 일치, 테마 전환 리로드 불요) · 가드 4파일 = catalog 값 언어(`boxShadow` 키 이름 깊이 탐색) + **CSS↔토큰 수치 대칭(계획 외 추가 — 서로 참조하지 않는 손-유지 사본 2벌의 조용한 발산 차단)** + generator 경유(source-order) + `resolveToken` theme 분기 · specs 662 / shared 749 PASS · type-check PASS                                                                                                         |
...
| G3   | Phase 3 종료             | **✅ 통과 — 2026-07-25 실측** (임시 프로젝트 `adr166-shadow-check`). light: Popover `dy4/blur12/α.08 + dy2/blur6/α.04 + dy0/blur2/α.12` = `lightShadows.md` · Tooltip = `.sm` · Modal = `.lg`. dark: 전 레이어 α ×3, 기하 동일 = `darkShadows.*`. 테마 전환에 리로드 없이 반응(메모이즈 theme 고착 없음). Tooltip/Modal 은 **캔버스 그림자가 처음으로 표시**                                                              | (통과 — N/A)                                                                  |
| G4   | Phase 4 (primitive 제거) | **✅ 통과 — 2026-07-25 실측** (임시 프로젝트 `adr166-shadow-check`). 제거 후 Popover 노드 = catalog 3레이어 유지(`dy4/σ5.10/α.08 + dy2/σ2.55/α.04 + dy0/σ0.85/α.12` = `lightShadows.md`, inline 없음) + `line` 자식 2 (=`popover_arrow` 생존). Dialog = effects 0 + `box` 자식 1 (=`overlay_backdrop` 생존) — DOM `Dialog.css` box-shadow 0건과 대칭. 이 **차등**(Dialog 만 0)이 신규 dist 가 실제 로드됐음을 동시에 증명 | primitive 존치 + Phase 3 배선 재점검. 제거는 다음 사이클로 이월               |
