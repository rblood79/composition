...
> **G0 조항 축소 (2026-08-31, 사용자 결정)**: G0 는 원래 파일럿이 "기대 픽셀 값으로"
> 두 leg 에서 PNG 에 도달할 것을 요구했다. Phase 0 실측에서 두 leg 이 한 fixture
> (checksum `1ad05caa`) 로 각각 PNG 에 도달했으나 **결과가 불일치**했다 — Preview 는
> 3/3 노드를 칠하고 Skia 는 균일 백색이다. 이는 host 나 seam 의 결함이 아니라
> 하니스가 잡아낸 D3 발산이므로, G0 의 실패 경로("revise host/seam")가 적용되지
> 않는다. 따라서 G0 는 **도달 가능성**(두 leg 이 한 checksum 에서 PNG 을 낸다)까지로
> 좁히고, **픽셀 일치**는 원래 그 판정을 맡도록 설계된 G3 의 L3 로 넘긴다.
> Skia 백색의 원인 규명은 §7 에 따라 별도 작업이며 Phase 1 을 막지 않는다.
...
| Phase 4b — cross-leg 예산 교정 (G3 positive)          | **완료 2026-08-31 (예산 무변경)**                       | 두 프로덕션 leg 을 처음 맞댄 결과 "예산이 문제가 아니다" 로 결론 — 발산 3건을 ratchet 으로 고정 (`9e5c7ce50`). 이후 사용자 승인으로 원인 규명 착수: **basic 은 수리 완료** — 프레임 테두리 미방출(배경과 같은 자리)이 원인이었고, 고친 뒤 예산 한 줄 안 건드리고 L3 통과 (ratio 0.0221→0.00000 / 0.0557→0.00068, `866b428fa`). **catalog 는 기전 확정, 수리 방식 결정 대기** — 명시 폭 block 형제 + inline-level 형제 조합에서 IFC 시뮬레이션이 줄을 안 나눈다 (`0aa52b68a`). text/raster 는 미착수                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| Phase 5 — CI · 개발 워크플로 통합                     | **부분 완료 2026-08-31 · CI 절반 실측 정정 2026-09-07** | 로컬 절반 충족 — doctor 우선 러너 + smoke/full (실측 smoke 72 tests/7.4s·full 88/8.3s, 예산 90s/300s), 경로-스코프 pre-push 게이트, `/cross-check` §5.6 라우팅. CI 절반은 08-31 기록 ("deploy.yml 이 없는 `wasm:build` 를 불러 관측 못 함") 이 **stale** — `7334e8668` 로 같은 날 수리됐고 smoke job 은 09-01 부터 D3 push 마다 실행됐다. 실측 (2026-09-07, GitHub API): scope=true run 의 smoke 는 **Linux runner 에서 0/N 통과** (성공 run 은 전부 scope 밖 skip), 로컬 macOS pre-push 는 매번 통과 → 환경 발산이며 G2 결정성은 macOS 에서만 측정됐다. 09-05 추가된 ADR-205 G4 drift step 이 scope `if:` 가드 없이 의존성 미설치 run 에서도 돌아 prettier import 로 실패 → **deploy 는 09-04 19:11 이후 전부 실패** (gh-pages builder 배포 0건, publish 보류 방침이라 현재 비용 0). 로그는 `gh` 미인증 (익명 API 403). 재개 시: drift step 가드 1줄 → smoke 를 deploy `needs:` 에서 빼거나 `continue-on-error` → Linux 에서 G2 10-run 결정성 재측정 후 되돌림. 폰트 hinting 발산이면 CI 는 L1 까지만 차단하고 L3/L4 는 artifact 보고. G5 는 로컬 절반만 충족 |
| Phase 6 — 대표 D3 matrix + 종결                       | **보류 2026-09-07** (사용자 결정)                       | 위 Status 재판정 참조. G6 는 "실제 CI run 통과" 를 요구하는데 Linux 0/N 이라 matrix 를 늘려도 종결에 닿지 못한다                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| R14 | The host returns element screenshots at a scale other than 1:1, so the two legs are compared at different resolutions and resampling error hides inside the budget. Boundary: `@vitest/browser-playwright::getIframeScale` — Vitest sizes the tester iframe to `browser.viewport` and scales it with CSS to fit the real window.                                           |         MEDIUM (found in Phase 3)          | **Measured 2026-08-31**: with the default 414x896 viewport a CSS 240x180 element captured as 193x145 (scale 0.804); at 1280x900 it was 192x144 (0.800); at 1280x720 it was 240x180 (1.000). `devicePixelRatio` was 1 on both sides and `getBoundingClientRect()` inside the iframe reported the unscaled values, so no geometry or DPR check sees it, and the downscale is systematic rather than flaky so every determinism gate stays green. Mitigation: viewport pinned below the window size in `vitest.visual-parity.config.ts`, plus a per-run `shot.width === rect.width` assertion so a smaller-window host fails loudly. |

The selected alternative has HIGH operational risks R1-R5 and R11 (R13 was
measured down to MEDIUM in Phase 0, R14 was found in Phase 3 at MEDIUM). Each
has a blocking Gate below; none may be accepted as a permanent flaky baseline.

## Gates

| Gate | Timing                             | Pass condition                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Failure path                                                                                       |
| ---- | ---------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| G0   | Phase 0 inventory                  | Current Skia scene entry, Preview canonical message entry, font/image readiness, wall-clock time sources, surface backend selection, package ownership, and existing oracle overlap are frozen with file/function evidence. The doctor fixture matches its expected pixel value, and a no-text pilot reaches PNG on **both** legs from one fixture checksum without production behavior changes — pixel _agreement_ between the legs is judged by L3 in G3, not here. The SW↔GL raster delta is measured per paint family with the amplitude ceiling recorded. (R11/R12/R13) | Keep ADR Proposed; revise host/seam before implementation.                                         |
