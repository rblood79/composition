...
- **후속 — WebKit (Safari 엔진) 의 다시 시도 (Playwright WebKit, `adr242-live.mjs --only-l6 --webkit`)**: 복구되지 않는다. WebKit 은 실패한 module URL 을 문서 밖에 기억해 **새로고침 뒤에도** 같은 URL 의 `import()` 가 요청 없이 실패한다 (진단 `--probe`: 같은 URL `fetch` 200 · `import` 실패 · `?r=1` 을 붙인 `import` 성공). 실패 방식 3가지 (네트워크 실패 · 404 응답 · 서버에서 파일을 치웠다가 되돌림) 모두 같다. Chrome 은 3가지 모두 새로고침으로 복구. 배포 교체 (옛 chunk 404) 는 새로고침이 새 해시 URL 을 받아 복구되지만, 일시적 네트워크 실패는 idle 선로드 한 번의 실패로도 그 탭에서 패널이 계속 열리지 않는다. 수리 방향 (다시 시도가 다른 URL 로 받기 등) 은 사용자 판단 대상. Firefox 는 이 환경에서 실행 불가.
- **후속 — 폰트 디코드 경고 원인**: ADR-235 · 이 ADR 과 무관한 기존 결함. `apps/builder/src/index.css` 의 `@import "pretendard/.../pretendard.css"` 가 builder `main-*.css` 에 URL 재작성 없이 인라인돼 `./woff2/` · `./woff/` 상대 경로가 `/composition/assets/woff*/` 로 풀리고 404 (HTML 응답) 가 된다. 같은 CSS 를 JS 로 import 하는 Preview (`preview-*.css`) 는 해시 경로로 정상.
