...
- **후속 — lazy 패널을 연 채 저장된 레이아웃으로 부팅 (사용자 확인 대상 → 확인, 2026-09-26)**: (1) 사용자 Chrome 프로필 (dev 서버 · 실제 프로젝트) 에서 데이터 · 테마 · 설정 · 인터랙션 · 히스토리를 연 채 새로고침 → 다섯 패널 모두 실제 내용 (사용자 히스토리 50건 등) · 로딩 표시 · 오류 표시 0 · 콘솔 오류 0. 창이 가려져 (`document.hidden`) 캔버스 부팅은 95% 에서 멈춰 있어 이 부분은 (2) 로 봤다. 확인 뒤 사용자 레이아웃을 원래 값으로 되돌림 (바이트 동일). (2) production 빌드 Chrome · WebKit: 같은 다섯 패널 저장 → 새로고침 → 부팅 중 패널 골격 안 스피너를 잠깐 거쳐 약 0.5 s 에 다섯 패널 내용 · 캔버스 부팅 완료 · 패널 chunk 요청 5 · 오류 표시 0 · page error 0. 관측된 404 는 패널과 무관한 `/appIcon.svg` (production `/composition/` 경로를 빼고 요청 — 헤더 로고 · 파비콘이 production 에서 깨짐, 기존 결함).
