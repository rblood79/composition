...
Implemented — 2026-08-29 (**delivered scope = Phase 0–8**, 사용자 confirm 2026-08-29). Phase 9 (외부 코딩 에이전트 ACP/SDK embed + MCP server, **G7**) 는 **Electron 마이그레이션 반영 시점의 별도 ADR 로 이관** — 착수 전제가 문서상 "미확정" 이 아니라 **코드에 실재하지 않는다** (electron 의존성 0건 · `apps/desktop`/`electron` 디렉터리 부재 · 전용 ADR 부재, 2026-08-29 실측). G7 은 "embed 1종 이상 + MCP 도구 표면으로 문서 조작 실측" 이라 embed 없이는 **측정 자체가 성립하지 않고**, Phase 9 산출물 4개 중 웹 단계에서 가능한 유일 항목 (MCP 호환 도구 표면) 은 Phase 3 이 이미 흡수했다 (`toLLMToolDefinitions()` 가 내는 `{name, description, parameters}` 가 곧 MCP tool schema 형태). 이 처분은 ADR 자신의 "Phase 9 실패 시 대안" (Phase 1-8 자립 운영 유지) 과 같은 결론이며, ADR-149 (2026-07-19) 의 delivered scope 종결과 같은 형태다. **잔여 3건 종결 (2026-08-29, Ollama 0.33.0 + qwen3:14b 실측)**: ① **G6 통과** — 실물 endpoint wire 정합 확인 ② **G5 통과** — props 키 정확도 18/18 (1.000) ③ **Phase 6 시나리오 부분** — 4기준 중 3 충족, '도구 오류 0' 미충족 (A 2건 · B 7건, 전부 동일 원인). 상세는 §Live Exercise 8차. Accepted 2026-08-28 (리뷰 round 2 승인 — round 1 이슈 8건 HIGH 1 / MED 3 / LOW 4 전부 fixed, pending 0) · Proposed 2026-05-13
**노선 개정 — 2026-08-18**: 자체 로컬 LLM 내장 노선 (Ollama 1st + node-llama-cpp Electron 내장 + Qwen 고정) 을 폐기하고, reference 수렴 노선 (**에이전트 중심 멀티 프로바이더 BYOK + 외부 코딩 에이전트/MCP 준비**) 으로 교체. **벤더 SDK 완전 제거 방침은 유지**. 근거: [PENCIL_ECOSYSTEM_ANALYSIS.md](../../explanation/research/PENCIL_ECOSYSTEM_ANALYSIS.md) (2026-08-17 갱신) + [HOLAOS_ANALYSIS.md](../../explanation/research/HOLAOS_ANALYSIS.md) (2026-08-18) + [XAI_ORG_ANALYSIS.md](../../explanation/research/XAI_ORG_ANALYSIS.md) (2026-08-18 — grok-build 를 5번째 수렴 사례로 추가). 본 개정은 전제 확정 종결 계약의 재개 조건 (a) 사용자 재제기에 따른 것 — 통합 형태 (단일 ADR, 대안 A) 결정은 유지하고 **인프라 노선만 재결정** (§인프라 노선 재결정 2026-08-18).
...
| Phase | 상태                            | 근거                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| ----- | ------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 0     | Implemented (08-28)             | G0 통과 — inventory baseline freeze. **클라우드 LLM 결합이 `AgentService.ts` 1개 파일에 국소화** (값 import 1 · type-only 2 · `dangerouslyAllowBrowser` · 모델 id · env 키 전부 동일 파일, 서비스 소비자 `useAgentLoop` 1곳) · **`AIAgentProvider` 인터페이스는 존재하나 소비자 0** (`ai.types.ts:59`, dormant — Phase 1 은 추상화 신설이 아니라 소비 경로 전환) · **Cloud Edge Function 인프라 미존재** (D10 프록시는 Phase 2 에 배포 스캐폴딩 포함) · `systemPrompt.ts` 이미 provider 중립 · 회귀 gate baseline `Transform`/`props.events`/`group_N`/`SerializedEvent` 전부 0 · scope inflation Phase 1~4·8 전부 1.5x 미만. 산출물 `~/.claude/plans/adr-134-baseline-inventory.md`, breakdown §2 Phase 0 실측 결과 · §12 |
...
- **번들 grep** — 프로덕션 산출물에 이전 벤더 SDK · `dangerouslyAllowBrowser` · 원격 LLM endpoint · 빌드타임 API 키 **전부 0건**.
...
**2026-08-29 (8차) · Ollama 실물** — 잔여 3건 종결 측정. 환경: ollama 0.33.0 · qwen3:14b (9.3GB) · M4 Pro 24GB · ~19 tok/s · **n_ctx=32768**.

- **측정 무효를 먼저 잡았다** — ollama 기본 `n_ctx` 는 4096 인데 주입 시스템 프롬프트가 **6,454 tok** 이라 카탈로그가 잘린 채로 측정되고 있었다. 절단 상태의 Button props 는 `text` (오답), 32768 로 올린 뒤 같은 요청이 `children`+`variant` (정답). **대조군이 성립하므로 아래 수치는 vacuous 하지 않다.** 이 함정은 [로컬 endpoint 가이드](../../how-to/development/ai-local-endpoint.md) 에 경고로 기재.
- **G6 통과** — Ollama OpenAI 호환 `/v1` 의 wire 가 `OpenAICompatibleProvider` 파싱과 일치: `tool_calls`(index·id·function.arguments 조각) + `finish_reason:"tool_calls"` + `[DONE]`. 역할별 프로파일(planner/executor/verifier) 라우팅 동작, 도구 왕복 성공, 캔버스 실제 변경.
- **G5 통과 — props 키 정확도 18/18 (1.000) ≥ 0.90**, enum 값 6/6. 표본 10 요청 (Button/TextField/Checkbox/Select/Badge/Slider/Switch/Heading/ProgressBar/frame). oracle = `resolveEditContract` 의 semantic 필드 키 (셀렉터와 독립). 모델이 RAC 어휘를 그대로 쓴다 — `isSelected`(≠checked) · `minValue/maxValue`(≠min/max) · `isDisabled`(≠disabled) · `labelPosition`/`selectionMode`.
- **Phase 6 시나리오 2건 — 4기준 중 3 충족**. A 사용자 관리 대시보드: 계획 6단계 · 도구 28 · **오류 2** · 검증 5건 지적 → 수리 1 → 3건 지적 → 수리 2 → **이상 없음 수렴** · 32 요소 (Heading + Table/TableHeader/TableBody + Pagination + ProgressBar×3 + Metric + Button×9). B 이커머스 카탈로그: 계획 6단계 · 도구 38 · **오류 7** · 수리 2회 소진 후 **6건 지적 잔존** (bounded repair 상한 도달 → 사람에게 넘김, 설계대로) · 41 요소 (Heading + Table + Card/GridList ref×5 + Pagination + Image + Metric).
- **오류 11건 전부 같은 원인 (재현 3/3 회)** — 모델이 `create_element` 가 돌려준 `data.elementId` 대신 **자리표시자 id** 를 다음 도구에 넘긴다: `created-element-id` / `gridListId` / `paginationId` / `cardId` / `created_table_id` / `Table-123` / `Pagination-789`. 실패는 안전하게 처리되고(도구가 "요소를 찾을 수 없습니다" 반환) 모델이 `get_editor_state`/`search_elements` 로 복구하지만, 턴과 시간을 크게 낭비한다. **원인은 프롬프트 공백**: `systemPrompt.ts` 는 `elementId: "selected"` 만 안내하고 "생성 결과의 elementId 를 이어서 쓰라" 는 규칙이 없다. 수정은 별도 작업.
- **정리** — 프로파일은 로컬 endpoint 구성 그대로 유지 (사용자 요청으로 설치·구동). 생성된 요소는 로컬 검증 프로젝트 데이터라 그대로 둠.
...
ADR-054 Proposed (2026-04-05) 의 대안 A (Ollama → node-llama-cpp + 온라인 모델 선택) 는 본 ADR 작성 시점 기준 반영 0건 (Proposed 상태 유지). Electron 마이그레이션 시점 미확정 (ADR-054 Soft Constraint 1) 이 해소되지 않은 상태에서 로컬 LLM 내장 단계 차단 위험. 또한 ADR-054 의 Hard Constraint (`클라우드 LLM SDK` 완전 제거 + Provider 추상화 + 폐쇄망 + 컴포넌트 카탈로그 Tier 2 주입) 와 ADR-011 의 Phase A1~A4 반영 산출물 (도구 7개 + AIPanel + AbortController + G.3) 의 정합 합의 부재.
...
사용자 재제기 (2026-08-18): "기존 클라우드 LLM 은 제거 예정이고, pencil 앱이나 PENCIL_ECOSYSTEM_ANALYSIS.md 분석처럼 open-pencil 방향으로 전환해야 한다" — 이에 따라 §인프라 노선 재결정에서 노선을 재평가한다.
...
통합 형태 (대안 A) 는 유지한 채, **"벤더 SDK 제거 후 무엇으로 대체하는가"** 의 노선을 재평가한다.
...
- **D10** (신규 2026-08-18): **secret isolation** — `dangerouslyAllowBrowser` 제거 + browser 번들에서 외부 provider 직접 호출 0건 (Hard Constraint 13). BYOK 키 보관·경유 경계: 원격 provider 는 프록시 경유 (Cloud Edge Function 등 — Phase 2 에서 확정), 로컬 endpoint (localhost) 는 직접 호출 허용. 키의 브라우저 저장은 사용자 명시 opt-in (localStorage 평문 금지)
- **D11** (신규 2026-08-18): **외부 에이전트/MCP 준비** — 7+ 도구 정의를 MCP tool schema 와 호환되는 형태 (JSON Schema 파라미터 + 명세 분리) 로 유지. ACP/에이전트 SDK embed (Claude Code / Codex — Pencil.app dual embed·holaOS 하니스·grok-build ACP 패턴) 는 Electron 반영 후 Phase 9 에서 재평가. 도구 표면이 커질 때의 지연 로딩은 grok-build `search_tool`+`use_tool` 패턴 (manifest 안정 = KV cache 보존 — 3중 독립 수렴 확인, XAI_ORG_ANALYSIS §5-1 #1) 을 정본 reference 로 한다
  - **정합 (2026-08-28, ADR-196 Phase 4)**: 명령 실행 도구는 D11 이 새로 정의하지 않고 [ADR-196](196-agent-command-surface.md) 의 descriptor (`listAgentCommands()` — `COMMAND_META` allowlist 파생 JSON Schema) 를 **그대로 소비**한다. 현재 AI tool `run_command` 로 배선돼 있고, D11 이 MCP/새 provider 로 옮길 때 정의 재작성 없이 배선만 이동한다 (196 R6). 실측 (2026-08-28): 현행 `AgentService` 의 모델 id `llama-3.3-70b-versatile` 이 만료돼 (`404 model_not_found`) AI 패널 도구 8종 전부 도달 불가 — Phase 0 baseline 항목.
...
- Phase 0 — inventory baseline freeze (ADR-011 반영 영역 인벤토리 + 4 격차 측정 + 클라우드 LLM 표면 실측)
- Phase 1 — LLM Provider 추상화 + 에이전트 프로파일 (D1) — **G1**
- Phase 2 — 벤더 SDK 완전 제거 + secret isolation (D10) — **G2**
...
| R9  | 클라우드 LLM SDK 완전 제거 시 기존 Phase A1~A4 산출물 회귀                                                                                                        |  MED   | Phase 2 G2 — 대체 provider (에이전트 프로파일 경유) 로 기존 7개 도구 전수 통과 + AbortController + G.3 시각 피드백 보존                                                                                   |
...
| R12 | (신규) 원격 provider 프록시 경계 부재 시 BYOK 키 브라우저 노출 재발 (클라우드 LLM 사례 반복)                                                                      |  MED   | Phase 2 G2 — `dangerouslyAllowBrowser` 0 + browser 번들 내 원격 provider 직접 호출 0 grep gate. 프록시 방식 (Cloud Edge Function 등) Phase 2 확정                                                         |
...
| G2   | Phase 2 완료                                   | 벤더 SDK 0 grep + `dangerouslyAllowBrowser` 0 grep + browser 번들 원격 provider 직접 호출 0 + 대체 provider 로 7개 도구 전수 통과                                                                                                                                                 | 키 경유 방식 재설계 (프록시 ↔ 로컬 gateway), fallback 단계 추가                               |
...
| G5   | Phase 5 완료 **통과 (2026-08-29 실물 측정)**   | 컴포넌트 카탈로그 Props 정확도 ≥ 90% (RAC / RSP 문서 기반 검증, executor 프로파일 기준 모델) — **실측 18/18 = 1.000**, enum 값 6/6, 표본 10 요청 · qwen3:14b · oracle `resolveEditContract` · 대조군(카탈로그 절단) 오답 확인                                                     | 카탈로그 형식 재설계, 동적 주입 전략 변경 (RAG 도입) — **불요**                               |
| G6   | Phase 7 완료 **통과 (2026-08-29 Ollama 실물)** | 에이전트 프로파일 라우팅 (D7 분해) 동작 + 폐쇄망 시나리오 — 전 프로파일 로컬 endpoint 바인딩으로 도구 통과 1회 실측. **Ollama 0.33.0 `/v1` wire 정합 확인** (tool_calls·finish_reason·[DONE])                                                                                     | 프로파일 구성 UX 보강, 로컬 endpoint 가이드 확충 — 가이드에 `OLLAMA_CONTEXT_LENGTH` 경고 추가 |
...
- **클라우드 LLM SDK 완전 제거 + secret isolation**: 벤더 종속 해소 + `dangerouslyAllowBrowser: true` 제거가 1급 결정 (Hard Constraint 13) 으로 승격 — API 키 브라우저 노출 구조 재발 차단
...
- 원격 provider 프록시 (Cloud Edge Function 등) 운영 표면 추가 (R12)
...
| 2026-08-28 | **Accepted 승격 + Phase 0 반영** — 리뷰 round 2 승인 기록(pending 0)으로 Status 전이, 사용자 `/execute-adr 134` 착수. Phase 0 inventory baseline freeze 완료 (클라우드 LLM 표면 국소화 · `AIAgentProvider` dormant · Edge Function 미존재 · 회귀 gate baseline 0). 사용자 결정 "클라우드 LLM 는 더 이상 사용하지 않는다" 확인 — 현행 모델 id 만료(404)로 AI 패널이 이미 정지 상태이며 Phase 1+2 가 복구 경로. ADR-196 descriptor 소비 정합 1줄 (D11) |
