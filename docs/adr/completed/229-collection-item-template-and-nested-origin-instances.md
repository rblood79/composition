...
> **[ADR-234](../234-variant-instances-and-slot-filled-collections.md) 가 일부 대체 (2026-09-23)**: TagGroup slot 위치 (root → TagList) · Slot "+" 의 동작 (F30 `items` 등록 → TagList 의 Tag instance 자식) · 정적 chip 의 정본 (`items` 행 → Tag instance 자식, 바인딩 TagGroup 은 `items` + 템플릿 유지) · `Tag/Selected` 복제본 (→ origin = 선택 상태 · 휴지 = `--unselected` ref 변형). 조합 자식 = origin instance 규칙 (Phase 2) 은 그대로.
