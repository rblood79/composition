...
5. **번들**: 초기 < 500KB 유지 — 팔레트 CSS 추가분 ≤ 20KB (286 변수 원문 — Phase 1 실측 14.5KB).
...
| R3  | oklch→sRGB 변환 오차로 Skia(hex) ≠ DOM(oklch) — 특히 gamut 밖 값(green-400 등) clamp                                                                                                                               |  LOW   | 수식은 canvas 실측 일치 확인. G1 이 286 전수 Δ≤2 검증. clamp 는 브라우저 sRGB 렌더와 동일 동작                                                                      |
...
| G1   | Phase 1 종료 | 생성 TS hex vs 브라우저 canvas 실측 sRGB — 286 전수 Δ≤2; drift 테스트 GREEN                                                                                                 | 변환 수식/파서 교정 후 재생성                       |
