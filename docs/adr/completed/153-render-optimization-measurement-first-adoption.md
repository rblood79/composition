...
1. **Picture(display list) 캐시 0건** — `PictureRecorder`/`drawPicture` 사용 전무. command stream 캐시는 단일 global all-or-nothing (`renderCommands.ts:270-318`) 이라 요소 1개 편집이 전체 stream 재빌드 + `contentSurface` 전체 재페인트를 유발
...
**2026-07-27 상호작용 프레임 내부 분해 실측 (2 스케일 — 소규모 모바일 2페이지 63% 줌 / desktop 1920 콘텐츠 밀집 60% 줌)**: ADR-069 `window.__composition_PERF__` 인프라 + SkiaRenderer 임시 flush/record 분해 라벨로 상호작용 프레임 내부를 분해했다 (기록: 메모리 `project-render-frame-decomposition-flush-vs-js`, baseline 수치 표는 design breakdown §Phase 0):

- **JS 조립 축은 상호작용 중 미미** — `buildSkiaFrameContent` + `buildFrameRenderPlan` 합계 0.07~0.13ms/frame (프레임의 2~7%). 기존 command stream 캐시가 상호작용 경로에서 실효 중이므로 격차 2 (incremental build budget) 의 우선순위는 실측상 낮다 — 단 registry 변경(요소 편집) 시 캐시 미스 경로는 미측정 (Phase 1 측정 대상, G4 판정과 연결).
- **content 재렌더 프레임의 지배 축은 CanvasKit 내부** — record (씬 재기록) 1.73~2.05ms + flush/snapshot 1.49~2.91ms 로 활성 프레임의 90%+. record 는 가시 요소 수에 비례한다 (뷰포트 분량 1.7ms → 전 콘텐츠 가시 p95 3.6ms) — Phase 3 Picture 캐시의 이득이 페이지 규모에 비례해 커진다는 직접 근거.
- **격차 5 (신규): content 재렌더마다 `makeImageSnapshot` 재생성** — `renderContent` 가 활성 프레임마다 스냅샷을 새로 만들어, 다음 프레임 clear 시 거대 content surface (화면+패딩 1024px, dpr 2 기준 수십 MB 텍스처) 의 copy-on-write 복사를 유발할 수 있는 구조. flush.content 는 max 117.9ms(소규모)/60.9ms(desktop) 스파이크 포함. 처방 후보는 Phase 3 에 편입 — 모션 중 content 승격 강등 (cleanup 재렌더 시만 재래스터+스냅샷) 또는 content surface ping-pong (설계 결정: breakdown §Phase 3, R7).
- **팬(camera-only) 경로는 씬 크기 무관 0.9~1.0ms/frame** (오버레이/blit 재기록 ~0.5 + flush.main 0.2~0.33) — dual-surface 설계가 목적대로 작동 중. 오버레이 picture 캐시는 이득 상한이 ~0.5ms/frame 이라 후순위.
- **WebGPU 백엔드 전환 제안에 대한 판정 근거 (재발 방지 기록)**: GPU API 교체가 건드리는 축 (GL 제출 오버헤드) 은 flush 의 일부일 뿐이고, record 는 WASM CPU 기록이라 무관 — 자체 WebGPU 엔진 재설계·백엔드 전환은 본 ADR 의 캐시 레버 대비 효과/비용이 성립하지 않는다. 재개 조건: CanvasKit Graphite/WebGPU 빌드 안정화 **및** Phase 3 반영 후에도 flush 축 지배 실측.
...
1. HIGH 0 이면서 유일하게 본질 격차 (content 변경 전량 재기록 + 스냅샷 CoW 재생성 + 측정 부재) 를 해소하는 대안. Phase 4 를 실측 evidence 게이트 (G4) 뒤에 두어 "필요 없으면 도입하지 않는" 종결 경로를 내장 — 복잡도 위험을 구조적으로 상한
...
| ID  | 위험                                                                                                                                                                                                                                                                                                                                                                       |  심각도  | 대응                                                                                                                                                                                                                                                  |
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :------: | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| R1  | Picture 캐시 invalidate 키 누락 → stale 시각 회귀. 코드 경로: `renderCommands.ts:270-318` 단일 캐시 키, `SkiaRenderer.ts:127` invalidateContent all-or-nothing, ADR-136 `buildSceneStructureSnapshot()` signature 입력 목록                                                                                                                                                | **HIGH** | invalidate 키를 sceneVersion signature SSOT 로 강제 + Phase 1 miss 사유 분류를 선행 도입해 miss/stale 을 1급 신호화 + G2 (cross-check + 편집 시나리오 exercise)                                                                                       |
...
| R6  | 애니메이션/드래그 중 이동 노드의 Picture 캐시 churn — transition/animation tick 승격 구간 (`SkiaRenderer.ts:483-535`) 에서 매 프레임 re-record+delete 발생 시 record+replay 가 direct draw 보다 비싸 오히려 회귀                                                                                                                                                           |   MED    | volatile 면제 — 활성 애니메이션/드래그 노드는 캐시 제외 후 direct draw (open-pencil `hasVolatileOverlay` skip 패턴) + 위치-불변 record (노드-로컬 좌표 record, 이동은 draw 시 변환 적용 — 이동만으로 re-record 없음) + G2 애니메이션 구간 비회귀 실측 |
| R7  | 스냅샷 정책 변경 (격차 5 처방) 의 양면 위험 — (a) 모션 중 content 승격 강등형은 모션 중 품질 저하 (리샘플 blur/커버리지 빈 영역) 가 cleanup(200ms) 까지 지속, (b) ping-pong 형은 content surface 1장 추가로 WASM/GPU 메모리 증가 (dpr 2 기준 수십 MB). "재렌더 유지 + 스냅샷만 생략" 은 present 가 snapshot blit 전용이라 성립 불가 (breakdown §Phase 3)                   |   MED    | Phase 3 설계에서 두 옵션 trade-off 실측 후 선택 + G2 모션 종료 시각 무결성 + G3 heap 상한에 ping-pong surface 포함                                                                                                                                    |
...
| G2   | Phase 3 종료      | `/cross-check` 시각 대칭 PASS (캐시 hit/miss 양 경로) + 요소 1개 편집 시 재기록 범위가 변경 노드+조상 한정 실측 + 편집/undo stale 렌더 0 + 드래그/애니메이션 구간 프레임 타임 Phase 0 baseline 비회귀 (volatile 면제 + 위치-불변 키 검증, R6) + 스냅샷 정책 변경 시 flush.content 누적 감소·모션 종료 시각 무결성 실측 (R7) | invalidate 키 보강 후 재검증. 2회 실패 시 Picture 캐시 rollback (독립 커밋) |
...
> **상호작용 구동 함정 (2026-07-27 실측 교훈)**: ① 합성 wheel 이벤트로 팬을 구동하면 선택 요소가 1개 있을 때 스크롤 라우팅 분기 (`useViewportControl` handleWheel Phase E) 가 삼켜 카메라가 움직이지 않는다 — 팬 구동은 중버튼 드래그 경로 (mousedown button:1 → window mousemove) 를 사용한다. ② Chrome 창이 최소화/가려지면 rAF·타이머 정지로 수집이 통째로 무효가 된다 — 수집 전 창 전면화 + 드라이버에 `visibilityState` 가드 필수. ③ 대량 요소 씬이 필요하면 UI 붙여넣기 증식이 아니라 canonical 문서 seed 로 결정적으로 생성한다 (툴바 N/N 카운터는 요소 수가 아니라 undo 히스토리 위치 — 증식 검증 지표로 쓰면 잘못 판정).
