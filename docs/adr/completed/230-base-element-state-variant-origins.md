...
Implemented — 2026-09-22 (Proposed 2026-09-21 → Accepted 같은 날 사용자 `/execute-adr 230` · Phase 0 G0 · Phase 1 G1·G2 · Phase 2 G2·G3 · Phase 3 G4 통과 · Components 배치 사용자 confirm 2026-09-22)

> **[ADR-234](../234-variant-instances-and-slot-filled-collections.md) 가 대체 (2026-09-23)**: 상태 변형 = 복제본 + 관리 키 overlay + Preview `<style>` · `--co-*` 변수 채널 → **변형 = origin 의 `reusable` ref + 유효값 차분** (키 제한 없음). 두 leg 는 층 모듈 하나 (`stateVariantLayers.ts`) 로 RAC 상태 층을 겹치고, Preview 는 RAC render props 로 적용한다. 230 의 `<style>` 규칙 · `--co-*` 변수 · `data-state-origin` · `_stateVariants` 는 제거됐다. 저장된 문서는 hydration 에서 이관된다 (`stateVariantMigration.ts`).

리뷰 round 1 h1/m2/m3 설계 보완 완료 (round 2, 이슈 0). **Phase 0 (G0) 완료** — inventory freeze (breakdown §2 F13~F21) · 기본 요소 집합 5 (Button · ToggleButton · Link · Checkbox · Switch) · Δnode 12 · 리뷰 반증 3건 현재 코드 재현. **Phase 1 (G1 · G2) 완료 2026-09-22** — selected/disabled 변형 origin seed + 두 leg 유효 상태 overlay + opacity 단일 적용 + Preview CSS 변수/RAC 상태 selector 1장 · unit 23 · live 8/8 (breakdown §7). **Phase 2 (G2 interaction · G3) 완료 2026-09-22** — hover/pressed/focus-visible 변형 origin 15 root 시드 · Preview CSS 순서 selected→focus-visible→hover→pressed→disabled + `:not([data-disabled])` 차단 · 변형 origin 자신은 캔버스에 catalog hover/pressed 토큰 정적 표시 · G3 `scene.build` p95 Δ −1.4 ms (600 ref · 상태 50%) · `<style>` 1.2 KB. Preview iframe live 는 사용자 판정으로 보류 (unit 고정 · 사용자 확인 대상). **Phase 3 (G4) 완료 2026-09-22** — BC unit 6 (Δnode 33 · Δbyte 9,793 · 필드 추가 0 · 재hydration Δ0) · BC live 6/6 (IndexedDB 저장 층 Phase 1 모양 reload) · Properties 정체 칩 상태 배지 · Components 배치 (default 오른쪽 같은 행, Selected → Disabled → Hover → Pressed → Focus) 사용자 confirm "확인했다". 보류 = Navigator 그룹 접기 (LOW 후속).
