...
2026-05-21 추가 판정: ColorWheel date/color primitive catalog pilot 을 land 했다.
`packages/shared/src/catalog/primitives/colorWheel.ts` 와 `toColorWheelRacProps()` 가
ColorWheel color/hue/outerRadius/innerRadius/size/disabled props 를 RAC ColorWheel
surface 로 정규화한다. `componentCatalog` 는 ColorWheel 을 `cutover:"catalog"`
active `date-color` primitive 로 등록한다. shared `ColorWheel.tsx` 는 catalog
projection 을 소비하고, Preview `CanonicalNodeRenderer` 는 ColorWheel resolved node 를
legacy `rendererMap` 보다 primitive branch 에서 먼저 렌더한다. generic Skia path 는
ColorWheel hue arc ring/thumb 를 arc+box node 로 렌더하고
`ColorWheelSpec.render.shapes()` 를 호출하지 않는 fixture 를 가진다.

2026-05-21 추가 판정: ColorPicker date/color primitive catalog pilot 을 land 했다.
`packages/shared/src/catalog/primitives/colorPicker.ts` 와 `toColorPickerRacProps()` 가
ColorPicker defaultValue/variant/size/disabled props 를 RAC ColorPicker surface 로
정규화한다. `componentCatalog` 는 ColorPicker 를 `cutover:"catalog"` active
`date-color` primitive 로 등록하고 ColorArea/ColorSlider/ColorField child placement 를
제공한다. shared `ColorPicker.tsx` 는 catalog projection 을 소비하고, Preview
`CanonicalNodeRenderer` 는 ColorPicker resolved node 를 legacy `rendererMap` 보다
primitive branch 에서 먼저 렌더한다. generic Skia path 는 dedicated ColorPicker draw
module 없이 resolved ColorArea/ColorSlider/ColorField children 을 재귀 렌더하고
`ColorPickerSpec.render.shapes()` 를 호출하지 않는 fixture 를 가진다.

2026-05-21 추가 판정: Calendar date/color primitive catalog pilot 을 land 했다.
`packages/shared/src/catalog/primitives/calendar.ts` 와 `toCalendarRacProps()` 가
Calendar variant/size/month paging/date state props 를 RAC Calendar surface 로
정규화한다. `componentCatalog` 는 Calendar 를 `cutover:"catalog"` active
`date-color` primitive 로 등록한다. shared `Calendar.tsx` 는 catalog projection 을
소비하고, Preview `CanonicalNodeRenderer` 는 Calendar resolved node 를 legacy
`rendererMap` 보다 primitive branch 에서 먼저 렌더한다. generic Skia path 는 month
heading, weekday row, day cells 를 생성하고 `CalendarSpec.render.shapes()` 를 호출하지
않는 fixture 를 가진다.

2026-05-21 추가 판정: RangeCalendar date/color primitive catalog pilot 을 land 했다.
`packages/shared/src/catalog/primitives/rangeCalendar.ts` 와
`toRangeCalendarRacProps()` 가 RangeCalendar range start/end/variant/size/month paging
props 를 RAC RangeCalendar surface 로 정규화한다. `componentCatalog` 는
RangeCalendar 를 `cutover:"catalog"` active `date-color` primitive 로 등록한다.
shared `RangeCalendar.tsx` 는 catalog projection 을 소비하고, Preview
`CanonicalNodeRenderer` 는 RangeCalendar resolved node 를 legacy `rendererMap` 보다
primitive branch 에서 먼저 렌더한다. generic Skia path 는 Calendar month grid 를
재사용하면서 range band 를 추가하고 `RangeCalendarSpec.render.shapes()` 를 호출하지
않는 fixture 를 가진다.

2026-05-21 추가 판정: DatePicker date/color primitive catalog pilot 을 land 했다.
`packages/shared/src/catalog/primitives/datePicker.ts` 와 `toDatePickerRacProps()` 가
DatePicker label/defaultValue/size/granularity/calendar icon/date state props 를 RAC
DatePicker surface 로 정규화한다. `componentCatalog` 는 DatePicker 를
`cutover:"catalog"` active `date-color` primitive 로 등록한다. shared
`DatePicker.tsx` 는 catalog projection 을 소비하고, Preview `CanonicalNodeRenderer`
는 DatePicker resolved node 를 legacy `rendererMap` 보다 primitive branch 에서 먼저
렌더한다. generic Skia path 는 label/input/date text/calendar icon 을 생성하고
`DatePickerSpec.render.shapes()` 를 호출하지 않는 fixture 를 가진다.

2026-05-21 추가 판정: DateRangePicker date/color primitive catalog pilot 을 land 했다.
`packages/shared/src/catalog/primitives/dateRangePicker.ts` 와
`toDateRangePickerRacProps()` 가 DateRangePicker label/start/end/size/granularity/
calendar icon/date range state props 를 RAC DateRangePicker surface 로 정규화한다.
`componentCatalog` 는 DateRangePicker 를 `cutover:"catalog"` active `date-color`
primitive 로 등록한다. shared `DateRangePicker.tsx` 는 catalog projection 을
소비하고, Preview `CanonicalNodeRenderer` 는 DateRangePicker resolved node 를 legacy
`rendererMap` 보다 primitive branch 에서 먼저 렌더한다. generic Skia path 는
label/input/date range text/calendar icon 을 생성하고
`DateRangePickerSpec.render.shapes()` 를 호출하지 않는 fixture 를 가진다.
