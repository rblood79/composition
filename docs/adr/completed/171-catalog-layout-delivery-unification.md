...
| R6  | 수기 배선 18종 제거 시 size 의존 값(padding 등)이 catalog 로 표현 불가해 회귀 — **Phase 0(2026-07-29) 에서 완화**: 표현 불가가 아니라 resolver 가 `sizes[size]` 축을 **읽지 않았을 뿐**이다(차단 L3). 잔존 위험은 비-size prop 의존 4분기로 좁혀졌다                                                                                                                                  |  LOW   | 존치 사유를 비-size prop 의존(table `heightMode` · gridlist `layout` · toolbar/togglebuttongroup `orientation`)으로 한정 + 주석                                       |
...
아래는 **실행 순서**대로다 (Phase 1 → 2 → 3 → 5 → 3-b → 4 → 6 — fixture 가 인라인 제거보다 앞선다. Phase 3-b 는 Phase 5 fixture 가 잡아낸 size 축 게이트 갈림의 해소라 5 뒤에 온다).

| Gate | 시점                          | 통과 조건                                                                                                                                                                                                                                                                                                                                                                                                            | 실패 시 대안                                         |
| ---- | ----------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------- |
| G1   | Phase 1 종료                  | 9종 catalog 정정 후 실효 computed 값이 정정 전과 **불변**                                                                                                                                                                                                                                                                                                                                                            | 해당 종은 정정 보류 + 수동 CSS 를 SSOT 로 명문화     |
| G2   | Phase 3 종료 (3-b 재확인)     | 비대칭 19종(Phase 0 확정)의 Skia box 가 실효 DOM 값과 일치 (MenuItem `280×32` 등 — 차단 3층 L1·L2·L3 를 모두 열어야 성립, design §2-0). 기존 `resolveContainerStylesFallback.test.ts` 47 케이스는 갱신 후 GREEN (R8). **Phase 3-b 재확인**: size 축 판정을 생성기 규칙(`ownsContainerBox`/`skipPadding`/`skipGap`) 미러로 교체한 뒤 라이브 Toolbar `263×29 → 215×29`(실효 DOM padding 0 정합) + 기존 6종 회귀 0      | 경로 B 게이트 복원 후 개별 진단                      |
| G3   | Phase 5 도입 (**Phase 4 앞**) | 신설 fixture 를 Phase 3 을 일시 되돌린 상태에서 돌려 **RED** 임을 확인                                                                                                                                                                                                                                                                                                                                               | 오라클이 결함을 못 잡음 → fixture 재설계             |
| G4   | Phase 4 종료                  | factory 인라인 제거 전후 Skia box **불변**(fixture GREEN 유지) **＋** Inspector 의 표시값·"수정 N" 뱃지·reset 결과가 제거 전과 동등 (R7 live exercise). **PASS (2026-07-29)**: 라이브 A/B 29요소 byte-identical · parity 931 GREEN · baseline 미러 양방향 계약 테스트 신설(민감도 확인). "실패 시 대안" 이 실제로 발동 — 22종 71선언 중 **6종 15선언만** 제거하고 나머지는 사유별로 존치(design §Phase 4 3자 대조표) | 해당 컴포넌트 인라인 존치 + catalog 미도달 사유 기록 |
