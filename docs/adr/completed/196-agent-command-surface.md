...
Implemented — 2026-08-28 (Phase 0~~4 / G0~~G4 종결). Accepted 2026-08-28 (리뷰 round 1 승인 — MED 3 / LOW 3 fixed, pending 0; 사용자 승격 지시 2026-08-28) · Proposed 2026-08-28
...
6. **비용** — per-keydown 경로 코드 추가 0 · **초기 번들** Δ ≤ +3KB gz (2026-08-28 사용자 판정: 기준은 초기 번들 — 명령 표면·승인 UI 는 지연 로딩으로 초기 번들 밖에 둔다) · 외부 의존 0 · `window.__compositionAgent` 는 DEV 빌드 한정.
...
- **가시성** — `.ai-command-log` 목록에 2건 (`alignLeft 조건 미충족 · multi-select-mode-off` / `zoomIn 실행`) 표시. `host` 가 `ai-panel` 로, 2026-08-28 측정의 `chrome-mcp` 와 구분된다 — 패널 경로가 실제로 처음 측정됐다.
- **Phase 3 배선 회귀 없음** — ADR-134 Phase 8 이후 AIPanel/useAgentLoop 가 바뀌었으나 consumer 4종(`run_command` 등록 · `AgentCommandLogList` · `AgentCommandConfirmDialogHost` · `window.__compositionAgent` allowlist 40) 전부 유지, 관련 테스트 38 PASS.
