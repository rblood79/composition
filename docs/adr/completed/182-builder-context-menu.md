...
| ---- | ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------- |
| G1   | Phase 1 착수 직후   | RAC `Popover` 가 임의 클릭 좌표 앵커에서 열림·플립·dismiss 정상 + **dismiss 클릭이 캔버스 포인터 파이프라인으로 전달되지 않음**(선택/마퀴 미동작) 확인 (스파이크 1파일)              | 자체 포지셔닝 래퍼 + RAC `Menu` 로 렌더러만 교체 (§3-3) |
