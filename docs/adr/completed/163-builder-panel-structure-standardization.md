...
- ~~**`properties/editors/` dead chain**~~ → **해소 (2026-07-25, 사용자 승인 후 진행)**: 배럴 소비처 0 (live 경로는 `useEditContract` + GenericFieldRenderer, `PropertiesPanel` 은 `./editors` 에서 `ElementSlotSelector` 만 import). 완전 dead 21파일 삭제(-3,639줄) — per-type 에디터 15 + `editorUtils`/`propertyEditorNode` + 배럴 + 전용 CSS 2 + 전용 정적 테스트. **보존**: `ElementSlotSelector`/`ResponsiveVisibilityEditor`(LIVE), 페이지·레이아웃 오소링 서브트리(PageBodyEditor / LayoutBodyEditor / PageLayoutSelector / PageParentSelector / LayoutPresetSelector — 미배선 기능, 사용자 결정). **실측 정정**: `LayoutPresetSelector` 는 test-only 가 아니라 LIVE — `panels/styles/hooks/useResetStyles.ts` 가 `presetDefinitions`/`presetStyle` 을 직접 import (심볼 기반 검사가 서브모듈 named import 를 놓쳤던 것).
- ~~`components/styles/index.css`(1,169줄 잡화) 분할~~ → **해소 (2026-07-25)**: 2단계 — ① 짝 없는 고아 CSS 4클러스터 제거(slot-editor / schema·auto-generate 는 원래부터 고아, datatable / layout-slug 는 위 삭제로 고아화) 1,169→929줄, ② 소비자별 co-locate 분할 8파일 신설 930→95줄. 잔여는 공용 진입점 성격만(상단 @import 4 / `[data-focus-visible]` / `source-dot` 미배선 쌍 / `.spin` 공용 유틸). 선택자 117개 손실 0 대조 확인.
