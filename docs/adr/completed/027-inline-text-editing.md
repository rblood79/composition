...
Implemented (2026-09-20) — Phase A+B+C (TextEditOverlay + Quill + 멀티페이지 좌표 보정 + 컴포넌트 텍스트 편집) + **Phase D 전환 무결성** (2026-09-20 "리치 텍스트" 에서 재정의 — §Phase D): D0 캔버스 변위 차단 `bdd55079d` · D1 wrap 파리티 `a91f1a7d5` · D2 정렬 파리티 `a7fb02034` · D3 타입 8 × 줌 2 픽셀 게이트 16/16 `063b787eb` + 사용자 참관 live (아래 §Live Exercise). 리치 텍스트·툴바는 보류 (재개 조건: canonical 텍스트 모델 ADR 선행).
...
- D3 (headless Playwright, `apps/builder/scripts/adr027-text-edit-parity.mjs`, 타입 8 × 줌 100/200% = 16 케이스 **16/16 PASS**): 같은 요소를 (A) Skia 가 그린 채 · (B) 편집 진입 뒤 캡처해 텍스트 지도 (텍스트 색 거리를 요소 배경색으로 정규화, 선택 상자 띠는 stroke 색에 가까운 픽셀만 제외) 의 **shift** (±6px SAD 최소 정수 이동) 와 **반치 bbox** 차를 잰다 — 각 축 ≤ 1 CSS px (200% 는 화면 2px: DOM 이 baseline 을 device px 로 스냅하는 0.5px 양자화). 케이스: Text 4줄 wrap · center + padding 12 + border 2 · fontSize 28 · 한글 단일행 · pre-wrap `\n` 3줄 · Button · Badge · Link (TextArea 는 `TEXT_EDITABLE_TAGS` 밖 — 오버레이 편집 자체가 없어 게이트 대상 아님). 실측 shift 전부 0 (heading 200% dy −1 화면 px = 0.5 CSS px). **하니스가 드러낸 결함 6 (전부 수리)**:
  1. 오버레이 `font-feature-settings` 부재 — Skia paragraph 와 Preview body 는 Pretendard cv02·03·04·11 을 싣는데 빌더 문서는 `--default-font-feature-settings` 가 없어 오버레이가 `normal`. Latin 글리프 폭이 갈려 Heading 205.94 ↔ 204.41 (200% ink 우측 3px), 한글만 있는 줄은 정확히 같았다 → `resolveOverlayFontFeatures` (overlayWrap.ts) 로 같은 feature 를 싣는다.
  2. fit-content 라벨의 마지막 단어 줄바꿈 — Skia 는 Canvas 2D 힌트 경로에서 `max(maxWidth, ceil(c2dWidth) + 1)` 로 layout 해 상자보다 1px 넓다. 오버레이가 상자 폭 그대로 pre-wrap 하면 (feature 적용 뒤 112.56 in 112) "자세히" 가 둘째 줄로 → `wrapWidthExtra` (paragraph `getMaxWidth()` − 노드 maxWidth) 만큼 root 를 넓힌다 (wrap 모드만).
  3. 200% 에서 D2 nudge 가 반 크기 컨테이너에서 측정됨 — `position`/`size` prop 이 scene 좌표라 첫 렌더 컨테이너가 zoom 전 크기 (Save 라벨 textLeft 12 · lineTop −2.4 · nudge 5.94/7.4). livePos/liveSize 를 `getSceneBounds` + 카메라로 초기화해 첫 렌더부터 맞춘다.
  4. 세로 nudge 는 baseline 끼리 — line box top 이 같아도 두 엔진의 half-leading 배분이 달라 글리프가 1 CSS px 갈렸다 (Text 4줄 · Heading: shift dy −1 @100% · −2 @200%). DOM 은 첫 줄 앞에 0×0 inline-block probe 로 baseline 을 재고 Skia 는 `lineMetrics[0].baseline`.
  5. **Skia 한글 단일행이 descent 만큼 위에 그려짐** (2026-04-08 `15db472bc` 부터, ADR-027 밖 D3 발산이지만 게이트가 잡았다): `computeDrawY` 가 CJK 포함 텍스트에 `getIdeographicBaseline()` (= 줄 아래 변) 을 원점으로 쓰면서 ink ascent 는 Canvas 2D alphabetic 기준 → 16px Text "가운데 정렬" y −6.32 · Button "저장" 라벨 y −0.05 vs "Save" 4.96. Preview 는 line box 안이라 Canvas 만 6px 높았다 → `resolveSingleLineGlyphTop` (textGlyphCentering.ts) 로 alphabetic 단일화, 정적 게이트 (`getIdeographicBaseline` 0).
  6. **엔진 텍스트 높이 2 (ADR-027 밖, 하니스 시드가 재현)**: (a) 고정 px 폭 텍스트 leaf 의 줄바꿈 높이를 부모 content 폭으로 재고 (block body 1920 → 1줄 24), Step 4.5 2-pass 는 px 폭을 "enrich 가 쓴 폭" 으로 가정해 (300 == 300) 재측정을 건너뛰어 엔진 300×24 ↔ Skia 3줄 넘침 → `enrichWithIntrinsicSize` 가 텍스트 leaf 도 자기 px 폭으로 잰다 (INTRINSIC_MEASURE_TAGS 와 같이). (b) pre 계열의 `\n` — 레이아웃 활성 측정기는 항상 `Canvas2DTextMeasurer` (CanvasKit 측정기는 설치되는 곳이 없다) 이고 `\n` 을 공백 토큰으로 봐 pre-wrap "a\nb\nc" 가 1줄 → §4.9 가 white-space 를 측정기에 넘기고 (`measureWrappedTextHeight` 10번째 인자 · 캐시 키) 측정기가 pre 계열에서 줄마다 재서 더한다 (`pre` 는 폭 무한). D1 의 "Enter = 줄바꿈" 경로가 이 결함 위에 있었다.
  - 하니스 함정: 팔레트 절대 배치 케이스는 page 높이 (1080) 안에 (밖이면 클릭 미도달 → "편집 진입 실패") · 색 tolerance ink 마스크는 DOM 글리프가 Skia 보다 굵어 (ink 1.3~~1.9×) AA 가장자리 1~~2px 를 잡는다 · 텍스트가 선택 stroke 위에 놓이면 A 는 stroke-over-text, B 는 text-over-stroke 라 띠 전체를 지우면 bbox 위가 6px 어긋난다.
  - G-D3 사용자 참관 live: 아래 §Live Exercise (D3).

### Live Exercise (D3, 2026-09-20 — Chrome MCP foreground · 사용자 참관 · 프로젝트 `adr027-check` · 100% / 200%)

시드는 production 경로 (팔레트 클릭 → `updateElementProps`): 붉은 테두리 + padding 4 의 pre-wrap 가운데 정렬 Text 2줄 · Button "저장하기 Save" · Link "Learn more 자세히" · 300px Text "한글 단일행 라벨 Korean".

| #   | 케이스                         | 결과                                                                                                                                                                                                                                                                                        |
| --- | ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | pre-wrap Text 진입 (100%)      | nudge 0/0.19 (Skia x 83.25 == DOM 83.25 · baseline 17.69 vs 17.5) · 편집기 pre-wrap · `font-feature-settings` cv02·03·04·11 · 2 `<p>` 각 24 · pan/`workspace` scroll 불변. 육안: 선택 상자 안 텍스트 동일 (테두리는 선택 상자에 가려질 뿐 자리 같음)                                        |
| 2   | Enter + 타이핑 "셋째 줄 third" | 3 `<p>` · 편집기 높이 82 == 레이아웃 82 (58 → 82, 아래 형제가 밀림) · Cmd+Enter 커밋 → store `\n` 2개 · Skia 3줄. **Esc 는 취소** (Pencil 규약 — D1 표의 "Esc = 완료" 는 오기, 정정)                                                                                                        |
| 3   | 커밋 뒤 Skia (100%)            | **결함 → 수리**: 커밋 직후 Skia 3줄이 12px 아래 (paddingTop 17 / 17 — baseline middle 의 textBlockHeight 측정이 `\n` 을 공백으로 봐 2줄 48). `buildSpecNodeData` 가 `style.whiteSpace` 를 text shape 에 싣고 `specShapeConverter` 가 측정기에 넘긴다 → paddingTop 5 / 5, 편집기와 같은 자리 |
| 4   | Button "저장하기 Save" (100%)  | nudge 0/0.39 (textLeft 12.53 == 12.53) · 한글 라벨이 상자 세로 중앙 (종전 −5px 위 결함 없음)                                                                                                                                                                                                |
| 5   | Link (100%)                    | 편집기 `calc(100% + 1px)` · underline · 1줄 ("자세히" 안 떨어짐) · nudge 0/−0.04                                                                                                                                                                                                            |
...
| D1 wrap 파리티      | 오버레이 white-space 를 Skia 가 쓴 paragraph 입력 (maxWidth · wrap 여부) 에서 파생 — 고정폭 텍스트는 `pre-wrap` + 컨테이너 폭, fit-content 라벨은 nowrap 유지. 다중행 타입은 Enter = 줄바꿈, Cmd+Enter / 외부 클릭 = 완료 · Esc = 취소 (Pencil 규약) | `TextEditOverlay.tsx`, `useTextEdit.ts`             | G-D1: 3줄 문장에서 오버레이 줄 수 == Skia 줄 수, 줄 y ±1px                                                                    |
...
**결정 (2026-09-20 후속 3·4, 사용자 판정 — 아래 D1 결정을 대체)**: **Enter 는 항상 줄바꿈** (Figma · Framer 규약). 완료는 Cmd/Ctrl+Enter · **Esc** · 바깥 클릭 — Esc 도 완료 (후속 4, Figma 규약; 종전 Pencil 규약의 취소는 폐기, 되돌리기는 커밋 뒤 Undo). `TextEditOverlay` 의 `onCancel` prop 삭제 (`cancelEdit` 훅은 남는다 — 키에 매이지 않은 취소 경로). live: " ESC" 타이핑 → Esc → 저장됨. normal · nowrap 에 들어간 `\n` 은 커밋 때 `whiteSpace` 를 pre 계열로 승격해 (후속 2) CSS · Skia 가 같이 그린다 — D1 이 Enter 를 white-space 로 나눈 이유 (두 consumer 가 갈린다) 가 승격으로 사라졌다. `resolveOverlayWrap` 의 `enterInsertsNewline` 삭제. live: `width: 50%` normal Text 에서 "QWRRGREFER" 뒤 Enter → 편집기 문단 2 → Cmd+Enter → `pre-wrap` 저장, 상자 106 (4줄).
...
**후속 8 (2026-09-20, 사용자 live: `verticalAlign: top` 에 padding-top 을 줘도 Skia 는 0)**: `buildCatalogShapes` 의 위 기준 텍스트 y 가 rule `size.paddingY` 만 읽었다 (Text 는 rule paddingY 없음 → 0). 사용자 `style.paddingTop ?? padding` 을 paddingX 와 같은 우선순위로 (인라인 → rule → 0), inline leaf (baseline top) 도 같은 y — 후속 7 로 인라인 border 가 더는 box archetype 이 아니라 위 기준으로 그리므로 padding-top 을 직접 실어야 한다. 단위 `buildCatalogShapes.textPaddingTop.test` (inline + 24 → y 24 · Button top 10 > rule 5 · middle 은 0) · specs fixture 의 archetype 식을 shared 와 같게. live: pt 25 (= 24 + border 1) · 하니스 40/40 · parity 1430.
