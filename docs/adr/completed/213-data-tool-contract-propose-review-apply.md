...
- **후속 반영 (2026-09-14)**: 변수 읽기 tool `list_variables` (`services/ai/tools/listVariables.ts` — 입력 `getProjectVariableDefinitions` + `collectDocumentVariables`, 소유자 project / page / element + `usedBy`, detailed 는 defaultValue · persist · source · usages; 런타임 값 없음) 를 본 ADR 의 읽기 tool 4 와 같은 계약 (format concise/detailed · 쓰기 0 · `toolLabels` 어휘) 으로 추가했다 — live `apps/builder/scripts/adr214-followup-list-variables-live.mjs` 7/7 (Ollama qwen3:14b, AI 패널에서 tool 호출 → count(project) · agree(element Checkbox) 답변). 변수 쓰기 (`define_variable`) 는 삭제와 같은 op 라 `HUMAN_ONLY_DATA_OPS` 에 남는다 (214 Phase 1 결정).
