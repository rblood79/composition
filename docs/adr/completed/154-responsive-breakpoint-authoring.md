...
> - **후속 fix (2026-07-23, valueless-toggle)**: ~~eligible 이지만 값 없는 속성(minWidth/maxWidth/flexGrow/alignSelf/aspectRatio 등)은 seed 대상 부재로 토글 no-stick~~ → **해소**: 토글 ON 이 effective 값이 없을 때 `resolveEligibleSeedDefault` (responsiveWriteRouting.ts) 의 CSS-initial 값(카테고리 기반 — length→`auto`, spacing→`0`, enum→유효 기본, flexGrow→`0`/flexShrink→`1`)으로 seed. 새 schema 필드 없이 기존 override 파이프라인만 사용, 초기값이 시각 무변화라 토글 순간 화면 무변경. 32개 eligible prop 전수 non-empty seed 정적 가드(`responsiveWriteRouting.test.ts`). live(project 2332, mobile): alignSelf→`auto`/flexGrow→`0`/maxWidth→`auto` seed → 토글 고정 + OFF 시 tier clear + base props.style 8키 무변경 (`__composition_STORE__` 6-exercise ON/OFF).
