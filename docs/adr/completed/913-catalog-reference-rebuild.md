...
| R6  | rule table 편집 후 generated CSS 재생성 누락 시 Skia(런타임 직독)만 갱신되고 CSS(커밋 빌드 산출물)는 stale → preview↔Skia 발산 (= 본 ADR 이 해소하려는 문제의 재발 경로). rule table 은 `packages/shared/` 경로라 `spec-rebuild-flag.sh`(packages/specs 만 매칭)·turbo build 가 `generate:css` 를 자동 트리거하지 않음 |  HIGH  | G6 CSS 재생성 정합 Gate — slice 변환 시 `pnpm generate:css` 재실행 + CSS diff 커밋 + `validate:sync` PASS 의무화. **✅ 자동화 봉합 (2026-06-22)**: `spec-rebuild-flag.sh` 에 `packages/shared/src/catalog/**` 매칭 → `.css-regen-pending` flag + `type-check-gate.sh`(Stop hook) 가 flag 소비 시 `generate:css`+`validate:sync` 재실행 + CSS diff 발생 시 커밋 누락 알림(asyncRewake). slice workflow 밖 rule 편집도 hook 가 자동 봉합 — 작업자 규율 의존 제거. negative-path 검증: rule table 변경 시 hook 가 Badge.css 재생성 + diff 알림 확인 |
...
### slice 5 — Overlay family — 2026-06-19

> **정찰 우선 실행**: slice 1~4 교훈(추측 금지, starter 대조 evidence 동반) 반영 — Dialog/Popover/Tooltip/Modal 4 멤버를 starter 레퍼런스 + generated/manual CSS + catalog rule + STRUCTURE_META + Skia primitive(shadow/arrow/backdrop) + portal/overlay 렌더 경로 6 각도로 병렬 정밀 측정(Workflow w9btzgoyy, 13 agent measure→적대적 verify→synthesize). **실재 갭 11 측정 → 적대적 verify 후 fix 대상 2 확정 + overclaim 기각 3**(slice 3/4 의 overclaim 차단 패턴 강하게 작동 — measure 의 Dialog shadow "24 vs 32 12% sigma" 오비교 / Modal cascade 를 cutover 회귀로 오귀속 / Tooltip starter-한정 shadow 를 catalog 갭으로 오판 — 3건 모두 generated=container-only vs starter=DOM-full 단순비교 또는 잘못된 reference 인용). color/padding/gap/font 축은 21건 "변경 0 = 정렬" proof 로 닫힘(catalog rule ↔ generated CSS ↔ Skia primitive 3-way 토큰 정합 + overlay backdrop/shadow/arrow primitive 실재 wiring 확인).

**fix-in-slice5 (2건, 사용자 confirm 2026-06-19 — "slice 5에 흡수 + arrow 1줄"):**

1. **radius primitive `xs`/`2xl` 키 누락 (HIGH, css-skia-asymmetry)** — shared-tokens.css 가 `--radius-xs:0.125rem`(2px) / `--radius-2xl:1rem`(16px) 를 정의하고 catalog `COMPONENT_RULES_TABLE` 가 `{radius.xs}` 10회 + `{radius.2xl}` 3회(Dialog lg/xl line 2196/2204 + Form line 2659 + ComboBox/DatePicker/Table/Tree/Breadcrumbs 등) 참조하는데, primitives `radius` 객체 + `RadiusTokens` 타입에 두 키가 없어 → `resolveToken` undefined 반환 → buildCatalogShapes 다운스트림 Skia pipeline 에서 0px. DOM 은 `var(--radius-xs/2xl)`=2/16px → **D3 Skia↔CSS 대칭 위반**(영향 13 컴포넌트, 대부분 Overlay 밖 — 공유 토큰 결함). measure agent under-scope 했으나 synthesize 가 catalog grep 으로 13 use 전수 확인. **Overlay family 경계를 넘는 공유 토큰 fix 라 사용자 scope confirm 거쳐 slice 5 에 흡수**(AskUserQuestion 2026-06-19, 차단 메모리 `feedback-execute-adr-surface-minimization` 우선 평가 — fix surface 자체는 작고 G4 byte-diff 0). 수정: `radius.ts` 에 `xs:2`/`"2xl":16` + `RadiusTokens` 인터페이스 2키 추가(RadiusTokenRef 템플릿 type-valid 화 + stale 주석 md/lg/xl 정정). 3xl/4xl 은 CSS 정의 있으나 catalog 미사용 → 미추가(0 drift). 토큰 _정의_ 불변, 신규 키만 = R2 re-scale 없음(ADR-081 G1 snapshot diff = xs/2xl 2줄만, 기존 7값 불변). 위치: `packages/specs/src/primitives/radius.ts` + `types/token.types.ts`.

2. **Popover OverlayArrow stroke-width 비대칭 (LOW, css-skia-asymmetry)** — manual `Popover.css:51` 이 `stroke-width:1px`, starter `react-aria-starter/src/Popover.css:26` + Skia `popover_arrow` `strokeWidth:2` 는 2px → DOM(1px)↔Skia(2px) arrow 선두께 비대칭(composition manual CSS 단독 발산). 3-way 대조로 레퍼런스 정본(2px) 확정 후 정렬. 수정: `Popover.css:51` `1px→2px`(한 줄, 계약 얽힘 0). 위치: `packages/shared/src/components/styles/Popover.css`.

**defer (4건, surface minimization — 차단 메모리 `feedback-execute-adr-surface-minimization` 우선 평가):**

- **Modal cascade 충돌 (LOW) → 별도 ADR**: manual `overlays.css .react-aria-Modal{bg:--bg-overlay;border:--border-hover}` 가 generated `Modal.css{bg:transparent;border:none}` 에 cascade override 당함(동일 specificity, generated 후 import). standalone Modal 이 transparent+borderless → starter surface 와 발산. **ADR-141 이전부터 pre-existing(legacy Modal.spec.ts 도 transparent emit) — cutover 회귀 아님**. import 순서 정책 또는 Modal-as-transparent-portal-wrapper 비준 결정 = portal/overlay 렌더 모델 + cascade 정책 → 별도 ADR.
- **Popover bg-raised vs inset 토큰 정본 (LOW) → defer-separate-slice**: active DOM base=manual `--bg-raised`(cascade-wins) / Skia·catalog surface fill=`{color.layer-2}`=`--bg-inset` / css-tokens.md normative=`--bg-raised`. 3-way 토큰 정본 결정 + manual CSS·catalog·Skia 동시 정렬 필요(one-liner 아님).
- **Dialog/Modal drop-shadow element-ownership 분기 (LOW) → defer-separate-slice**: Skia dialog_shadow(Dialog box 부착, offsetY8/blur24/alpha0.2) vs DOM `--drop-shadow-md`(Modal box 부착, 2-layer). 강도·offset·소유 element 모두 발산. shadow 정본 source 결정 + portal 렌더 구조 얽힘 → 기존 rac-starter shadow-audit backlog 편입.
- **generated CSS provenance 헤더 stale (LOW) → defer-separate-slice**: Dialog.css/DialogFooter.css 헤더가 삭제된 `DialogSpec` 을 `Source:` 인용(값은 catalog 1:1, 시각 영향 0). cutover-deleted spec 전체 동일 → generator 헤더 일괄 정리 pass 로 batch.

**검증 (live behavior — Chrome MCP, 앱 인스턴스 dist 모듈 직독):**

- **radius fix**: 앱이 소비하는 `@composition/specs/dist/index.js` `resolveToken("{radius.xs}")=2` / `resolveToken("{radius.2xl}")=16`(이전 undefined→0px) live 직독 확증 + 기존 키 sm4/md6/lg8/xl12 불변. 인과 닫음: `buildCatalogShapes:75 parsePxValue` 가 TokenRef passthrough → 다운스트림 Skia `resolveToken` 해석(cssValueParser.ts:23-25 주석) → 이제 16/2px shape radius. DOM 불변(generated byte-diff 0)이라 Skia 만 0→정상 = 대칭 복원. 단위 테스트 tokenResolver 11 PASS(radius xs/2xl 4 + baseline 5 + 기존 2) + ADR-081 G1 snapshot 갱신(xs/2xl 2줄만).
- **Popover arrow**: starter Popover.css:26(2px) + Skia popover_arrow strokeWidth:2 와 정렬 — manual CSS 단독 1px 발산 해소.
- **G4 byte-diff 격리**: `pnpm generate:css`(88 files) 후 generated CSS byte-diff **0** — radius 는 Skia 런타임 직독만 교정(CSS 는 shared-tokens.css 가 값 정의해 처음부터 정상), Popover arrow 는 manual CSS 단일 편집. slice 외 family 영향 0. R2 공유 토큰 정의 불변(신규 키만).
