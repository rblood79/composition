# ADR-111: Layout/Slot Frameset 완전 재설계 — pencil app 호환

## Status

Implemented — 2026-05-02 direct cutover + G5 Pencil import/export parity closure. 2026-05-03 post-cutover fix 군은 2026-05-05 rollback 으로 폐기됐고, 2026-05-06 current main follow-up 패치로 Frames 탭 / Skia frame body / Slot 표시 / shared frame multi-page refresh / Nodes panel Frames tree parity 회귀를 다시 닫았다.

> **재개 사유** (2026-04-30): 본 ADR 은 ADR-112 (Editing Semantics UI — reusable component + slot 추상) 의 **frame-bundled preset 편의 확장** 임이 framing 재정의 됐고, 2026-04-30 ADR-112 가 Component/Slot base 를 `Implemented` 로 승격했다. 본 ADR 은 여전히 ADR-112 에 영향을 주거나 기준을 제공하지 않는다. `Frame.reusable` / `Frame.slot` / `Ref.descendants` schema 의 사용자 가시 편집 base 는 ADR-112 기준을 따른다. 본 ADR 의 이전 ##Slot section## 소유권 표현은 잘못된 설계 전제였고, ADR-112 기준으로 supersede 된다. **재개 범위**: P3-ε / P3-ζ 는 FramesTab / frame preset UX 가 완료된 ADR-112 기능을 더 쉽게 쓰게 하는 보조 흐름으로만 재설계한다.

> **동결 보존 범위**: Phase 0~2 (Implemented) + Phase 3 의 P3-α/β/γ/δ + δ fix #1+#2+#3+#4 + B1 filter + θ scope land + θ regression fix #1 (~commit `e4f24697` + 세션 49 후속) 모두 보존. P3-θ regression fix #1 은 frame instance composition body 채택 정책 GREEN — frame schema 자체 land 는 ADR-112 base 와 무관하게 실 사용자 가시 동작 (frame default + page slot fill) 보존 가치.

> **재개 조건 및 closure 결과**: 충족됨. ADR-112 Component/Slot base 완료(2026-04-30) 후 P3-ε / P3-ζ 는 ADR-112 기능의 frame authoring 편의 확장으로 재설계했고, 사용자 브라우저 회귀 검증까지 완료했다. 이후 2026-05-02 direct cutover 로 G3 canonical frame action surface 와 G4 legacy layout store/명칭 충돌 잔여를 닫았고, G5 Pencil import/export parity 를 완료해 본 ADR 을 `Implemented` 로 승격한다.
>
> **ADR-116 우선순위 조정 closure (2026-05-02)**: ADR-116 이 `CompositionDocument` canonical store/API 및 import/export adapter boundary 를 닫은 뒤, 본 ADR 의 G3/G4/G5 잔여도 순차 closure 했다. ADR-114 는 completed archive 에 Superseded 로 이동했으며, `imports` resolver/cache 잔여 scope 는 ADR-116 canonical import registry 와 본 ADR G5 Pencil adapter 로 흡수됐다.

> **Direct cutover 정정 (2026-05-02)**: 개발 단계라 기존 사용자/데이터 보존, feature flag, dev migration trigger, backup/rollback 을 유지하지 않는다. `VITE_FRAMES_TAB_CANONICAL` / `isFramesTabCanonical()` / `migrationP111` / FramesTab dev migration UI 를 제거했고, FramesTab/PageLayoutSelector read path 는 canonical reusable FrameNode surface 로 고정했다. G4 명칭 충돌 항목인 `PanelSlot` / `BottomPanelSlot` 은 `PanelArea` / `BottomPanelArea` 로 rename 완료했다. `useLayoutsStore` / `layoutActions` legacy store 본체도 제거했다.

### 진행 로그

- **2026-05-06 — rollback follow-up frame/page tree closure**: 2026-05-05 rollback 후 재현된 page-body frame 적용/refresh 시 body/Slot/Skia hatch marker 누락, Layout Preset Slot 누적, frame/page tab body auto-selection, shared frame multi-page refresh projection, synthetic projection `order_num` DB write 오류를 current main 패치군으로 닫았다. Frames 탭 UI tree 도 Pages 탭과 같은 `TreeBase` / `VirtualizedTree`, `section` / `section-content`, `frame-tree` 단일 class 기준으로 정리했다. 검증: FramesTab targeted vitest 4 files / 39 tests PASS + Pages tab parity baseline 2 files / 6 tests PASS + frame/component canonical regression sweep 및 `pnpm run codex:preflight` PASS.
- **2026-05-05 — rollback note**: 2026-05-03 post-cutover FramesTab / Skia frame scope fix 군은 build error 정합화 및 canonical-hydration fix 3종과 함께 사용자 환경 누적 회귀로 폐기됐다. 폐기 baseline 은 `225532ea9` 이며, 해당 작업은 완료 근거가 아니라 2026-05-06 follow-up patch 의 재진입 배경으로만 보존한다.
- **2026-05-03 — post-cutover FramesTab / Skia frame scope fix (ROLLED BACK 2026-05-05)**: direct cutover 이후 page/body 가 frame 수만큼 Layers tree 에 중복 표시되고 frame Slot 이 Skia 에 정상 표시되지 않던 회귀를 canonical frame scope/write-through 기준으로 닫았던 작업. targeted vitest 는 PASS 했지만 사용자 browser-side 회귀가 누적되어 rollback 됐다.
- **2026-05-02 — G5 final closure / Implemented**: `apps/builder/src/adapters/pencil/` 에 Pencil document-level import/export adapter, schema map, type re-export, 5개 `.pen` fixture, import/roundtrip tests 를 추가했다. `packages/shared/src/types/pencil-adapter.types.ts` 의 Phase 5+ stub 함수는 실제 순수 매핑 함수로 승격했고, ADR-116 import payload adapter 는 같은 mapper 를 사용해 `.pen` top-level node 를 reusable canonical master 로 normalize 한다. 검증: targeted vitest 3 files / 24 tests PASS + `pnpm run codex:typecheck` PASS.
...
- **2026-05-02 — ADR-113/116 G5 thirteenth cleanup slice**: Component semantics mirror read/write 를 `componentSemanticsMirror` adapter 로 분리했다. `ComponentSlotFillSection`, editing semantics fixture, store bridge, instance lifecycle action 은 component marker payload 를 직접 legacy helper 에서 읽거나 쓰지 않는다. 검증: targeted vitest 6 files / 57 tests PASS.
- **2026-05-02 — ADR-116 projection removal follow-up**: FramesTab/PageLayoutSelector/ComponentsPanel 과 BuilderCanvas/canvas drag-drop read path 는 active canonical document 를 사용하고, `selectCanonicalDocument()` projection rebuild 를 더 이상 수행하지 않는다. 검증: targeted vitest 8 files / 25 tests PASS.
- **2026-05-02 — ADR-116 projection removal follow-up #2**: BuilderCore frame refresh/publish, usePageManager hydrate, elementCreation history/reorder caller 경로도 active canonical document 또는 frame mirror adapter 로 전환했다. caller/store action 레벨의 `selectCanonicalDocument()` rebuild 는 제거했고, 잔여는 sync/adapter boundary 로 축소했다. 검증: targeted vitest 12 files / 60 tests PASS.
...
- **2026-04-27 (세션 36)**: Phase 1 함수 layer 완결 — `convertTemplateToCanonicalFrame` / `flattenTemplateElements` / `buildDescendantsFromSlots` / `hoistLayoutAsReusableFrame` / `dryRunMigrationP111` / `applyMigrationP111` (vitest 45/45 PASS, commits batch in PR #249 + `f4047af1`)
...
- **2026-04-27 (세션 37 후속)**: **PR-E3: dev-only canonical migration trigger** (PR pending)
  ...
  - `dryRunMigrationP111(adapter, projectId, canonicalDoc)` + `applyMigrationP111(canonicalDoc, result)` (in-memory) 호출. 결과 콘솔 group 로그
  ...
  - 검증: type-check 0 / FramesTab 33/33 회귀 0 / migrationP111 45/45 회귀 0 / db 전체 126/126 회귀 0
- **2026-04-27 (세션 37 후속)**: **PR-E4: Phase 2 cutover — flag default true 전환** (PR pending)
  ...
  - 검증: type-check 0 / 156/156 vitest PASS (FramesTab 33 + frameActions 7 + migrationP111 45 + canonical adapters 71) — vitest 는 mockState 격리로 default 변경 영향 0
...
- **2026-04-27 (세션 43)**: **회귀 fix #2 — canonical legacyProps element top-level fields 미보존** (PR #272 / `ccc06b30`)
  ...
  - **monitoring 카운터 2차 reset** — fix #2 land 시점부터 새 1주 시작 (~2026-05-04+ 추가 연장). 이후 ADR-116 선행 결정으로 monitoring 기반 Phase 2 Implemented 승격 경로는 historical record 로만 유지한다.
...
- **2026-04-27 (세션 45)**: **Phase 2 closure 5단계 사전 체크리스트 land** — `docs/adr/design/111-closure-checklist.md`
  - monitoring 종결 (~2026-05-04+) 후 즉시 Implemented 승격 가능하도록 사전 작성했으나, 이후 ADR-116 선행 결정으로 checklist 는 historical 문서가 됨
...
- **2026-04-28 (세션 46)**: **Phase 3 fundamental 결함 발견 — frame canvas authoring 시각 path 미구현 + monitoring 종결 framing 정정**
  ...
  - **결함 위치**: ADR-111 cutover commit `7b6f4eb9` 가 단순 `featureFlags` default flip (4 file / 38/-8 라인, 실 logic 0건) — frame canvas authoring 시각 path 는 dual-mode 시절부터 미구현. cutover 의 회귀가 아니라 **ADR-111 자체의 fundamental 미완성**
  - **Gate G2 (시각 회귀 0) 위반 확정** — Phase 2 closure 5단계 체크리스트는 보류. monitoring 6일 대기 framing 무의미 (사용자 결정: monitoring 종결 의미 없음, fix 가 본질)
  - **본 세션 진행**: design breakdown 신규 sub-phase land 만 (`docs/adr/design/111-phase3-frame-canvas-authoring-breakdown.md`). 본격 fix (1주+ HIGH) 는 별도 세션
  - **scope 분리**: 본 결함 = frame body **base render** (영역 자체 캔버스 표시) — ADR-111 의 P3 신규 sub-phase. ADR-112 의 시각 마커 (reusable/ref/override 표식) 는 본 base render 위에 land 가능 — prerequisite 관계
...
- **2026-04-28 (세션 47 후속, 본문 보강)**: **revision 2 — Hard Constraint #6 + Gate G6 신설 (Properties 패널 ##Slot section##)**
  ...
  - Gate G6 신설: section 노출 + slot field 상태 표시 + 컨트롤 동작 + ADR-112 ##Component section## 와 패널 공존 / UI 충돌 0
  - **2026-04-28 세션 50+ hardening 으로 superseded**: 위 Slot section / Gate G6 소유권은 ADR-112 로 이관. 본 ADR 은 해당 base UI 를 구현하지 않는다
  - References 보강: pencil components docs + keyboard-shortcuts docs URL + `.pen` 2.11 핵심 type 명시
  - ADR-112 본문 동시 revision 2 — 6 요소 확장 (⑥ Origin 토글 추가, `Cmd+Opt+K`/`Cmd+Opt+X` pencil 단축키 호환 baseline, Properties 패널 ##Component section## 위치 결정)
...
- **2026-04-28 (세션 49)**: **P3-θ Slot Fill Resolution land — Gate G3-θ a/b/c/e 충족 (frame instance composition 진입)**
  ...
  - **D8=A (legacy slot_name 매칭)**: page root element 의 `slot_name` (props.slot_name 또는 element.slot_name) 이 frame Slot 의 `name` 과 일치 시 page element 의 `parent_id` 를 해당 Slot 의 id 로 재매핑. 매칭 안 된 slot_name 은 `"content"` fallback. canonical descendants 전환은 ADR-113 Phase 5-A (`slot_name` cleanup) 시 동기화.
...
- **2026-04-28 (세션 49 후속) — 동결 결정**: **본 ADR Phase 3 후속 (P3-ε / P3-ζ) 진행 정지**
  - **결정 사유**: ADR-112 codex 3차 리뷰 + 사용자 framing 재정의 — 본 ADR 은 ADR-112 (reusable component 추상) 의 **frame-bundled preset 응용**. 의존 방향이 거꾸로 박혀 있었음 (baseline ADR-903 Phase 4 framing 의 stale).
  ...
  - **정지 영역**: P3-ε (FramesTab inline frame editing 진입), P3-ζ (Chrome MCP 회귀 검증 추가), G3-θ (d) Chrome MCP screenshot — 모두 ADR-112 Component/Slot base 완료 후 재개.
  - **다음 진입**: ADR-112 Component/Slot base 완료 → 본 ADR 은 P3-ε / P3-ζ 를 frame authoring 편의 확장으로 재설계 진입.
- **2026-04-28 (세션 50+ 후속) — 의존 방향 hardening**:
  - 이전 "ADR-111 ##Slot section##" 소유권 표현은 잘못된 설계 전제였음.
  - Slot section base 는 ADR-112 가 완료한다. 본 ADR 은 이를 재정의하지 않고, 완료된 기능을 FramesTab / preset UX 에서 편하게 쓰게 하는 확장만 담당한다.
- **2026-04-30 — 재개 조건 충족**:
  - ADR-112 가 Component/Slot base 와 G4-A~H gate 를 `Implemented` 로 닫음.
  - 본 ADR 의 P3-ε / P3-ζ 는 이제 재개 가능하나, scope 는 ADR-112 기능을 호출하거나 추천값/preset/FramesTab 편의 진입점을 제공하는 확장으로만 제한한다.
...
- **2026-04-30 — P3-ζ browser regression closure**:
  ...
  - 당시 잔여는 ADR-111 본문 Gate 기준의 G3 cascade 회귀 0, G4 legacy adapter 0건 + 명칭 충돌 해소, G5 pencil import/export schema-equivalent 검증이었다. 이후 2026-05-02 direct cutover + G5 final closure 로 닫았다.
- **2026-04-30 — G3 cascade slice #1 (`duplicateLayout` immediate merge)**:
  ...
  - 당시 잔여: G3 전체 완료 조건인 canonical-native `deleteReusableFrame` / `duplicateReusableFrame` / `setPageFrameRef` 전환, 50+ fixture roundtrip, undo/redo 검증은 ADR-116 G2 canonical store/API 이후 재개하기로 했다. 이후 direct cutover 에서 canonical frame action surface 로 정리했다.
- **2026-04-30 — G3 cascade slice #2 (`deleteLayout` orphan page-ref cleanup)**:
  ...
  - 당시 잔여: element cascade 자체는 `layout_id` legacy fallback 기반이었고, canonical-native frame subtree mutation 으로의 완전 전환은 ADR-116 G2 이후 후속 G3 작업이었다. 이후 adapter boundary + active `CompositionDocument` path 로 정리했다.
- **2026-04-30 — G3 cascade slice #3 (`setPages` page frame binding invalidation)**:
  ...
  - 잔여: canonical-native `setPageFrameRef` write API 로 전환되면 이 live invalidation 계약도 ADR-116 canonical mutation boundary 로 이동해야 한다.
...
- **G3 (b)**: NodesPanel `LayoutsTab` → `FramesTab` 재설계 (canonical `ref`/`slot`/`descendants` 직접 조작). 사용자 가시 frame authoring base 는 닫혔고, 직접 canonical mutation 은 ADR-116 G2 이후 재개
- **G3 (c)**: repo-wide 결합 해체 — 잔여 caller (FramesTab/layoutActions/usePageManager/PageLayoutSelector 등 다수) `el.layout_id === X` 매칭이 여전히 legacy fallback 으로 동작. adapter-only 격리는 ADR-116 G5 field quarantine 에서 닫음
- **G3 (d)**: layout-vs-page 이원화 UI 전원 canonical frame authoring UI 로 치환 — 기능 상실 0. 최종 write source 전환은 ADR-116 G4/G5 이후 확정
...
4. **ADR-112 우선 조건** — 본 ADR 은 ADR-112 의 UI 토대를 제공하지 않는다. ADR-112 가 Component/Slot base 를 먼저 완료해야 하며, 본 ADR 은 그 기능 위 편의 확장만 제공한다
...
6. **Properties 패널 ##Slot section## 은 ADR-112 base 로 이관** (revision 2 표현 supersede):
   - frame 1 개 선택 시 Properties (Inspector) 패널의 별도 ##Slot section## 에 노출되는 base UI 는 ADR-112 가 소유
   - `Frame.slot: false | string[]` schema 호환과 `[+]` / `[-]` 기본 동작도 ADR-112 gate 에서 완료
   - 본 ADR 은 Slot section 을 새로 정의하지 않는다. 이후 FramesTab / preset UX 에서 해당 ADR-112 기능을 호출하거나 추천값을 제공하는 편의 확장만 가능
...
| ---------------------------------------- | ---------------------------------------- | ------------------------------------------------------ |
| `Element.tag`                            | `node.type`                              | **rename** (ADR-113 scope)                             |
...
| `LayoutsTab` (UI)                                                        | `FramesTab`                     | pencil 표준 정합 — 이미 ADR-111 결정                                                               |
...
> 구현 상세: [111-layout-frameset-pencil-redesign-breakdown.md](../design/111-layout-frameset-pencil-redesign-breakdown.md) — 후속 세션에 작성 (Phase 분해 + 파일 변경 목록 + 마이그레이션 도구 + 검증 시나리오)
...
| **G3**: cascade 회귀 0 (R3 매핑)              | ADR-116 G2 이후 Phase 3 완료         | (a) deleteLayout / cloneLayout / addPageToLayout / removePageFromLayout 50+ fixture roundtrip read-write-read 정합 0 drift / (b) undo/redo 정상 / (c) canonical-native `deleteReusableFrame` / `duplicateReusableFrame` / `setPageFrameRef` API 사용                                                                                                                                                                                            | layoutActions 재작성 보강                      |
| **G4**: legacy adapter 0건 + 명칭 충돌 해소   | ADR-116 G5 안에서 Phase 4 완료       | (a) `apps/builder/src/builder/stores/layouts.ts` 본체 삭제 / (b) repo-wide grep `LayoutsTab` / `legacy layout_id` 결과 0 / (c) `useLayoutsStore` 호출 site 0건 / (d) **명칭 충돌 해소** — `PanelSlot.tsx` → `PanelArea.tsx`, `BottomPanelSlot.tsx` → `BottomPanelArea.tsx` rename land (Builder UI panel slot ↔ pencil `slot` 의미 격리). repo-wide grep `PanelSlot\|BottomPanelSlot` 결과 0 (`apps/builder/src/builder/layout/` 디렉토리 한정) | adapter shim 디렉토리 한정 + dead code 제거    |
| **G5**: pencil 호환 검증                      | ADR-116 G6 이후 Phase 5 완료         | (a) 샘플 pencil `.pen` 파일 5종 import → composition canonical document 변환 → roundtrip export → 원본과 schema-equivalent (binary diff 가능 영역만) / (b) ADR-116 이 흡수한 `imports` resolver/cache adapter 인터페이스와 통합 가능                                                                                                                                                                                                            | composition 확장 필드 namespace 격리 보강      |
| ~~G6: Properties 패널 ##Slot section## land~~ | ~~Phase 5 완료 또는 별도 sub-phase~~ | **Superseded by ADR-112**. Slot section base, `Frame.slot: false \| string[]` 기본 표시, `[+]` / `[-]` 기본 동작은 ADR-112 가 소유. 본 ADR 은 완료된 ADR-112 Slot 기능에 추천값/preset/FramesTab 편의 진입점만 제공 가능                                                                                                                                                                                                                        | ADR-112 완료 전 본 ADR 에서 구현 금지          |
...
- ADR-116 이 흡수한 ADR-903 P5-D/E (`imports` resolver/cache) 와 자연스럽게 통합 — DesignKit 통합은 [ADR-115](115-remove-designkit-system.md) 로 제거됐고 ADR-114 는 Superseded 됨
...
- [ADR-116](116-canonical-document-ssot-transition.md) — canonical document SSOT 전환. 본 ADR 의 남은 G3/G4/G5 실행 순서를 선행 gate 로 재정렬
- [ADR-112](112-editing-semantics-ui-5elements.md) — Editing Semantics UI 6요소 + Slot section base. 본 ADR 은 ADR-112 의 영향을 주지 않으며, 완료된 ADR-112 Component/Slot 기능 위 frame authoring 편의 확장만 제공
- [ADR-114](114-imports-resolver-designkit-integration.md) — Superseded. `imports` resolver/cache 잔여 scope 는 ADR-116 + 본 ADR G5 로 흡수
...
- 사용자 직접 확인 (2026-04-28 세션 47 / 세션 50+ 후속 정정) — Properties 패널 우측에 Component section + Slot section 별도 노출, `[-]` `[+]` `Create component` 컨트롤 명시. 후속 정정: Component/Slot base 는 ADR-112 소유, ADR-111 은 편의 확장
