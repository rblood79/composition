...
Deprecated — 2026-07-30 (실사용 회귀로 **코드 전량 되돌림**). 이력: Proposed 2026-07-29 → 리뷰 round 1 승인 (`reviews/172.md`, 이슈 2건 전건 fixed) → Implemented 2026-07-30 (G1~G5 전부 통과) → **당일 되돌림**

> 본 문서는 **음성 결과 기록**으로 보존한다. 게이트를 전부 통과하고도 실사용에서 실패한 사례이므로, 같은 설계를 다시 시도하기 전에 아래 §되돌림 기록을 먼저 읽을 것.

## 되돌림 기록 (2026-07-30)

### 무엇이 일어났나

Implemented 승격 당일, 사용자가 빌더에서 다음을 보고했다:

- 재래스터 후 **컴포넌트 텍스트가 불특정하게 렌더되지 않음** (초기 화면 밖 영역으로 스크롤 시 어떤 곳은 나오고 어떤 곳은 안 나옴)
- **Page 6~18 이 정상 표시되지 않음**
- **성능 개선이 체감되지 않음** — "퍼포먼스도 업그레이드 하지못하고 오류만 지속적으로 발생"

ADR-173(본 ADR 의 후속 축)과 함께 `61a191b35` 로 전량 되돌렸다.

### 왜 게이트가 이를 못 걸렀나 — 측정 설계 결함

G1~G5 를 전부 통과했으나, 측정 조건이 **본 ADR 이 최적화한 경우만** 골라 재고 있었다:

| 측정한 것                                                   | 측정하지 않은 것                    |
| ----------------------------------------------------------- | ----------------------------------- |
| 팬 프레임 + **가시 집합 불변** (캐싱에 가장 유리)           | 새 영역으로 스크롤 (가시 집합 변경) |
| 합성 벤치 문서 (5,046 요소 중 대부분 `bench-*` 필러 텍스트) | 실제 컴포넌트 구성 문서             |
| 파생 재계산 **횟수 0**                                      | 프레임 전체 벽시계 시간             |
| —                                                           | 브레이크포인트 전환 / 편집 경로     |

"20배 규모 end-to-end 실측 통과" 는 그래서 실사용을 대표하지 못했다. **작업량 지표(count=0)가 0 이어도 프레임 총비용은 개선되지 않을 수 있다** — 제거한 파생 비용이 실제 병목이 아니었기 때문이다.

### 재시도 시 선결 조건

1. **먼저 프레임 총비용을 실문서에서 분해할 것.** 파생 계층이 몇 %인지 모르는 상태에서 그 계층만 최적화하면 개선 상한이 그 비율에 갇힌다.
2. **측정 문서를 합성 필러로 채우지 말 것.** `bench-*` 텍스트 5,000개는 실제 컴포넌트 트리와 비용 구조가 다르다.
3. **게이트에 "가시 집합이 변하는 경로"를 반드시 포함할 것.** 팬 중 가시 집합 불변은 최선의 경우이지 대표 경우가 아니다.
