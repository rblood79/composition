...
Implemented — 2026-09-10. P0–P5 완료, G0–G5 PASS. 최종 제품 revision `53c761c8b`. **G4-policy 는 사용자 승인으로 종결** (2026-09-10): Builder/Preview 전체 초기 번들 (gzip 파일별 합계) 에만 한시 상한 **Builder ≤1,297,311 B · Preview ≤630,625 B** (최종 revision 실측값, 여유분 없음) 를 적용한다. Publish 는 기본 <500,000 B 유지. 유효 기간은 **2026-10-10** 또는 초기 번들에 영향을 주는 변경 중 먼저 오는 시점까지이며 (ADR-209 B 안과 같은 날), 그 뒤에는 재측정과 축소 계획 또는 재승인이 필요하다. ADR-209 B 안 상한 (1,289,801 / 626,424) 은 이 승인이 대체한다. 차트 순증 (≤10 KiB) · lazy (≤200 KiB) · runtime 조건은 유지한다. 원래 Proposed 2026-09-10 (코드 기준 `baf535258`, round 1·2 리뷰 승인) → In Progress → Implemented.
...
- 2026-09-10 P4: before `baf535258` / after `c8ce1666c` (판독 수리 뒤 최종 `53c761c8b` 로 번들·W800·frame 재측정: initial +7,510/+4,201/+2,821 B · columns W800 41.3/75.2/42.1/42.9 ms · frame Δ 최악 +0.6 — 같은 판정) 를 **별도 clean worktree + 원래 lockfile** (sha `1196b859e4db8ffb`, 두 commit 사이 lockfile·엔진 diff 0) 로 측정. **번들** (final production, gzip): initial 순증 Builder +7,473 / Preview +4,196 / Publish +2,820 B (한도 10 KiB) · lazy chart graph +305/+305/+284 B (한도 200 KiB) · Recharts initial 유입 0. **runtime static** (`adr210-runtime-perf.mjs`, headed, 3+12 표본): columns W800 4종 p95 area 41.4 · bar 75.4 · line 41.1 · radar 41.3 ms (≤100 PASS; adverse 긴 한글 이름+KRW+expand 최대 81.7) · W200 6종 before ≈ after · 같은 마크 수 group800 bar 117.8 → 77.4 · 8/16 시리즈·5000행 비용 보고 (bar 1,600 rect 114 · 3,200 rect 325 · 20,000 rect 1,363 ms). 첫 측정은 bar W800 116ms **FAIL** → `9758c169c` Bar plain path shape (Recharts `Rectangle` 의 막대당 ref/state/`getTotalLength` 제거) + 시리즈 색일 때 Cell 생략으로 수리. **Builder frame A/B** (`adr209-builder-frame-ab.mjs`, DEV 두 worktree, W200 6-chart, 5쌍 순서 교대): Δ p95 −0.6/−0.6/−0.6/−0.5/−0.5 (한도 +1 PASS). 첫 측정은 쌍 1 Δ +1.1 **FAIL** + longtask 2배 → 원인 Properties 의 ADR-210 컨트롤 3개가 매 op 재렌더 (24 op 당 19회) → `c8ce1666c` 컨트롤 입력 필드만 골라 memo · `useCollections` memo · key remount → `useOwnedState` 로 수리 (24 op 당 7회, select 당 mount ≈10ms DEV 만 남음, longtask median 11 → 15 기록). **Builder wide W800 별도 보고**: 같은 after 빌드에서 columns800 vs 같은 종류 group800 = `render.frame` p95 24 vs 10 ms — Canvas `CHART_SAMPLE_ROWS=200` 이 **행** 을 자르므로 columns 는 캔버스 마크 4× · 문서 행 1/4 (group 은 store longtask 2×). 셀 기준 cap 은 사용자-가시 변경이라 **후속 판정 항목** (미변경). **production network** (`adr209-f4-production-network.mjs`, after dist) 9/9. `/review` 1회 HIGH 0. 근거 [P4 evidence](../evidence/210-p4-bundle-runtime-frame.md) (로컬). **G4-engineering PASS · G4-policy 미결**: 전체 initial <500,000 B 는 Publish (393,113) 만 충족, Builder 1,297,274 / Preview 630,620 은 기본과 ADR-209 B 한시 상한 (1,289,801 / 626,424) 을 각각 +7,473 / +4,196 초과 — 예외 자동 승계 없음, **사용자 재승인 또는 축소가 P5 종결 조건**.

- 2026-09-10 P5: 최종 `53c761c8b` 에서 `pnpm codex:preflight` FAIL 0/WARN 0 · builder unit 714 files/5,689 · shared 1,174 · specs 1,160 · chart Chromium 212 + dark/reduced-motion 149 · 엔진 parity (아래 §Live Exercise) · live P2 6/6 · P3 7/7 · ADR-209 Series 흐름 (legacy 호환) 재실행. 호환·rollback 제한은 아래 §호환 · rollback 제한 에 확정 기재. **G5: 열린 필수 조건 1 (G4-policy) — Implemented 미승격, In Progress 유지.** 후속 판정 항목 2: Canvas `CHART_SAMPLE_ROWS` 셀 기준 cap · Properties Chart 컨트롤 select 당 mount 비용 (UI 축소).
- 2026-09-10 G4-policy 승인 · G5 종결: 사용자가 예산 예외를 승인했다 — Builder ≤1,297,311 / Preview ≤630,625 B (실측값 상한, 여유분 없음), 만료 2026-10-10 또는 initial 영향 변경 시, Publish 기본 유지. 축소 대안 (ADR-209 B 상한 이하로 되돌리기, P4 재측정 필요) 과 여유분 부여 대안 (+10 KiB) 은 기각. 이로써 열린 필수 조건 0 → **G5 PASS, Implemented 승격 · completed 이관**. ADR-209 Series 하니스 재실행의 Export 단계 FAIL 1 (17/18) 은 하니스 타이밍 flake 로 분류하되 미통과 사실을 그대로 남긴다 (같은 경로는 P3 하니스 7/7). 후속 판정 항목 2 (Canvas `CHART_SAMPLE_ROWS` 셀 기준 cap · Chart 컨트롤 select 당 mount 비용) 는 별도 조사로 이관 — 표시 정확성과 성능을 같이 비교한 뒤 정책을 정한다. 근거 [P5 evidence](../evidence/210-p5-closure.md) §5 (로컬).

### 호환 · rollback 제한 (T12 · breakdown §8)

- 새 8키를 쓰지 않은 기존 문서: 강제 재직렬화 0, 자동 migration write 0, 기존 export 3종의 scene byte 동일 (구버전 `305e4c4f7` probe).
- 새 설정 (columns · seriesConfig · valueFormat …) 문서를 구버전에 넣으면 **무손실 렌더를 지원하지 않는다** — 구버전은 알 수 없는 props 를 무시해 빈 차트 또는 legacy `metric` 단일 시리즈로 그린다 (데이터 손실 0). 지원되는 rollback 은 **업데이트 전 원본 export/DB 백업 복원**이다. wide→long 자동 export adapter 는 범위 밖.
- 새 클라이언트에서 `dataMode:"group"` / `valueFormat:"auto"` 로 명시 전환하면 보존된 legacy `metric`/`color` 로 돌아가지만 시리즈 표현 (이름·토큰 색·순서) 을 보존하는 downgrade 는 아니다.
- ref 인스턴스의 `seriesConfig`/`valueFields` override 는 배열 전체 교체다 — origin 의 이후 항목 추가가 자동으로 합쳐지지 않는다 (명시 고정 버튼이 그 계약을 드러낸다).

### Live Exercise

실제 Builder (DEV 5173, 로그인 세션, headless Chromium) 에서 팔레트 Chart → Properties 의 시리즈 원천/시리즈 설정/숫자 형식 컨트롤 mount → 값 컬럼 Apply (canonical 단일 patch, Skia 픽셀 변화) → Undo 1회 → 통화 묶음 → reload 보존 (P2 6/6, `adr210-chart-p2-live.mjs`); 실제 패널로 표시 이름·`--chart-series-5`·USD 설정 → Skia 주색 → Themes dark 스위치 → Compare Mode Preview 의 문자열/fill 이 Skia 픽셀과 일치 (light/dark) → hover·키보드 tooltip canonical write 0 → 메뉴 Export → 독립 publish (3001) 같은 문자열·fill (P3 7/7, `adr210-chart-p3-live.mjs`); 로그인된 production 빌드에서 cold boot/편집 chunk 0 · Preview lazy 1회 · 독립 Publish (P4 F4 9/9). 근거 [P2](../evidence/210-p2-properties-semantic-write.md) · [P3](../evidence/210-p3-consumers-preview-publish.md) · [P4](../evidence/210-p4-bundle-runtime-frame.md) (로컬 evidence, 화면 `210-p2/`·`210-p3/`).
