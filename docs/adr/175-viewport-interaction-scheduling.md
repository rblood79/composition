...
| ---------------------------- | --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| G1 Baseline                  | Phase 0   | 제공된 Builder route와 실제 다페이지 문서에서 drag, wheel pan, `Ctrl`/`Cmd + wheel`, toolbar/keyboard zoom의 raw event 수, controller listener 수, mirror write 수를 기록한다. 장비·refresh rate·sample 수·window를 명시하고, `input.*` handler body, RAF 간 wall-frame interval, `render.frame`, `longtask.*`를 서로 다른 표본으로 저장한다. | 계측이 없거나 synthetic-only이거나 표본 정의가 섞이면 구현 시작 금지                               |
