...
- 문서의 결론 "Uppy + tus 서버" 는 라이브러리 채택 안이다. Uppy core + Dashboard + Tus 플러그인은 100KB+ gz (추정) 이고 UI 가 RAC/catalog 밖이라 D1/D3 대칭이 깨진다. 문서에서 가져올 것은 **TUS 1.0 프로토콜** (POST 생성 → HEAD offset 조회 → PATCH 청크 반복, 오픈 표준 — Vimeo/Cloudflare/Cloud 채택) 이다. ADR-194 (차트) 가 같은 상황에서 "라이브러리 비교 → 의존 0 자체 구현" 으로 간 선례다.
