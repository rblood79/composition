...
3. **하위 호환** — 기존 `global`→project, `page+page_id`→page, `component`→project로 결정적으로 변환한다. 모든 component 변환에 `migrationStatus:"owner-unresolved"`와 가시적 인덱스 배지/로그를 남기며 조용한 변환은 0%다. G0에서 프로젝트별·전체 비율을 계수하고 저장 전까지 원본은 재직렬화하지 않는다.
