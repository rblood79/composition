...
> **진행 상태 (2026-06-03 실측 — review-adr round 7)**: 코드 착수 진행 중. Status 는 **Proposed 유지** — 단계 4 collection 전체 발효 + 단계 5 구 정본 제거가 남아 ADR 전체 미완(Implemented 아님). land 완료 범위:
>
...
> - **단계 4 (collection 발효) — 부분 (ListBox proof)**: ListBox Skia generic 발효(`173765201`) + projected→canonical write 변환 인프라(`d5da74c72`) + text measurement generic 전환(`831079766`). **3계약(C1/C2/C3) 나머지 6 collection+Table 발효는 미착수**(설계만 `5dae6811b`).
> - **단계 5 (구 정본 제거) — 미착수**: 선행 의존 실측만 기록(`06ddfc44d`).
>
> 즉 본문 Risk/Gate 의 "미증명"·"증명 예정" 중 단계 1~3 영역은 land·증명됐고, 단계 4(collection 전체)/5 만 미완. 아래 Risk 표의 증명 상태 열 참조.
...
잔존 HIGH 위험: R-1 / R-2 / R-3 / R-4 (4건). **증명 진행 (2026-06-03 실측)**: R-2 증명됨(G-adapter PASS), R-1·R-4 부분 증명(Button slice / racStateAttrs disabled), R-3 부분(ListBox proof). 미완 = R-1 family 전역(단계 4/5) / R-3 generic projector 60fps(단계 4) / R-4 interaction threading(후속). 대안 B 는 family 격리(ADR-910)를 포기하므로 R-1 의 전역 영향이 대안 A 보다 크나, Button vertical slice 우선 증명(완료) + 구 정본 제거 지연(단계 5)으로 회귀 표면을 단계별로 닫는다.
