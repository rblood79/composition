...
> **진행 상태 (2026-06-04 갱신 — 단계 4 Menu 발효 = collection 7종 전부 발효 완료)**: 코드 착수 진행 중. Status 는 **Proposed 유지** — 단계 5 구 정본 제거가 남아 ADR 전체 미완(Implemented 아님). 단계 4(collection 발효)는 7종 전부 완료. land 완료 범위:
>
...
> 구현 상세: [912-rac-pencil-rebuild-cutover-breakdown.md](design/912-rac-pencil-rebuild-cutover-breakdown.md) — 진단 확정 / 갈아엎기 경계(제거·재사용·신규 3분류) / 영역 A(schema+어댑터) / 영역 B(collection 발효 3계약: C1 generic projector + C2 rule fill 정렬 + C3 children·text 중복 방지) / 영역 C(편집계약+state) / HC 1:1 / 구현 순서. 실행 phase 분해 + sub-group 결정은 사용자 confirm 후(adr-writing.md fork checkpoint).
...
- 컴포넌트당 `render.shapes()` Skia source 가 폐기 방향(theme rule 대체) — 단 ADR-907 Layer B/908 fill/909 longhand 는 보존(override layer 유지). ADR-036 status 재평가 필요. **단계 5 선행 의존(실측 2026-06-04 갱신)**: `render.shapes()` 는 단일 그리기가 아니라 3 call site — (1) Skia 그리기(`buildSpecNodeData.ts:1187`, 현재 `{ skiaLegacy: true }` entry 가 의존) — (1a) collection 7(Menu/Select/ComboBox/Tabs/TagGroup/GridList/Table) 은 **단계 4 가 전부 발효·해소 완료(2026-06-04)** + (1b) 잔여(Tooltip + date 4; Dialog/Modal/Popover/DropZone 은 이미 발효, Slider/color 는 skiaLegacy 없음)는 skiaPrimitive escape hatch + (2/3) 텍스트 측정(`specTextStyle.ts:167` + `specTextStyleForOverlay.ts:61`, 29 TEXT_BEARING_SPECS type → generic 측정 전환 후 제거). 따라서 단계 5 의 `render.shapes` 제거는 잔여 skiaPrimitive escape + 텍스트 측정 generic 전환 이후로 순서 재정의(breakdown ⑦ 단계 5 선행 의존 표). collection 발효 주체는 ADR-920(Superseded, 비착수)이 아니라 본 ADR 단계 4 — 사용자 결정 2026-06-03. 단계 4 완료로 (1a) 의존 해소됨.
