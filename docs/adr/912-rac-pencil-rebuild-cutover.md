...
- **theme rule base 층 source = starter CSS + design structured source → generated** (사용자 방향 2026-06-02, codex 결함 2 정정의 재정정): 현재 `COMPONENT_RULES_TABLE` 은 `generate-rules.ts` 가 124 spec 의 variants/sizes/fill 을 build-time 변환해 생성한다(`packages/specs/package.json` `build:specs` → `generate:rules`). spec 124 를 삭제하면 이 변환기의 입력이 사라져 base 층(HC#3 의 base)이 통째로 정의되지 않는다. **해법은 base 의 입력 source 를 spec(전환기 부채)에서 `packages/react-aria-starter/src/*.css`(RAC 공식 starter 시각 원천) + design structured source 로 교체**하는 것이다. 149KB `componentRulesTable.ts` 를 사람이 손으로 저작하는 것은 비현실적이므로, **starter style inventory → structured generated source(`packages/design-system/*.ts` 또는 `design.generated.json`) → `componentRulesTable.ts` + theme tokens + `reactAriaGenerated.css` 생성** 흐름으로 둔다. `packages/design.md` 는 정본이 아니라 그 structured source 의 **human audit / 검증 문서**로 유지한다(design.md §Mapping 이 이미 "starter 측 정의는 css-tokens.md 가 정본, 본 표는 참조" 로 자기 위상 선언). spec 124 + `generate-rules.ts`(spec→table 변환기) + `pnpm generate:rules` step 은 동시 삭제하되, base source 가 한 순간도 정의되지 않는 구간이 없도록 starter-based 생성을 먼저 land 한다. `resolveComponentRule` 은 위치·시그니처 무변경(생성된 테이블을 read). 상세 전환·대안은 breakdown ②-6.
...
| ----------- | -------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| G-slice     | 공통 기반 (R-1)                  | Button 노드 `size="md"→"sm"` 편집 1회 → DOM / Skia / Properties Panel / Style Panel / Publish 5곳이 같은 노드 값(`fontSize:14`) + 같은 generated theme rule base(starter→structured→table 단일 source 파생)를 즉시 동일 반영 + `/cross-check` 대칭 PASS. 단일 공급원(HC#1·#2) 코드 증명. **성공 조건 = legacy seam 실제 제거 + 같은 source**: Button 경로에서 `resolveComponentVisual` / `render.shapes()` / buildCatalogShapes·CSSGenerator 의 spec 읽기를 **제거하고도**, DOM generated CSS 와 Skia runtime rule 이 같은 generated source 에서 파생되어 5곳 작동. "신규 함수가 작동하지만 legacy seam(spec) 을 fallback 으로 유지" = **실패**(dual-SSOT 재현). **kill criteria**: 깨끗하게 안 나오면 → 전면 확장 금지, ③ 영역 A 설계 재보정. **1차 kill 시점 = 1A common spine 직후(7~10h)** — Button spine 이 지저분하거나 seam 호출 필요 시 1B(consumer wiring)/1C(검증) 진입 전 즉시 재보정(breakdown ⑦). 단계 1 전체 예상 20~28h(보수적 3일) | **kill: 전면 확장 중단 + ③ 공통 기반 설계 재보정** (1A 직후 1차 판단, 단계 2~5 진입 금지) |
| G-adapter   | base⊕override 어댑터 (R-2)       | DOM base = generated CSS + `data-*`(build-time), Skia base = 같은 generated theme rule runtime resolve — **둘이 같은 generated source 파생**(starter→structured→table). `toReactStyle` 은 `props.style` override 전용(DOM 색/size base 는 generated CSS 가 담당, 인라인 색 주입 아님), `toSkiaStyle` 은 같은 rule base ⊕ override runtime 병합. `/cross-check` 시각 대칭 + reset-to-default(`delete props.style[k]` → base 복귀) round-trip. base/override 2층(HC#3) 증명. **성공 조건 = fallback 0 + 같은 source**: `applyInlineBorderOverlay` 류 사후 override 우회 / spec seam fallback 없이, DOM generated CSS 와 Skia runtime rule 이 같은 generated source 에서 나온다. ("같은 inline object" 가 아니라 "같은 source 파생" 이 성공 기준). **kill criteria**: DOM generated CSS 와 Skia runtime rule 이 같은 source 로 시각 대칭이 안 나오면 → 어댑터/source 책임 분해 재설계                                                                 | **kill: 전면 확장 중단 + 어댑터/source 책임 분해 재설계** (단계 2~5 진입 금지)            |
...
- theme rule base 층의 입력 source 가 spec(전환기 부채)에서 starter CSS + design structured source(`packages/design-system/*.ts` 또는 `design.generated.json`)로 바뀐다 — base 시각 값을 spec 124 가 아니라 starter 원천에서 generated 한다. `generate-rules.ts`(spec→table) 가 starter→table 생성기로 교체되며, 새 생성기 신뢰성(starter CSS 파싱·design source 구조)이 새 부담이다(R-2 와 연계). 단 `componentRulesTable.ts` 를 149KB 손 저작하는 것보다 현실적이고, spec dual-SSOT 소멸 + base 단일 source 라는 이득이 상회. `design.md` 는 정본 아니라 audit/검증 문서로 유지(markdown 표 파싱 취약성 회피 — structured source 가 정본 입력). ADR-907/908/909 의 fill/spacing/longhand 구조는 생성된 테이블의 필드로 직접 표현된다.
