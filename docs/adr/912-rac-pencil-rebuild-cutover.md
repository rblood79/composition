...
> - **단계 4 (collection 발효) — 부분 (ListBox + 비-data-bound 4종)**:
...
>   - **잔여 — 미착수**: Menu(popup↔trigger 본질 미확정, 사용자 보류 2026-06-03) / GridList·Table(data-bound row 순회 → C1 generic collectionProjector 필요, appendListBoxRowProjection→CollectionProjectorConfig 일반화). 설계는 `5dae6811b` breakdown ④.
> - **단계 5 (구 정본 제거) — 미착수**: 선행 의존 실측만 기록(`06ddfc44d`).
>
> 즉 본문 Risk/Gate 의 "미증명"·"증명 예정" 중 단계 1~3 + 단계 4 비-data-bound 4종은 land·증명됐고, 단계 4 잔여(Menu/GridList/Table)/단계 5 만 미완. 아래 Risk 표의 증명 상태 열 참조.
...
| R-3 | collection Interactive Projected Tree 가 60fps + 깊은 노드 편집 동시 성립하는지 — projected id ↔ canonical write target 분리 정합                                                                                                                                                                                                                                                            | **HIGH** | **부분 (ListBox proof)**: projected→canonical write 변환 인프라 land(`d5da74c72`) + ListBox 발효(`173765201`). Gate G-projected(10k row 60fps + 깊은 노드 + projected id 비유입)는 단계 4 generic projector 확장 시 증명 — **미완**                                                                                                                                                                  |
...
잔존 HIGH 위험: R-1 / R-2 / R-3 / R-4 (4건). **증명 진행 (2026-06-03 실측)**: R-2 증명됨(G-adapter PASS, 비-data-bound 4종 발효 후 generated CSS diff 0 부분 확증), R-1 부분 증명 확대(Button slice + 비-data-bound 4종 발효 회귀 0), R-4 부분 증명(racStateAttrs disabled), R-3 부분(ListBox proof). 미완 = R-1 잔여 family 전역(Menu/GridList/Table, 단계 4/5) / R-3 generic projector 60fps(단계 4 GridList/Table) / R-4 interaction threading(후속). 대안 B 는 family 격리(ADR-910)를 포기하므로 R-1 의 전역 영향이 대안 A 보다 크나, Button vertical slice + 비-data-bound 4종 단계 증명 + 구 정본 제거 지연(단계 5)으로 회귀 표면을 단계별로 닫는다.
