...
> **진행 상태 (2026-06-03 갱신 — 단계 4 Table 2D C1 발효)**: 코드 착수 진행 중. Status 는 **Proposed 유지** — 단계 4 잔여 collection(Menu) 발효 + 단계 5 구 정본 제거가 남아 ADR 전체 미완(Implemented 아님). land 완료 범위:
>
...
> - **단계 4 (collection 발효) — 부분 (ListBox + 비-data-bound 4종 + GridList C1 + Table 2D C1)**:
...
>   - **잔여 — 미착수**: Menu(popup↔trigger 본질 미확정, 사용자 보류 2026-06-03 — `.react-aria-Menu` popup 리스트 vs rule trigger 버튼, Menu.css 정본 경계 별도 결정). 설계는 `5dae6811b` breakdown ④.
> - **단계 5 (구 정본 제거) — 미착수**: 선행 의존 실측만 기록(`06ddfc44d`).
>
> 즉 본문 Risk/Gate 의 "미증명"·"증명 예정" 중 단계 1~3 + 단계 4(비-data-bound 4종 + GridList C1 + Table 2D C1)는 land·증명됐고, 단계 4 잔여(Menu)/단계 5 만 미완. 아래 Risk 표의 증명 상태 열 참조.
...
| ID  | 위험                                                                                                                                                                                                                                                                                                                                                                                         |  심각도  | 대응                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :------: | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| R-1 | generic 공통 기반(resolve + generic DOM/Skia + 단일 어댑터 + theme resolve) 1 버그가 family 격리 없이 전 컴포넌트 동시 회귀                                                                                                                                                                                                                                                                  | **HIGH** | **부분 증명 확대 (2026-06-03)**: Gate G-slice — Button vertical slice 공통 기반 land·증명(`d7be36b53` 1C). + 단계 4 비-data-bound 4종(Select/ComboBox/Tabs/TagGroup) 발효(`0e9c501d4`)가 generic buildCatalogShapes 경로 전역 영향 실측 — vitest 회귀 0(+7 통과)·drift 102/102·generated CSS diff 0. 잔여 family(Menu/GridList/Table) 전역 영향 + 구 정본 제거(단계 5)는 발효 검증 후로 지연(미착수)                                                                                                                                                                                                                                                                                               |
| R-2 | base⊕override 병합이 DOM/Skia 동일 시각 결과를 내는지. 1A-(c) 실측: 분리 코어 `resolveMergedStyle` + 두 backend 어댑터 `toReactStyle`/`toSkiaStyle` 모두 **shared/catalog** 집중(의존 그래프 shared→specs 라 `toSkiaStyle` 만 `resolveToken`[specs] 추가 import). `buildCatalogShapes`(specs generic 생성기, KEEP)가 이미 산재 13키 ad-hoc 병합 중 → 이를 merged map 소비로 수렴해야 drift 0 | **HIGH** | **증명됨 (2026-06-03)**: Gate G-adapter PASS — `toSkiaStyle`(`7022b8d84`, G-adapter 단위 증명) + Skia dispatch merged map 소비(`b94fff418`). 두 어댑터가 같은 `resolveMergedStyle` 코어 재사용(병합 복제 0). collection family 시각 대칭: 비-data-bound 4종(`0e9c501d4`) 발효 후 generated CSS diff 0(DOM 불변, C2 가 Skia 를 DOM 에 정렬) — 부분 확증. Menu/GridList/Table 잔여                                                                                                                                                                                                                                                                                                                   |
...
잔존 HIGH 위험: R-1 / R-2 / R-3 / R-4 (4건). **증명 진행 (2026-06-03 실측)**: R-2 증명됨(G-adapter PASS, 비-data-bound 4종 발효 후 generated CSS diff 0 부분 확증), R-1 부분 증명 확대(Button slice + 비-data-bound 4종 + GridList C1 + Table 2D C1 발효 회귀 0), R-4 부분 증명(racStateAttrs disabled), R-3 부분 증명 확대(ListBox proof + GridList C1 1단 + Table 2D C1 2단 projector — cell 깊은 노드 + columnId write-target land). 미완 = R-1 잔여 family 전역(Menu, 단계 4/5) / R-3 generic projector 60fps 실측(window cap 100, 가상화 스크롤 미land) + cell drill-in data edit UI / R-4 interaction threading(후속). 대안 B 는 family 격리(ADR-910)를 포기하므로 R-1 의 전역 영향이 대안 A 보다 크나, Button vertical slice + 비-data-bound 4종 + GridList C1 + Table 2D C1 단계 증명 + 구 정본 제거 지연(단계 5)으로 회귀 표면을 단계별로 닫는다.
...
- 컴포넌트당 `render.shapes()` Skia source 가 폐기 방향(theme rule 대체) — 단 ADR-907 Layer B/908 fill/909 longhand 는 보존(override layer 유지). ADR-036 status 재평가 필요. **단계 5 선행 의존(실측 2026-06-03)**: `render.shapes()` 는 단일 그리기가 아니라 3 call site — (1) Skia 그리기(`buildSpecNodeData.ts:1180`, 현재 `{ skiaLegacy: true }` 12 entry 가 의존) — (1a) collection 7(Menu/Select/ComboBox/Tabs/TagGroup/GridList + Table) 은 **단계 4(C1/C2/C3, ④ 영역 B)가 직접 발효·해소** + (1b) 잔여 5(Tooltip + date 4; Dialog/Modal/Popover/DropZone 은 이미 발효, Slider/color 는 skiaLegacy 없음)는 skiaPrimitive escape hatch + (2/3) 텍스트 측정(`specTextStyle.ts:167` + `specTextStyleForOverlay.ts:61`, 29 TEXT_BEARING_SPECS type → generic 측정 전환 후 제거). 따라서 단계 5 의 `render.shapes` 제거는 단계 4 collection 발효 + 잔여 skiaPrimitive escape + 텍스트 측정 generic 전환 이후로 순서 재정의(breakdown ⑦ 단계 5 선행 의존 표). collection 발효 주체는 ADR-920(Superseded, 비착수)이 아니라 본 ADR 단계 4 — 사용자 결정 2026-06-03. 현 상태에서 "의존 0" 안전 제거 spec 교집합은 거의 비어있음.
