...
| Track B  | ready 이후 Navigator 선택 fan-out        | **완료** — LayerTree에서만 RAC `Virtualizer`를 opt-in하고 scoped `[role="row"]` 행 수·`rowSize`·단일 scroll owner·D1/DnD·600/5k 총비용을 검증. PageTree·LayoutList·LayoutElementTree (ADR-225 전 FrameList·FrameElementTree) 호출부는 유지 | ADR-203 Implemented                    |
