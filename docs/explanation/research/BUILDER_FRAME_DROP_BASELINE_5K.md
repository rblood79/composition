...
> **수리 완료 (2026-07-31, ADR-174 Implemented)**: 아래 성립 조건 3개가 전부 제거됐다 — ① 프레임 중 즉시 `.delete()` → 지연 폐기 큐 (`deferredDisposal.ts`, Phase 1) ② 전역 LRU 상한/퇴거 → **paragraph 소유 = 텍스트 노드** (수명 = 노드 수명, 상한·퇴거 개념 소멸 — Phase 2/3, Pen 형) ③ 수리 과정에서 발견된 선행 잠복 버그 — per-call `ParagraphBuilder.Make` 의 fontVariations 폰트 인스턴스 5.78 MB/paragraph 복제 → 공유 FontCollection (`a3414a526`). 실측 (22페이지 실문서): 힙 1,090→**128 MB**, 줌 10%↔100% ×2 왕복 텍스트 전량 유지. 아래 본문은 수리 전 상태의 진단 기록으로 보존.

ADR-172/173 되돌림은 "P1 반경 확대가 문턱을 넘게 한 원인"을 제거한 것이지, **문턱 자체를 제거한 것이 아니다**. 소실 버그의 성립 조건은 지금 main 에도 그대로 있다.

### 기제 — 2단으로 갈라진다

1. **스래싱 (성능)**: 한 프레임 walk 의 **고유 텍스트 수** (캐시 키 기준 — content 키라 동일 텍스트·스타일·폭은 노드 간 공유, `nodeRendererText.ts:130`) > 전역 LRU 상한 **1,000** → 프레임마다 전량 퇴거·재생성.
...
**문턱 변수는 요소 총수가 아니라 walk 당 고유 캐시 키 수**다 (동일 키 draw 는 히트 — draw 수보다 작거나 같다). 요소 총수는 줌아웃(가시 ≈ 전 문서)을 경유해서만 문턱에 관여한다 — 컬링 반경 200 에서도 텍스트 밀도·고유도가 높은 큰 문서를 줌아웃하면 넘을 수 있다 (실문서 문턱은 ADR-174 Phase 0 중복 계수 실측이 확정).

### 실측 경계

> ⚠️ **출처 주의 (2026-07-31 정정)**: 아래 두 행은 모두 **필러 5,046 문서** (`bench-*` 합성 요소 — 172/173 을 오염시킨 그 문서) 측정치다. 기제 격리 증거로는 유효하나 (버그 재현은 문서 사실성과 무관), **경계 수치로는 일반화 불가** — bench-\* 텍스트는 고유 문자열이라 중복 계수 ≈ 1 (draw 수 ≈ 고유 캐시 키 수) 인 반면 실문서는 라벨 반복으로 고유 키가 훨씬 적을 수 있다. 실문서 경계는 ADR-174 Phase 0 (클린 5,069 문서 중복 계수) 이 대체한다. 리뷰 round 1 M1/L1 연계 (`docs/adr/reviews/174.md`).

| 조건                                                   | walk 당 텍스트 draw | 결과                                              |
| ------------------------------------------------------ | ------------------- | ------------------------------------------------- |
| 반경 512 (ADR-173 P1) + 필러 5,046 요소                | 1,416               | **재현** (`49d71dbd3` 격리)                       |
| 반경 200 + 필러 5,046 요소 + 줌 15% 왕복 (되돌림 검증) | < 1,000 추정        | 미재현 — 단 그 문서·그 줌 한정 사실이지 보증 아님 |
