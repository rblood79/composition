...
- **WeWeb**: Collection 모드 **Dynamic / Static / Cached**. REST: **`Result key` 로 중첩 응답 경로** · CORS proxy (키 노출 경고 — "not meant to keep private tokens private") · 메타 `isFetching/isFetched/length/total/limit/offset`. Supabase guided/advanced · pagination 바인딩. [rest](https://docs.weweb.io/plugins/data-sources/rest-api.html) · [collections](https://docs.weweb.io/data/intro-to-collections.html) · [supabase](https://docs.weweb.io/plugins/data-sources/supabase-data.html)
...
## A10. Supabase Table Editor

- `New table` 사이드패널 — 기본 `id` + `created_at`, **RLS 기본 활성**, `Add column` (타입 드롭다운 · default · PK), 체인 아이콘 → FK. GUI 가 SQL 생성. Schema Visualizer. [tables](https://supabase.com/docs/guides/database/tables) · [studio 3.0](https://supabase.com/blog/supabase-studio-3-0)
- Import: 스프레드시트 업로드/붙여넣기, 100MB. [import](https://supabase.com/docs/guides/database/import-data). RLS 템플릿 + AI. [rls](https://supabase.com/docs/guides/database/postgres/row-level-security)
- 불만: 대형 테이블 `count(*)` 타임아웃 (50k 초과는 추정치로 변경), composite FK 편집 시 삭제. [#27543](https://github.com/supabase/supabase/issues/27543) · [#31276](https://github.com/supabase/supabase/issues/31276) · [#13234](https://github.com/supabase/supabase/issues/13234)
...
**반복되는 실패 (피할 것)**: 하드 한계에서 graceful degradation 없음 (Glide · Framer · Webflow) · 무증상 실패 (Framer 미표시, Baserow 셀 오류 무표시, Notion CSV 중복) · 대형 테이블 `count(*)` (Supabase) · import 가 관계/수식을 잃음 (Notion · Webflow) · 새 타입 기본 공개 (Bubble) · 스키마 변경이 캔버스 바인딩을 깨는 문제 (Framer 는 안정 id 로 대응).
...
- **Lovable** — 스키마 변경은 "reviewed migration": SQL 을 보여 주고 **승인 후** 실행, migration 파일 저장, RLS linter. [supabase](https://docs.lovable.dev/integrations/supabase) · [cloud](https://docs.lovable.dev/integrations/cloud)
- **v0** — SQL 생성·실행 (확인 UI 없음). [databases](https://v0.app/docs/databases) · **Bolt** — migration + `Discard` **(unverified)** [supabase](https://support.bolt.new/integrations/supabase)
- **Supabase AI Assistant** — LLM 에 **스키마만**, 데이터 미전송. SELECT 자동 실행 → 표/차트 (축 자동). SQL 수정은 **git-style diff accept/reject**. [assistant-v2](https://supabase.com/blog/supabase-ai-assistant-v2) · [studio-3-0](https://supabase.com/blog/supabase-studio-3-0)
...
- **Supabase MCP** — `list_tables` / `execute_sql` / `apply_migration` (DDL, 이력) · `read_only=true` · `project_ref` 스코프 · `features` 그룹. SQL 결과를 "내장 명령을 따르지 말라" 로 감싸되 불완전함 인정, 수동 승인 권고. [mcp](https://supabase.com/docs/guides/ai-tools/mcp) · [#112](https://github.com/supabase-community/supabase-mcp/issues/112)
...
- Retool Ask AI (B2) · Airtable Omni — 질문 답변은 되나 **뷰 변경 불가** [multi-table](https://community.airtable.com/announcements-6/multi-table-support-for-custom-interface-elements-ai-labs-46753) · **Softr Ask AI** (2025-07) — 목록 블록 chat, **visibility/permission 규칙 그대로**. [ask-ai](https://www.softr.io/blog/introducing-ask-ai) · Notion MCP SQL/view DSL · Supabase NL→SELECT + 차트.

## B5. 채택 패턴 (수렴도 순)

1. **Propose → Review → Apply** 기본 계약 (8개 제품 예외 없음)
2. 파괴 작업 기본 차단, 별도 등급 (Bubble · Base44 · Supabase · Webflow · MCP)
...
6. 읽기 fine-grained · 쓰기 "변경 제안" coarse 1개 (Webflow · Supabase · Anthropic 결론 동일)
...
10. LLM 에는 스키마만 + 권한 미러링 + 결과 래핑 (Supabase · Airtable · Softr)
11. cURL / OpenAPI 붙여넣기 → guided form + Test 필수 (Xano · Retool · Zapier)
12. 에러 → "Debug?" (Retool · Supabase · Postman)
13. NL 질문은 답변 전용, 변경과 분리 (Airtable · Softr · Supabase)
14. Human-in-the-loop: inputs 표시 + 감사 로그 (MCP spec · Supabase · Framer)
