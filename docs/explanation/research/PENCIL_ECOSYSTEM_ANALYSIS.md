...
**작성일**: 2026-05-27
**갱신일**: 2026-09-07 (`/Users/admin/work/pencil` 이 Pen v1.2.8 로 교체 — provenance 확정, §1·§2·§5·§9·§10 갱신) · 2026-08-17 (세 제품 최신 artifact/release + Figma/Framer 공식 기능 교차 검증, AI·제품화 수준·총점 추가 — §1~§10)
...
> **♻️ provenance 확정 (2026-09-07) — `/Users/admin/work/pencil` = Pen v1.2.8**: 아래 2026-08-17 배너의 "두 artifact 분리" 전제는 해소됐다. 같은 경로의 `Info.plist` 가 이제 **Pen v1.2.8** (`dev.pencil.desktop`, 빌드 2026-09-02) 이고, `app.asar` 전개 결과 v1.2.1 추출본과 같은 계보의 후속 릴리스임이 확인됐다 (koffi 부재 · 자체 `pencil.wasm` Skia m149 · 평문 `.pen` v2.17). v1.1.57 판정은 **교체 전 번들의 역사 기록**으로만 남긴다 (§2). 델타 정본: [PEN_V1.2.8_DELTA_2026-09.md](PEN_V1.2.8_DELTA_2026-09.md) — MCP 도구 10→6 (`execute` JS API · `browser` 신설) · 스킬 번들 원격 갱신 · 브라우저 import · 대시보드/draft · Comments. **§5-3·§5-4 의 Pencil/Pen 점수는 재채점하지 않았다** — AI·확장성·협업 축에 상향 요인이 생겼으나 방법론 전체 재적용이 필요해 보류.
>
> **⚠️ 버전 분리 (2026-08-17, 2026-09-07 해소) — Pencil.app / Pen artifact**: `/Users/admin/work/pencil`의 실제 `Info.plist`는 **Pencil.app v1.1.57** (`dev.pencil.desktop`)다. 별도 문서의 v1.2.1 추출본(`docs/pencil-extracted/`)은 **Pen v1.2.1** artifact이므로 현재 `/Users/admin/work/pencil`의 버전·렌더러를 덮어쓰지 않는다. 아래 §2와 §5는 v1.1.57 로컬 번들과 v1.2.1 별도 분석을 분리한다.
>
...
| 디렉토리                            | 형태                                                  | 정체                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ----------------------------------- | ----------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **`/Users/admin/work/pencil`**      | **macOS `.app` bundle 내용물 (closed-source binary)** | **Pen v1.2.8** (`dev.pencil.desktop`, Electron 42.5, 60+ 언어, 빌드 2026-09-02 — 2026-09-07 교체 확인). `Resources/app.asar` 138MB + `app.asar.unpacked` (Claude Agent SDK 0.3.257 · Codex 0.144.1 · Cursor SDK · Go MCP 서버 · `skills/pen-dev`). 교체 전 번들은 Pencil.app v1.1.57 (§2 역사 기록). v1.2.1 추출본 (`docs/pencil-extracted/`) 과 같은 계보 — 델타는 [PEN_V1.2.8_DELTA_2026-09.md](PEN_V1.2.8_DELTA_2026-09.md). |
...
- **Pencil.app / Pen**: live update feed가 아니라 `/Users/admin/work/pencil`에 보관된 bundle을 판정한 것이다. 2026-09-07 현재 이 artifact 는 **Pen v1.2.8** 이며, v1.2.1 추출본과 파일 단위 diff 로 같은 계보임을 확인했다 (구 v1.1.57 판정은 교체 전 번들 기준).
...
## 2. 원본 Pencil.app v1.1.57 기술 스택 — 로컬 bundle 기준 (교체 전 역사 기록)

> **2026-09-07**: 본 절이 판정한 v1.1.57 번들은 같은 경로에서 **Pen v1.2.8 로 교체**됐다. 아래 "koffi native bridge" 서술은 교체 전 artifact 의 메타데이터 판정이며 현재 번들에는 koffi 가 없다 — 현재 스택은 [PEN_V1.2.1_RENDERING_UIUX_ANALYSIS.md](PEN_V1.2.1_RENDERING_UIUX_ANALYSIS.md) §1~§3 (렌더링, v1.2.8 에서 동일 재확인) + [PEN_V1.2.8_DELTA_2026-09.md](PEN_V1.2.8_DELTA_2026-09.md) (AI·셸 델타) 가 정본이다. 절 본문은 수정하지 않는다.
...
| ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- | ----------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------- | -------------------------------------------------------------------------------- |
| **렌더링**        | v1.2.8 (현재 번들): 자체 `pencil.wasm` (Skia m149, C API 396) + JS 레이아웃 — v1.2.1 과 동일                                                                          | native `skia-safe` + browser CanvasKit/WebGL2 shared host                         | CanvasKit WASM/WebGL2, Canvas 2D 폴백 없음                              | CanvasKit WASM 단일 renderer + Rust layout/Skia consumer                                                         | hosted renderer 내부 비공개                                                  | hosted React/edge delivery, editor renderer 내부 비공개                          |
| **데이터 모델**   | `.pen` 평문 JSON schema v2.17 (`@ha/schema`, 노드 13종 — v2.14 대비 공개 표면 무변경)                                                                                 | `.op` JSON `PenDocument`, 21 node variants, events/bindings/lifecycle/route       | `.fig` + `.pen` + Kiwi override + HTML/CSS/Tailwind/JSX 입출력          | `CompositionDocument` + canonical RefNode/frame/slot/events/actions                                              | node/file model + Variables/Libraries, proprietary                           | visual site/document + components/CMS, proprietary                               |
...
| **AI 통합**       | 5 agent families + pen.dev Pro 프록시 + 커스텀 provider; MCP 6 tools (`execute` JS DSL · `browser` · `spawn_agents`), 스킬 번들 원격 갱신, 외부 CLI 11 타깃 자동 설치 | `op-ai` + skills + concurrent orchestrator + ACP/MCP + multi-provider             | Design/Review/Fast/Vision role models + ACP/MCP + bounded Vision        | 클라우드 LLM `llama-3.3-70b-versatile`, streaming Tool Calling, 7 tools; provider abstraction/Ollama는 plan-only | First Draft/Agent, text/image/interaction/asset tools, paid Full seat·credit | Agents가 editable page/section/copy/visual을 생성·수정·publish; external agents  |
| **협업**          | realtime 편집 contract 미확인 (번들에 Yjs/WebSocket 0); Comments 는 공유 링크 API 기반 (feature flag)                                                                 | authenticated P2P + relay + regional hub, pairing/cursors/conflict replay         | Trystero WebRTC P2P + relay/hub fallback + Yjs CRDT                     | 실시간 document collaboration 없음. Cloud Auth는 identity surface                                                | multiplayer, version history, Organization/Enterprise branching              | branches/staging/version workflow; realtime editor contract는 본 비교에서 미확인 |
| **성능·검증**     | 교차 benchmark 없음; v1.2.1 WebGL/WASM 실측이 v1.2.8 에서 심볼 단위 재확인                                                                                            | README reported 10k snapshot ~0.68s; release/CI/cargo-deny/source tests           | virtualized layers, retained backing, 30 engine domains, visual-oracles | ADR-153 Implemented: Picture/paint/cache/profiler/ping-pong + SpatialIndex; p50/p95/p99 우선                     | version history/Dev Mode/multiplayer scaling; renderer p50/p95/p99 공개 없음 | site optimization/Lighthouse/staging; editor frame-time benchmark 공개 없음      |
| **운영·출력**     | Electron desktop + `.pen`/URL scheme + 브라우저 import (DOM→캔버스) + HTML/PDF export                                                                                 | native desktop/web/CLI/wasm viewer, Figma/Git/VS Code/Chrome, codegen/deck export | Tauri desktop/web/CLI/MCP/headless Vue SDK, `.pptx`/HTML/PDF/SVG        | Builder + publish/preview, canonical mutation path                                                               | Dev Mode/Code Connect/export/Figma Sites                                     | CMS, custom code, staging, custom domain, hosting, publish                       |
...
| **fallback 정책** | custom WASM 단일 (native bridge 없음 — v1.1.57 판정은 교체 전 번들)                                                                                                   | Paper.js runtime fallback 없음; native PathOp/web contour parity 별도 검증        | Canvas 2D 폴백 없음                                                     | fallback 없음. 지원범위와 parity를 명시                                                                          | SaaS 내부 fallback 비공개                                                    | hosting/runtime 최적화는 제공하지만 editor fallback 내부는 비공개                |
...
| ADR-134 AI Assistant 통합                                  | Pencil v1.1.57 dual SDK, openpencil Rust agent/orchestrator, open-pencil role models/ACP/MCP              | composition은 **기능이 전혀 없는 상태가 아니라** legacy Tool Calling 7 tools가 landed다. 다만 Provider abstraction/Ollama/offline/routing/Plan→Execute→Verify는 ADR-134 Proposed plan-only라 reference 대비 격차가 남는다.                                     |
...
| 제품화·AI 운영                                             | Figma AI Agent/First Draft, Framer Agents·external agents, openpencil/OpenPencil의 multi-provider·ACP/MCP | composition의 7-tool 클라우드 LLM loop는 기능적으로 landed지만 provider abstraction·offline·secret isolation·Plan→Execute→Verify가 미완료다. 따라서 기능 격차보다 **운영 가능한 AI contract 격차**로 평가한다.                                                 |
...
- AI는 7개 tool + streaming agent loop + abort/visual feedback이 landed지만, `AgentService`의 browser API key, 단일 provider, `llama-3.3-70b-versatile`, `dangerouslyAllowBrowser`는 ADR-134가 해결하려는 현재 gap이다. Figma·Framer·open-pencil 계열과의 차이는 AI 기능 수보다 provider·권한·검증·운영 contract의 제품화 수준이다.
...
7. **Pencil.app AI agent dual embed (Codex + Claude SDK)** — composition은 legacy 클라우드 LLM agent loop가 landed했지만 ADR-134의 provider/offline/verification 확장은 plan-only → reference가 더 진보
...
1. **Pencil 계열은 두 artifact로 나눠야 한다.** ~~`/Users/admin/work/pencil`은 v1.1.57 Electron bundle이며 Codex/Claude SDK와 `koffi`가 실제 unpacked dependency로 확인된다.~~ → **2026-09-07 해소**: 같은 경로가 Pen v1.2.8 로 교체돼 v1.2.1 추출본과 같은 계보임이 확정됐다. 현재 정본은 custom `pencil.wasm`/Skia m149 + 평문 `.pen` schema (v2.17) 이며 native bridge 서술은 역사 기록이다.
...
5. **composition의 가장 큰 잔존 gap은 AI productization이다.** 현재 `AgentService`는 `llama-3.3-70b-versatile`, streaming Tool Calling, 7 tools, max 10 turns, 429 retry를 실제 제공한다. 그러나 browser-side `dangerouslyAllowBrowser`와 단일 provider는 ADR-134가 해결하려는 문제이고, Provider abstraction/Ollama/offline/routing/Plan→Execute→Verify는 아직 Proposed plan-only다. 따라서 “AI가 없다”가 아니라 “기본 agent loop는 있으나 enterprise-grade provider·verification·offline contract가 없다”가 정확하다.
6. **협업은 제품 scope 차이로 분류한다.** openpencil은 authenticated P2P + public relay + regional hub, open-pencil은 Trystero WebRTC + relay/hub fallback + Yjs CRDT다. composition의 Cloud Auth는 identity surface이지 실시간 document collaboration 구현이 아니므로, 이를 schema/rendering gap으로 잘못 계산하지 않는다.
...
- ~~**Pencil artifact provenance 고정**~~ — **완료 (2026-09-07)**: `/Users/admin/work/pencil` 이 Pen v1.2.8 로 교체돼 v1.2.1 과 같은 계보 확정. 후속 후보로 대체: **Pencil/Pen §5-3·§5-4 재채점** (AI · 확장성 · 협업 축 상향 요인 — `execute` JS DSL · 스킬 번들 · 브라우저 import · Comments) 은 방법론 전체를 다시 적용할 때 함께 한다.
...
- **Worker thread / GPU 분리 ADR** — Pen v1.2.8 도 완전 단일 스레드 (pthread/SAB/Worker 0) 이므로 helper process 는 Electron 표준 구성일 뿐 참조 지점이 아니다; composition에서는 Worker isolation이 실제 frame-time p95/interaction latency를 개선하는지 측정 후 제안한다.
...
- composition `apps/builder/src/services/ai/AgentService.ts`·`services/ai/tools/index.ts`로 현재 클라우드 LLM model/tool loop를, ADR-134 본문으로 Proposed/plan-only provider·offline 계획을 확인했다.
