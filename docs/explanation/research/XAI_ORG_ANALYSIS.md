...
| 3   | **Retry/backoff 행동 스펙** (429 Retry-After 전액 대기 + 별도 시도 상한, 413 이미지 strip 1회, 영구-fatal 분류, 서버 override 헤더)        | grok-build §2-2                            | Phase 1 어댑터의 retry 정책 초안으로 이식 — 현행 AgentService 의 429 retry 를 대체하는 명세                                                                                                                                           |
...
engine 과 Skia 씬 자료구조에 적용 가능한 **이디엄 카탈로그** (당장 반영이 아니라 다음 최적화·리팩토링 때 꺼내 쓰는 목록):
