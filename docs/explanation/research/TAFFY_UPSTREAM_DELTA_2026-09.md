...
> `packages/engine` 은 Taffy 0.10.0 을 참조해 자체 구현한 Rust 레이아웃 엔진이다 (의존 0, ADR-916). Taffy 가 그 뒤 5개 릴리스에서 고친 항목을 엔진과 대조하고, 의심 항목은 Chrome 차등 하니스 (`tests/parity/harness.ts` `domLeg` vs `engineLeg`) 로 실측했다. **오라클은 Chrome 이다** — Taffy 가 고쳤다는 사실이 아니라 Chrome 과 어긋나는지가 판정 기준이다.
>
...
| ②    | ✅ 2026-09-07 — grid 명시 배치가 explicit grid 초과 시 암묵 트랙 생성 (10,000 가드 제거)            | `grid.rs:754-767` `block_fits` · `:1004-1051` 배치 루프 · 암묵 열 생성           | G4 fixture                                             | Taffy 는 CSS-GRID §8.5 대로 암묵 트랙을 늘린다. 가드는 clamp (10,000 라인) 로 바꾼다 (#986)                                                                                  |
| ③ ✅ | abs-pos used size clamp                                                                             | `tree.rs:1992-1998` (덮어쓰기 제거) · `tree.rs:5401-5404` (stretch 에 clamp)     | B4 · B4c fixture → `phase4_5` E11 계열                 | 작음                                                                                                                                                                         |
| ④    | ✅ 2026-09-07 — 암묵 열 기본 `auto` — 정폭 grid 의 template 없음 · `grid-auto-columns` 목록         | `grid.rs:681-686` 폴백 제거 · `:1094` 토큰 순환 · `tree.rs:3374` 분기 확장       | G12 · G10 · G11 fixture                                | 먼저 live 로 catalog 6 규칙 (MeterTrack 등) 이 실제로 어긋나는지 확인 — implicitStyles 가 가리면 우선순위 하향                                                               |
| ⑤ ✅ | block leaf `aspect-ratio`                                                                           | `tree.rs:1686` `!children.is_empty()` 조건                                       | B6 fixture                                             | Pencil import 이미지 프레임에 도달                                                                                                                                           |
| ⑥    | ✅ 2026-09-07 — grid 파서 — 대괄호 이름 · auto-fit collapse · auto-repeat 최소 · auto 축 `repeat()` | `grid.rs:80-140` tokenize/parse · `:235-253` · `:476` · `tree.rs:4640`           | G3 · G1 · G1b · G10 fixture                            | 도달 낮음. 함께 하면 한 phase. `presetDefinitions.ts:21` 회피 주석 정정 동반                                                                                                 |
| ⑦    | ✅ 2026-09-07 — 정렬 키워드 — `baseline` (flex) · `safe`/`unsafe` · `self-*` (flex)                 | `tree.rs:4235-4272` 파서 · `flex.rs` baseline 참여                               | F4 · F8b fixture                                       | baseline 은 ADR-923 block baseline 슬롯 (`leaf_baseline`) 을 flex 로 확장하는 형태 — 가장 큼                                                                                 |
| ⑧    | ✅ 2026-09-07 — `display: flow-root` BFC · block `align-content`                                    | `tree.rs:4859` `node_establishes_bfc` · `solve_block`                            | B8 · B5 fixture                                        | 도달 낮음                                                                                                                                                                    |
| ⑨    | ✅ 2026-09-07 — padding 있는 텍스트 leaf 의 intrinsic 이중 가산 (#1018)                             | `tree.rs:4071-4110` ↔ `flex.rs:285` 슬롯 계약                                    | 텍스트 케이스라 `pipelineLeg` 로 실측 먼저             | 정적 판정만 — **실측 전 착수 금지**                                                                                                                                          |
...
- **2026-09-07 밤 — ⑨ 완료 (ADR 없이 `/fix` 절차)**: pipeline 실측으로 확정 (Text `paddingLeft 12` in flex row: Chrome 94.4 / 엔진 107 · `12+8`: 102.4 / 123). 원인 = leaf 스칼라 경로만 border-box 보고 (컨테이너 auto 축은 content-box 계약). leaf 를 content-box 보고로 통일하고, 반환을 border-box 로 읽던 grid 기여·배치가 auto 축 자식에 pad/border 를 더하도록 — 그 과정에서 **grid 의 padded 컨테이너가 padding 만큼 모자라던 잠복 결함** (auto 트랙 50/70 · justify start 50/70 · auto 행 20/40) 도 같이 수리. 기여의 `%` padding 은 0 (shrink-to-fit grid + `paddingLeft:10%` Chrome 120). 게이트 `paddedLeafIntrinsic.browser.test.ts` 16 (baseline 13 RED) · `shrinkToFitInline` 구 [잔존] → 132 · unit 6. ledger §26. **별개 발견 → 같은 날 판정·수리**: "fit-content 컨테이너 400" 은 pipeline fixture 의 root 자리에 둔 하니스 배치 오류 (body 폭 주입) — 결함 아님. `max-content`/`min-content` block 부모 · Container Align 아래 Text 의 스칼라 소실 (400 / 12 / **0**, Chrome 82.4 / 94.4 / 41.5) 은 `enrichWithIntrinsicSize` 의 `(isFlexChild || isGridChild)` 게이트가 원인인 **TS 공급 결함** — 게이트 제거 (ledger §27, `textLeafScalarBlockParent.browser.test.ts` 8).
- **2026-09-07 밤 — ⑦ 완료 (`/fix` 절차) — C 묶음 (⑥⑦⑧) 종결, §4 전 항목 반영**: flex.rs `FLEX_FIELD_COUNT` 21 → 22 (슬롯 21 `baseline_plus_one`, 0 = absent 합성) + `baseline_group` (참여 = baseline 해소 ∧ cross auto margin 없음, column 은 start; 라인 cross = max(outer, 그룹 extent)) + `ALIGN_BASELINE 4` · safe 코드 (align_items 5/6 · align_self 5~~7 · justify 6/7 · align_content 6/7 — 여유 `max(0)`) · 파서 4개가 `strip_overflow_position` 공유 + `self-*` → start/end, grid 는 `grid_align_items_code` 로 0~~3 접음. 게이트 `flexBaselineSafeAlign.browser.test.ts` 52 (baseline 34 RED) · unit 2 · cargo 410 (golden 22) · parity 1380 (기존 2 제외) · layout 단위 488 · live Playwright (a.y 40 · Text 55.9/DOM 55 · safe 0 · unsafe −100 · self-end 150). ledger §30. LOW deferred: first/last baseline 구분 · grid baseline · flex `align-content: space-evenly` (기존 gap). **§4 잔여 = "—" API 견고성 후순위뿐.**
- **2026-09-07 밤 — ⑥ 완료 (`/fix` 절차)**: `tokenize_template_with_line_names` (대괄호 이름 분리, 공백 이름 여럿) + `resolve_line_name` (`b` · `x 2`) 을 tree.rs `build_grid_placement_spec` 이 번호로 · `auto_repeat_count` §7.2.3.2 (max definite 면 max 아니면 min, **반복 밖 트랙 + gutter 선차감** — 컨테이너 전체로 세면 `50px repeat(auto-fill,100px) 50px` 가 한 반복 과다, Chrome e.w 50) · `expand_auto_repeat_tokens` 로 tree.rs 가 definite 축에서 토큰 펼침 · auto-fit 은 배치 뒤 미점유 트랙 **토큰 삭제** (gutter 포함, Chrome gap 20 → 290/290 b.x 310) + 인덱스 당김 + `placement_spec` 숫자 재작성 (명시 `grid-column-start: 3` → x 0 w 600). 게이트 `gridLineNamesAutoRepeat.browser.test.ts` 38 (baseline 22 RED) · unit 3 · cargo 408 · parity 1328 (기존 2 제외) · layout+preset 단위 682 · live Playwright 3 케이스 Chrome 동일. ledger §29. `presetDefinitions.ts` 회피 주석: 엔진 사유 전부 소멸 (명시 나열 = 설계 선택). **⑦ 이어서.**
- **2026-09-07 밤 — ⑧ 완료 (`/fix` 절차, C 묶음 재개 — 사용자 재제기)**: `node_establishes_bfc` / `solve_block` `creates_bfc` 에 `display_creates_bfc` (inner flow-root = `flow-root` · `inline-block`, ∨ abs/fixed — **abs-pos BFC 는 live 대조군이 드러낸 인접 사각**, Chrome abs h 50 / 종전 10). block `align-content` 는 `parse_block_align_content` (safe/unsafe 접두 분리 · 분배 폴백) + `solve_block` `align_offset` (여유 = used content − in-flow 묶음, 기본 safe, `unsafe` 만 음수) + normal 이 아니면 (`start` 포함) `can_collapse_top/bottom` 닫힘 (Chrome: 묶음이 margin box — auto 높이 center > mt20 h50 → blk h 70). TS: `normalizeCssDisplay` 가 `flow-root` 를 `block` 으로 접던 것 보존 (`EngineDisplay` 확장) · block 어댑터가 style `alignContent` 운반. 게이트 `blockFlowRootAlignContent.browser.test.ts` 72 (baseline 48 RED · 엔진만 수리 시 pipeline 23 RED) · unit 4 · cargo 405 (⑨ 삽입 때 `#[test]` 를 잃은 2개 복구 포함) · parity 1290 (기존 2 제외) · live Playwright (fr h 50 · child y 40 · sib y 50 / blk center mt20 → 85 — Chrome 동일). ledger §28. **⑥ ⑦ 은 이어서** (사용자 결정 09-07: ⑧ → ⑥ → ⑦ `/fix` 체인).
- **2026-09-07 밤 — ② ④ 완료 (ADR-206 Phase 2)**: grid.rs `block_fits` 명시 축 한계 해제 + `implicit_minor_count` (자동 축 폭 = max(명시, 명시 end, auto span)) + 10,000 가드 5곳 → `MAX_GRID_LINE` 라인 clamp (Chrome 실측 상한은 10,000,000 — 의도된 편차 기록) + 셀 폴백 100 제거 3곳 + `with_implicit_tracks` 토큰 순환 · tree.rs 암묵 열 합성을 `inline_intrinsic` 밖으로 + 암묵 행 flow 무관 + `expand_repeat_tokens` + `combine_grid_line` end-only. G4 · G4' · G12 · G10 · G11 전부 GREEN (`gridImplicitTracks.browser.test.ts` 30/30), `shrinkToFitInline` 구 `[잔존] flow:column` 해소. `presetDefinitions.ts:21` 회피 주석 정정 (auto-repeat 만 잔존). §7 drift 1건 해소.
