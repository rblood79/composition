...
> **환경 차단 (2026-09-17)** — 작성 머신에 JDK · Maven · tusd · docker 가 없어 `mvn -q test` · tusd 대조군 · 1GB e2e 를 **실행하지 못했다**. 소스는 Java 8 문법·API 만으로 작성하고 두 번 정독했다. 실행 결과는 아래 §사용자 실행 명령 을 사용자 머신에서 돌린 로그로 채운다 (evidence `docs/adr/evidence/201-phase2-server.md`).
...
| `src/tuslib/java/…/TusLibraryController.java`                      | 변형 ② — [tus-java-server](https://github.com/tomdesair/tus-java-server) 위임 스켈레톤. Maven 프로파일 `tus-lib` 에서만 컴파일 (`mvn -P tus-lib`). **라이브러리 API 호출은 컴파일 미검증**                                     |
...
```bash
# JDK 8 + Maven (macOS)
brew install --cask temurin@8            # 또는: brew install openjdk@8 maven
brew install maven
...
## 사용자 실행 명령 (G2 — 작성 머신에서 차단된 항목)

| 항목                       | 명령                                                                                                                                                                                                              | 기대                                                                           |
| -------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------ |
| G2b-1 단위·MockMvc         | `cd examples/upload-server-spring && mvn -q test`                                                                                                                                                                 | `BUILD SUCCESS`, 실패 0 (`target/surefire-reports/`)                           |
| G2b-2 기동 거부 실물       | `mvn cargo:run -Dupload.storage.dir="$PWD/target/cargo/configurations/tomcat9x/webapps/upload/WEB-INF/uploads"`                                                                                                   | 컨텍스트 기동 실패, 로그에 `must be outside the web root (ADR-201 S1)`         |
...
| 변형 ② 컴파일              | `mvn -q -P tus-lib compile`                                                                                                                                                                                       | 컴파일 성공 (라이브러리 API 이름 미검증 — 실패 시 `TusLibraryController` 수정) |
