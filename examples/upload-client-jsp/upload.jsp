...
<body>
  ...
  <%-- 엔진 IIFE 1줄 — packages/upload-engine/dist/vanilla.global.js 를 js/composition-upload.iife.js 로 복사 (README) --%>
  ...
  <script>
    (function () {
      ...
      ready.then(function () {
        ...
        uploader.subscribe(function (event) {
          // event: { type, item } — item.status: queued|creating|uploading|paused|done|error, item.error?.code
          var item = event && event.item;
          if (!item) { return; }
          var pct = item.size ? Math.floor((item.offset / item.size) * 100) : 0;
          line("[" + (event.type || "update") + "] " + item.name + " " + item.status + " " + pct + "%"
            + (item.error ? " " + item.error.code + " " + (item.error.message || "") : ""));
          if (item.error && item.error.code === "E_UNAUTHORIZED") {
            line("→ 세션 만료 또는 CSRF 토큰 불일치. 페이지를 새로고침한 뒤 재개하면 HEAD offset 부터 이어진다.");
          }
        });
